    ...
    for(ASTUnit::stored_diag_iterator D = AU->stored_diag_begin()
       , DEnd = AU->stored_diag_end();
        ...
        if(D->getLevel() >= DiagnosticsEngine::Error &&
            ...
            diag::DiagCat_AST_Deserialization_Issue) return true;
    }
    return false;
}
...
static RangeComparisonResult RangeCompare(SourceManager &SM, SourceRange R1, SourceRange R2){
    assert(R1.isValid() && "First range is invalid?");
    assert(R2.isValid() && "Second range is invalid?");
    if(R1.getEnd() != R2.getBegin() &&
        SM.isBeforeInTranslationUnit(R1.getEnd(), R2.getBegin())) return RangeBefore;
    if(R2.getEnd() != R1.getBegin() &&
        SM.isBeforeInTranslationUnit(R2.getEnd(), R1.getBegin())) return RangeAfter;
    return RangeOverlap;
}

/// \brief Determine if a source location falls within, before, or after a
///   a given source range.
static RangeComparisonResult LocationCompare(SourceManager &SM, SourceLocation L, SourceRange R){
    assert(R.isValid() && "First range is invalid?");
    assert(L.isValid() && "Second range is invalid?");
    if(L == R.getBegin() || L == R.getEnd()) return RangeOverlap;
    if(SM.isBeforeInTranslationUnit(L, R.getBegin())) return RangeBefore;
    if(SM.isBeforeInTranslationUnit(R.getEnd(), L)) return RangeAfter;
    return RangeOverlap;
}
...
CXSourceRange cxloc::translateSourceRange(const SourceManager &SM, const LangOptions &LangOpts, const CharSourceRange &R){
    // We want the last character in this location, so we will adjust the
    // location accordingly.
    SourceLocation EndLoc = R.getEnd();
    if(EndLoc.isValid() && EndLoc.isMacroID() && ! SM.isMacroArgExpansion(EndLoc)) EndLoc = SM.getExpansionRange(EndLoc).second;
...
bool CursorVisitor::Visit(CXCursor Cursor, bool CheckedRegionOfInterest){
    if(clang_isInvalid(Cursor.kind)) return false;
    if(clang_isDeclaration(Cursor.kind)){
        ...
        if(D->isImplicit() && ! isa<ObjCMethodDecl>(D)) return false;
    }
    // If we have a range of interest, and this cursor doesn't intersect with it,
    // we're done.
    if(RegionOfInterest.isValid() && ! CheckedRegionOfInterest){
        SourceRange Range = getRawCursorExtent(Cursor);
        if(Range.isInvalid() || CompareRegionOfInterest(Range)) return false;
    }
    switch(Visitor(Cursor, Parent, ClientData)){
        ...
        case CXChildVisit_Recurse: {
            ...
            if(PostChildrenVisitor)
                    if(PostChildrenVisitor(Cursor, ClientData)) return true;
            return ret;
        }
    }
...
static bool visitPreprocessedEntitiesInRange(SourceRange R, PreprocessingRecord &PPRec, CursorVisitor &Visitor){
    ...
    if(! Visitor.shouldVisitIncludedEntities()){
        ...
        if(FID != SM.getFileID(SM.getFileLoc(R.getEnd()))) FID = FileID();
    }
...
bool CursorVisitor::visitFileRegion(){
    if(RegionOfInterest.isInvalid()) return false;
    ...
    if(! VisitDeclsOnly && ! VisitPreprocessorLast)
            if(visitPreprocessedEntitiesInRegion()) return true;// visitation break.
    if(visitDeclsFromFileRegion(File, Offset, Length)) return true;	// visitation break.
    if(! VisitDeclsOnly && VisitPreprocessorLast) return visitPreprocessedEntitiesInRegion();
    return false;
}
...
static bool isInLexicalContext(Decl *D, DeclContext *DC){
    ...
    for(DeclContext *DeclDC = D->getLexicalDeclContext();
        ...
        if(DeclDC == DC) return true;
    }
    return false;
}

bool CursorVisitor::visitDeclsFromFileRegion(FileID File, unsigned Offset, unsigned Length){
    ASTUnit			*Unit	= cxtu::getASTUnit(TU);
    SourceManager	&SM		= Unit->getSourceManager();
    ...
    while(Decls.empty() || Decls.front()->isTopLevelDeclInObjCContainer()){
        ...
        if(SLEntry.isFile()) Outer = SLEntry.getFile().getIncludeLoc();
        else Outer = SLEntry.getExpansion().getExpansionLocStart();
        if(Outer.isInvalid()) return false;
    ...
    for(SmallVectorImpl<Decl *>::iterator DE = Decls.end(); DIt != DE; ++ DIt){
        Decl *D = *DIt;
        if(D->getSourceRange().isInvalid()) continue;
        if(isInLexicalContext(D, CurDC)) continue;
        CurDC = dyn_cast<DeclContext>(D);
        if(TagDecl *TD = dyn_cast<TagDecl>(D))
                if(! TD->isFreeStanding()) continue;
        RangeComparisonResult CompRes = RangeCompare(SM, D->getSourceRange(), Range);
        if(CompRes == RangeBefore) continue;
        if(CompRes == RangeAfter) break;
        assert(CompRes == RangeOverlap);
        VisitedAtLeastOnce = true;
        if(isa<ObjCContainerDecl>(D)){
            FileDI_current	= &DIt;
            FileDE_current	= DE;
        }else FileDI_current = nullptr;
        if(Visit(MakeCXCursor(D, TU, Range), /*CheckedRegionOfInterest=*/ true)) return true;// visitation break.
    }
    if(VisitedAtLeastOnce) return false;
    ...
    while(DC && ! DC->isTranslationUnit()){
        ...
        if(CurDeclRange.isInvalid()) break;
...
bool CursorVisitor::visitPreprocessedEntitiesInRegion(){
    if(! AU->getPreprocessor().getPreprocessingRecord()) return false;
    ...
    if(RegionOfInterest.isValid()){
        ...
        if(AU->isInPreambleFileID(B)){
            if(SM.isLoadedSourceLocation(E)) return visitPreprocessedEntitiesInRange(SourceRange(B, E)
                                                                                    , PPRec, *this);
    ...
    if(OnlyLocalDecls) return visitPreprocessedEntities(PPRec.local_begin(), PPRec.local_end()
                                                       , PPRec);
    return visitPreprocessedEntities(PPRec.begin(), PPRec.end(), PPRec);
}

template<typename InputIterator>
bool CursorVisitor::visitPreprocessedEntities(InputIterator First, InputIterator Last, PreprocessingRecord &PPRec, FileID FID){
    for(; First != Last; ++ First){
        if(! FID.isInvalid() && ! PPRec.isEntityInFileID(First, FID)) continue;
        PreprocessedEntity *PPE = *First;
        if(! PPE) continue;
        if(MacroExpansion *ME = dyn_cast<MacroExpansion>(PPE)){
            if(Visit(MakeMacroExpansionCursor(ME, TU))) return true;
            continue;
        }
        if(MacroDefinitionRecord *MD = dyn_cast<MacroDefinitionRecord>(PPE)){
            if(Visit(MakeMacroDefinitionCursor(MD, TU))) return true;
            continue;
        }
        if(InclusionDirective *ID = dyn_cast<InclusionDirective>(PPE)){
            if(Visit(MakeInclusionDirectiveCursor(ID, TU))) return true;
            continue;
        }
    }
    return false;
...
bool CursorVisitor::VisitChildren(CXCursor Cursor){
    ...
    if(clang_isStatement(Cursor.kind)){
        if(const Stmt *S = getCursorStmt(Cursor)) return Visit(S);
        return false;
    }
    if(clang_isExpression(Cursor.kind)){
        if(const Expr *E = getCursorExpr(Cursor)) return Visit(E);
        return false;
    }
                ...
                if(! CXXUnit->isMainFileAST() && CXXUnit->getOnlyLocalDecls() &&
                    ...
                    for(ASTUnit::top_level_iterator TL = CXXUnit->top_level_begin()
                       , TLEnd = CXXUnit->top_level_end();
                        ...
                        CXXUnit->getASTContext().getTranslationUnitDecl())) return true;
                continue;
            }
            // Walk the preprocessing record.
            if(CXXUnit->getPreprocessor().getPreprocessingRecord()) visitPreprocessedEntitiesInRegion();
        }
        return false;
    }
    if(Cursor.kind == CXCursor_CXXBaseSpecifier){
        if(const CXXBaseSpecifier *Base = getCursorCXXBaseSpecifier(Cursor)){
            if(TypeSourceInfo *BaseTSInfo = Base->getTypeSourceInfo()) return Visit(BaseTSInfo->getTypeLoc());
        }
    }
    ...
    if(Cursor.kind == CXCursor_MacroDefinition &&
        ...
        if(MacroDefinitionRecord *MacroDef =
            checkForMacroInMacroDefinition(MI, Loc, TU)) return Visit(cxcursor::MakeMacroExpansionCursor(MacroDef, BeginLoc, TU));
    }
...
bool CursorVisitor::VisitBlockDecl(BlockDecl *B){
    if(TypeSourceInfo *TSInfo = B->getSignatureAsWritten())
            if(Visit(TSInfo->getTypeLoc())) return true;
    if(Stmt *Body = B->getBody()) return Visit(MakeCXCursor(Body, StmtParent, TU, RegionOfInterest));
    return false;
}
...
Optional<bool> CursorVisitor::shouldVisitCursor(CXCursor Cursor){
    if(RegionOfInterest.isValid()){
        ...
        if(Range.isInvalid()) return None;
...
bool CursorVisitor::VisitDeclContext(DeclContext *DC){
    ...
    for(; I != E; ++ I){
        Decl *D = *I;
        if(D->getLexicalDeclContext() != DC) continue;
...
Optional<bool> CursorVisitor::handleDeclForVisitation(const Decl *D){
    ...
    if(auto *ivarD = dyn_cast<ObjCIvarDecl>(D)){
        if(ivarD->getSynthesize()) return None;
    }
    // FIXME: ObjCClassRef/ObjCProtocolRef for forward class/protocol
    // declarations is a mismatch with the compiler semantics.
    if(Cursor.kind == CXCursor_ObjCInterfaceDecl){
        auto *ID = cast<ObjCInterfaceDecl>(D);
        if(! ID->isThisDeclarationADefinition()) Cursor = MakeCursorObjCClassRef(ID, ID->getLocation(), TU);
    }else if(Cursor.kind == CXCursor_ObjCProtocolDecl){
        auto *PD = cast<ObjCProtocolDecl>(D);
        if(! PD->isThisDeclarationADefinition()) Cursor = MakeCursorObjCProtocolRef(PD, PD->getLocation(), TU);
    }
    const Optional<bool> V = shouldVisitCursor(Cursor);
    if(! V.hasValue()) return None;
    if(! V.getValue()) return false;
    if(Visit(Cursor, true)) return true;
    return None;
}
...
bool CursorVisitor::VisitTypeAliasTemplateDecl(TypeAliasTemplateDecl *D){
    if(VisitTemplateParameters(D->getTemplateParameters())) return true;
    return Visit(MakeCXCursor(D->getTemplatedDecl(), TU, RegionOfInterest));
}

bool CursorVisitor::VisitTypeAliasDecl(TypeAliasDecl *D){
    if(TypeSourceInfo *TSInfo = D->getTypeSourceInfo()) return Visit(TSInfo->getTypeLoc());
    return false;
}
...
bool CursorVisitor::VisitClassTemplateSpecializationDecl(ClassTemplateSpecializationDecl *D){
    ...
    if(TypeSourceInfo *SpecType = D->getTypeAsWritten()){
        ...
        if(TemplateSpecializationTypeLoc TSTLoc =
            ...
            for(unsigned I = 0, N = TSTLoc.getNumArgs(); I != N; ++ I)
                    if(VisitTemplateArgumentLoc(TSTLoc.getArgLoc(I))) return true;
        }
    }
...
bool CursorVisitor::VisitClassTemplatePartialSpecializationDecl(ClassTemplatePartialSpecializationDecl *D){
    ...
    for(unsigned I = 0, N = Info->NumTemplateArgs; I != N; ++ I)
            if(VisitTemplateArgumentLoc(TemplateArgs[I])) return true;
...
bool CursorVisitor::VisitTemplateTypeParmDecl(TemplateTypeParmDecl *D){
    ...
    if(D->hasDefaultArgument() && ! D->defaultArgumentWasInherited())
            if(TypeSourceInfo *DefArg = D->getDefaultArgumentInfo())
                    if(Visit(DefArg->getTypeLoc())) return true;
    return false;
}

bool CursorVisitor::VisitEnumConstantDecl(EnumConstantDecl *D){
    if(Expr *Init = D->getInitExpr()) return Visit(MakeCXCursor(Init, StmtParent, TU, RegionOfInterest));
    return false;
}
...
bool CursorVisitor::VisitDeclaratorDecl(DeclaratorDecl *DD){
    ...
    for(unsigned i = 0; i < NumParamList; i ++){
        ...
        if(VisitTemplateParameters(Params)) return true;
    }
    if(TypeSourceInfo *TSInfo = DD->getTypeSourceInfo())
            if(Visit(TSInfo->getTypeLoc())) return true;
    // Visit the nested-name-specifier, if present.
    if(NestedNameSpecifierLoc QualifierLoc = DD->getQualifierLoc())
            if(VisitNestedNameSpecifierLoc(QualifierLoc)) return true;
    return false;
}

/// \brief Compare two base or member initializers based on their source order.
static int CompareCXXCtorInitializers(CXXCtorInitializer *const *X, CXXCtorInitializer *const *Y){
...
bool CursorVisitor::VisitFunctionDecl(FunctionDecl *ND){
    ...
    if(TypeSourceInfo *TSInfo = ND->getTypeSourceInfo()){
        ...
        if((FTL && ! isa<CXXConversionDecl>(ND) && Visit(FTL.getReturnLoc())) ||
            (! FTL && Visit(TL))) return true;
        // Visit the nested-name-specifier, if present.
        if(NestedNameSpecifierLoc QualifierLoc = ND->getQualifierLoc())
                if(VisitNestedNameSpecifierLoc(QualifierLoc)) return true;
        // Visit the declaration name.
        if(! isa<CXXDestructorDecl>(ND))
                if(VisitDeclarationNameInfo(ND->getNameInfo())) return true;
        // FIXME: Visit explicitly-specified template arguments!
        // Visit the function parameters, if we have a function type.
        if(FTL && VisitFunctionTypeLoc(FTL, true)) return true;
        // FIXME: Attributes?
    }
            ...
            for(unsigned I = 0, N = WrittenInits.size(); I != N; ++ I){
                ...
                if(Init->isAnyMemberInitializer()){
                    if(Visit(MakeCursorMemberRef(Init->getAnyMember()
                                                , Init->getMemberLocation(), TU))) return true;
                }else if(TypeSourceInfo *TInfo = Init->getTypeSourceInfo()){
                    if(Visit(TInfo->getTypeLoc())) return true;
                }
                // Visit the initializer value.
                if(Expr *Initializer = Init->getInit())
                        if(Visit(MakeCXCursor(Initializer, ND, TU, RegionOfInterest))) return true;
            }
        }
        if(Visit(MakeCXCursor(ND->getBody(), StmtParent, TU, RegionOfInterest))) return true;
    }
    return false;
}

bool CursorVisitor::VisitFieldDecl(FieldDecl *D){
    if(VisitDeclaratorDecl(D)) return true;
    if(Expr *BitWidth = D->getBitWidth()) return Visit(MakeCXCursor(BitWidth, StmtParent, TU, RegionOfInterest));
    return false;
}

bool CursorVisitor::VisitVarDecl(VarDecl *D){
    if(VisitDeclaratorDecl(D)) return true;
    if(Expr *Init = D->getInit()) return Visit(MakeCXCursor(Init, StmtParent, TU, RegionOfInterest));
    return false;
}
...
bool CursorVisitor::VisitNonTypeTemplateParmDecl(NonTypeTemplateParmDecl *D){
    if(VisitDeclaratorDecl(D)) return true;
    if(D->hasDefaultArgument() && ! D->defaultArgumentWasInherited())
            if(Expr *DefArg = D->getDefaultArgument()) return Visit(MakeCXCursor(DefArg, StmtParent, TU, RegionOfInterest));
    return false;
}
...
bool CursorVisitor::VisitTemplateTemplateParmDecl(TemplateTemplateParmDecl *D){
    if(VisitTemplateParameters(D->getTemplateParameters())) return true;
    if(D->hasDefaultArgument() && ! D->defaultArgumentWasInherited() &&
        VisitTemplateArgumentLoc(D->getDefaultArgument())) return true;
    return false;
}
...
bool CursorVisitor::VisitObjCMethodDecl(ObjCMethodDecl *ND){
    ...
    for(const auto *P : ND->parameters()){
        if(Visit(MakeCXCursor(P, TU, RegionOfInterest))) return true;
    }
...
static void addRangedDeclsInContainer(DeclIt *DI_current, DeclIt DE_current, SourceManager &SM, SourceLocation EndLoc, SmallVectorImpl<Decl *> &Decls){
...
bool CursorVisitor::VisitObjCContainerDecl(ObjCContainerDecl *D){
    ...
    // this nesting here.
    if(! DI_current && ! FileDI_current) return VisitDeclContext(D);
    ...
    // The common case.
    if(DeclsInContainer.empty()) return VisitDeclContext(D);
    // Get all the Decls in the DeclContext, and sort them with the
    // additional ones we've collected.  Then visit them.
    for(auto *SubDecl : D->decls()){
        if(! SubDecl || SubDecl->getLexicalDeclContext() != D ||
            SubDecl->getLocStart().isInvalid()) continue;
...
bool CursorVisitor::VisitObjCCategoryDecl(ObjCCategoryDecl *ND){
    if(Visit(MakeCursorObjCClassRef(ND->getClassInterface(), ND->getLocation()
                                   , TU))) return true;
    if(VisitObjCTypeParamList(ND->getTypeParamList())) return true;
    ObjCCategoryDecl::protocol_loc_iterator PL = ND->protocol_loc_begin();
    for(ObjCCategoryDecl::protocol_iterator I = ND->protocol_begin()
       , E = ND->protocol_end(); I != E; ++ I, ++ PL)
            if(Visit(MakeCursorObjCProtocolRef(*I, *PL, TU))) return true;
    return VisitObjCContainerDecl(ND);
}

bool CursorVisitor::VisitObjCProtocolDecl(ObjCProtocolDecl *PID){
    if(! PID->isThisDeclarationADefinition()) return Visit(MakeCursorObjCProtocolRef(PID, PID->getLocation(), TU));
...
bool CursorVisitor::VisitObjCPropertyDecl(ObjCPropertyDecl *PD){
    if(PD->getTypeSourceInfo() && Visit(PD->getTypeSourceInfo()->getTypeLoc())) return true;
    // FIXME: This implements a workaround with @property declarations also being
    // installed in the DeclContext for the @interface.  Eventually this code
    // should be removed.
    ObjCCategoryDecl *CDecl = dyn_cast<ObjCCategoryDecl>(PD->getDeclContext());
    if(! CDecl || ! CDecl->IsClassExtension()) return false;
    ...
    ObjCPropertyDecl	*prevDecl	=
        ObjCPropertyDecl::findPropertyDecl(cast<DeclContext>(ID), PropertyId
                                          , PD->getQueryKind());
    if(! prevDecl) return false;
    // Visit synthesized methods since they will be skipped when visiting
    // the @interface.
    if(ObjCMethodDecl *MD = prevDecl->getGetterMethodDecl())
            if(MD->isPropertyAccessor() && MD->getLexicalDeclContext() == CDecl)
                    if(Visit(MakeCXCursor(MD, TU, RegionOfInterest))) return true;
...
bool CursorVisitor::VisitObjCTypeParamList(ObjCTypeParamList *typeParamList){
    if(! typeParamList) return false;
    for(auto *typeParam : *typeParamList){
        // Visit the type parameter.
        if(Visit(MakeCXCursor(typeParam, TU, RegionOfInterest))) return true;
    }
    return false;
}
...
bool CursorVisitor::VisitObjCInterfaceDecl(ObjCInterfaceDecl *D){
    ...
    if(VisitObjCTypeParamList(D->getTypeParamListAsWritten())) return true;
    ...
    if(TypeSourceInfo *SuperClassTInfo = D->getSuperClassTInfo())
            if(Visit(SuperClassTInfo->getTypeLoc())) return true;
...
bool CursorVisitor::VisitObjCCategoryImplDecl(ObjCCategoryImplDecl *D){
    ...
    if(ObjCInterfaceDecl *ID = D->getClassInterface())
            if(Visit(MakeCursorObjCClassRef(ID, D->getLocation(), TU))) return true;
...
bool CursorVisitor::VisitObjCPropertyImplDecl(ObjCPropertyImplDecl *PD){
    if(ObjCIvarDecl *Ivar = PD->getPropertyIvarDecl())
            if(PD->isIvarNameSpecified()) return Visit(MakeCursorMemberRef(Ivar, PD->getPropertyIvarDeclLoc(), TU));
    return false;
}
...
bool CursorVisitor::VisitUsingDecl(UsingDecl *D){
    ...
    if(Visit(MakeCursorOverloadedDeclRef(D, D->getLocation(), TU))) return true;
...
bool CursorVisitor::VisitUnresolvedUsingTypenameDecl(UnresolvedUsingTypenameDecl *D){
    // Visit nested-name-specifier.
    if(NestedNameSpecifierLoc QualifierLoc = D->getQualifierLoc())
            if(VisitNestedNameSpecifierLoc(QualifierLoc)) return true;
    return false;
...
bool CursorVisitor::VisitStaticAssertDecl(StaticAssertDecl *D){
    if(Visit(MakeCXCursor(D->getAssertExpr(), StmtParent, TU, RegionOfInterest))) return true;
    if(StringLiteral *Message = D->getMessage())
            if(Visit(MakeCXCursor(Message, StmtParent, TU, RegionOfInterest))) return true;
    return false;
}
...
bool CursorVisitor::VisitDeclarationNameInfo(DeclarationNameInfo Name){
    switch(Name.getName().getNameKind()){
        ...
        case clang::DeclarationName::CXXConversionFunctionName:
            if(TypeSourceInfo *TSInfo = Name.getNamedTypeInfo()) return Visit(TSInfo->getTypeLoc());
            return false;
...
bool CursorVisitor::VisitNestedNameSpecifier(NestedNameSpecifier *NNS, SourceRange Range){
    ...
    while(NestedNameSpecifier *Prefix = NNS->getPrefix()) NNS = Prefix;
    switch(NNS->getKind()){
        ...
        case NestedNameSpecifier::TypeSpec: {
            ...
            if(const TypedefType *Typedef = dyn_cast<TypedefType>(T)) return Visit(MakeCursorTypeRef(Typedef->getDecl(), Range.getBegin(), TU));
            if(const TagType *Tag = dyn_cast<TagType>(T)) return Visit(MakeCursorTypeRef(Tag->getDecl(), Range.getBegin(), TU));
            if(const TemplateSpecializationType *TST
                    = dyn_cast<TemplateSpecializationType>(T)) return VisitTemplateName(TST->getTemplateName(), Range.getBegin());
            break;
        }
...
bool CursorVisitor::VisitNestedNameSpecifierLoc(NestedNameSpecifierLoc Qualifier){
    SmallVector<NestedNameSpecifierLoc, 4> Qualifiers;
    for(; Qualifier; Qualifier = Qualifier.getPrefix()) Qualifiers.push_back(Qualifier);
    while(! Qualifiers.empty()){
        ...
        switch(NNS->getKind()){
            ...
            case NestedNameSpecifier::TypeSpec:
            case NestedNameSpecifier::TypeSpecWithTemplate:
                if(Visit(Q.getTypeLoc())) return true;
                break;
...
bool CursorVisitor::VisitTemplateParameters(const TemplateParameterList *Params){
...
bool CursorVisitor::VisitTemplateName(TemplateName Name, SourceLocation Loc){
    switch(Name.getKind()){
        ...
        case TemplateName::OverloadedTemplate:
            ...
            if(Visit(MakeCursorOverloadedDeclRef(Name, Loc, TU))) return true;
            return false;
...
bool CursorVisitor::VisitTemplateArgumentLoc(const TemplateArgumentLoc &TAL){
    switch(TAL.getArgument().getKind()){
        ...
        case TemplateArgument::Type:
            if(TypeSourceInfo *TSInfo = TAL.getTypeSourceInfo()) return Visit(TSInfo->getTypeLoc());
            return false;

        case TemplateArgument::Declaration:
            if(Expr *E = TAL.getSourceDeclExpression()) return Visit(MakeCXCursor(E, StmtParent, TU, RegionOfInterest));
            return false;

        case TemplateArgument::NullPtr:
            if(Expr *E = TAL.getSourceNullPtrExpression()) return Visit(MakeCXCursor(E, StmtParent, TU, RegionOfInterest));
            return false;

        case TemplateArgument::Expression:
            if(Expr *E = TAL.getSourceExpression()) return Visit(MakeCXCursor(E, StmtParent, TU, RegionOfInterest));
            return false;

        case TemplateArgument::Template:
        case TemplateArgument::TemplateExpansion:
            if(VisitNestedNameSpecifierLoc(TAL.getTemplateQualifierLoc())) return true;
...
#include "clang/AST/BuiltinTypes.def"
    ...
    if(! VisitType.isNull()){
        if(const TypedefType *Typedef = VisitType->getAs<TypedefType>()) return Visit(MakeCursorTypeRef(Typedef->getDecl(), TL.getBuiltinLoc()
                                                                                                       , TU));
    }
    return false;
}
...
bool CursorVisitor::VisitTagTypeLoc(TagTypeLoc TL){
    if(TL.isDefinition()) return Visit(MakeCXCursor(TL.getDecl(), TU, RegionOfInterest));
    return Visit(MakeCursorTypeRef(TL.getDecl(), TL.getNameLoc(), TU));
}
...
bool CursorVisitor::VisitObjCTypeParamTypeLoc(ObjCTypeParamTypeLoc TL){
    if(Visit(MakeCursorTypeRef(TL.getDecl(), TL.getLocStart(), TU))) return true;
...
bool CursorVisitor::VisitObjCObjectTypeLoc(ObjCObjectTypeLoc TL){
    if(TL.hasBaseTypeAsWritten() && Visit(TL.getBaseLoc())) return true;
    for(unsigned I = 0, N = TL.getNumTypeArgs(); I != N; ++ I){
        if(Visit(TL.getTypeArgTInfo(I)->getTypeLoc())) return true;
    }
    for(unsigned I = 0, N = TL.getNumProtocols(); I != N; ++ I){
        if(Visit(MakeCursorObjCProtocolRef(TL.getProtocol(I), TL.getProtocolLoc(I)
                                          , TU))) return true;
...
bool CursorVisitor::VisitFunctionTypeLoc(FunctionTypeLoc TL, bool SkipResultType){
    if(! SkipResultType && Visit(TL.getReturnLoc())) return true;
    for(unsigned I = 0, N = TL.getNumParams(); I != N; ++ I)
            if(Decl *D = TL.getParam(I))
                    if(Visit(MakeCXCursor(D, TU, RegionOfInterest))) return true;
    return false;
}

bool CursorVisitor::VisitArrayTypeLoc(ArrayTypeLoc TL){
    if(Visit(TL.getElementLoc())) return true;
    if(Expr *Size = TL.getSizeExpr()) return Visit(MakeCXCursor(Size, StmtParent, TU, RegionOfInterest));
    return false;
}
...
bool CursorVisitor::VisitTemplateSpecializationTypeLoc(TemplateSpecializationTypeLoc TL){
    // Visit the template name.
    if(VisitTemplateName(TL.getTypePtr()->getTemplateName()
                        , TL.getTemplateNameLoc())) return true;
    // Visit the template arguments.
    for(unsigned I = 0, N = TL.getNumArgs(); I != N; ++ I)
            if(VisitTemplateArgumentLoc(TL.getArgLoc(I))) return true;
    return false;
}
...
bool CursorVisitor::VisitTypeOfTypeLoc(TypeOfTypeLoc TL){
    if(TypeSourceInfo *TSInfo = TL.getUnderlyingTInfo()) return Visit(TSInfo->getTypeLoc());
    return false;
}
...
bool CursorVisitor::VisitDependentTemplateSpecializationTypeLoc(DependentTemplateSpecializationTypeLoc TL){
    // Visit the nested-name-specifier, if there is one.
    if(TL.getQualifierLoc() &&
        VisitNestedNameSpecifierLoc(TL.getQualifierLoc())) return true;
    // Visit the template arguments.
    for(unsigned I = 0, N = TL.getNumArgs(); I != N; ++ I)
            if(VisitTemplateArgumentLoc(TL.getArgLoc(I))) return true;
    return false;
...
bool CursorVisitor::VisitElaboratedTypeLoc(ElaboratedTypeLoc TL){
    if(VisitNestedNameSpecifierLoc(TL.getQualifierLoc())) return true;
...
bool CursorVisitor::VisitDecltypeTypeLoc(DecltypeTypeLoc TL){
    if(Expr *E = TL.getUnderlyingExpr()) return Visit(MakeCXCursor(E, StmtParent, TU));
    return false;
}
...
bool CursorVisitor::VisitCXXRecordDecl(CXXRecordDecl *D){
    ...
    if(D->isCompleteDefinition()){
        for(const auto &I : D->bases()){
            if(Visit(cxcursor::MakeCursorCXXBaseSpecifier(&I, TU))) return true;
        }
    }
...
bool CursorVisitor::VisitAttributes(Decl *D){
    for(const auto *I : D->attrs())
            if(Visit(MakeCXCursor(I, D, TU))) return true;
    return false;
}
...
#undef DEF_JOB
    ...
    class ExplicitTemplateArgsVisit : public VisitorJob {
        public:
            ExplicitTemplateArgsVisit(const TemplateArgumentLoc *Begin, const TemplateArgumentLoc *End, CXCursor parent)
    ...
    class EnqueueVisitor : public ConstStmtVisitor<EnqueueVisitor, void> {
        ...
        public:
            ...
            void VisitOMPCancellationPointDirective(const OMPCancellationPointDirective *D);
            ...
            void VisitOMPTargetParallelForDirective(const OMPTargetParallelForDirective *D);
            ...
            void VisitOMPTargetParallelForSimdDirective(const OMPTargetParallelForSimdDirective *D);
            ...
            void VisitOMPTeamsDistributeParallelForDirective(const OMPTeamsDistributeParallelForDirective *D);
            void VisitOMPTargetTeamsDistributeParallelForDirective(const OMPTargetTeamsDistributeParallelForDirective *D);
            void VisitOMPTargetTeamsDistributeDirective(const OMPTargetTeamsDistributeDirective *D);
            void VisitOMPTargetTeamsDistributeSimdDirective(const OMPTargetTeamsDistributeSimdDirective *D);
            void VisitOMPTeamsDistributeParallelForSimdDirective(const OMPTeamsDistributeParallelForSimdDirective *D);

        private:
            void AddDeclarationNameInfo(const Stmt *S);
            void AddNestedNameSpecifierLoc(NestedNameSpecifierLoc Qualifier);
            void AddExplicitTemplateArgs(const TemplateArgumentLoc *A, unsigned NumTemplateArgs);
...
void EnqueueVisitor::AddNestedNameSpecifierLoc(NestedNameSpecifierLoc Qualifier){
    if(Qualifier) WL.push_back(NestedNameSpecifierLocVisit(Qualifier, Parent));
}

void EnqueueVisitor::AddStmt(const Stmt *S){
    if(S) WL.push_back(StmtVisit(S, Parent));
}
void EnqueueVisitor::AddDecl(const Decl *D, bool isFirst){
    if(D) WL.push_back(DeclVisit(D, Parent, isFirst));
}
void EnqueueVisitor::AddExplicitTemplateArgs(const TemplateArgumentLoc *A, unsigned NumTemplateArgs){
...
void EnqueueVisitor::AddMemberRef(const FieldDecl *D, SourceLocation L){
    if(D) WL.push_back(MemberRefVisit(D, L, Parent));
}
void EnqueueVisitor::AddTypeLoc(TypeSourceInfo *TI){
    if(TI) WL.push_back(TypeLocVisit(TI->getTypeLoc(), Parent));
}
void EnqueueVisitor::EnqueueChildren(const Stmt *S){
    unsigned size = WL.size();
    for(const Stmt *SubStmt : S->children()) AddStmt(SubStmt);
    if(size == WL.size()) return;
...
#include "clang/Basic/OpenMPKinds.def"
    ...
    void OMPClauseEnqueue::VisitOMPClauseWithPreInit(const OMPClauseWithPreInit *C){
        Visitor->AddStmt(C->getPreInitStmt());
    }

    void OMPClauseEnqueue::VisitOMPClauseWithPostUpdate(const OMPClauseWithPostUpdate *C){
    ...
    void OMPClauseEnqueue::VisitOMPClauseList(T *Node){
        for(const auto *I : Node->varlists()) Visitor->AddStmt(I);
    }

    void OMPClauseEnqueue::VisitOMPPrivateClause(const OMPPrivateClause *C){
        VisitOMPClauseList(C);
        for(const auto *E : C->private_copies()) Visitor->AddStmt(E);
    }
    void OMPClauseEnqueue::VisitOMPFirstprivateClause(const OMPFirstprivateClause *C){
        VisitOMPClauseList(C);
        VisitOMPClauseWithPreInit(C);
        for(const auto *E : C->private_copies()) Visitor->AddStmt(E);
        for(const auto *E : C->inits()) Visitor->AddStmt(E);
    }
    void OMPClauseEnqueue::VisitOMPLastprivateClause(const OMPLastprivateClause *C){
        VisitOMPClauseList(C);
        VisitOMPClauseWithPostUpdate(C);
        for(auto *E : C->private_copies()) Visitor->AddStmt(E);
        for(auto *E : C->source_exprs()) Visitor->AddStmt(E);
        for(auto *E : C->destination_exprs()) Visitor->AddStmt(E);
        for(auto *E : C->assignment_ops()) Visitor->AddStmt(E);
    }
    ...
    void OMPClauseEnqueue::VisitOMPReductionClause(const OMPReductionClause *C){
        VisitOMPClauseList(C);
        VisitOMPClauseWithPostUpdate(C);
        for(auto *E : C->privates()) Visitor->AddStmt(E);
        for(auto *E : C->lhs_exprs()) Visitor->AddStmt(E);
        for(auto *E : C->rhs_exprs()) Visitor->AddStmt(E);
        for(auto *E : C->reduction_ops()) Visitor->AddStmt(E);
    }
    void OMPClauseEnqueue::VisitOMPLinearClause(const OMPLinearClause *C){
        VisitOMPClauseList(C);
        VisitOMPClauseWithPostUpdate(C);
        for(const auto *E : C->privates()) Visitor->AddStmt(E);
        for(const auto *E : C->inits()) Visitor->AddStmt(E);
        for(const auto *E : C->updates()) Visitor->AddStmt(E);
        for(const auto *E : C->finals()) Visitor->AddStmt(E);
...
void EnqueueVisitor::VisitCompoundStmt(const CompoundStmt *S){
    for(auto &I : llvm::reverse(S->body())) AddStmt(I);
}
void EnqueueVisitor::VisitMSDependentExistsStmt(const MSDependentExistsStmt *S){
    AddStmt(S->getSubStmt());
    AddDeclarationNameInfo(S);
    if(NestedNameSpecifierLoc QualifierLoc = S->getQualifierLoc()) AddNestedNameSpecifierLoc(QualifierLoc);
}

void EnqueueVisitor::VisitCXXDependentScopeMemberExpr(const CXXDependentScopeMemberExpr *E){
    if(E->hasExplicitTemplateArgs()) AddExplicitTemplateArgs(E->getTemplateArgs(), E->getNumTemplateArgs());
    AddDeclarationNameInfo(E);
    if(NestedNameSpecifierLoc QualifierLoc = E->getQualifierLoc()) AddNestedNameSpecifierLoc(QualifierLoc);
    if(! E->isImplicitAccess()) AddStmt(E->getBase());
}
void EnqueueVisitor::VisitCXXNewExpr(const CXXNewExpr *E){
    ...
    for(unsigned I = E->getNumPlacementArgs(); I > 0; -- I) AddStmt(E->getPlacementArg(I - 1));
}
void EnqueueVisitor::VisitCXXOperatorCallExpr(const CXXOperatorCallExpr *CE){
    for(unsigned I = CE->getNumArgs(); I > 1 /* Yes, this is 1 */; -- I) AddStmt(CE->getArg(I - 1));
    AddStmt(CE->getCallee());
    AddStmt(CE->getArg(0));
}
void EnqueueVisitor::VisitCXXPseudoDestructorExpr(const CXXPseudoDestructorExpr *E){
...
void EnqueueVisitor::VisitCXXScalarValueInitExpr(const CXXScalarValueInitExpr *E){
    AddTypeLoc(E->getTypeSourceInfo());
}
void EnqueueVisitor::VisitCXXTemporaryObjectExpr(const CXXTemporaryObjectExpr *E){
    EnqueueChildren(E);
    AddTypeLoc(E->getTypeSourceInfo());
}
void EnqueueVisitor::VisitCXXTypeidExpr(const CXXTypeidExpr *E){
    EnqueueChildren(E);
    if(E->isTypeOperand()) AddTypeLoc(E->getTypeOperandSourceInfo());
}

void EnqueueVisitor::VisitCXXUnresolvedConstructExpr(const CXXUnresolvedConstructExpr *E){
    EnqueueChildren(E);
    AddTypeLoc(E->getTypeSourceInfo());
}
...
void EnqueueVisitor::VisitDeclRefExpr(const DeclRefExpr *DR){
    if(DR->hasExplicitTemplateArgs()) AddExplicitTemplateArgs(DR->getTemplateArgs(), DR->getNumTemplateArgs());
    WL.push_back(DeclRefExprParts(DR, Parent));
}
void EnqueueVisitor::VisitDependentScopeDeclRefExpr(const DependentScopeDeclRefExpr *E){
    if(E->hasExplicitTemplateArgs()) AddExplicitTemplateArgs(E->getTemplateArgs(), E->getNumTemplateArgs());
    AddDeclarationNameInfo(E);
...
void EnqueueVisitor::VisitDesignatedInitExpr(const DesignatedInitExpr *E){
    ...
    for(const DesignatedInitExpr::Designator &D :
        ...
        if(D.isFieldDesignator()){
            if(FieldDecl *Field = D.getField()) AddMemberRef(Field, D.getFieldLoc());
            continue;
        }
...
void EnqueueVisitor::VisitInitListExpr(const InitListExpr *IE){
    ...
    if(InitListExpr *Syntactic = IE->getSyntacticForm()) IE = Syntactic;
    EnqueueChildren(IE);
}
...
void EnqueueVisitor::VisitUnaryExprOrTypeTraitExpr(const UnaryExprOrTypeTraitExpr *E){
    EnqueueChildren(E);
    if(E->isArgumentType()) AddTypeLoc(E->getArgumentTypeInfo());
}
...
void EnqueueVisitor::VisitTypeTraitExpr(const TypeTraitExpr *E){
    for(unsigned I = E->getNumArgs(); I > 0; -- I) AddTypeLoc(E->getArg(I - 1));
}
...
void EnqueueVisitor::VisitUnresolvedMemberExpr(const UnresolvedMemberExpr *U){
    VisitOverloadExpr(U);
    if(! U->isImplicitAccess()) AddStmt(U->getBase());
}
...
void EnqueueVisitor::VisitOpaqueValueExpr(const OpaqueValueExpr *E){
    ...
    if(Expr *SourceExpr = E->getSourceExpr()) return Visit(SourceExpr);
}
...
void EnqueueVisitor::VisitOMPExecutableDirective(const OMPExecutableDirective *D){
    EnqueueChildren(D);
    for(ArrayRef<OMPClause *>::iterator I = D->clauses().begin()
       , E = D->clauses().end();
        I != E; ++ I) EnqueueChildren(*I);
}
...
void EnqueueVisitor::VisitOMPParallelForDirective(const OMPParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPParallelForSimdDirective(const OMPParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPParallelSectionsDirective(const OMPParallelSectionsDirective *D){
    VisitOMPExecutableDirective(D);
}
...
void EnqueueVisitor::VisitOMPTaskyieldDirective(const OMPTaskyieldDirective *D){
    VisitOMPExecutableDirective(D);
}
...
void EnqueueVisitor::VisitOMPTaskgroupDirective(const OMPTaskgroupDirective *D){
    VisitOMPExecutableDirective(D);
}
...
void EnqueueVisitor::VisitOMPTargetEnterDataDirective(const OMPTargetEnterDataDirective *D){
    VisitOMPExecutableDirective(D);
}

void EnqueueVisitor::VisitOMPTargetExitDataDirective(const OMPTargetExitDataDirective *D){
    VisitOMPExecutableDirective(D);
}

void EnqueueVisitor::VisitOMPTargetParallelDirective(const OMPTargetParallelDirective *D){
    VisitOMPExecutableDirective(D);
}

void EnqueueVisitor::VisitOMPTargetParallelForDirective(const OMPTargetParallelForDirective *D){
    VisitOMPLoopDirective(D);
}
...
void EnqueueVisitor::VisitOMPCancellationPointDirective(const OMPCancellationPointDirective *D){
    VisitOMPExecutableDirective(D);
}
...
void EnqueueVisitor::VisitOMPTaskLoopSimdDirective(const OMPTaskLoopSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPDistributeDirective(const OMPDistributeDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPDistributeParallelForDirective(const OMPDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPDistributeParallelForSimdDirective(const OMPDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPDistributeSimdDirective(const OMPDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetParallelForSimdDirective(const OMPTargetParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetSimdDirective(const OMPTargetSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTeamsDistributeDirective(const OMPTeamsDistributeDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTeamsDistributeSimdDirective(const OMPTeamsDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetTeamsDirective(const OMPTargetTeamsDirective *D){
    VisitOMPExecutableDirective(D);
}

void EnqueueVisitor::VisitOMPTeamsDistributeParallelForDirective(const OMPTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetTeamsDistributeParallelForDirective(const OMPTargetTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetTeamsDistributeDirective(const OMPTargetTeamsDistributeDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTargetTeamsDistributeSimdDirective(const OMPTargetTeamsDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void EnqueueVisitor::VisitOMPTeamsDistributeParallelForSimdDirective(const OMPTeamsDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}
        ...
        switch(LI.getKind()){
            case VisitorJob::DeclVisitKind: {
                ...
                if(Visit(MakeCXCursor(D, TU, RegionOfInterest
                                     , cast<DeclVisit>(&LI)->isFirst()))) return true;
                continue;
            }
            case VisitorJob::ExplicitTemplateArgsVisitKind: {
                for(const TemplateArgumentLoc &Arg :
                    *cast<ExplicitTemplateArgsVisit>(&LI)){
                    if(VisitTemplateArgumentLoc(Arg)) return true;
                }
                continue;
            }
            case VisitorJob::TypeLocVisitKind: {
                // Perform default visitation for TypeLocs.
                if(Visit(cast<TypeLocVisit>(&LI)->get())) return true;
                continue;
            }
            ...
            case VisitorJob::NestedNameSpecifierLocVisitKind: {
                ...
                if(VisitNestedNameSpecifierLoc(V->get())) return true;
                continue;
            }
            ...
            case VisitorJob::MemberRefVisitKind: {
                ...
                if(Visit(MakeCursorMemberRef(V->get(), V->getLoc(), TU))) return true;
                continue;
            }
            case VisitorJob::StmtVisitKind: {
                ...
                if(! IsInRegionOfInterest(Cursor)) continue;
                switch(Visitor(Cursor, Parent, ClientData)){
                    case CXChildVisit_Break: return true;
                    case CXChildVisit_Continue: break;
                    case CXChildVisit_Recurse:
                        if(PostChildrenVisitor) WL.push_back(PostChildrenVisit(nullptr, Cursor));
            ...
            case VisitorJob::MemberExprPartsKind: {
                ...
                if(NestedNameSpecifierLoc QualifierLoc = M->getQualifierLoc())
                        if(VisitNestedNameSpecifierLoc(QualifierLoc)) return true;
                // Visit the declaration name.
                if(VisitDeclarationNameInfo(M->getMemberNameInfo())) return true;
            ...
            case VisitorJob::DeclRefExprPartsKind: {
                ...
                if(VisitDeclarationNameInfo(DR->getNameInfo())) return true;
                continue;
            }
            case VisitorJob::OverloadExprPartsKind: {
                ...
                if(NestedNameSpecifierLoc QualifierLoc = O->getQualifierLoc())
                        if(VisitNestedNameSpecifierLoc(QualifierLoc)) return true;
                // Visit the declaration name.
                if(VisitDeclarationNameInfo(O->getNameInfo())) return true;
                // Visit the overloaded declaration reference.
                if(Visit(MakeCursorOverloadedDeclRef(O, TU))) return true;
                continue;
            }
            ...
            case VisitorJob::LambdaExprPartsKind: {
                ...
                if(E->hasExplicitParameters() || E->hasExplicitResultType()){
                    ...
                    if(E->hasExplicitParameters() && E->hasExplicitResultType()){
                        ...
                        if(Visit(TL)) return true;
                    }else if(FunctionProtoTypeLoc Proto =
                        TL.getAs<FunctionProtoTypeLoc>()){
                        if(E->hasExplicitParameters()){
                            // Visit parameters.
                            for(unsigned I = 0, N = Proto.getNumParams(); I != N; ++ I)
                                    if(Visit(MakeCXCursor(Proto.getParam(I), TU))) return true;
                        }else{
                            // Visit result type.
                            if(Visit(Proto.getReturnLoc())) return true;
                        }
                    }
                }
                break;
            ...
            case VisitorJob::PostChildrenVisitKind:
                if(PostChildrenVisitor(Parent, ClientData)) return true;
                break;
        }
    }
    return false;
...
namespace {
    ...
    RefNamePieces buildPieces(unsigned NameFlags, bool IsMemberRefExpr, const DeclarationNameInfo &NI, SourceRange QLoc, const SourceRange *TemplateArgsLoc = nullptr){
        ...
        if(WantQualifier && QLoc.isValid()) Pieces.push_back(QLoc);
        if(Kind != DeclarationName::CXXOperatorName || IsMemberRefExpr) Pieces.push_back(NI.getLoc());
        if(WantTemplateArgs && TemplateArgsLoc && TemplateArgsLoc->isValid()) Pieces.push_back(*TemplateArgsLoc);
...
static void fatal_error_handler(void *user_data, const std::string &reason, bool gen_crash_diag){
...
extern "C" {
    CXIndex clang_createIndex(int excludeDeclarationsFromPCH, int displayDiagnostics){
        ...
        // enable it.
        if(! getenv("LIBCLANG_DISABLE_CRASH_RECOVERY")) llvm::CrashRecoveryContext::Enable();
        ...
        if(excludeDeclarationsFromPCH) CIdxr->setOnlyLocalDecls();
        if(displayDiagnostics) CIdxr->setDisplayDiagnostics();
        if(getenv("LIBCLANG_BGPRIO_INDEX")) CIdxr->setCXGlobalOptFlags(CIdxr->getCXGlobalOptFlags() |
                    CXGlobalOpt_ThreadBackgroundPriorityForIndexing);
        if(getenv("LIBCLANG_BGPRIO_EDIT")) CIdxr->setCXGlobalOptFlags(CIdxr->getCXGlobalOptFlags() |
                    CXGlobalOpt_ThreadBackgroundPriorityForEditing);
        return CIdxr;
    }

    void clang_disposeIndex(CXIndex CIdx){
        if(CIdx) delete static_cast<CIndexer *>(CIdx);
    }

    void clang_CXIndex_setGlobalOptions(CXIndex CIdx, unsigned options){
        if(CIdx) static_cast<CIndexer *>(CIdx)->setCXGlobalOptFlags(options);
    }

    unsigned clang_CXIndex_getGlobalOptions(CXIndex CIdx){
        if(CIdx) return static_cast<CIndexer *>(CIdx)->getCXGlobalOptFlags();
        return 0;
    }

    void clang_toggleCrashRecovery(unsigned isEnabled){
        if(isEnabled) llvm::CrashRecoveryContext::Enable();
        else llvm::CrashRecoveryContext::Disable();
    }
    ...
    enum CXErrorCode clang_createTranslationUnit2(CXIndex CIdx, const char *ast_filename, CXTranslationUnit *out_TU){
        if(out_TU) *out_TU = nullptr;
        if(! CIdx || ! ast_filename || ! out_TU) return CXError_InvalidArguments;
        LOG_FUNC_SECTION {
            *Log << ast_filename;
        }
    ...
    CXTranslationUnit clang_createTranslationUnitFromSourceFile(CXIndex CIdx, const char *source_filename, int num_command_line_args, const char *const *command_line_args, unsigned num_unsaved_files, struct CXUnsavedFile *unsaved_files){
    ...
    static CXErrorCode clang_parseTranslationUnit_Impl(CXIndex CIdx, const char *source_filename, const char *const *command_line_args, int num_command_line_args, ArrayRef<CXUnsavedFile> unsaved_files, unsigned options, CXTranslationUnit *out_TU){
        // Set up the initial return values.
        if(out_TU) *out_TU = nullptr;
        // Check arguments.
        if(! CIdx || ! out_TU) return CXError_InvalidArguments;
        CIndexer *CXXIdx = static_cast<CIndexer *>(CIdx);
        if(CXXIdx->isOptEnabled(CXGlobalOpt_ThreadBackgroundPriorityForIndexing)) setThreadBackgroundPriority();
        ...
        if(options & CXTranslationUnit_KeepGoing) Diags->setFatalsAsError(true);
        ...
        Args->insert(Args->end(), command_line_args
                    , command_line_args + num_command_line_args);
        if(! FoundSpellCheckingArgument) Args->insert(Args->begin() + 1, "-fno-spell-checking");
        // The 'source_filename' argument is optional.  If the caller does not
        // specify it then it is assumed that the source file is specified
        // in the actual argument list.
        // Put the source file after command_line_args otherwise if '-x' flag is
        // present it will be unused.
        if(source_filename) Args->push_back(source_filename);
        ...
        if(! Unit && ! ErrUnit) return CXError_ASTReadError;
        if(NumErrors != Diags->getClient()->getNumErrors()){
            // Make sure to check that 'Unit' is non-NULL.
            if(CXXIdx->getDisplayDiagnostics()) printDiagsToStderr(Unit ? Unit.get() : ErrUnit.get());
        }
        if(isASTReadError(Unit ? Unit.get() : ErrUnit.get())) return CXError_ASTReadError;
    ...
    enum CXErrorCode clang_parseTranslationUnit2(
        CXIndex CIdx, const char *source_filename, const char *const *command_line_args, int num_command_line_args, struct CXUnsavedFile *unsaved_files, unsigned num_unsaved_files, unsigned options, CXTranslationUnit *out_TU){
    ...
    enum CXErrorCode clang_parseTranslationUnit2FullArgv(
        ...
        LOG_FUNC_SECTION {
            *Log << source_filename << ": ";
            for(int i = 0; i != num_command_line_args; ++ i) *Log << command_line_args[i] << " ";
        }
        if(num_unsaved_files && ! unsaved_files) return CXError_InvalidArguments;
        ...
        if(! RunSafely(CRC, ParseTranslationUnitImpl)){
            ...
            if(CXTranslationUnit *TU = out_TU) PrintLibclangResourceUsage(*TU);
        }
        return result;
    }
    ...
    static const IdentifierInfo *getMacroIdentifier(CXCursor C){
        if(C.kind == CXCursor_MacroDefinition){
            if(const MacroDefinitionRecord *MDR = getCursorMacroDefinition(C)) return MDR->getName();
    ...
    unsigned clang_Cursor_isMacroFunctionLike(CXCursor C){
        ...
        if(const MacroInfo *MI = PP.getMacroInfo(II)) return MI->isFunctionLike();
        return false;
    }
    ...
    unsigned clang_Cursor_isMacroBuiltin(CXCursor C){
        ...
        ASTUnit			*ASTU	= getCursorASTUnit(C);
        Preprocessor	&PP		= ASTU->getPreprocessor();
        if(const MacroInfo *MI = PP.getMacroInfo(II)) return MI->isBuiltinMacro();
        return false;
    }
    ...
    static StringLiteral *getCFSTR_value(CallExpr *callExpr){
        if(callExpr->getNumArgs() != 1) return nullptr;
        ...
        if(arg->getStmtClass() == Stmt::ImplicitCastExprClass){
            ...
            if(subExpr->getStmtClass() != Stmt::StringLiteralClass) return nullptr;
            S = static_cast<StringLiteral *>(I->getSubExprAsWritten());
        }else if(arg->getStmtClass() == Stmt::StringLiteralClass) S = static_cast<StringLiteral *>(callExpr->getArg(0));
        else return nullptr;
        return S;
    }
    ...
    struct ExprEvalResult {
        ...
        ~ExprEvalResult(){
            if(EvalType != CXEval_UnExposed && EvalType != CXEval_Float &&
                EvalType != CXEval_Int) delete EvalData.stringVal;
        }
    };
    ...
    static const ExprEvalResult *evaluateExpr(Expr *expr, CXCursor C){
        ...
        if(! expr->EvaluateAsRValue(ER, ctx)) return nullptr;
        QualType	rettype;
        CallExpr	*callExpr;
        ...
        if(expr->getStmtClass() == Stmt::CStyleCastExprClass){
            ...
            if(rettype->isVectorType() || callExpr->getNumArgs() > 1) return nullptr;
            if(rettype->isIntegralType(ctx) || rettype->isRealFloatingType()){
                if(callExpr->getNumArgs() == 1 &&
                    ! callExpr->getArg(0)->getType()->isIntegralType(ctx)) return nullptr;
    ...
    CXEvalResult clang_Cursor_Evaluate(CXCursor C){
        ...
        if(D){
            ...
            if(auto *Var = dyn_cast<VarDecl>(D)) expr = Var->getInit();
            else if(auto *Field = dyn_cast<FieldDecl>(D)) expr = Field->getInClassInitializer();
            if(expr) return const_cast<CXEvalResult>(reinterpret_cast<const void *>(
        ...
        if(compoundStmt){
            ...
            for(auto *bodyIterator : compoundStmt->body()){
                if((expr = dyn_cast<Expr>(bodyIterator))) break;
            }
            if(expr) return const_cast<CXEvalResult>(
    ...
    static CXSaveError clang_saveTranslationUnit_Impl(CXTranslationUnit TU, const char *FileName, unsigned options){
    ...
    int clang_saveTranslationUnit(CXTranslationUnit TU, const char *FileName, unsigned options){
        ...
        if(! CXXUnit->hasSema()) return CXSaveError_InvalidTU;
        CXSaveError result;
        ...
        if(! CXXUnit->getDiagnostics().hasUnrecoverableErrorOccurred() ||
            ...
            if(getenv("LIBCLANG_RESOURCE_USAGE")) PrintLibclangResourceUsage(TU);
            return result;
        }
        ...
        }else if(getenv("LIBCLANG_RESOURCE_USAGE")) PrintLibclangResourceUsage(TU);
        return result;
    }
    ...
    void clang_disposeTranslationUnit(CXTranslationUnit CTUnit){
        if(CTUnit){
            ...
            if(Unit && Unit->isUnsafeToFree()) return;
    ...
    static CXErrorCode clang_reparseTranslationUnit_Impl(CXTranslationUnit TU, ArrayRef<CXUnsavedFile> unsaved_files, unsigned options){
        // Check arguments.
        if(isNotUsableTU(TU)){
            LOG_BAD_TU(TU);
        ...
        CIndexer *CXXIdx = TU->CIdx;
        if(CXXIdx->isOptEnabled(CXGlobalOpt_ThreadBackgroundPriorityForEditing)) setThreadBackgroundPriority();
        ASTUnit						*CXXUnit = cxtu::getASTUnit(TU);
        ASTUnit::ConcurrencyCheck	Check(*CXXUnit);
        ...
        if(! CXXUnit->Reparse(CXXIdx->getPCHContainerOperations()
                             , *RemappedFiles.get())) return CXError_Success;
        if(isASTReadError(CXXUnit)) return CXError_ASTReadError;
...
extern "C" {
    CXString clang_getFileName(CXFile SFile){
        if(! SFile) return cxstring::createNull();
...
static const Decl *getDeclFromExpr(const Stmt *E){
    if(const ImplicitCastExpr *CE = dyn_cast<ImplicitCastExpr>(E)) return getDeclFromExpr(CE->getSubExpr());
    if(const DeclRefExpr *RefExpr = dyn_cast<DeclRefExpr>(E)) return RefExpr->getDecl();
    if(const MemberExpr *ME = dyn_cast<MemberExpr>(E)) return ME->getMemberDecl();
    if(const ObjCIvarRefExpr *RE = dyn_cast<ObjCIvarRefExpr>(E)) return RE->getDecl();
    if(const ObjCPropertyRefExpr *PRE = dyn_cast<ObjCPropertyRefExpr>(E)){
        if(PRE->isExplicitProperty()) return PRE->getExplicitProperty();
        ...
        if(PRE->isMessagingSetter()) return PRE->getImplicitPropertySetter();
        return PRE->getImplicitPropertyGetter();
    }
    if(const PseudoObjectExpr *POE = dyn_cast<PseudoObjectExpr>(E)) return getDeclFromExpr(POE->getSyntacticForm());
    if(const OpaqueValueExpr *OVE = dyn_cast<OpaqueValueExpr>(E))
            if(Expr *Src = OVE->getSourceExpr()) return getDeclFromExpr(Src);
    if(const CallExpr *CE = dyn_cast<CallExpr>(E)) return getDeclFromExpr(CE->getCallee());
    if(const CXXConstructExpr *CE = dyn_cast<CXXConstructExpr>(E))
            if(! CE->isElidable()) return CE->getConstructor();
    if(const CXXInheritedCtorInitExpr *CE =
        dyn_cast<CXXInheritedCtorInitExpr>(E)) return CE->getConstructor();
    if(const ObjCMessageExpr *OME = dyn_cast<ObjCMessageExpr>(E)) return OME->getMethodDecl();
    if(const ObjCProtocolExpr *PE = dyn_cast<ObjCProtocolExpr>(E)) return PE->getProtocol();
    if(const SubstNonTypeTemplateParmPackExpr *NTTP
            = dyn_cast<SubstNonTypeTemplateParmPackExpr>(E)) return NTTP->getParameterPack();
    if(const SizeOfPackExpr *SizeOfPack = dyn_cast<SizeOfPackExpr>(E))
            if(isa<NonTypeTemplateParmDecl>(SizeOfPack->getPack()) ||
                isa<ParmVarDecl>(SizeOfPack->getPack())) return SizeOfPack->getPack();
    return nullptr;
}

static SourceLocation getLocationFromExpr(const Expr *E){
    if(const ImplicitCastExpr *CE = dyn_cast<ImplicitCastExpr>(E)) return getLocationFromExpr(CE->getSubExpr());
    if(const ObjCMessageExpr *Msg = dyn_cast<ObjCMessageExpr>(E)) return /*FIXME:*/ Msg->getLeftLoc();
    if(const DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(E)) return DRE->getLocation();
    if(const MemberExpr *Member = dyn_cast<MemberExpr>(E)) return Member->getMemberLoc();
    if(const ObjCIvarRefExpr *Ivar = dyn_cast<ObjCIvarRefExpr>(E)) return Ivar->getLocation();
    if(const SizeOfPackExpr *SizeOfPack = dyn_cast<SizeOfPackExpr>(E)) return SizeOfPack->getPackLoc();
    if(const ObjCPropertyRefExpr *PropRef = dyn_cast<ObjCPropertyRefExpr>(E)) return PropRef->getLocation();
...
// define and call the block manually, so the
    ...
    static CXString getDeclSpelling(const Decl *D){
        if(! D) return cxstring::createEmpty();
        const NamedDecl *ND = dyn_cast<NamedDecl>(D);
        if(! ND){
            if(const ObjCPropertyImplDecl *PropImpl =
                dyn_cast<ObjCPropertyImplDecl>(D))
                    if(ObjCPropertyDecl *Property = PropImpl->getPropertyDecl()) return cxstring::createDup(Property->getIdentifier()->getName());
            if(const ImportDecl *ImportD = dyn_cast<ImportDecl>(D))
                    if(Module *Mod = ImportD->getImportedModule()) return cxstring::createDup(Mod->getFullModuleName());
            return cxstring::createEmpty();
        }
        if(const ObjCMethodDecl *OMD = dyn_cast<ObjCMethodDecl>(ND)) return cxstring::createDup(OMD->getSelector().getAsString());
        ...
        if(isa<UsingDirectiveDecl>(D)) return cxstring::createEmpty();
        SmallString<1024>			S;
    ...
    CXString clang_getCursorSpelling(CXCursor C){
        if(clang_isTranslationUnit(C.kind)) return clang_getTranslationUnitSpelling(getCursorTU(C));
            ...
            switch(C.kind){
                ...
                case CXCursor_OverloadedDeclRef: {
                    ...
                    if(const Decl *D = Storage.dyn_cast<const Decl *>()){
                        if(const NamedDecl *ND = dyn_cast<NamedDecl>(D)) return cxstring::createDup(ND->getNameAsString());
                        return cxstring::createEmpty();
                    }
                    if(const OverloadExpr *E = Storage.dyn_cast<const OverloadExpr *>()) return cxstring::createDup(E->getName().getAsString());
                    OverloadedTemplateStorage *Ovl
                        = Storage.get<OverloadedTemplateStorage *>();
                    if(Ovl->size() == 0) return cxstring::createEmpty();
        ...
        if(clang_isExpression(C.kind)){
            ...
            if(C.kind == CXCursor_ObjCStringLiteral ||
                ...
                if(const ObjCStringLiteral *OSL = dyn_cast<ObjCStringLiteral>(E)) SLit = OSL->getString();
                else SLit = cast<StringLiteral>(E);
                SmallString<256>			Buf;
            ...
            if(D) return getDeclSpelling(D);
            return cxstring::createEmpty();
        }
        if(clang_isStatement(C.kind)){
            const Stmt *S = getCursorStmt(C);
            if(const LabelStmt *Label = dyn_cast_or_null<LabelStmt>(S)) return cxstring::createRef(Label->getName());
            return cxstring::createEmpty();
        }
        if(C.kind == CXCursor_MacroExpansion) return cxstring::createRef(getCursorMacroExpansion(C).getName()
                           ->getNameStart());
        if(C.kind == CXCursor_MacroDefinition) return cxstring::createRef(getCursorMacroDefinition(C)->getName()
                           ->getNameStart());
        if(C.kind == CXCursor_InclusionDirective) return cxstring::createDup(getCursorInclusionDirective(C)->getFileName());
        if(clang_isDeclaration(C.kind)) return getDeclSpelling(getCursorDecl(C));
        ...
        if(C.kind == CXCursor_PackedAttr) return cxstring::createRef("packed");
    ...
    CXSourceRange clang_Cursor_getSpellingNameRange(CXCursor C, unsigned pieceIndex, unsigned options){
        if(clang_Cursor_isNull(C)) return clang_getNullRange();
        ASTContext &Ctx = getCursorContext(C);
        if(clang_isStatement(C.kind)){
            const Stmt *S = getCursorStmt(C);
            if(const LabelStmt *Label = dyn_cast_or_null<LabelStmt>(S)){
                if(pieceIndex > 0) return clang_getNullRange();
        ...
        if(C.kind == CXCursor_ObjCMessageExpr){
            if(const ObjCMessageExpr *
                ...
                if(pieceIndex >= ME->getNumSelectorLocs()) return clang_getNullRange();
        ...
        if(C.kind == CXCursor_ObjCInstanceMethodDecl ||
            ...
            if(const ObjCMethodDecl *
                ...
                if(pieceIndex >= MD->getNumSelectorLocs()) return clang_getNullRange();
        ...
        if(C.kind == CXCursor_ObjCCategoryDecl ||
            ...
            if(const ObjCCategoryDecl *
                CD = dyn_cast_or_null<ObjCCategoryDecl>(getCursorDecl(C))) return cxloc::translateSourceRange(Ctx, CD->getCategoryNameLoc());
            if(const ObjCCategoryImplDecl *
                CID = dyn_cast_or_null<ObjCCategoryImplDecl>(getCursorDecl(C))) return cxloc::translateSourceRange(Ctx, CID->getCategoryNameLoc());
        }
        if(C.kind == CXCursor_ModuleImportDecl){
            ...
            if(const ImportDecl *ImportD =
                ...
                if(! Locs.empty()) return cxloc::translateSourceRange(Ctx
    ...
    CXString clang_Cursor_getMangling(CXCursor C){
        if(clang_isInvalid(C.kind) || ! clang_isDeclaration(C.kind)) return cxstring::createEmpty();
        // Mangling only works for functions and variables.
        const Decl *D = getCursorDecl(C);
        if(! D || ! (isa<FunctionDecl>(D) || isa<VarDecl>(D))) return cxstring::createEmpty();
    ...
    CXStringSet *clang_Cursor_getCXXManglings(CXCursor C){
        if(clang_isInvalid(C.kind) || ! clang_isDeclaration(C.kind)) return nullptr;
        const Decl *D = getCursorDecl(C);
        if(! (isa<CXXRecordDecl>(D) || isa<CXXMethodDecl>(D))) return nullptr;
        ASTContext					&Ctx = D->getASTContext();
        index::CodegenNameGenerator CGNameGen(Ctx);
    ...
    CXString clang_getCursorDisplayName(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return clang_getCursorSpelling(C);
        const Decl *D = getCursorDecl(C);
        if(! D) return cxstring::createEmpty();
        PrintingPolicy Policy = getCursorContext(C).getPrintingPolicy();
        if(const FunctionTemplateDecl *FunTmpl = dyn_cast<FunctionTemplateDecl>(D)) D = FunTmpl->getTemplatedDecl();
        if(const FunctionDecl *Function = dyn_cast<FunctionDecl>(D)){
            SmallString<64>				Str;
            llvm::raw_svector_ostream	OS(Str);
            OS << *Function;
            if(Function->getPrimaryTemplate()) OS << "<>";
            OS << "(";
        ...
        if(const ClassTemplateDecl *ClassTemplate = dyn_cast<ClassTemplateDecl>(D)){
            ...
            for(unsigned I = 0, N = Params->size(); I != N; ++ I){
                ...
                if(TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(Param)) OS << (TTP->wasDeclaredWithTypename() ? "typename" : "class");
                else if(NonTypeTemplateParmDecl *NTTP
                        = dyn_cast<NonTypeTemplateParmDecl>(Param)) OS << NTTP->getType().getAsString(Policy);
                else OS << "template<...> class";
            }
            OS << ">";
            return cxstring::createDup(OS.str());
        }
        if(const ClassTemplateSpecializationDecl *ClassSpec
            ...
            if(TypeSourceInfo *TSInfo = ClassSpec->getTypeAsWritten()) return cxstring::createDup(TSInfo->getType().getAsString(Policy));
            SmallString<128>			Str;
            llvm::raw_svector_ostream	OS(Str);
            OS << *ClassSpec;
    ...
    struct GetCursorData {
        ...
        GetCursorData(SourceManager &SM, SourceLocation tokenBegin, CXCursor &outputCursor)
    ...
    static enum CXChildVisitResult GetCursorVisitor(CXCursor cursor, CXCursor parent, CXClientData client_data){
        ...
        // cursor.
        if(cursor.kind == CXCursor_MacroExpansion && Data->PointsAtMacroArgExpansion) return CXChildVisit_Recurse;
        if(clang_isDeclaration(cursor.kind)){
            // Avoid having the implicit methods override the property decls.
            if(const ObjCMethodDecl *MD
                    = dyn_cast_or_null<ObjCMethodDecl>(getCursorDecl(cursor))){
                if(MD->isImplicit()) return CXChildVisit_Break;
                    ...
                    BestCursor->kind == CXCursor_ObjCClassRef)
                        if(const ObjCInterfaceDecl *PrevID
                            ...
                            if(PrevID != ID &&
                                ...
                                ! ID->isThisDeclarationADefinition()) return CXChildVisit_Break;
                        }
                ...
                // 'Foo' even though the cursor location was at 'Foo'.
                if(Data->VisitedDeclaratorDeclStartLoc == StartLoc) return CXChildVisit_Break;
                ...
                // source ranges for both start at '@', so 'Bar' will end up overriding
                // 'Foo' even though the cursor location was at 'Foo'.
                if(Data->VisitedObjCPropertyImplDecl) return CXChildVisit_Break;
        ...
        if(clang_isExpression(cursor.kind) &&
            ...
            if(const Decl *D = getCursorDecl(*BestCursor)){
                ...
                if(D->getLocation().isValid() && Data->TokenBeginLoc.isValid() &&
                    D->getLocation() == Data->TokenBeginLoc) return CXChildVisit_Break;
            }
        }
        ...
        // update it further.
        if(BestCursor->kind == CXCursor_ObjCSuperClassRef) return CXChildVisit_Break;
        *BestCursor = cursor;
        return CXChildVisit_Recurse;
    }
    ...
    unsigned clang_equalCursors(CXCursor X, CXCursor Y){
        ...
        if(clang_isDeclaration(X.kind)) X.data[1] = nullptr;
        if(clang_isDeclaration(Y.kind)) Y.data[1] = nullptr;
        return X == Y;
    }

    unsigned clang_hashCursor(CXCursor C){
        unsigned Index = 0;
        if(clang_isExpression(C.kind) || clang_isStatement(C.kind)) Index = 1;
    ...
    CXSourceLocation clang_getCursorLocation(CXCursor C){
        if(clang_isReference(C.kind)){
            switch(C.kind){
                ...
                case CXCursor_CXXBaseSpecifier: {
                    ...
                    if(! BaseSpec) return clang_getNullLocation();
                    if(TypeSourceInfo *TSInfo = BaseSpec->getTypeSourceInfo()) return cxloc::translateSourceLocation(getCursorContext(C)
        ...
        if(clang_isExpression(C.kind)) return cxloc::translateSourceLocation(getCursorContext(C)
                                                                            , getLocationFromExpr(getCursorExpr(C)));
        if(clang_isStatement(C.kind)) return cxloc::translateSourceLocation(getCursorContext(C)
        ...
        if(! clang_isDeclaration(C.kind)) return clang_getNullLocation();
        const Decl *D = getCursorDecl(C);
        if(! D) return clang_getNullLocation();
        ...
        if(const VarDecl *VD = dyn_cast<VarDecl>(D)){
            if(! cxcursor::isFirstInDeclGroup(C)) Loc = VD->getLocation();
        }
        // For ObjC methods, give the start location of the method name.
        if(const ObjCMethodDecl *MD = dyn_cast<ObjCMethodDecl>(D)) Loc = MD->getSelectorStartLoc();
...
CXCursor cxcursor::getCursor(CXTranslationUnit TU, SourceLocation SLoc){
    ...
    if(SLoc.isInvalid()) return clang_getNullCursor();
    ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
...
static SourceRange getRawCursorExtent(CXCursor C){
    ...
    if(clang_isExpression(C.kind)) return getCursorExpr(C)->getSourceRange();
    if(clang_isStatement(C.kind)) return getCursorStmt(C)->getSourceRange();
    if(clang_isAttribute(C.kind)) return getCursorAttr(C)->getRange();
    if(C.kind == CXCursor_PreprocessingDirective) return cxcursor::getCursorPreprocessingDirective(C);
    if(C.kind == CXCursor_MacroExpansion){
    ...
    if(clang_isDeclaration(C.kind)){
        ...
        // and if so, whether it is the first decl.
        if(const VarDecl *VD = dyn_cast<VarDecl>(D)){
            if(! cxcursor::isFirstInDeclGroup(C)) R.setBegin(VD->getLocation());
        }
        return R;
    }
    return SourceRange();
...
static SourceRange getFullCursorExtent(CXCursor C, SourceManager &SrcMgr){
    if(clang_isDeclaration(C.kind)){
        ...
        if(const DeclaratorDecl *DD = dyn_cast<DeclaratorDecl>(D)){
            if(TypeSourceInfo *TI = DD->getTypeSourceInfo()) StartLoc = TI->getTypeLoc().getLocStart();
        }else if(const TypedefDecl *Typedef = dyn_cast<TypedefDecl>(D)){
            if(TypeSourceInfo *TI = Typedef->getTypeSourceInfo()) StartLoc = TI->getTypeLoc().getLocStart();
        }
        if(StartLoc.isValid() && R.getBegin().isValid() &&
            SrcMgr.isBeforeInTranslationUnit(StartLoc, R.getBegin())) R.setBegin(StartLoc);
        // FIXME: Multiple variables declared in a single declaration
        // currently lack the information needed to correctly determine their
        // ranges when accounting for the type-specifier.  We use context
        // stored in the CXCursor to determine if the VarDecl is in a DeclGroup,
...
extern "C" {
    CXSourceRange clang_getCursorExtent(CXCursor C){
        ...
        if(R.isInvalid()) return clang_getNullRange();
        return cxloc::translateSourceRange(getCursorContext(C), R);
    }

    CXCursor clang_getCursorReferenced(CXCursor C){
        if(clang_isInvalid(C.kind)) return clang_getNullCursor();
        CXTranslationUnit tu = getCursorTU(C);
        if(clang_isDeclaration(C.kind)){
            const Decl *D = getCursorDecl(C);
            if(! D) return clang_getNullCursor();
            if(const UsingDecl *Using = dyn_cast<UsingDecl>(D)) return MakeCursorOverloadedDeclRef(Using, D->getLocation(), tu);
            if(const ObjCPropertyImplDecl *PropImpl =
                dyn_cast<ObjCPropertyImplDecl>(D))
                    if(ObjCPropertyDecl *Property = PropImpl->getPropertyDecl()) return MakeCXCursor(Property, tu);
            return C;
        }
        if(clang_isExpression(C.kind)){
            const Expr	*E	= getCursorExpr(C);
            ...
            if(const OverloadExpr *Ovl = dyn_cast_or_null<OverloadExpr>(E)) return MakeCursorOverloadedDeclRef(Ovl, tu);
            return clang_getNullCursor();
        }
        if(clang_isStatement(C.kind)){
            const Stmt *S = getCursorStmt(C);
            if(const GotoStmt *Goto = dyn_cast_or_null<GotoStmt>(S))
                    if(LabelDecl *label = Goto->getLabel())
                            if(LabelStmt *labelS = label->getStmt()) return MakeCXCursor(labelS, getCursorDecl(C), tu);
            return clang_getNullCursor();
        }
        if(C.kind == CXCursor_MacroExpansion){
            if(const MacroDefinitionRecord *Def =
                getCursorMacroExpansion(C).getDefinition()) return MakeMacroDefinitionCursor(Def, tu);
        }
        if(! clang_isReference(C.kind)) return clang_getNullCursor();
        switch(C.kind){
            case CXCursor_ObjCSuperClassRef:
            ...
            case CXCursor_ObjCProtocolRef: {
                ...
                if(const ObjCProtocolDecl *Def = Prot->getDefinition()) return MakeCXCursor(Def, tu);
                return MakeCXCursor(Prot, tu);
            }

            case CXCursor_ObjCClassRef: {
                const ObjCInterfaceDecl *Class = getCursorObjCClassRef(C).first;
                if(const ObjCInterfaceDecl *Def = Class->getDefinition()) return MakeCXCursor(Def, tu);
    ...
    CXCursor clang_getCursorDefinition(CXCursor C){
        ...
        if(C.kind == CXCursor_MacroExpansion) return clang_getCursorReferenced(C);
        if(! clang_isDeclaration(C.kind)) return clang_getNullCursor();
        const Decl *D = getCursorDecl(C);
        if(! D) return clang_getNullCursor();
        switch(D->getKind()){
            ...
            case Decl::ClassTemplatePartialSpecialization:
                if(TagDecl *Def = cast<TagDecl>(D)->getDefinition()) return MakeCXCursor(Def, TU);
                return clang_getNullCursor();
            ...
            case Decl::CXXConversion: {
                ...
                if(cast<FunctionDecl>(D)->getBody(Def)) return MakeCXCursor(Def, TU);
                return clang_getNullCursor();
            }
            ...
            case Decl::Decomposition: {
                ...
                if(const VarDecl *Def = cast<VarDecl>(D)->getDefinition()) return MakeCXCursor(Def, TU);
                return clang_getNullCursor();
            }

            case Decl::FunctionTemplate: {
                const FunctionDecl *Def = nullptr;
                if(cast<FunctionTemplateDecl>(D)->getTemplatedDecl()->getBody(Def)) return MakeCXCursor(Def->getDescribedFunctionTemplate(), TU);
                return clang_getNullCursor();
            }

            case Decl::ClassTemplate: {
                if(RecordDecl *Def = cast<ClassTemplateDecl>(D)->getTemplatedDecl()
                    ->getDefinition()) return MakeCXCursor(cast<CXXRecordDecl>(Def)->getDescribedClassTemplate()
                                                          , TU);
                return clang_getNullCursor();
            }
            ...
            case Decl::VarTemplate: {
                if(VarDecl *Def =
                    cast<VarTemplateDecl>(D)->getTemplatedDecl()->getDefinition()) return MakeCXCursor(cast<VarDecl>(Def)->getDescribedVarTemplate(), TU);
                return clang_getNullCursor();
            }
            ...
            case Decl::ObjCMethod: {
                ...
                if(Method->isThisDeclarationADefinition()) return C;
                ...
                if(const ObjCInterfaceDecl *Class
                        ...
                        if(ObjCImplementationDecl *ClassImpl = Class->getImplementation())
                                if(ObjCMethodDecl *Def = ClassImpl->getMethod(Method->getSelector()
                                        ...
                                        if(Def->isThisDeclarationADefinition()) return MakeCXCursor(Def, TU);
                return clang_getNullCursor();
            }

            case Decl::ObjCCategory:
                if(ObjCCategoryImplDecl *Impl
                        = cast<ObjCCategoryDecl>(D)->getImplementation()) return MakeCXCursor(Impl, TU);
                return clang_getNullCursor();

            case Decl::ObjCProtocol:
                if(const ObjCProtocolDecl *Def = cast<ObjCProtocolDecl>(D)->getDefinition()) return MakeCXCursor(Def, TU);
                return clang_getNullCursor();
            ...
            case Decl::ObjCInterface: {
                ...
                if(WasReference){
                    if(const ObjCInterfaceDecl *Def = IFace->getDefinition()) return MakeCXCursor(Def, TU);
                }else if(ObjCImplementationDecl *Impl = IFace->getImplementation()) return MakeCXCursor(Impl, TU);
                return clang_getNullCursor();
            }
            ...
            case Decl::Friend:
                if(NamedDecl *Friend = cast<FriendDecl>(D)->getFriendDecl()) return clang_getCursorDefinition(MakeCXCursor(Friend, TU));
                return clang_getNullCursor();

            case Decl::FriendTemplate:
                if(NamedDecl *Friend = cast<FriendTemplateDecl>(D)->getFriendDecl()) return clang_getCursorDefinition(MakeCXCursor(Friend, TU));
                return clang_getNullCursor();
        }
        return clang_getNullCursor();
    }
    ...
    CXCursor clang_getCanonicalCursor(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return C;
        if(const Decl *D = getCursorDecl(C)){
            if(const ObjCCategoryImplDecl *CatImplD = dyn_cast<ObjCCategoryImplDecl>(D))
                    if(ObjCCategoryDecl *CatD = CatImplD->getCategoryDecl()) return MakeCXCursor(CatD, getCursorTU(C));
            if(const ObjCImplDecl *ImplD = dyn_cast<ObjCImplDecl>(D))
                    if(const ObjCInterfaceDecl *IFD = ImplD->getClassInterface()) return MakeCXCursor(IFD, getCursorTU(C));
    ...
    unsigned clang_getNumOverloadedDecls(CXCursor C){
        if(C.kind != CXCursor_OverloadedDeclRef) return 0;
        OverloadedDeclRefStorage Storage = getCursorOverloadedDeclRef(C).first;
        if(const OverloadExpr *E = Storage.dyn_cast<const OverloadExpr *>()) return E->getNumDecls();
        if(OverloadedTemplateStorage *S
                = Storage.dyn_cast<OverloadedTemplateStorage *>()) return S->size();
        const Decl *D = Storage.get<const Decl *>();
        if(const UsingDecl *Using = dyn_cast<UsingDecl>(D)) return Using->shadow_size();
        return 0;
    }

    CXCursor clang_getOverloadedDecl(CXCursor cursor, unsigned index){
        if(cursor.kind != CXCursor_OverloadedDeclRef) return clang_getNullCursor();
        if(index >= clang_getNumOverloadedDecls(cursor)) return clang_getNullCursor();
        CXTranslationUnit			TU		= getCursorTU(cursor);
        OverloadedDeclRefStorage	Storage = getCursorOverloadedDeclRef(cursor).first;
        if(const OverloadExpr *E = Storage.dyn_cast<const OverloadExpr *>()) return MakeCXCursor(E->decls_begin()[index], TU);
        if(OverloadedTemplateStorage *S
                = Storage.dyn_cast<OverloadedTemplateStorage *>()) return MakeCXCursor(S->begin()[index], TU);
        const Decl *D = Storage.get<const Decl *>();
    ...
    void clang_getDefinitionSpellingAndExtent(CXCursor C, const char * *startBuf, const char * *endBuf, unsigned *startLine, unsigned *startColumn, unsigned *endLine, unsigned *endColumn){
    ...
    CXSourceRange clang_getCursorReferenceNameRange(CXCursor C, unsigned NameFlags, unsigned PieceIndex){
        ...
        switch(C.kind){
            case CXCursor_MemberRefExpr:
                if(const MemberExpr *E = dyn_cast<MemberExpr>(getCursorExpr(C))) Pieces = buildPieces(NameFlags, true, E->getMemberNameInfo()
            ...
            case CXCursor_CallExpr:
                if(const CXXOperatorCallExpr *OCE =
                    ...
                    if(const ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(Callee)) Callee = ICE->getSubExpr();
                    if(const DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(Callee)) Pieces = buildPieces(NameFlags, false, DRE->getNameInfo()
        ...
        if(Pieces.empty()){
            if(PieceIndex == 0) return clang_getCursorExtent(C);
        }else if(PieceIndex < Pieces.size()){
            SourceRange R = Pieces[PieceIndex];
            if(R.isValid()) return cxloc::translateSourceRange(getCursorContext(C), R);
        }
        return clang_getNullRange();
    }
...
extern "C" {
    ...
    CXString clang_getTokenSpelling(CXTranslationUnit TU, CXToken CXTok){
        ...
        ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
        if(! CXXUnit) return cxstring::createEmpty();
        ...
        if(Invalid) return cxstring::createEmpty();
    ...
    CXSourceLocation clang_getTokenLocation(CXTranslationUnit TU, CXToken CXTok){
        ...
        ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
        if(! CXXUnit) return clang_getNullLocation();
    ...
    CXSourceRange clang_getTokenExtent(CXTranslationUnit TU, CXToken CXTok){
        ...
        ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
        if(! CXXUnit) return clang_getNullRange();
        return cxloc::translateSourceRange(CXXUnit->getASTContext()
                                          , SourceLocation::getFromRawEncoding(CXTok.int_data[1]));
    }

    static void getTokens(ASTUnit *CXXUnit, SourceRange Range, SmallVectorImpl<CXToken> &CXTokens){
        ...
        if(BeginLocInfo.first != EndLocInfo.first) return;
        // Create a lexer
        bool		Invalid = false;
        StringRef	Buffer
        ...
        do {
            ...
            if(Tok.isLiteral()){
                ...
                if((II->getObjCKeywordID() != tok::objc_not_keyword) && previousWasAt) CXTok.int_data[0] = CXToken_Keyword;
                else{
    ...
    void clang_tokenize(CXTranslationUnit TU, CXSourceRange Range, CXToken * *Tokens, unsigned *NumTokens){
        ...
        ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
        if(! CXXUnit || ! Tokens || ! NumTokens) return;
        ASTUnit::ConcurrencyCheck Check(*CXXUnit);
...
static enum CXChildVisitResult	AnnotateTokensVisitor(CXCursor cursor, CXCursor parent, CXClientData client_data);
static bool						AnnotateTokensPostChildrenVisitor(CXCursor cursor, CXClientData client_data);
...
namespace {
    class AnnotateTokensWorker {
        ...
        public:
            AnnotateTokensWorker(CXToken *tokens, CXCursor *cursors, unsigned numTokens, CXTranslationUnit TU, SourceRange RegionOfInterest)
...
static inline void updateCursorAnnotation(CXCursor &Cursor, const CXCursor &updateC){
    if(clang_isInvalid(updateC.kind) || ! clang_isInvalid(Cursor.kind)) return;
    Cursor = updateC;
}
...
void AnnotateTokensWorker::annotateAndAdvanceTokens(CXCursor updateC, RangeComparisonResult compResult, SourceRange range){
    while(MoreTokens()){
        const unsigned I = NextToken();
        if(isFunctionMacroToken(I))
                if(! annotateAndAdvanceFunctionMacroTokens(updateC, compResult, range)) return;
...
bool AnnotateTokensWorker::annotateAndAdvanceFunctionMacroTokens(
    ...
    for(; I < NumTokens && isFunctionMacroToken(I); ++ I){
        ...
        if(TokLoc.isFileID()) continue;	// not macro arg token, it's parens or comma.
        if(LocationCompare(SrcMgr, TokLoc, range) == compResult){
            if(clang_isInvalid(clang_getCursorKind(Cursors[I]))) Cursors[I] = updateC;
        }else atLeastOneCompFail = true;
    }
    if(atLeastOneCompFail) return false;
    TokIdx = I;	// All of the tokens were handled, advance beyond all of them.
    return true;
}

enum CXChildVisitResult AnnotateTokensWorker::Visit(CXCursor cursor, CXCursor parent){
    SourceRange cursorRange = getRawCursorExtent(cursor);
    if(cursorRange.isInvalid()) return CXChildVisit_Recurse;
    if(! HasContextSensitiveKeywords){
        // Objective-C properties can have context-sensitive keywords.
        if(cursor.kind == CXCursor_ObjCPropertyDecl){
            if(const ObjCPropertyDecl *Property
                    = dyn_cast_or_null<ObjCPropertyDecl>(getCursorDecl(cursor))) HasContextSensitiveKeywords = Property->getPropertyAttributesAsWritten() != 0;
        }
        // Objective-C methods can have context-sensitive keywords.
            ...
            if(const ObjCMethodDecl *Method
                    = dyn_cast_or_null<ObjCMethodDecl>(getCursorDecl(cursor))){
                if(Method->getObjCDeclQualifier()) HasContextSensitiveKeywords = true;
                else{
        ...
        // C++ methods can have context-sensitive keywords.
            ...
            if(const CXXMethodDecl *Method
                ...
                if(Method->hasAttr<FinalAttr>() || Method->hasAttr<OverrideAttr>()) HasContextSensitiveKeywords = true;
            }
        }
        // C++ classes can have context-sensitive keywords.
            ...
            if(const Decl *D = getCursorDecl(cursor))
                    if(D->hasAttr<FinalAttr>()) HasContextSensitiveKeywords = true;
        }
    }
    // Don't override a property annotation with its getter/setter method.
    if(cursor.kind == CXCursor_ObjCInstanceMethodDecl &&
        parent.kind == CXCursor_ObjCPropertyDecl) return CXChildVisit_Continue;
            ...
            switch(LocationCompare(SrcMgr, TokLoc, cursorRange)){
                ...
                case RangeOverlap:
                    ...
                    if(cursor.kind == CXCursor_MacroExpansion){
                        if(TokLoc == cursorRange.getBegin()) Cursors[I] = cursor;
                        AdvanceToken();
                        break;
                    }
                    // We may have already annotated macro names inside macro definitions.
                    if(Cursors[I].kind != CXCursor_MacroExpansion) Cursors[I] = cursor;
                    AdvanceToken();
                    continue;
            }
            break;
    ...
    if(cursorRange.isInvalid()) return CXChildVisit_Continue;
...
bool AnnotateTokensWorker::postVisitChildren(CXCursor cursor){
    if(PostChildrenInfos.empty()) return false;
    const PostChildrenInfo &Info = PostChildrenInfos.back();
    if(! clang_equalCursors(Info.Cursor, cursor)) return false;
    ...
    for(unsigned I = BeforeChildren; I != AfterChildren; ++ I){
        if(! clang_isInvalid(clang_getCursorKind(Cursors[I]))) break;
        Cursors[I] = cursor;
    }
    // Attributes are annotated out-of-order, rewind TokIdx to when we first
    // encountered the attribute cursor.
    if(clang_isAttribute(cursor.kind)) TokIdx = Info.BeforeReachingCursorIdx;
...
/// AnnotateTokensWorker.
    class MarkMacroArgTokensVisitor {
        ...
        public:
            MarkMacroArgTokensVisitor(SourceManager &SM, CXToken *tokens, unsigned numTokens)
                : SM(SM), Tokens(tokens), NumTokens(numTokens), CurIdx(0){}

            CXChildVisitResult visit(CXCursor cursor, CXCursor parent){
                if(cursor.kind != CXCursor_MacroExpansion) return CXChildVisit_Continue;
                SourceRange macroRange = getCursorMacroExpansion(cursor).getSourceRange();
                if(macroRange.getBegin() == macroRange.getEnd()) return CXChildVisit_Continue;	// it's not a function macro.
                for(; CurIdx < NumTokens; ++ CurIdx){
                    if(! SM.isBeforeInTranslationUnit(getTokenLoc(CurIdx)
                                                     , macroRange.getBegin())) break;
                }
                if(CurIdx == NumTokens) return CXChildVisit_Break;
                for(; CurIdx < NumTokens; ++ CurIdx){
                    SourceLocation tokLoc = getTokenLoc(CurIdx);
                    if(! SM.isBeforeInTranslationUnit(tokLoc, macroRange.getEnd())) break;
...
static CXChildVisitResult MarkMacroArgTokensVisitorDelegate(CXCursor cursor, CXCursor parent, CXClientData client_data){
...
static bool lexNext(Lexer &Lex, Token &Tok, unsigned &NextIdx, unsigned NumTokens){
    if(NextIdx >= NumTokens) return true;
    ++ NextIdx;
    Lex.LexFromRawLexer(Tok);
    return Tok.is(tok::eof);
}

static void annotatePreprocessorTokens(CXTranslationUnit TU, SourceRange RegionOfInterest, CXCursor *Cursors, CXToken *Tokens, unsigned NumTokens){
    ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
    ...
    if(Buffer.empty() || Invalid) return;
    Lexer Lex(SourceMgr.getLocForStartOfFile(BeginLocInfo.first)
             , CXXUnit->getASTContext().getLangOpts()
    ...
    while(true){
        Token Tok;
        if(lexNext(Lex, Tok, NextIdx, NumTokens)) break;
...
reprocess:
        if(Tok.is(tok::hash) && Tok.isAtStartOfLine()){
            ...
            for(; TokIdx <= LastIdx; ++ TokIdx) updateCursorAnnotation(Cursors[TokIdx], Cursor);
            if(finished) break;
            goto reprocess;
        }
    }
...
static void clang_annotateTokensImpl(CXTranslationUnit TU, ASTUnit *CXXUnit, CXToken *Tokens, unsigned NumTokens, CXCursor *Cursors){
    CIndexer *CXXIdx = TU->CIdx;
    if(CXXIdx->isOptEnabled(CXGlobalOpt_ThreadBackgroundPriorityForEditing)) setThreadBackgroundPriority();
    ...
    // location so we can have the full context when annotating semantically.
    {
        ...
        if(Loc.isMacroID()) RegionOfInterest.setBegin(SM.getExpansionLoc(Loc));
    }
            ...
            if(Cursors[I].kind == CXCursor_ObjCPropertyDecl){
                ...
                if(const ObjCPropertyDecl *Property
                    ...
                    if(Property->getPropertyAttributesAsWritten() != 0 &&
                        ...
                        .Case("weak", true)
                        .Case("class", true)
                        .Default(false)) Tokens[I].int_data[0] = CXToken_Keyword;
                }
                continue;
            }
...
extern "C" {
    void clang_annotateTokens(CXTranslationUnit TU, CXToken *Tokens, unsigned NumTokens, CXCursor *Cursors){
        ...
        for(unsigned I = 0; I != NumTokens; ++ I) Cursors[I] = C;
        ASTUnit *CXXUnit = cxtu::getASTUnit(TU);
        if(! CXXUnit) return;
        ASTUnit::ConcurrencyCheck Check(*CXXUnit);
        ...
        llvm::CrashRecoveryContext CRC;
        if(! RunSafely(CRC, AnnotateTokensImpl, GetSafetyThreadStackSize() * 2)) fprintf(stderr, "libclang: crash detected while annotating tokens\n");
    }
}	// end: extern "C"
...
extern "C" {
    CXLinkageKind clang_getCursorLinkage(CXCursor cursor){
        if(! clang_isDeclaration(cursor.kind)) return CXLinkage_Invalid;
...
extern "C" {
    CXVisibilityKind clang_getCursorVisibility(CXCursor cursor){
        if(! clang_isDeclaration(cursor.kind)) return CXVisibility_Invalid;
        const Decl *D = cxcursor::getCursorDecl(cursor);
        if(const NamedDecl *ND = dyn_cast_or_null<NamedDecl>(D))
...
extern "C" {
    static CXAvailabilityKind getCursorAvailabilityForDecl(const Decl *D){
        if(isa<FunctionDecl>(D) && cast<FunctionDecl>(D)->isDeleted()) return CXAvailability_NotAvailable;
        switch(D->getAvailability()){
            case AR_Available:
            case AR_NotYetIntroduced:
                if(const EnumConstantDecl *EnumConst = dyn_cast<EnumConstantDecl>(D)) return getCursorAvailabilityForDecl(
    ...
    enum CXAvailabilityKind clang_getCursorAvailability(CXCursor cursor){
        if(clang_isDeclaration(cursor.kind))
                if(const Decl *D = cxcursor::getCursorDecl(cursor)) return getCursorAvailabilityForDecl(D);
        return CXAvailability_Available;
    }
    ...
    static CXVersion convertVersion(VersionTuple In){
        ...
        if(Minor.hasValue()) Out.Minor = *Minor;
        else return Out;
        Optional<unsigned> Subminor = In.getSubminor();
        if(Subminor.hasValue()) Out.Subminor = *Subminor;
        return Out;
    }

    static int getCursorPlatformAvailabilityForDecl(const Decl *D, int *always_deprecated, CXString *deprecated_message, int *always_unavailable, CXString *unavailable_message, CXPlatformAvailability *availability, int availability_size){
        bool	HadAvailAttr	= false;
        int		N				= 0;
        for(auto A : D->attrs()){
            if(DeprecatedAttr *Deprecated = dyn_cast<DeprecatedAttr>(A)){
                HadAvailAttr = true;
                if(always_deprecated) *always_deprecated = 1;
            ...
            if(UnavailableAttr *Unavailable = dyn_cast<UnavailableAttr>(A)){
                HadAvailAttr = true;
                if(always_unavailable) *always_unavailable = 1;
    ...
    int clang_getCursorPlatformAvailability(CXCursor cursor, int *always_deprecated, CXString *deprecated_message, int *always_unavailable, CXString *unavailable_message, CXPlatformAvailability *availability, int availability_size){
        if(always_deprecated) *always_deprecated = 0;
        if(deprecated_message) *deprecated_message = cxstring::createEmpty();
        if(always_unavailable) *always_unavailable = 0;
        if(unavailable_message) *unavailable_message = cxstring::createEmpty();
        if(! clang_isDeclaration(cursor.kind)) return 0;
        const Decl *D = cxcursor::getCursorDecl(cursor);
        if(! D) return 0;
    ...
    CXLanguageKind clang_getCursorLanguage(CXCursor cursor){
        if(clang_isDeclaration(cursor.kind)) return getDeclLanguage(cxcursor::getCursorDecl(cursor));
    ...
    static const Decl *maybeGetTemplateCursor(const Decl *D){
        ...
        if(const FunctionDecl *FD = dyn_cast<FunctionDecl>(D))
                if(FunctionTemplateDecl *FunTmpl = FD->getDescribedFunctionTemplate()) return FunTmpl;
        if(const CXXRecordDecl *RD = dyn_cast<CXXRecordDecl>(D))
                if(ClassTemplateDecl *ClassTmpl = RD->getDescribedClassTemplate()) return ClassTmpl;
        return D;
    }
    ...
    enum CX_StorageClass clang_Cursor_getStorageClass(CXCursor C){
        ...
        const Decl		*D	= getCursorDecl(C);
        if(D){
            if(const FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) sc = FD->getStorageClass();
            else if(const VarDecl *VD = dyn_cast<VarDecl>(D)) sc = VD->getStorageClass();
    ...
    CXCursor clang_getCursorSemanticParent(CXCursor cursor){
        if(clang_isDeclaration(cursor.kind)){
            if(const Decl *D = getCursorDecl(cursor)){
                ...
                if(! DC) return clang_getNullCursor();
                return MakeCXCursor(maybeGetTemplateCursor(cast<Decl>(DC))
                                   , getCursorTU(cursor));
            }
        }
        if(clang_isStatement(cursor.kind) || clang_isExpression(cursor.kind)){
            if(const Decl *D = getCursorDecl(cursor)) return MakeCXCursor(D, getCursorTU(cursor));
        }
        return clang_getNullCursor();
    }
    ...
    CXFile clang_getIncludedFile(CXCursor cursor){
        if(cursor.kind != CXCursor_InclusionDirective) return nullptr;
    ...
    unsigned clang_Cursor_getObjCPropertyAttributes(CXCursor C, unsigned reserved){
        if(C.kind != CXCursor_ObjCPropertyDecl) return CXObjCPropertyAttr_noattr;
...
#undef SET_CXOBJCPROP_ATTR
    ...
    unsigned clang_Cursor_getObjCDeclQualifiers(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return CXObjCDeclQualifier_None;
        Decl::ObjCDeclQualifier QT	= Decl::OBJC_TQ_None;
        const Decl				*D	= getCursorDecl(C);
        if(const ObjCMethodDecl *MD = dyn_cast<ObjCMethodDecl>(D)) QT = MD->getObjCDeclQualifier();
        else if(const ParmVarDecl *PD = dyn_cast<ParmVarDecl>(D)) QT = PD->getObjCDeclQualifier();
        if(QT == Decl::OBJC_TQ_None) return CXObjCDeclQualifier_None;
    ...
    unsigned clang_Cursor_isObjCOptional(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return 0;
        const Decl *D = getCursorDecl(C);
        if(const ObjCPropertyDecl *PD = dyn_cast<ObjCPropertyDecl>(D)) return PD->getPropertyImplementation() == ObjCPropertyDecl::Optional;
        if(const ObjCMethodDecl *MD = dyn_cast<ObjCMethodDecl>(D)) return MD->getImplementationControl() == ObjCMethodDecl::Optional;
        return 0;
    }
    ...
    unsigned clang_Cursor_isVariadic(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return 0;
        const Decl *D = getCursorDecl(C);
        if(const FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) return FD->isVariadic();
        if(const ObjCMethodDecl *MD = dyn_cast<ObjCMethodDecl>(D)) return MD->isVariadic();
        return 0;
    }

    CXSourceRange clang_Cursor_getCommentRange(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return clang_getNullRange();
        const Decl			*D			= getCursorDecl(C);
        ASTContext			&Context	= getCursorContext(C);
        const RawComment	*RC			= Context.getRawCommentForAnyRedecl(D);
        if(! RC) return clang_getNullRange();
        return cxloc::translateSourceRange(Context, RC->getSourceRange());
    }

    CXString clang_Cursor_getRawCommentText(CXCursor C){
        if(! clang_isDeclaration(C.kind)) return cxstring::createNull();
        const Decl			*D			= getCursorDecl(C);
        ASTContext			&Context	= getCursorContext(C);
        const RawComment	*RC			= Context.getRawCommentForAnyRedecl(D);
    ...
    CXModule clang_Cursor_getModule(CXCursor C){
        if(C.kind == CXCursor_ModuleImportDecl){
            if(const ImportDecl *ImportD =
                dyn_cast_or_null<ImportDecl>(getCursorDecl(C))) return ImportD->getImportedModule();
        }
        return nullptr;
    }
    ...
    CXString clang_Module_getName(CXModule CXMod){
        if(! CXMod) return cxstring::createEmpty();
        Module *Mod = static_cast<Module *>(CXMod);
    ...
    unsigned clang_Module_getNumTopLevelHeaders(CXTranslationUnit TU, CXModule CXMod){
        if(isNotUsableTU(TU)){
            LOG_BAD_TU(TU);
            return 0;
        }
    ...
    CXFile clang_Module_getTopLevelHeader(CXTranslationUnit TU, CXModule CXMod, unsigned Index){
        if(isNotUsableTU(TU)){
            LOG_BAD_TU(TU);
            return nullptr;
        }
        ...
        ArrayRef<const FileEntry *> TopHeaders = Mod->getTopHeaders(FileMgr);
        if(Index < TopHeaders.size()) return const_cast<FileEntry *>(TopHeaders[Index]);
        return nullptr;
    }
}	// end: extern "C"
...
extern "C" {
    ...
    unsigned clang_CXXField_isMutable(CXCursor C){
        ...
        if(const auto D = cxcursor::getCursorDecl(C))
                if(const auto FD = dyn_cast_or_null<FieldDecl>(D)) return FD->isMutable() ? 1 : 0;
        return 0;
    }
...
extern "C" {
    CXType clang_getIBOutletCollectionType(CXCursor C){
        if(C.kind != CXCursor_IBOutletCollectionAttr) return cxtype::MakeCXType(QualType(), cxcursor::getCursorTU(C));
        const IBOutletCollectionAttr *A =
...
static inline void createCXTUResourceUsageEntry(MemUsageEntries &entries, enum CXTUResourceUsageKind k, unsigned long amount){
...
extern "C" {
    ...
    CXTUResourceUsage clang_getCXTUResourceUsage(CXTranslationUnit TU){
        ...
        if(GlobalCodeCompletionAllocator *completionAllocator =
            astUnit->getCachedCompletionAllocator().get()) completionBytes = completionAllocator->getTotalMemory();
    ...
    void clang_disposeCXTUResourceUsage(CXTUResourceUsage usage){
        if(usage.data) delete (MemUsageEntries *) usage.data;
    }
    ...
    CXSourceRangeList *clang_getSkippedRanges(CXTranslationUnit TU, CXFile file){
        ...
        for(std::vector<SourceRange>::const_iterator i = SkippedRanges.begin(), ei = SkippedRanges.end();
            i != ei; ++ i){
            if(sm.getFileID(i->getBegin()) == wantedFileID || sm.getFileID(i->getEnd()) == wantedFileID) wantedRanges.push_back(*i);
        }
        skipped->count	= wantedRanges.size();
        skipped->ranges = new CXSourceRange[skipped->count];
        for(unsigned i = 0, ei = skipped->count; i != ei; ++ i) skipped->ranges[i] = cxloc::translateSourceRange(Ctx, wantedRanges[i]);
        return skipped;
    }
    ...
    CXSourceRangeList *clang_getAllSkippedRanges(CXTranslationUnit TU){
        ...
        skipped->ranges = new CXSourceRange[skipped->count];
        for(unsigned i = 0, ei = skipped->count; i != ei; ++ i) skipped->ranges[i] = cxloc::translateSourceRange(Ctx, SkippedRanges[i]);
        return skipped;
    }
...
namespace clang {
    bool RunSafely(llvm::CrashRecoveryContext &CRC, llvm::function_ref<void()> Fn, unsigned Size){
        if(! Size) Size = GetSafetyThreadStackSize();
        if(Size) return CRC.RunSafelyOnThread(Fn, Size);
...
void clang::setThreadBackgroundPriority(){
    if(getenv("LIBCLANG_BGPRIO_DISABLE")) return;
#ifdef USE_DARWIN_THREADS
...
MacroInfo *cxindex::getMacroInfo(const IdentifierInfo &II, SourceLocation MacroDefLoc, CXTranslationUnit TU){
    if(MacroDefLoc.isInvalid() || ! TU) return nullptr;
    if(! II.hadMacroDefinition()) return nullptr;
    ASTUnit			*Unit	= cxtu::getASTUnit(TU);
    ...
    if(MD){
        for(MacroDirective::DefInfo
            ...
            if(MacroDefLoc == Def.getMacroInfo()->getDefinitionLoc()) return Def.getMacroInfo();
        }
    }
    return nullptr;
}

const MacroInfo *cxindex::getMacroInfo(const MacroDefinitionRecord *MacroDef, CXTranslationUnit TU){
    if(! MacroDef || ! TU) return nullptr;
    const IdentifierInfo *II = MacroDef->getName();
    if(! II) return nullptr;
    return getMacroInfo(*II, MacroDef->getLocation(), TU);
}

MacroDefinitionRecord *cxindex::checkForMacroInMacroDefinition(const MacroInfo *MI, const Token &Tok, CXTranslationUnit TU){
    if(! MI || ! TU) return nullptr;
    if(Tok.isNot(tok::raw_identifier)) return nullptr;
    if(MI->getNumTokens() == 0) return nullptr;
    ...
    SourceManager &SM = Unit->getSourceManager();
    if(SM.isBeforeInTranslationUnit(Tok.getLocation(), DefRange.getBegin())) return nullptr;
    if(SM.isBeforeInTranslationUnit(DefRange.getEnd(), Tok.getLocation())) return nullptr;
    Preprocessor		&PP		= Unit->getPreprocessor();
    PreprocessingRecord *PPRec	= PP.getPreprocessingRecord();
    if(! PPRec) return nullptr;
    ...
    if(std::find(MI->arg_begin(), MI->arg_end(), &II) != MI->arg_end()) return nullptr;
    MacroDirective *InnerMD = PP.getLocalMacroDirectiveHistory(&II);
    if(! InnerMD) return nullptr;
    return PPRec->findMacroDefinition(InnerMD->getMacroInfo());
}

MacroDefinitionRecord *cxindex::checkForMacroInMacroDefinition(const MacroInfo *MI, SourceLocation Loc, CXTranslationUnit TU){
    if(Loc.isInvalid() || ! MI || ! TU) return nullptr;
    if(MI->getNumTokens() == 0) return nullptr;
    ASTUnit			*Unit	= cxtu::getASTUnit(TU);
    Preprocessor	&PP		= Unit->getPreprocessor();
    if(! PP.getPreprocessingRecord()) return nullptr;
    Loc = Unit->getSourceManager().getSpellingLoc(Loc);
    Token Tok;
    if(PP.getRawToken(Loc, Tok)) return nullptr;
...
Logger &cxindex::Logger::operator <<(CXTranslationUnit TU){
    if(TU){
        if(ASTUnit *Unit = cxtu::getASTUnit(TU)){
            ...
            if(Unit->isMainFileAST()) LogOS << " (" << Unit->getASTFileName() << ')';
            return *this;
        }
    }else LogOS << "<NULL TU>";
    return *this;
