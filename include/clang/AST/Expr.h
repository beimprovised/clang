        ...
        class Expr : public Stmt {
                ...
                public:
                        ...
                        bool hasPlaceholderType(BuiltinType::Kind K) const {
                                ...
                                if(const BuiltinType *BT = dyn_cast<BuiltinType>(getType())) return BT->getKind() == K;
                                return false;
                        }
                        ...
                        static bool isPotentialConstantExpr(const FunctionDecl *FD, SmallVectorImpl<
                        ...
                        const Expr *skipRValueSubobjectAdjustments(
                                SmallVectorImpl<const Expr *> &CommaLHS, SmallVectorImpl<SubobjectAdjustment> &Adjustments) const;
...
///       refers to an enclosed local or a captured variable.
        class DeclRefExpr final
                ...
                DeclRefExpr(const ASTContext &Ctx, NestedNameSpecifierLoc QualifierLoc, SourceLocation TemplateKWLoc, ValueDecl *D, bool RefersToEnlosingVariableOrCapture, const DeclarationNameInfo &NameInfo, NamedDecl *FoundD, const TemplateArgumentListInfo *TemplateArgs, QualType T, ExprValueKind VK);
                ...
                public:
                        ...
                        static DeclRefExpr *Create(const ASTContext &Context, NestedNameSpecifierLoc QualifierLoc, SourceLocation TemplateKWLoc, ValueDecl *D, bool RefersToEnclosingVariableOrCapture, const DeclarationNameInfo &NameInfo, QualType T, ExprValueKind VK, NamedDecl *FoundD = nullptr, const TemplateArgumentListInfo *TemplateArgs = nullptr);
                        ...
                        NestedNameSpecifierLoc getQualifierLoc() const {
                                if(! hasQualifier()) return NestedNameSpecifierLoc();
                        ...
                        void copyTemplateArgumentsInto(TemplateArgumentListInfo &List) const {
                                if(hasExplicitTemplateArgs()) getTrailingObjects<ASTTemplateKWAndArgsInfo>()->copyInto(
...
/// ASTContext's allocator for memory allocation.
        class APNumericStorage {
                ...
                protected:
                        ...
                        llvm::APInt getIntValue() const {
                                ...
                                if(NumWords > 1) return llvm::APInt(BitWidth, NumWords, pVal);
                                else return llvm::APInt(BitWidth, VAL);
                        }
        ...
        class FloatingLiteral : public Expr, private APFloatStorage {
                ...
                FloatingLiteral(const ASTContext &C, const llvm::APFloat &V, bool isexact, QualType Type, SourceLocation L);
        ...
        class StringLiteral : public Expr {
                ...
                public:
                        ...
                        StringRef getBytes() const {
                                ...
                                if(CharByteWidth == 1) return StringRef(StrData.asChar, getByteLength());
                                if(CharByteWidth == 4) return StringRef(reinterpret_cast<const char *>(StrData.asUInt32)
                                                                                                           , getByteLength());
                        ...
                        uint32_t getCodeUnit(size_t i) const {
                                ...
                                if(CharByteWidth == 1) return static_cast<unsigned char>(StrData.asChar[i]);
                                if(CharByteWidth == 4) return StrData.asUInt32[i];
                                assert(CharByteWidth == 2 && "unsupported CharByteWidth");
                        ...
                        bool containsNonAsciiOrNull() const {
                                ...
                                for(unsigned i = 0, e = Str.size(); i != e; ++ i)
                                                if(! isASCII(Str[i]) || ! Str[i]) return true;
                                return false;
                        }
...
/// we can represent and evaluate the expression @c offsetof(struct T, s[2].d).
        ...
        class OffsetOfExpr final
                ...
                OffsetOfExpr(const ASTContext &C, QualType type, SourceLocation OperatorLoc, TypeSourceInfo *tsi, ArrayRef<OffsetOfNode> comps, ArrayRef<Expr *> exprs, SourceLocation RParenLoc);
...
/// "str1 + str2" to resolve to a function call.
        class CallExpr : public Expr {
                ...
                protected:
                        ...
                        CallExpr(const ASTContext &C, StmtClass SC, Expr *fn, ArrayRef<Expr *> args, QualType t, ExprValueKind VK, SourceLocation rparenloc);
        ...
        class MemberExpr final
                ...
                public:
                        ...
                        DeclAccessPair getFoundDecl() const {
                                if(! HasQualifierOrFoundDecl) return DeclAccessPair::make(getMemberDecl()
                        ...
                        /// information.
                        NestedNameSpecifierLoc getQualifierLoc() const {
                                if(! HasQualifierOrFoundDecl) return NestedNameSpecifierLoc();
        ...
        class CompoundLiteralExpr : public Expr {
                ...
                public:
                        ...
                        SourceLocation getLocStart() const LLVM_READONLY {
                                ...
                                if(! Init) return SourceLocation();
                                if(LParenLoc.isInvalid()) return Init->getLocStart();
                                return LParenLoc;
                        }
                        SourceLocation getLocEnd() const LLVM_READONLY {
                                // FIXME: Init should never be null.
...
/// @endcode
        class ImplicitCastExpr final
                ...
                private:
                        ImplicitCastExpr(QualType ty, CastKind kind, Expr *op, unsigned BasePathLength, ExprValueKind VK)
        ...
        inline Expr *Expr::IgnoreImpCasts(){
                Expr *e = this;
                while(ImplicitCastExpr *ice = dyn_cast<ImplicitCastExpr>(e)) e = ice->getSubExpr();
                return e;
        }
...
/// (Type)expr. For example: @c (int)f.
        class CStyleCastExpr final
                ...
                CStyleCastExpr(QualType exprTy, ExprValueKind vk, CastKind kind, Expr *op, unsigned PathSize, TypeSourceInfo *writtenTy, SourceLocation l, SourceLocation r)
        ...
        class BinaryOperator : public Expr {
                ...
                public:
                        ...
                        static Opcode getOpForCompoundAssignment(Opcode Opc){
                                ...
                                if(Opc >= BO_AndAssign) return Opcode(unsigned (Opc) - BO_AndAssign + BO_And);
                                else return Opcode(unsigned (Opc) - BO_MulAssign + BO_Mul);
                        }
...
/// every client ends up needing to know that these are different.
        ...
        inline Expr *AbstractConditionalOperator::getCond() const {
                if(const ConditionalOperator *co = dyn_cast<ConditionalOperator>(this)) return co->getCond();
                return cast<BinaryConditionalOperator>(this)->getCond();
        }

        inline Expr *AbstractConditionalOperator::getTrueExpr() const {
                if(const ConditionalOperator *co = dyn_cast<ConditionalOperator>(this)) return co->getTrueExpr();
                return cast<BinaryConditionalOperator>(this)->getTrueExpr();
        }

        inline Expr *AbstractConditionalOperator::getFalseExpr() const {
                if(const ConditionalOperator *co = dyn_cast<ConditionalOperator>(this)) return co->getFalseExpr();
        ...
        class InitListExpr : public Expr {
                ...
                public:
                        ...
                        child_range children(){
                                ...
                                if(InitExprs.empty()) return child_range(child_iterator(), child_iterator());
...
/// designator for @c .y. The initialization expression will be 1.0.
        class DesignatedInitExpr final
                ...
                private:
                        ...
                        DesignatedInitExpr(const ASTContext &C, QualType Ty, llvm::ArrayRef<Designator> Designators, SourceLocation EqualOrColonLoc, bool GNUSyntax, ArrayRef<Expr *> IndexExprs, Expr *Init);
                        ...
                        class Designator {
                                ...
                                public:
                                        ...
                                        FieldDecl *getField() const {
                                                ...
                                                if(Field.NameOrField & 0x01) return nullptr;
                                                else return reinterpret_cast<FieldDecl *>(Field.NameOrField);
                                        }
                                        ...
                                        SourceLocation getLocStart() const LLVM_READONLY {
                                                if(Kind == FieldDesignator) return getDotLoc().isInvalid() ? getFieldLoc() : getDotLoc();
                                                else return getLBracketLoc();
                                        }
                                        SourceLocation getLocEnd() const LLVM_READONLY {
        ...
        class GenericSelectionExpr : public Expr {
                ...
                public:
                        ...
                        QualType getAssocType(unsigned i) const {
                                if(const TypeSourceInfo *TS = getAssocTypeSourceInfo(i)) return TS->getType();
...
/// modelling design.
        class PseudoObjectExpr final
                ...
                PseudoObjectExpr(QualType type, ExprValueKind VK, Expr *syntactic, ArrayRef<Expr *> semantic, unsigned resultIndex);
                ...
                public:
                        ...
                        Expr *getResultExpr(){
                                if(PseudoObjectExprBits.ResultIndex == 0) return nullptr;
...
#include "clang/Basic/Builtins.def"
                ...
                public:
                        ...
                        Expr *getVal1() const {
                                if(Op == AO__c11_atomic_init) return cast<Expr>(SubExprs[ORDER]);
                        ...
                        Expr *getVal2() const {
                                if(Op == AO__atomic_exchange) return cast<Expr>(SubExprs[ORDER_FAIL]);
...
/// still needs to be performed and/or an error diagnostic emitted.
        ...
        class HOMPDeviceSpecifierExpr : public Expr {
                HOMPDeviceSpecifierExpr(StmtClass SC, QualType T, ExprValueKind VK, ExprObjectKind OK, bool TD, bool VD, bool ID, bool ContainsUnexpandedParameterPack)
                        : Expr(SC, T, VK, OK, TD, VD, ID, ContainsUnexpandedParameterPack){}
        };
}	// end namespace clang
