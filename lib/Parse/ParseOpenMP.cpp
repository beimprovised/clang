...
static OpenMPDirectiveKind ParseOpenMPDirectiveKind(Parser &P){
    ...
    for(unsigned i = 0; i < llvm::array_lengthof(F); ++ i){
        if(DKind != F[i][0]) continue;
        Tok = P.getPreprocessor().LookAhead(0);
        unsigned SDKind =
            Tok.isAnnotation()
            ? static_cast<unsigned>(OMPD_unknown)
            : getOpenMPDirectiveKindEx(P.getPreprocessor().getSpelling(Tok));
        if(SDKind == OMPD_unknown) continue;
        if(SDKind == F[i][1]){
            P.ConsumeToken();
            DKind = F[i][2];
        }
...
Parser::DeclGroupPtrTy Parser::ParseOpenMPDeclareReductionDirective(AccessSpecifier AS){
    ...
    if(Name.isEmpty() && Tok.is(tok::annot_pragma_openmp_end)) return DeclGroupPtrTy();
    // Consume ':'.
    bool IsCorrect = ! ExpectAndConsume(tok::colon);
    if(! IsCorrect && Tok.is(tok::annot_pragma_openmp_end)) return DeclGroupPtrTy();
    ...
    do {
        ...
        if(Tok.is(tok::colon) || Tok.is(tok::annot_pragma_openmp_end)) break;
        // Consume ','.
    ...
    if(! IsCorrect && Tok.is(tok::annot_pragma_openmp_end)) return DeclGroupPtrTy();
    // Consume ':'.
    if(ExpectAndConsume(tok::colon)) IsCorrect = false;
    if(Tok.is(tok::annot_pragma_openmp_end)){
    ...
    for(auto *D : DRD.get()){
        ...
        if(Tok.isNot(tok::annot_pragma_openmp_end)){
            ...
            if(Tok.is(tok::identifier) &&
                Tok.getIdentifierInfo()->isStr("initializer")) ConsumeToken();
            else{
    ...
    class FNContextRAII final {
        ...
        public:
            FNContextRAII(Parser &P, Parser::DeclGroupPtrTy Ptr) : P(P){
                ...
                if(HasTemplateScope) Actions.ActOnReenterTemplateScope(Actions.getCurScope(), D);
                // If the Decl is on a function, add function parameters to the scope.
                HasFunScope = D->isFunctionOrFunctionTemplate();
                FnScope		= new Parser::ParseScope(&P, Scope::FnScope | Scope::DeclScope
                                                    , HasFunScope);
                if(HasFunScope) Actions.ActOnReenterFunctionContext(Actions.getCurScope(), D);
            }
            ~FNContextRAII(){
                if(HasFunScope){
                ...
                if(HasTemplateScope) TempScope->Exit();
                delete FnScope;
                delete TempScope;
                delete ThisScope;
            }
...
static bool parseDeclareSimdClauses(
    Parser &P, OMPDeclareSimdDeclAttr::BranchStateTy &BS, ExprResult &SimdLen, SmallVectorImpl<Expr *> &Uniforms, SmallVectorImpl<Expr *> &Aligneds, SmallVectorImpl<Expr *> &Alignments, SmallVectorImpl<Expr *> &Linears, SmallVectorImpl<unsigned> &LinModifiers, SmallVectorImpl<Expr *> &Steps){
    SourceRange BSRange;
    const Token &Tok	= P.getCurToken();
    bool		IsError = false;
    while(Tok.isNot(tok::annot_pragma_openmp_end)){
        if(Tok.isNot(tok::identifier)) break;
        ...
        if(OMPDeclareSimdDeclAttr::ConvertStrToBranchStateTy(ClauseName, Out)){
            ...
            if(SimdLen.isInvalid()) IsError = true;
        }else{
            ...
            if(CKind == OMPC_uniform || CKind == OMPC_aligned ||
                ...
                auto						*Vars = &Uniforms;
                if(CKind == OMPC_aligned) Vars = &Aligneds;
                else if(CKind == OMPC_linear) Vars = &Linears;
                P.ConsumeToken();
                if(P.ParseOpenMPVarList(OMPD_declare_simd
                                       , getOpenMPClauseKind(ClauseName), *Vars, Data)) IsError = true;
                if(CKind == OMPC_aligned) Alignments.append(Aligneds.size() - Alignments.size(), Data.TailExpr);
                else if(CKind == OMPC_linear){
                    if(P.getActions().CheckOpenMPLinearModifier(Data.LinKind
                                                               , Data.DepLinMapLoc)) Data.LinKind = OMPC_LINEAR_val;
        ...
        // Skip ',' if any.
        if(Tok.is(tok::comma)) P.ConsumeToken();
    }
    return IsError;
}

/// Parse clauses for '#pragma omp declare simd'.
Parser::DeclGroupPtrTy Parser::ParseOMPDeclareSimdClauses(Parser::DeclGroupPtrTy Ptr, CachedTokens &Toks, SourceLocation Loc){
    PP.EnterToken(Tok);
    ...
    if(Tok.isNot(tok::annot_pragma_openmp_end)){
        ...
        while(Tok.isNot(tok::annot_pragma_openmp_end)) ConsumeAnyToken();
    }
...
Parser::DeclGroupPtrTy Parser::ParseOpenMPDeclarativeDirectiveWithExtDecl(
    AccessSpecifier &AS, ParsedAttributesWithRange &Attrs, DeclSpec::TST TagType, Decl *Tag){
    ...
    switch(DKind){
        ...
        case OMPD_declare_simd: {
            ...
            DeclGroupPtrTy Ptr;
            if(Tok.is(tok::annot_pragma_openmp)) Ptr = ParseOpenMPDeclarativeDirectiveWithExtDecl(AS, Attrs, TagType, Tag);
        ...
        case OMPD_declare_target: {
            ...
            if(Tok.isNot(tok::annot_pragma_openmp_end)){
                ...
                while(Tok.isNot(tok::annot_pragma_openmp_end)){
                    ...
                    if(ParseOpenMPSimpleVarList(OMPD_declare_target, Callback, true)) break;
                    // Consume optional ','.
            ...
            // Skip the last annot_pragma_openmp_end.
            ConsumeAnyToken();
            if(! Actions.ActOnStartOpenMPDeclareTargetDirective(DTLoc)) return DeclGroupPtrTy();
            ...
            while(DKind != OMPD_end_declare_target && DKind != OMPD_declare_target &&
                ...
                if(Tok.isAnnotation() && Tok.is(tok::annot_pragma_openmp)){
                    ...
                    ConsumeToken();
                    DKind = ParseOpenMPDirectiveKind(*this);
                    if(DKind != OMPD_end_declare_target) TPA.Revert();
                    else TPA.Commit();
                }
                // Save the declarations so that we can create the declare target group
                // later on.
                if(Ptr)
                        for(auto *V : Ptr.get()) Decls.push_back(V);
            }
            ...
            // later on.
            if(! Decls.empty()) return Actions.BuildDeclaratorGroup(Decls, /*TypeMayContainAuto=*/ false);
            return DeclGroupPtrTy();
        }
        case OMPD_unknown:
...
///         annot_pragma_openmp_end
///
StmtResult Parser::ParseOpenMPDeclarativeOrExecutableDirective(AllowedContsructsKind Allowed){
    assert(Tok.is(tok::annot_pragma_openmp) && "Not an OpenMP directive!");
    ParenBraceBracketBalancer	BalancerRAIIObj(*this);
    ...
    switch(DKind){
        ...
        case OMPD_declare_reduction:
            ...
            if(auto Res = ParseOpenMPDeclareReductionDirective(/*AS=*/ AS_none)){
                ...
                ConsumeAnyToken();
                Directive = Actions.ActOnDeclStmt(Res, Loc, Tok.getLocation());
            }else SkipUntil(tok::annot_pragma_openmp_end);
            break;
        case OMPD_flush:
            ...
            if(DKind == OMPD_critical){
                ...
                if(! T.consumeOpen()){
                    if(Tok.isAnyIdentifier()){
                        ...
                        ConsumeAnyToken();
                    }else Diag(Tok, diag::err_omp_expected_identifier_for_critical);
                    T.consumeClose();
                }
            }else if(DKind == OMPD_cancellation_point || DKind == OMPD_cancel){
                CancelRegion = ParseOpenMPDirectiveKind(*this);
                if(Tok.isNot(tok::annot_pragma_openmp_end)) ConsumeToken();
            }
            if(isOpenMPLoopDirective(DKind)) ScopeFlags |= Scope::OpenMPLoopDirectiveScope;
            if(isOpenMPSimdDirective(DKind)) ScopeFlags |= Scope::OpenMPSimdDirectiveScope;
...
bool Parser::ParseOpenMPSimpleVarList(
    OpenMPDirectiveKind Kind, const llvm::function_ref<void (CXXScopeSpec &, DeclarationNameInfo)> &
    Callback, bool AllowScopeSpecifier){
    // Parse '('.
    BalancedDelimiterTracker T(*this, tok::l_paren, tok::annot_pragma_openmp_end);
    if(T.expectAndConsume(diag::err_expected_lparen_after
                         , getOpenMPDirectiveName(Kind))) return true;
    bool	IsCorrect		= true;
    ...
    while(Tok.isNot(tok::r_paren) && Tok.isNot(tok::annot_pragma_openmp_end)){
        ...
        }else Callback(SS, Actions.GetNameFromUnqualifiedId(Name));
        // Consume ','.
        if(Tok.is(tok::comma)) ConsumeToken();
    }
    if(NoIdentIsFound){
...
OMPClause *Parser::ParseOpenMPClause(OpenMPDirectiveKind DKind, OpenMPClauseKind CKind, bool FirstClause){
    ...
    switch(CKind){
        ...
        case OMPC_hint:
            ...
            if(CKind == OMPC_ordered && PP.LookAhead(/*N=*/ 0).isNot(tok::l_paren)) Clause = ParseOpenMPClause(CKind);
            else Clause = ParseOpenMPSingleExprClause(CKind);
            break;
        case OMPC_default:
        case OMPC_proc_bind:
...
ExprResult Parser::ParseOpenMPParensExpr(StringRef ClauseName, SourceLocation &RLoc){
    BalancedDelimiterTracker T(*this, tok::l_paren, tok::annot_pragma_openmp_end);
    if(T.expectAndConsume(diag::err_expected_lparen_after, ClauseName.data())) return ExprError();
...
OMPClause *Parser::ParseOpenMPSimpleClause(OpenMPClauseKind Kind){
    ...
    BalancedDelimiterTracker T(*this, tok::l_paren, tok::annot_pragma_openmp_end);
    if(T.expectAndConsume(diag::err_expected_lparen_after
                         , getOpenMPClauseName(Kind))) return nullptr;
    unsigned Type = getOpenMPSimpleClauseType(
        Kind, Tok.isAnnotation() ? "" : PP.getSpelling(Tok));
    SourceLocation TypeLoc = Tok.getLocation();
    if(Tok.isNot(tok::r_paren) && Tok.isNot(tok::comma) &&
        Tok.isNot(tok::annot_pragma_openmp_end)) ConsumeAnyToken();
    // Parse ')'.
    T.consumeClose();
...
OMPClause *Parser::ParseOpenMPSingleExprWithArgClause(OpenMPClauseKind Kind){
    ...
    if(Kind == OMPC_schedule){
        ...
        if(KindModifier > OMPC_SCHEDULE_unknown){
            ...
            else Diag(Tok, diag::warn_pragma_expected_colon) << "schedule modifier";
            KindModifier = getOpenMPSimpleClauseType(
                Kind, Tok.isAnnotation() ? "" : PP.getSpelling(Tok));
        }
        ...
        if((Arg[ScheduleKind] == OMPC_SCHEDULE_static ||
            ...
            Tok.is(tok::comma)) DelimLoc = ConsumeAnyToken();
        ...
        if(Tok.isNot(tok::r_paren) && Tok.isNot(tok::comma) &&
            Tok.isNot(tok::annot_pragma_openmp_end)) ConsumeAnyToken();
        if(Arg.back() == OMPC_DIST_SCHEDULE_static && Tok.is(tok::comma)) DelimLoc = ConsumeAnyToken();
        ...
        if(Arg.back() != OMPD_unknown){
            ConsumeToken();
            if(Tok.is(tok::colon)) DelimLoc = ConsumeToken();
            else Diag(Tok, diag::warn_pragma_expected_colon)
    ...
    // Parse ')'.
    T.consumeClose();
    if(NeedAnExpression && Val.isInvalid()) return nullptr;
...
bool Parser::ParseOpenMPVarList(OpenMPDirectiveKind DKind, OpenMPClauseKind Kind, SmallVectorImpl<Expr *> &Vars, OpenMPVarListDataTy &Data){
    ...
    BalancedDelimiterTracker T(*this, tok::l_paren, tok::annot_pragma_openmp_end);
    if(T.expectAndConsume(diag::err_expected_lparen_after
                         , getOpenMPClauseName(Kind))) return true;
    ...
    if(Kind == OMPC_reduction){
        ...
        if(Tok.is(tok::colon)) Data.ColonLoc = ConsumeToken();
        else Diag(Tok, diag::warn_pragma_expected_colon) << "reduction identifier";
        if(! InvalidReductionId) Data.ReductionId =
        ...
        if(IsMapClauseModifierToken(Tok)){
            if(PP.LookAhead(0).is(tok::colon)){
                if(Data.MapType == OMPC_MAP_unknown) Diag(Tok, diag::err_omp_unknown_map_type);
                else if(Data.MapType == OMPC_MAP_always) Diag(Tok, diag::err_omp_map_type_missing);
                ConsumeToken();
                ...
                if(IsMapClauseModifierToken(PP.LookAhead(1)) &&
                    ...
                    }else MapTypeModifierSpecified = true;
                    ConsumeToken();
                    ConsumeToken();
                    ...
                    if(Data.MapType == OMPC_MAP_unknown ||
                        Data.MapType == OMPC_MAP_always) Diag(Tok, diag::err_omp_unknown_map_type);
                    ConsumeToken();
                }else{
        ...
        if(Tok.is(tok::colon)) Data.ColonLoc = ConsumeToken();
        else if(ColonExpected) Diag(Tok, diag::warn_pragma_expected_colon) << "map type";
    }
    bool IsComma =
    ...
    while(IsComma || (Tok.isNot(tok::r_paren) && Tok.isNot(tok::colon) &&
        ...
        if(VarExpr.isUsable()) Vars.push_back(VarExpr.get());
        else{
            SkipUntil(tok::comma, tok::r_paren, tok::annot_pragma_openmp_end
                     , StopBeforeMatch);
        }
    ...
    if(NeedRParenForLinear) LinearT.consumeClose();
    ...
    if(MustHaveTail){
        ...
        if(Tail.isUsable()) Data.TailExpr = Tail.get();
        else SkipUntil(tok::comma, tok::r_paren, tok::annot_pragma_openmp_end
                      , StopBeforeMatch);
    }
    // Parse ')'.
    T.consumeClose();
    if((Kind == OMPC_depend && Data.DepKind != OMPC_DEPEND_unknown &&
        ...
        (MustHaveTail && ! Data.TailExpr) || InvalidReductionId) return true;
    return false;
}
...
OMPClause *Parser::ParseOpenMPVarListClause(OpenMPDirectiveKind DKind, OpenMPClauseKind Kind){
    SourceLocation			Loc		= Tok.getLocation();
    SourceLocation			LOpen	= ConsumeToken();
    SmallVector<Expr *, 4>	Vars;
    OpenMPVarListDataTy		Data;
    if(ParseOpenMPVarList(DKind, Kind, Vars, Data)) return nullptr;
