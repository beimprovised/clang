    ...
    class TreeTransform {
        ...
        public:
            ...
            class TemporaryBase {
                ...
                public:
                    TemporaryBase(TreeTransform &Self, SourceLocation Location, DeclarationName Entity) : Self(Self){
...
#include "clang/Basic/AttrList.inc"
            ...
            Decl *TransformDecl(SourceLocation Loc, Decl *D){
                ...
                if(Known != TransformedLocalDecls.end()) return Known->second;
                return D;
            }
...
#include "clang/Basic/OpenMPKinds.def"
            ...
            QualType RebuildDependentTemplateSpecializationType(
                ElaboratedTypeKeyword Keyword, NestedNameSpecifierLoc QualifierLoc, const IdentifierInfo *Name, SourceLocation NameLoc, TemplateArgumentListInfo &Args){
                ...
                TemplateName InstName
                    = getDerived().RebuildTemplateName(SS, *Name, NameLoc, QualType()
                                                      , nullptr);
                if(InstName.isNull()) return QualType();
                ...
                if(Keyword == ETK_None && QualifierLoc.getNestedNameSpecifier() == nullptr) return T;
                return SemaRef.Context.getElaboratedType(Keyword
                                                        , QualifierLoc.getNestedNameSpecifier()
                                                        , T);
            }
            ...
            QualType RebuildDependentNameType(ElaboratedTypeKeyword Keyword, SourceLocation KeywordLoc, NestedNameSpecifierLoc QualifierLoc, const IdentifierInfo *Id, SourceLocation IdLoc){
                ...
                if(Keyword == ETK_None || Keyword == ETK_Typename) return SemaRef.CheckTypenameType(Keyword, KeywordLoc, QualifierLoc
                                                                                                   , *Id, IdLoc);
                ...
                if(! DC) return QualType();
                if(SemaRef.RequireCompleteDeclContext(SS, DC)) return QualType();
                TagDecl *Tag = nullptr;
            ...
            StmtResult RebuildObjCForCollectionStmt(SourceLocation ForLoc, Stmt *Element, Expr *Collection, SourceLocation RParenLoc, Stmt *Body){
                StmtResult ForEachStmt = getSema().ActOnObjCForCollectionStmt(ForLoc
                                                                             ...
                                                                             , Collection
                                                                             , RParenLoc);
                if(ForEachStmt.isInvalid()) return StmtError();
            ...
            VarDecl *RebuildExceptionDecl(VarDecl *ExceptionDecl, TypeSourceInfo *Declarator, SourceLocation StartLoc, SourceLocation IdLoc, IdentifierInfo *Id){
                VarDecl *Var = getSema().BuildExceptionDeclaration(nullptr, Declarator
                                                                  , StartLoc, IdLoc, Id);
                if(Var) getSema().CurContext->addDecl(Var);
                return Var;
            }
                ...
                if(DeclStmt *RangeStmt = dyn_cast<DeclStmt>(Range)){
                    if(RangeStmt->isSingleDecl()){
                        if(VarDecl *RangeVar = dyn_cast<VarDecl>(RangeStmt->getSingleDecl())){
                            if(RangeVar->isInvalidDecl()) return StmtError();
                            Expr *RangeExpr = RangeVar->getInit();
                            if(! RangeExpr->isTypeDependent() &&
                                RangeExpr->getType()->isObjCObjectPointerType()) return getSema().ActOnObjCForCollectionStmt(ForLoc, LoopVar, RangeExpr
                                                                                                                            , RParenLoc);
                        }
                    }
                }
            ...
            ExprResult RebuildInitList(SourceLocation LBraceLoc, MultiExprArg Inits, SourceLocation RBraceLoc, QualType ResultTy){
                ...
                if(Result.isInvalid() || ResultTy->isDependentType()) return Result;
            ...
            ExprResult RebuildDependentScopeDeclRefExpr(
                NestedNameSpecifierLoc QualifierLoc, SourceLocation TemplateKWLoc, const DeclarationNameInfo &NameInfo, const TemplateArgumentListInfo *TemplateArgs, bool IsAddressOfOperand, TypeSourceInfo * *RecoveryTSI){
                CXXScopeSpec SS;
                SS.Adopt(QualifierLoc);
                if(TemplateArgs || TemplateKWLoc.isValid()) return getSema().BuildQualifiedTemplateIdExpr(SS, TemplateKWLoc, NameInfo
                                                                                                         , TemplateArgs);
            ...
            ExprResult RebuildCXXConstructExpr(QualType T, SourceLocation Loc, CXXConstructorDecl *Constructor, bool IsElidable, MultiExprArg Args, bool HadMultipleCandidates, bool ListInitialization, bool StdInitListInitialization, bool RequiresZeroInit, CXXConstructExpr::ConstructionKind ConstructKind, SourceRange ParenRange){
                ...
                if(getSema().CompleteConstructorCall(Constructor, Args, Loc
                                                    , ConvertedArgs)) return ExprError();
            ...
            TemplateArgumentLoc RebuildPackExpansion(TemplateArgumentLoc Pattern, SourceLocation EllipsisLoc, Optional<unsigned> NumExpansions){
                switch(Pattern.getArgument().getKind()){
                    case TemplateArgument::Expression: {
                        ...
                        if(Result.isInvalid()) return TemplateArgumentLoc();
                    ...
                    case TemplateArgument::Type:
                        if(TypeSourceInfo *Expansion
                                = getSema().CheckPackExpansion(Pattern.getTypeSourceInfo()
                                                              , EllipsisLoc
                                                              , NumExpansions)) return TemplateArgumentLoc(TemplateArgument(Expansion->getType())
                                                                                                          , Expansion);
                        break;
                }
        ...
        private:
            ...
            TypeSourceInfo *TransformTypeInObjectScope(TypeSourceInfo *TSInfo, QualType ObjectType, NamedDecl *FirstQualifierInScope, CXXScopeSpec &SS);

            TypeSourceInfo *TransformTSIInObjectScope(TypeLoc TL, QualType ObjectType, NamedDecl *FirstQualifierInScope, CXXScopeSpec &SS);
    };
...
#include "clang/AST/StmtNodes.inc"
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformInitializer(Expr *Init, bool NotCopyInit){
        // Initializers are instantiated like expressions, except that various outer
        // layers are stripped.
        if(! Init) return Init;
        if(ExprWithCleanups *ExprTemp = dyn_cast<ExprWithCleanups>(Init)) Init = ExprTemp->getSubExpr();
        if(MaterializeTemporaryExpr *MTE = dyn_cast<MaterializeTemporaryExpr>(Init)) Init = MTE->GetTemporaryExpr();
        while(CXXBindTemporaryExpr *Binder = dyn_cast<CXXBindTemporaryExpr>(Init)) Init = Binder->getSubExpr();
        if(ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(Init)) Init = ICE->getSubExprAsWritten();
        if(CXXStdInitializerListExpr *ILE =
            dyn_cast<CXXStdInitializerListExpr>(Init)) return TransformInitializer(ILE->getSubExpr(), NotCopyInit);
        ...
        if(! NotCopyInit && ! (Construct && Construct->isListInitialization())) return getDerived().TransformExpr(Init);
        ...
        if(isa<ImplicitValueInitExpr>(Init)) return getDerived().RebuildParenListExpr(SourceLocation(), None
                                                                                     , SourceLocation());
        ...
        if(! Construct || isa<CXXTemporaryObjectExpr>(Construct)) return getDerived().TransformExpr(Init);
        ...
        if(Construct && Construct->isStdInitListInitialization()) return TransformInitializer(Construct->getArg(0), NotCopyInit);
        SmallVector<Expr *, 8> NewArgs;
        bool ArgChanged = false;
        if(getDerived().TransformExprs(Construct->getArgs(), Construct->getNumArgs()
                                      ,	/*IsCall*/ true, NewArgs, &ArgChanged)) return ExprError();
        ...
        for(unsigned I = 0; I != NumInputs; ++ I){
            ...
            if(PackExpansionExpr *Expansion = dyn_cast<PackExpansionExpr>(Inputs[I])){
                ...
                if(! Expand){
                    ...
                    if(OutPattern.isInvalid()) return true;
                    ExprResult Out = getDerived().RebuildPackExpansion(OutPattern.get()
                                                                      , Expansion->getEllipsisLoc()
                                                                      , NumExpansions);
                    if(Out.isInvalid()) return true;
                    if(ArgChanged) *ArgChanged = true;
            ...
            if(Result.get() != Inputs[I] && ArgChanged) *ArgChanged = true;
    ...
    NestedNameSpecifierLoc TreeTransform<Derived>::TransformNestedNameSpecifierLoc(
        ...
        for(NestedNameSpecifierLoc Qualifier = NNS; Qualifier;
            Qualifier = Qualifier.getPrefix()) Qualifiers.push_back(Qualifier);
        CXXScopeSpec SS;
            ...
            switch(QNNS->getKind()){
                case NestedNameSpecifier::Identifier: {
                    ...
                    if(SemaRef.BuildCXXNestedNameSpecifier(/*Scope=*/ nullptr, IdInfo, false
                                                          , SS, FirstQualifierInScope, false)) return NestedNameSpecifierLoc();
                }
                break;
                ...
                case NestedNameSpecifier::TypeSpec: {
                    ...
                    if(! TL) return NestedNameSpecifierLoc();
                    if(TL.getType()->isDependentType() || TL.getType()->isRecordType() ||
                        (SemaRef.getLangOpts().CPlusPlus11 &&
                        TL.getType()->isEnumeralType())){
                        assert(! TL.getType().hasLocalQualifiers() &&
                            "Can't get cv-qualifiers here");
                        if(TL.getType()->isEnumeralType()) SemaRef.Diag(TL.getBeginLoc()
        ...
        if(SS.getScopeRep() == NNS.getNestedNameSpecifier() &&
            ! getDerived().AlwaysRebuild()) return NNS;
        ...
        if(SS.location_size() == NNS.getDataLength() &&
            memcmp(SS.location_data(), NNS.getOpaqueData(), SS.location_size()) == 0) return NestedNameSpecifierLoc(SS.getScopeRep(), NNS.getOpaqueData());
        // Allocate new nested-name-specifier location information.
        return SS.getWithLocInContext(SemaRef.Context);
    }

    template<typename Derived>
    DeclarationNameInfo TreeTransform<Derived>
    ::TransformDeclarationNameInfo(const DeclarationNameInfo &NameInfo){
        DeclarationName Name = NameInfo.getName();
        if(! Name) return DeclarationNameInfo();
        switch(Name.getNameKind()){
            ...
            case DeclarationName::CXXConversionFunctionName: {
                ...
                if(TypeSourceInfo *OldTInfo = NameInfo.getNamedTypeInfo()){
                    ...
                    if(! NewTInfo) return DeclarationNameInfo();
                    ...
                    if(NewT.isNull()) return DeclarationNameInfo();
    ...
    template<typename Derived>
    TemplateName TreeTransform<Derived>::TransformTemplateName(CXXScopeSpec &SS, TemplateName Name, SourceLocation NameLoc, QualType ObjectType, NamedDecl *FirstQualifierInScope){
        if(QualifiedTemplateName *QTN = Name.getAsQualifiedTemplateName()){
            ...
            if(! getDerived().AlwaysRebuild() &&
                ...
                TransTemplate == Template) return Name;
        ...
        if(SubstTemplateTemplateParmPackStorage *SubstPack
            ...
            if(! TransParam) return TemplateName();
            if(! getDerived().AlwaysRebuild() &&
                TransParam == SubstPack->getParameterPack()) return Name;
    ...
    void TreeTransform<Derived>::InventTemplateArgumentLoc(
        ...
        switch(Arg.getKind()){
            ...
            case TemplateArgument::TemplateExpansion: {
                ...
                if(DependentTemplateName *DTN = Template.getAsDependentTemplateName()) Builder.MakeTrivial(SemaRef.Context, DTN->getQualifier(), Loc);
                else if(QualifiedTemplateName *QTN = Template.getAsQualifiedTemplateName()) Builder.MakeTrivial(SemaRef.Context, QTN->getQualifier(), Loc);
    ...
    bool TreeTransform<Derived>::TransformTemplateArgument(
        const TemplateArgumentLoc &Input, TemplateArgumentLoc &Output, bool Uneval){
        ...
        switch(Arg.getKind()){
            ...
            case TemplateArgument::Type: {
                ...
                if(! DI) DI = InventTypeSourceInfo(Input.getArgument().getAsType());
            ...
            case TemplateArgument::Template: {
                ...
                if(Template.isNull()) return true;
...
/// for each of the template arguments in its underlying iterator.
    ...
    class TemplateArgumentLocInventIterator {
        ...
        public:
            ...
            explicit TemplateArgumentLocInventIterator(TreeTransform<Derived> &Self, InputIterator Iter)
                : Self(Self), Iter(Iter){}
    ...
    bool TreeTransform<Derived>::TransformTemplateArguments(
        InputIterator First, InputIterator Last, TemplateArgumentListInfo &Outputs, bool Uneval){
        for(; First != Last; ++ First){
            ...
            if(In.getArgument().isPackExpansion()){
                ...
                if(! Expand){
                    ...
                    Sema::ArgumentPackSubstitutionIndexRAII SubstIndex(getSema(), - 1);
                    if(getDerived().TransformTemplateArgument(Pattern, OutPattern, Uneval)) return true;
                    Out = getDerived().RebuildPackExpansion(OutPattern, Ellipsis
                                                           , NumExpansions);
                    if(Out.getArgument().isNull()) return true;
                ...
                for(unsigned I = 0; I != *NumExpansions; ++ I){
                    Sema::ArgumentPackSubstitutionIndexRAII SubstIndex(getSema(), I);
                    if(getDerived().TransformTemplateArgument(Pattern, Out, Uneval)) return true;
            ...
            // The simple case:
            if(getDerived().TransformTemplateArgument(In, Out, Uneval)) return true;
            Outputs.addArgument(Out);
        }
        return false;
    }
...
//= ==----------------------------------------------------------------------===//
    ...
    QualType TreeTransform<Derived>::TransformType(QualType T){
        if(getDerived().AlreadyTransformed(T)) return T;
    ...
    TypeSourceInfo *TreeTransform<Derived>::TransformType(TypeSourceInfo *DI){
        ...
        if(getDerived().AlreadyTransformed(DI->getType())) return DI;
        TypeLocBuilder TLB;
...
/// probably not for other clients.
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformQualifiedType(TypeLocBuilder &TLB, QualifiedTypeLoc T){
        ...
        // probably not for other clients.
        if(Result->isFunctionType() || Result->isReferenceType()) return Result;
        // Suppress Objective-C lifetime qualifiers if they don't make sense for the
        // resulting type.
        if(Quals.hasObjCLifetime()){
            if(! Result->isObjCLifetimeType() && ! Result->isDependentType()) Quals.removeObjCLifetime();
        ...
        if(! Quals.empty()){
            ...
            if(Result.hasLocalQualifiers()) TLB.push<QualifiedTypeLoc>(Result);
    ...
    template<typename Derived>
    TypeLoc TreeTransform<Derived>::TransformTypeInObjectScope(TypeLoc TL, QualType ObjectType, NamedDecl *UnqualLookup, CXXScopeSpec &SS){
        if(getDerived().AlreadyTransformed(TL.getType())) return TL;
        TypeSourceInfo *TSI =
            TransformTSIInObjectScope(TL, ObjectType, UnqualLookup, SS);
        if(TSI) return TSI->getTypeLoc();
        return TypeLoc();
    }

    template<typename Derived>
    TypeSourceInfo *TreeTransform<Derived>::TransformTypeInObjectScope(TypeSourceInfo *TSInfo, QualType ObjectType, NamedDecl *UnqualLookup, CXXScopeSpec &SS){
        if(getDerived().AlreadyTransformed(TSInfo->getType())) return TSInfo;
    ...
    TypeSourceInfo *TreeTransform<Derived>::TransformTSIInObjectScope(
        TypeLoc TL, QualType ObjectType, NamedDecl *UnqualLookup, CXXScopeSpec &SS){
        ...
        if(isa<TemplateSpecializationType>(T)){
            ...
            if(Template.isNull()) return nullptr;
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformBuiltinType(TypeLocBuilder &TLB, BuiltinTypeLoc T){
        BuiltinTypeLoc NewT = TLB.push<BuiltinTypeLoc>(T.getType());
        NewT.setBuiltinLoc(T.getBuiltinLoc());
        if(T.needsExtraLocalData()) NewT.getWrittenBuiltinSpecs() = T.getWrittenBuiltinSpecs();
        return T.getType();
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformComplexType(TypeLocBuilder &TLB, ComplexTypeLoc T){
        // FIXME: recurse?
        return TransformTypeSpecType(TLB, T);
    }

    template <typename Derived>
    QualType TreeTransform<Derived>::TransformAdjustedType(TypeLocBuilder &TLB, AdjustedTypeLoc TL){
        // Adjustments applied during transformation are handled elsewhere.
        return getDerived().TransformType(TLB, TL.getOriginalLoc());
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDecayedType(TypeLocBuilder &TLB, DecayedTypeLoc TL){
        QualType OriginalType = getDerived().TransformType(TLB, TL.getOriginalLoc());
        if(OriginalType.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
            OriginalType != TL.getOriginalLoc().getType()) Result = SemaRef.Context.getDecayedType(OriginalType);
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformPointerType(TypeLocBuilder &TLB, PointerTypeLoc TL){
        QualType PointeeType
            = getDerived().TransformType(TLB, TL.getPointeeLoc());
        if(PointeeType.isNull()) return QualType();
        QualType Result = TL.getType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformBlockPointerType(TypeLocBuilder &TLB, BlockPointerTypeLoc TL){
        QualType PointeeType
            = getDerived().TransformType(TLB, TL.getPointeeLoc());
        if(PointeeType.isNull()) return QualType();
        QualType Result = TL.getType();
...
/// or an r-value type.
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformReferenceType(TypeLocBuilder &TLB, ReferenceTypeLoc TL){
        ...
        if(isa<LValueReferenceType>(Result)) NewTL = TLB.push<LValueReferenceTypeLoc>(Result);
        else NewTL = TLB.push<RValueReferenceTypeLoc>(Result);
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformLValueReferenceType(TypeLocBuilder &TLB, LValueReferenceTypeLoc TL){
        return TransformReferenceType(TLB, TL);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformRValueReferenceType(TypeLocBuilder &TLB, RValueReferenceTypeLoc TL){
        return TransformReferenceType(TLB, TL);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformMemberPointerType(TypeLocBuilder &TLB, MemberPointerTypeLoc TL){
        QualType PointeeType = getDerived().TransformType(TLB, TL.getPointeeLoc());
        if(PointeeType.isNull()) return QualType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformConstantArrayType(TypeLocBuilder &TLB, ConstantArrayTypeLoc TL){
        const ConstantArrayType *T	= TL.getTypePtr();
        QualType ElementType		= getDerived().TransformType(TLB, TL.getElementLoc());
        if(ElementType.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformVariableArrayType(TypeLocBuilder &TLB, VariableArrayTypeLoc TL){
        const VariableArrayType *T	= TL.getTypePtr();
        QualType ElementType		= getDerived().TransformType(TLB, TL.getElementLoc());
        if(ElementType.isNull()) return QualType();
        ExprResult SizeResult
            = getDerived().TransformExpr(T->getSizeExpr());
        if(SizeResult.isInvalid()) return QualType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDependentSizedArrayType(TypeLocBuilder &TLB, DependentSizedArrayTypeLoc TL){
    ...
    QualType TreeTransform<Derived>::TransformDependentSizedExtVectorType(
        ...
        if(Size.isInvalid()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
            ElementType != T->getElementType() ||
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformVectorType(TypeLocBuilder &TLB, VectorTypeLoc TL){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformExtVectorType(TypeLocBuilder &TLB, ExtVectorTypeLoc TL){
        const VectorType *T		= TL.getTypePtr();
        QualType ElementType	= getDerived().TransformType(T->getElementType());
        if(ElementType.isNull()) return QualType();
        QualType Result = TL.getType();
    ...
    ParmVarDecl *TreeTransform<Derived>::TransformFunctionTypeParam(
        ParmVarDecl *OldParm, int indexAdjustment, Optional<unsigned> NumExpansions, bool ExpectParameterPack){
        ...
        }else NewDI = getDerived().TransformType(OldDI);
        if(! NewDI) return nullptr;
        if(NewDI == OldDI && indexAdjustment == 0) return OldParm;
    ...
    bool TreeTransform<Derived>::TransformFunctionTypeParams(
        SourceLocation Loc, ArrayRef<ParmVarDecl *> Params, const QualType *ParamTypes, const FunctionProtoType::ExtParameterInfo *ParamInfos, SmallVectorImpl<QualType> &OutParamTypes, SmallVectorImpl<ParmVarDecl *> *PVars, Sema::ExtParameterInfoBuilder &PInfos){
        ...
        for(unsigned i = 0; i != NumParams; ++ i){
                ...
                if(OldParm->isParameterPack()){
                    ...
                    if(ShouldExpand){
                        ...
                        for(unsigned I = 0; I != *NumExpansions; ++ I){
                            ...
                            if(! NewParm) return true;
                            if(ParamInfos) PInfos.set(OutParamTypes.size(), ParamInfos[i]);
                            OutParamTypes.push_back(NewParm->getType());
                            if(PVars) PVars->push_back(NewParm);
                        }
                        // If we're supposed to retain a pack expansion, do so by temporarily
                        // forgetting the partially-substituted parameter pack.
                        if(RetainExpansion){
            ...
            if(const PackExpansionType *Expansion
                ...
                if(ShouldExpand){
                    ...
                    for(unsigned I = 0; I != *NumExpansions; ++ I){
                        ...
                        if(PVars) PVars->push_back(nullptr);
                    }
                    // We're done with the pack expansion.
                    continue;
                }
            ...
            }else NewType = getDerived().TransformType(OldType);
            if(NewType.isNull()) return true;
            if(IsPackExpansion) NewType = getSema().Context.getPackExpansionType(NewType
                                                                                , NumExpansions);
            if(ParamInfos) PInfos.set(OutParamTypes.size(), ParamInfos[i]);
            OutParamTypes.push_back(NewType);
            if(PVars) PVars->push_back(nullptr);
        ...
        if(PVars){
            for(unsigned i = 0, e = PVars->size(); i != e; ++ i)
                    if(ParmVarDecl *parm = (*PVars)[i]) assert(parm->getFunctionScopeIndex() == i);
        }
#endif
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformFunctionProtoType(TypeLocBuilder &TLB, FunctionProtoTypeLoc TL){
    ...
    QualType TreeTransform<Derived>::TransformFunctionProtoType(
        TypeLocBuilder &TLB, FunctionProtoTypeLoc TL, CXXRecordDecl *ThisContext, unsigned ThisTypeQuals, Fn TransformExceptionSpec){
        ...
        if(T->hasTrailingReturn()){
            if(getDerived().TransformFunctionTypeParams(
                    TL.getBeginLoc(), TL.getParams()
                                                       ...
                                                       , ParamTypes, &ParamDecls, ExtParamInfos)) return QualType();
            {
        ...
        if(TransformExceptionSpec(EPI.ExceptionSpec, EPIChanged)) return QualType();
        ...
        if(auto NewExtParamInfos =
            ...
            if(! EPI.ExtParameterInfos ||
                ...
                != llvm::makeArrayRef(NewExtParamInfos, ParamTypes.size())) EPIChanged = true;
        ...
        for(unsigned i = 0, e = NewTL.getNumParams(); i != e; ++ i) NewTL.setParam(i, ParamDecls[i]);
        return Result;
    }

    template<typename Derived>
    bool TreeTransform<Derived>::TransformExceptionSpec(
        SourceLocation Loc, FunctionProtoType::ExceptionSpecInfo &ESI, SmallVectorImpl<QualType> &Exceptions, bool &Changed){
        ...
        if(ESI.Type == EST_ComputedNoexcept){
            ...
            if(NoexceptExpr.isInvalid()) return true;
            ...
            if(ESI.NoexceptExpr != NoexceptExpr.get()) Changed = true;
            ESI.NoexceptExpr = NoexceptExpr.get();
        }
        if(ESI.Type != EST_Dynamic) return false;
            ...
            if(const PackExpansionType *PackExpansion =
                ...
                if(getDerived().TryExpandParameterPacks(
                        Loc, SourceRange(), Unexpanded, Expand
                                                       , RetainExpansion, NumExpansions)) return true;
                if(! Expand){
                ...
                for(unsigned ArgIdx = 0; ArgIdx != *NumExpansions; ++ ArgIdx){
                    ...
                    QualType U = getDerived().TransformType(PackExpansion->getPattern());
                    if(U.isNull() || SemaRef.CheckSpecifiedExceptionType(U, Loc)) return true;
                    Exceptions.push_back(U);
                }
            }else{
        ...
        if(ESI.Exceptions.empty()) ESI.Type = EST_DynamicNone;
        return false;
    }
    ...
    QualType TreeTransform<Derived>::TransformFunctionNoProtoType(
        ...
        if(ResultType.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() || ResultType != T->getReturnType()) Result = getDerived().RebuildFunctionNoProtoType(ResultType);
    ...
    template<typename Derived> QualType TreeTransform<Derived>::TransformUnresolvedUsingType(TypeLocBuilder &TLB, UnresolvedUsingTypeLoc TL){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformTypedefType(TypeLocBuilder &TLB, TypedefTypeLoc TL){
        const TypedefType *T = TL.getTypePtr();
        TypedefNameDecl *Typedef
            = cast_or_null<TypedefNameDecl>(getDerived().TransformDecl(TL.getNameLoc()
                                                                      , T->getDecl()));
        if(! Typedef) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
            Typedef != T->getDecl()){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformTypeOfExprType(TypeLocBuilder &TLB, TypeOfExprTypeLoc TL){
        // typeof expressions are not potentially evaluated contexts
        EnterExpressionEvaluationContext Unevaluated(SemaRef, Sema::Unevaluated
                                                    , Sema::ReuseLambdaContextDecl);

        ExprResult E = getDerived().TransformExpr(TL.getUnderlyingExpr());
        if(E.isInvalid()) return QualType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformTypeOfType(TypeLocBuilder &TLB, TypeOfTypeLoc TL){
        TypeSourceInfo *Old_Under_TI	= TL.getUnderlyingTInfo();
        TypeSourceInfo *New_Under_TI	= getDerived().TransformType(Old_Under_TI);
        if(! New_Under_TI) return QualType();
        QualType Result = TL.getType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDecltypeType(TypeLocBuilder &TLB, DecltypeTypeLoc TL){
    ...
    QualType TreeTransform<Derived>::TransformUnaryTransformType(
        TypeLocBuilder &TLB, UnaryTransformTypeLoc TL){
        QualType Result = TL.getType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformAutoType(TypeLocBuilder &TLB, AutoTypeLoc TL){
        const AutoType *T	= TL.getTypePtr();
        QualType OldDeduced = T->getDeducedType();
        QualType NewDeduced;
        if(! OldDeduced.isNull()){
            NewDeduced = getDerived().TransformType(OldDeduced);
            if(NewDeduced.isNull()) return QualType();
        }
        QualType Result = TL.getType();
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformRecordType(TypeLocBuilder &TLB, RecordTypeLoc TL){
        const RecordType *T = TL.getTypePtr();
        RecordDecl *Record
            = cast_or_null<RecordDecl>(getDerived().TransformDecl(TL.getNameLoc()
                                                                 , T->getDecl()));
        if(! Record) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
            Record != T->getDecl()){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformEnumType(TypeLocBuilder &TLB, EnumTypeLoc TL){
        const EnumType *T = TL.getTypePtr();
        EnumDecl *Enum
            = cast_or_null<EnumDecl>(getDerived().TransformDecl(TL.getNameLoc()
                                                               , T->getDecl()));
        if(! Enum) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
            Enum != T->getDecl()){
    ...
    QualType TreeTransform<Derived>::TransformSubstTemplateTypeParmType(
        ...
        if(Replacement.isNull()) return QualType();
    ...
    QualType TreeTransform<Derived>::TransformTemplateSpecializationType(
        ...
        if(Template.isNull()) return QualType();
        return getDerived().TransformTemplateSpecializationType(TLB, TL, Template);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformAtomicType(TypeLocBuilder &TLB, AtomicTypeLoc TL){
        QualType ValueType = getDerived().TransformType(TLB, TL.getValueLoc());
        if(ValueType.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
    ...
    template <typename Derived>
    QualType TreeTransform<Derived>::TransformPipeType(TypeLocBuilder &TLB, PipeTypeLoc TL){
        QualType ValueType = getDerived().TransformType(TLB, TL.getValueLoc());
        if(ValueType.isNull()) return QualType();
        QualType Result = TL.getType();
    ...
    template <typename Derived>
    QualType TreeTransform<Derived>::TransformTemplateSpecializationType(
        TypeLocBuilder &TLB, TemplateSpecializationTypeLoc TL, TemplateName Template){
        ...
        if(getDerived().TransformTemplateArguments(ArgIterator(TL, 0)
                                                  ...
                                                  , NewTemplateArgs)) return QualType();
        ...
        if(! Result.isNull()){
            ...
            if(isa<DependentTemplateSpecializationType>(Result)){
                ...
                for(unsigned i = 0, e = NewTemplateArgs.size(); i != e; ++ i) NewTL.setArgLocInfo(i, NewTemplateArgs[i].getLocInfo());
                return Result;
            }
    ...
    QualType TreeTransform<Derived>::TransformDependentTemplateSpecializationType(
        TypeLocBuilder &TLB, DependentTemplateSpecializationTypeLoc TL, TemplateName Template, CXXScopeSpec &SS){
        TemplateArgumentListInfo NewTemplateArgs;
        NewTemplateArgs.setLAngleLoc(TL.getLAngleLoc());
        NewTemplateArgs.setRAngleLoc(TL.getRAngleLoc());
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformElaboratedType(TypeLocBuilder &TLB, ElaboratedTypeLoc TL){
        ...
        if(TL.getQualifierLoc()){
            ...
            if(! QualifierLoc) return QualType();
        }
        QualType NamedT = getDerived().TransformType(TLB, TL.getNamedTypeLoc());
        if(NamedT.isNull()) return QualType();
    ...
    QualType TreeTransform<Derived>::TransformAttributedType(
        ...
        if(modifiedType.isNull()) return QualType();
        QualType result = TL.getType();
        ...
        if(getDerived().AlwaysRebuild() ||
            ...
            if(equivalentType.isNull()) return QualType();
        ...
        if(TL.hasAttrOperand()) newTL.setAttrOperandParensRange(TL.getAttrOperandParensRange());
        if(TL.hasAttrExprOperand()) newTL.setAttrExprOperand(TL.getAttrExprOperand());
        else if(TL.hasAttrEnumOperand()) newTL.setAttrEnumOperandLoc(TL.getAttrEnumOperandLoc());
        return result;
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformParenType(TypeLocBuilder &TLB, ParenTypeLoc TL){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDependentNameType(TypeLocBuilder &TLB, DependentNameTypeLoc TL){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDependentTemplateSpecializationType(TypeLocBuilder &TLB, DependentTemplateSpecializationTypeLoc TL){
        NestedNameSpecifierLoc QualifierLoc;
        if(TL.getQualifierLoc()){
            QualifierLoc
                = getDerived().TransformNestedNameSpecifierLoc(TL.getQualifierLoc());
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformDependentTemplateSpecializationType(TypeLocBuilder &TLB, DependentTemplateSpecializationTypeLoc TL, NestedNameSpecifierLoc QualifierLoc){
        ...
        if(const ElaboratedType *ElabT = dyn_cast<ElaboratedType>(Result)){
            ...
            for(unsigned I = 0, E = NewTemplateArgs.size(); I != E; ++ I) NamedTL.setArgLocInfo(I, NewTemplateArgs[I].getLocInfo());
            ...
            for(unsigned I = 0, E = NewTemplateArgs.size(); I != E; ++ I) SpecTL.setArgLocInfo(I, NewTemplateArgs[I].getLocInfo());
        }else{
            TemplateSpecializationTypeLoc SpecTL
                = TLB.push<TemplateSpecializationTypeLoc>(Result);
            SpecTL.setTemplateKeywordLoc(TL.getTemplateKeywordLoc());
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformPackExpansionType(TypeLocBuilder &TLB, PackExpansionTypeLoc TL){
        QualType Pattern
            = getDerived().TransformType(TLB, TL.getPatternLoc());
        if(Pattern.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformObjCInterfaceType(TypeLocBuilder &TLB, ObjCInterfaceTypeLoc TL){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformObjCTypeParamType(TypeLocBuilder &TLB, ObjCTypeParamTypeLoc TL){
        ...
        if(TL.getNumProtocols()){
            ...
            for(unsigned i = 0, n = TL.getNumProtocols(); i != n; ++ i) NewTL.setProtocolLoc(i, TL.getProtocolLoc(i));
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::TransformObjCObjectType(TypeLocBuilder &TLB, ObjCObjectTypeLoc TL){
        // Transform base type.
        QualType BaseType = getDerived().TransformType(TLB, TL.getBaseLoc());
        if(BaseType.isNull()) return QualType();
            ...
            if(auto PackExpansionLoc = TypeArgLoc.getAs<PackExpansionTypeLoc>()){
                ...
                if(getDerived().TryExpandParameterPacks(
                        PackExpansionLoc.getEllipsisLoc(), PatternLoc.getSourceRange()
                                                       , Unexpanded, Expand, RetainExpansion, NumExpansions)) return QualType();
                if(! Expand){
                    // We can't expand this pack expansion into separate arguments yet;
                    // just substitute into the pattern and create a new pack expansion
                    // type.
                    ...
                    QualType NewPatternType = getDerived().TransformType(TypeArgBuilder
                                                                        , PatternLoc);
                    if(NewPatternType.isNull()) return QualType();
                ...
                for(unsigned ArgIdx = 0; ArgIdx != *NumExpansions; ++ ArgIdx){
                    ...
                    QualType NewTypeArg = getDerived().TransformType(TypeArgBuilder
                                                                    , PatternLoc);
                    if(NewTypeArg.isNull()) return QualType();
                    NewTypeArgInfos.push_back(
        ...
        for(unsigned i = 0, n = TL.getNumTypeArgs(); i != n; ++ i) NewT.setTypeArgTInfo(i, NewTypeArgInfos[i]);
        NewT.setTypeArgsRAngleLoc(TL.getTypeArgsRAngleLoc());
        NewT.setProtocolLAngleLoc(TL.getProtocolLAngleLoc());
        for(unsigned i = 0, n = TL.getNumProtocols(); i != n; ++ i) NewT.setProtocolLoc(i, TL.getProtocolLoc(i));
        NewT.setProtocolRAngleLoc(TL.getProtocolRAngleLoc());
        return Result;
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::TransformObjCObjectPointerType(TypeLocBuilder &TLB, ObjCObjectPointerTypeLoc TL){
        QualType PointeeType = getDerived().TransformType(TLB, TL.getPointeeLoc());
        if(PointeeType.isNull()) return QualType();
        QualType Result = TL.getType();
        if(getDerived().AlwaysRebuild() ||
...
//= ==----------------------------------------------------------------------===//
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformCompoundStmt(CompoundStmt *S, bool IsStmtExpr){
        ...
        for(auto *B : S->body()){
            ...
            if(Result.isInvalid()){
                ...
                if(isa<DeclStmt>(B)) return StmtError();
        ...
        if(SubStmtInvalid) return StmtError();
        if(! getDerived().AlwaysRebuild() &&
            ! SubStmtChanged) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformCaseStmt(CaseStmt *S){
        ...
        StmtResult Case = getDerived().RebuildCaseStmt(S->getCaseLoc()
                                                      ...
                                                      , RHS.get()
                                                      , S->getColonLoc());
        if(Case.isInvalid()) return StmtError();
        // Transform the statement following the case
        StmtResult SubStmt = getDerived().TransformStmt(S->getSubStmt());
        if(SubStmt.isInvalid()) return StmtError();
...
#include "clang/Basic/AttrList.inc"
    ...
    StmtResult TreeTransform<Derived>::TransformAttributedStmt(AttributedStmt *S){
        ...
        StmtResult SubStmt = getDerived().TransformStmt(S->getSubStmt());
        if(SubStmt.isInvalid()) return StmtError();
        if(SubStmt.get() == S->getSubStmt() && ! AttrsChanged) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformIfStmt(IfStmt *S){
        ...
        if(S->isConstexpr()) ConstexprConditionValue = Cond.getKnownValue();
        // Transform the "then" branch.
        StmtResult Then;
        if(! ConstexprConditionValue || *ConstexprConditionValue){
            Then = getDerived().TransformStmt(S->getThen());
            if(Then.isInvalid()) return StmtError();
        }else Then = new(getSema().Context)NullStmt(S->getThen()->getLocStart());
        // Transform the "else" branch.
        StmtResult Else;
        if(! ConstexprConditionValue || ! *ConstexprConditionValue){
            Else = getDerived().TransformStmt(S->getElse());
            if(Else.isInvalid()) return StmtError();
        }
        if(! getDerived().AlwaysRebuild() &&
            Init.get() == S->getInit() &&
            Cond.get() == std::make_pair(S->getConditionVariable(), S->getCond()) &&
            Then.get() == S->getThen() &&
            Else.get() == S->getElse()) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformSwitchStmt(SwitchStmt *S){
        ...
        StmtResult Switch
            = getDerived().RebuildSwitchStmtStart(S->getSwitchLoc()
                                                 , S->getInit(), Cond);
        if(Switch.isInvalid()) return StmtError();
    ...
    StmtResult TreeTransform<Derived>::TransformWhileStmt(WhileStmt *S){
        ...
        if(! getDerived().AlwaysRebuild() &&
            Cond.get() == std::make_pair(S->getConditionVariable(), S->getCond()) &&
            Body.get() == S->getBody()) return Owned(S);
    ...
    StmtResult TreeTransform<Derived>::TransformForStmt(ForStmt *S){
        ...
        if(getSema().getLangOpts().OpenMP && Init.isUsable()) getSema().ActOnOpenMPLoopInitialization(S->getForLoc(), Init.get());
        // Transform the condition
        Sema::ConditionResult Cond = getDerived().TransformCondition(
        ...
        if(S->getInc() && ! FullInc.get()) return StmtError();
        // Transform the body
        StmtResult Body = getDerived().TransformStmt(S->getBody());
        if(Body.isInvalid()) return StmtError();
        if(! getDerived().AlwaysRebuild() &&
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformIndirectGotoStmt(IndirectGotoStmt *S){
        ExprResult Target = getDerived().TransformExpr(S->getTarget());
        if(Target.isInvalid()) return StmtError();
        Target = SemaRef.MaybeCreateExprWithCleanups(Target.get());
        if(! getDerived().AlwaysRebuild() &&
            Target.get() == S->getTarget()) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformDeclStmt(DeclStmt *S){
        ...
        for(auto *D : S->decls()){
            ...
            if(! Transformed) return StmtError();
            if(Transformed != D) DeclChanged = true;
            Decls.push_back(Transformed);
        }
        if(! getDerived().AlwaysRebuild() && ! DeclChanged) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformGCCAsmStmt(GCCAsmStmt *S){
        ...
        if(! getDerived().AlwaysRebuild() && ! ExprsChanged) return S;
        // Go through the clobbers.
        for(unsigned I = 0, E = S->getNumClobbers(); I != E; ++ I) Clobbers.push_back(S->getClobberStringLiteral(I));
    ...
    StmtResult TreeTransform<Derived>::TransformMSAsmStmt(MSAsmStmt *S){
        ...
        for(unsigned i = 0, e = SrcExprs.size(); i != e; ++ i){
            ...
            if(! Result.isUsable()) HadError = true;
            else{
        ...
        if(! HadChange && ! getDerived().AlwaysRebuild()) return Owned(S);
...
// C++ Coroutines TS
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformCoroutineBodyStmt(CoroutineBodyStmt *S){
...
// Objective-C Statements.
    ...
    StmtResult TreeTransform<Derived>::TransformObjCAtTryStmt(ObjCAtTryStmt *S){
        ...
        if(TryBody.isInvalid()) return StmtError();
        // Transform the @catch statements (if present).
        bool AnyCatchChanged = false;
        SmallVector<Stmt *, 8> CatchStmts;
        for(unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++ I){
            StmtResult Catch = getDerived().TransformStmt(S->getCatchStmt(I));
            if(Catch.isInvalid()) return StmtError();
            if(Catch.get() != S->getCatchStmt(I)) AnyCatchChanged = true;
        ...
        if(S->getFinallyStmt()){
            ...
            if(Finally.isInvalid()) return StmtError();
        }
        // If nothing changed, just retain this statement.
        if(! getDerived().AlwaysRebuild() &&
            TryBody.get() == S->getTryBody() &&
            ! AnyCatchChanged &&
            Finally.get() == S->getFinallyStmt()) return S;
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCAtCatchStmt(ObjCAtCatchStmt *S){
        ...
        if(VarDecl *FromVar = S->getCatchParamDecl()){
            ...
            if(FromVar->getTypeSourceInfo()){
                ...
                if(! TSInfo) return StmtError();
            }
            QualType T;
            if(TSInfo) T = TSInfo->getType();
            else{
                T = getDerived().TransformType(FromVar->getType());
                if(T.isNull()) return StmtError();
            }
            Var = getDerived().RebuildObjCExceptionDecl(FromVar, TSInfo, T);
            if(! Var) return StmtError();
        }
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCAtFinallyStmt(ObjCAtFinallyStmt *S){
        // Transform the body.
        StmtResult Body = getDerived().TransformStmt(S->getFinallyBody());
        if(Body.isInvalid()) return StmtError();
        // If nothing changed, just retain this statement.
        if(! getDerived().AlwaysRebuild() &&
            Body.get() == S->getFinallyBody()) return S;
        // Build a new statement.
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCAtThrowStmt(ObjCAtThrowStmt *S){
        ExprResult Operand;
        if(S->getThrowExpr()){
            Operand = getDerived().TransformExpr(S->getThrowExpr());
            if(Operand.isInvalid()) return StmtError();
        }
        if(! getDerived().AlwaysRebuild() &&
            Operand.get() == S->getThrowExpr()) return S;
        return getDerived().RebuildObjCAtThrowStmt(S->getThrowLoc(), Operand.get());
    }

    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCAtSynchronizedStmt(ObjCAtSynchronizedStmt *S){
        // Transform the object we are locking.
        ExprResult Object = getDerived().TransformExpr(S->getSynchExpr());
        if(Object.isInvalid()) return StmtError();
        Object =
        ...
        if(! getDerived().AlwaysRebuild() &&
            ...
            Body.get() == S->getSynchBody()) return S;
        // Build a new statement.
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCAutoreleasePoolStmt(ObjCAutoreleasePoolStmt *S){
        // Transform the body.
        StmtResult Body = getDerived().TransformStmt(S->getSubStmt());
        if(Body.isInvalid()) return StmtError();
        // If nothing changed, just retain this statement.
        if(! getDerived().AlwaysRebuild() &&
            Body.get() == S->getSubStmt()) return S;
        // Build a new statement.
    ...
    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformObjCForCollectionStmt(ObjCForCollectionStmt *S){
        // Transform the element statement.
        StmtResult Element = getDerived().TransformStmt(S->getElement());
        if(Element.isInvalid()) return StmtError();
        // Transform the collection expression.
        ExprResult Collection = getDerived().TransformExpr(S->getCollection());
        if(Collection.isInvalid()) return StmtError();
        // Transform the body.
        StmtResult Body = getDerived().TransformStmt(S->getBody());
        if(Body.isInvalid()) return StmtError();
        // If nothing changed, just retain this statement.
    ...
    StmtResult TreeTransform<Derived>::TransformCXXCatchStmt(CXXCatchStmt *S){
        ...
        if(Handler.isInvalid()) return StmtError();
        if(! getDerived().AlwaysRebuild() && ! Var &&
            Handler.get() == S->getHandlerBlock()) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformCXXTryStmt(CXXTryStmt *S){
        ...
        if(! getDerived().AlwaysRebuild() && TryBlock.get() == S->getTryBlock() &&
            ! HandlerChanged) return S;
        return getDerived().RebuildCXXTryStmt(S->getTryLoc(), TryBlock.get()
                                             , Handlers);
    }

    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformCXXForRangeStmt(CXXForRangeStmt *S){
        StmtResult Range = getDerived().TransformStmt(S->getRangeStmt());
        if(Range.isInvalid()) return StmtError();
        StmtResult Begin = getDerived().TransformStmt(S->getBeginStmt());
        if(Begin.isInvalid()) return StmtError();
        StmtResult End = getDerived().TransformStmt(S->getEndStmt());
        if(End.isInvalid()) return StmtError();
        ExprResult Cond = getDerived().TransformExpr(S->getCond());
        if(Cond.isInvalid()) return StmtError();
        if(Cond.get()) Cond = SemaRef.CheckBooleanCondition(S->getColonLoc(), Cond.get());
        if(Cond.isInvalid()) return StmtError();
        if(Cond.get()) Cond = SemaRef.MaybeCreateExprWithCleanups(Cond.get());
        ExprResult Inc = getDerived().TransformExpr(S->getInc());
        if(Inc.isInvalid()) return StmtError();
        if(Inc.get()) Inc = SemaRef.MaybeCreateExprWithCleanups(Inc.get());
        StmtResult LoopVar = getDerived().TransformStmt(S->getLoopVarStmt());
        if(LoopVar.isInvalid()) return StmtError();
        ...
        if(getDerived().AlwaysRebuild() ||
            ...
            NewStmt = getDerived().RebuildCXXForRangeStmt(S->getForLoc()
                                                         ...
                                                         , Inc.get(), LoopVar.get()
                                                         , S->getRParenLoc());
            if(NewStmt.isInvalid()) return StmtError();
        }
        StmtResult Body = getDerived().TransformStmt(S->getBody());
        if(Body.isInvalid()) return StmtError();
        ...
        if(NewStmt.get() == S) return S;
        return FinishCXXForRangeStmt(NewStmt.get(), Body.get());
    }

    template<typename Derived>
    StmtResult TreeTransform<Derived>::TransformMSDependentExistsStmt(MSDependentExistsStmt *S){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformMSPropertyRefExpr(MSPropertyRefExpr *E){
        NestedNameSpecifierLoc QualifierLoc;
    ...
    template <typename Derived>
    ExprResult TreeTransform<Derived>::TransformMSPropertySubscriptExpr(MSPropertySubscriptExpr *E){
        auto BaseRes = getDerived().TransformExpr(E->getBase());
        if(BaseRes.isInvalid()) return ExprError();
        auto IdxRes = getDerived().TransformExpr(E->getIdx());
        if(IdxRes.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            BaseRes.get() == E->getBase() &&
            IdxRes.get() == E->getIdx()) return E;
    ...
    StmtResult TreeTransform<Derived>::TransformSEHTryStmt(SEHTryStmt *S){
        ...
        if(Handler.isInvalid()) return StmtError();
        if(! getDerived().AlwaysRebuild() && TryBlock.get() == S->getTryBlock() &&
            Handler.get() == S->getHandler()) return S;
    ...
    StmtResult TreeTransform<Derived>::TransformSEHHandler(Stmt *Handler){
        if(isa<SEHFinallyStmt>(Handler)) return getDerived().TransformSEHFinallyStmt(cast<SEHFinallyStmt>(Handler));
        else return getDerived().TransformSEHExceptStmt(cast<SEHExceptStmt>(Handler));
    }
...
//= ==----------------------------------------------------------------------===//
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPExecutableDirective(OMPExecutableDirective *D){
        ...
        for(ArrayRef<OMPClause *>::iterator I = Clauses.begin(), E = Clauses.end();
            ...
            if(*I){
                ...
                if(Clause) TClauses.push_back(Clause);
            }else TClauses.push_back(nullptr);
        }
        ...
        if(D->hasAssociatedStmt() && D->getAssociatedStmt()){
            ...
            if(AssociatedStmt.isInvalid()) return StmtError();
        }
        if(TClauses.size() != Clauses.size()) return StmtError();
        ...
        if(D->getDirectiveKind() == OMPD_cancellation_point) CancelRegion = cast<OMPCancellationPointDirective>(D)->getCancelRegion();
        else if(D->getDirectiveKind() == OMPD_cancel) CancelRegion = cast<OMPCancelDirective>(D)->getCancelRegion();
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPParallelDirective(OMPParallelDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPSimdDirective(OMPSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPForDirective(OMPForDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPForSimdDirective(OMPForSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPSectionsDirective(OMPSectionsDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPSectionDirective(OMPSectionDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPSingleDirective(OMPSingleDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPMasterDirective(OMPMasterDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPCriticalDirective(OMPCriticalDirective *D){
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPParallelForDirective(OMPParallelForDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPParallelForSimdDirective(OMPParallelForSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPParallelSectionsDirective(OMPParallelSectionsDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskDirective(OMPTaskDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskyieldDirective(OMPTaskyieldDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPBarrierDirective(OMPBarrierDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskwaitDirective(OMPTaskwaitDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskgroupDirective(OMPTaskgroupDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPFlushDirective(OMPFlushDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPOrderedDirective(OMPOrderedDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPAtomicDirective(OMPAtomicDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetDirective(OMPTargetDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetDataDirective(OMPTargetDataDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetEnterDataDirective(OMPTargetEnterDataDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetExitDataDirective(OMPTargetExitDataDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetParallelDirective(OMPTargetParallelDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetParallelForDirective(OMPTargetParallelForDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetUpdateDirective(OMPTargetUpdateDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTeamsDirective(OMPTeamsDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPCancellationPointDirective(OMPCancellationPointDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPCancelDirective(OMPCancelDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskLoopDirective(OMPTaskLoopDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTaskLoopSimdDirective(OMPTaskLoopSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPDistributeDirective(OMPDistributeDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPDistributeParallelForDirective(OMPDistributeParallelForDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPDistributeParallelForSimdDirective(OMPDistributeParallelForSimdDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPDistributeSimdDirective(OMPDistributeSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetParallelForSimdDirective(OMPTargetParallelForSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetSimdDirective(OMPTargetSimdDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTeamsDistributeDirective(OMPTeamsDistributeDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTeamsDistributeSimdDirective(OMPTeamsDistributeSimdDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetTeamsDirective(OMPTargetTeamsDirective *D){
        DeclarationNameInfo DirName;
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTeamsDistributeParallelForDirective(OMPTeamsDistributeParallelForDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetTeamsDistributeParallelForDirective(OMPTargetTeamsDistributeParallelForDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetTeamsDistributeParallelForSimdDirective(OMPTargetTeamsDistributeParallelForSimdDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetTeamsDistributeDirective(OMPTargetTeamsDistributeDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTargetTeamsDistributeSimdDirective(OMPTargetTeamsDistributeSimdDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
    ...
    template <typename Derived>
    StmtResult TreeTransform<Derived>::TransformOMPTeamsDistributeParallelForSimdDirective(OMPTeamsDistributeParallelForSimdDirective *D){
        DeclarationNameInfo DirName;
        getDerived().getSema().StartOpenMPDSABlock(
...
//= ==----------------------------------------------------------------------===//
    ...
    OMPClause *TreeTransform<Derived>::TransformOMPIfClause(OMPIfClause *C){
        ...
        if(Cond.isInvalid()) return nullptr;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPNumThreadsClause(OMPNumThreadsClause *C){
        ExprResult NumThreads = getDerived().TransformExpr(C->getNumThreads());
        if(NumThreads.isInvalid()) return nullptr;
        return getDerived().RebuildOMPNumThreadsClause(
            NumThreads.get(), C->getLocStart(), C->getLParenLoc(), C->getLocEnd());
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPSafelenClause(OMPSafelenClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPSimdlenClause(OMPSimdlenClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPCollapseClause(OMPCollapseClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPDefaultClause(OMPDefaultClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPProcBindClause(OMPProcBindClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPScheduleClause(OMPScheduleClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPOrderedClause(OMPOrderedClause *C){
        ExprResult E;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPNowaitClause(OMPNowaitClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPUntiedClause(OMPUntiedClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPMergeableClause(OMPMergeableClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPUpdateClause(OMPUpdateClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPCaptureClause(OMPCaptureClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPSeqCstClause(OMPSeqCstClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPThreadsClause(OMPThreadsClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPNogroupClause(OMPNogroupClause *C){
        // No need to rebuild this clause, no template-dependent parameters.
        return C;
    }

    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPPrivateClause(OMPPrivateClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
            if(EVar.isInvalid()) return nullptr;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPFirstprivateClause(OMPFirstprivateClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPLastprivateClause(OMPLastprivateClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPSharedClause(OMPSharedClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPReductionClause(OMPReductionClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
        ...
        if(NameInfo.getName()){
            NameInfo = getDerived().TransformDeclarationNameInfo(NameInfo);
            if(! NameInfo.getName()) return nullptr;
        }
        ...
        for(auto *E : C->reduction_ops()){
            ...
            }else UnresolvedReductions.push_back(nullptr);
        }
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPLinearClause(OMPLinearClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
        ...
        if(Step.isInvalid()) return nullptr;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPAlignedClause(OMPAlignedClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
        ...
        if(Alignment.isInvalid()) return nullptr;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPCopyinClause(OMPCopyinClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPCopyprivateClause(OMPCopyprivateClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPDependClause(OMPDependClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPDeviceClause(OMPDeviceClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPNumTeamsClause(OMPNumTeamsClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPThreadLimitClause(OMPThreadLimitClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPPriorityClause(OMPPriorityClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPGrainsizeClause(OMPGrainsizeClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPNumTasksClause(OMPNumTasksClause *C){
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPDistScheduleClause(OMPDistScheduleClause *C){
        ExprResult E = getDerived().TransformExpr(C->getChunkSize());
        if(E.isInvalid()) return nullptr;
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPDefaultmapClause(OMPDefaultmapClause *C){
        return C;
    }
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPUseDevicePtrClause(OMPUseDevicePtrClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    ...
    template <typename Derived>
    OMPClause *TreeTransform<Derived>::TransformOMPIsDevicePtrClause(OMPIsDevicePtrClause *C){
        llvm::SmallVector<Expr *, 16> Vars;
        Vars.reserve(C->varlist_size());
        for(auto *VE : C->varlists()){
            ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
...
//= ==----------------------------------------------------------------------===//
    ...
    ExprResult TreeTransform<Derived>::TransformDeclRefExpr(DeclRefExpr *E){
        ...
        if(NameInfo.getName()){
            NameInfo = getDerived().TransformDeclarationNameInfo(NameInfo);
            if(! NameInfo.getName()) return ExprError();
        }
        if(! getDerived().AlwaysRebuild() &&
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformFloatingLiteral(FloatingLiteral *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformImaginaryLiteral(ImaginaryLiteral *E){
        return E;
    }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCharacterLiteral(CharacterLiteral *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformUserDefinedLiteral(UserDefinedLiteral *E){
        if(FunctionDecl *FD = E->getDirectCallee()) SemaRef.MarkFunctionReferenced(E->getLocStart(), FD);
        return SemaRef.MaybeBindToTemporary(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformGenericSelectionExpr(GenericSelectionExpr *E){
        ExprResult ControllingExpr =
            getDerived().TransformExpr(E->getControllingExpr());
        if(ControllingExpr.isInvalid()) return ExprError();
        ...
        for(unsigned i = 0; i != E->getNumAssocs(); ++ i){
            ...
            if(TS){
                ...
                if(! AssocType) return ExprError();
                AssocTypes.push_back(AssocType);
            }else AssocTypes.push_back(nullptr);
            ExprResult AssocExpr = getDerived().TransformExpr(E->getAssocExpr(i));
            if(AssocExpr.isInvalid()) return ExprError();
    ...
    ExprResult TreeTransform<Derived>::TransformParenExpr(ParenExpr *E){
        ...
        if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getSubExpr()) return E;
...
/// object available.
    ...
    ExprResult TreeTransform<Derived>::TransformAddressOfOperand(Expr *E){
        if(DependentScopeDeclRefExpr *DRE = dyn_cast<DependentScopeDeclRefExpr>(E)) return getDerived().TransformDependentScopeDeclRefExpr(DRE, true, nullptr);
        else return getDerived().TransformExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformUnaryOperator(UnaryOperator *E){
        ExprResult SubExpr;
        if(E->getOpcode() == UO_AddrOf) SubExpr = TransformAddressOfOperand(E->getSubExpr());
        else SubExpr = TransformExpr(E->getSubExpr());
        if(SubExpr.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getSubExpr()) return E;
        ...
        for(unsigned I = 0, N = E->getNumComponents(); I != N; ++ I){
            ...
            switch(ON.getKind()){
                case OffsetOfNode::Array: {
                    ...
                    if(Index.isInvalid()) return ExprError();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformOpaqueValueExpr(OpaqueValueExpr *E){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformPseudoObjectExpr(PseudoObjectExpr *E){
        ...
        // should reapply.
        if(result.get()->hasPlaceholderType(BuiltinType::PseudoObject)) result = SemaRef.checkPseudoObjectRValue(result.get());
        return result;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *E){
        if(E->isArgumentType()){
            TypeSourceInfo *OldT = E->getArgumentTypeInfo();

            TypeSourceInfo *NewT = getDerived().TransformType(OldT);
            if(! NewT) return ExprError();
            if(! getDerived().AlwaysRebuild() && OldT == NewT) return E;
        ...
        if(auto *DRE =
            PE ? dyn_cast<DependentScopeDeclRefExpr>(PE->getSubExpr()) : nullptr) SubExpr = getDerived().TransformParenDependentScopeDeclRefExpr(
                    PE, DRE, false, &RecoveryTSI);
        else SubExpr = getDerived().TransformExpr(E->getArgumentExpr());
        if(RecoveryTSI){
            return getDerived().RebuildUnaryExprOrTypeTrait(
                RecoveryTSI, E->getOperatorLoc(), E->getKind(), E->getSourceRange());
        }else if(SubExpr.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getArgumentExpr()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformArraySubscriptExpr(ArraySubscriptExpr *E){
    ...
    template <typename Derived>
    ExprResult TreeTransform<Derived>::TransformOMPArraySectionExpr(OMPArraySectionExpr *E){
        ExprResult Base = getDerived().TransformExpr(E->getBase());
        if(Base.isInvalid()) return ExprError();
        ExprResult LowerBound;
        if(E->getLowerBound()){
            LowerBound = getDerived().TransformExpr(E->getLowerBound());
            if(LowerBound.isInvalid()) return ExprError();
        }
        ExprResult Length;
        if(E->getLength()){
            Length = getDerived().TransformExpr(E->getLength());
            if(Length.isInvalid()) return ExprError();
        }
        if(! getDerived().AlwaysRebuild() && Base.get() == E->getBase() &&
            LowerBound.get() == E->getLowerBound() && Length.get() == E->getLength()) return E;
    ...
    ExprResult TreeTransform<Derived>::TransformCallExpr(CallExpr *E){
        ...
        if(getDerived().TransformExprs(E->getArgs(), E->getNumArgs(), true, Args
                                      , &ArgChanged)) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            Callee.get() == E->getCallee() &&
            ! ArgChanged) return SemaRef.MaybeBindToTemporary(E);
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCompoundAssignOperator(CompoundAssignOperator *E){
        return getDerived().TransformBinaryOperator(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformBinaryConditionalOperator(BinaryConditionalOperator *e){
        // Just rebuild the common and RHS expressions and see whether we
        // get any changes.

        ExprResult commonExpr = getDerived().TransformExpr(e->getCommon());
        if(commonExpr.isInvalid()) return ExprError();
        ExprResult rhs = getDerived().TransformExpr(e->getFalseExpr());
        if(rhs.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            commonExpr.get() == e->getCommon() &&
            rhs.get() == e->getFalseExpr()) return e;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformConditionalOperator(ConditionalOperator *E){
        ExprResult Cond = getDerived().TransformExpr(E->getCond());
        if(Cond.isInvalid()) return ExprError();
        ExprResult LHS = getDerived().TransformExpr(E->getLHS());
        if(LHS.isInvalid()) return ExprError();
        ExprResult RHS = getDerived().TransformExpr(E->getRHS());
        if(RHS.isInvalid()) return ExprError();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformImplicitCastExpr(ImplicitCastExpr *E){
    ...
    ExprResult TreeTransform<Derived>::TransformCStyleCastExpr(CStyleCastExpr *E){
        ...
        if(! getDerived().AlwaysRebuild() &&
            ...
            SubExpr.get() == E->getSubExpr()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCompoundLiteralExpr(CompoundLiteralExpr *E){
        TypeSourceInfo *OldT	= E->getTypeSourceInfo();
        TypeSourceInfo *NewT	= getDerived().TransformType(OldT);
        if(! NewT) return ExprError();
        ExprResult Init = getDerived().TransformExpr(E->getInitializer());
        if(Init.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            OldT == NewT &&
            Init.get() == E->getInitializer()) return SemaRef.MaybeBindToTemporary(E);
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformExtVectorElementExpr(ExtVectorElementExpr *E){
        ExprResult Base = getDerived().TransformExpr(E->getBase());
        if(Base.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            Base.get() == E->getBase()) return E;
    ...
    ExprResult TreeTransform<Derived>::TransformInitListExpr(InitListExpr *E){
        if(InitListExpr *Syntactic = E->getSyntacticForm()) E = Syntactic;
        bool InitChanged = false;

        SmallVector<Expr *, 4> Inits;
        if(getDerived().TransformExprs(E->getInits(), E->getNumInits(), false
                                      , Inits, &InitChanged)) return ExprError();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformDesignatedInitExpr(DesignatedInitExpr *E){
        Designation Desig;
        ...
        for(const DesignatedInitExpr::Designator &D : E->designators()){
            ...
            if(Start.isInvalid()) return ExprError();
            ExprResult End = getDerived().TransformExpr(E->getArrayRangeEnd(D));
            if(End.isInvalid()) return ExprError();
...
// InitListExpr, then a DesignatedInitUpdateExpr is not encountered.
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformDesignatedInitUpdateExpr(DesignatedInitUpdateExpr *E){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformImplicitValueInitExpr(ImplicitValueInitExpr *E){
    ...
    ExprResult TreeTransform<Derived>::TransformParenListExpr(ParenListExpr *E){
        ...
        if(TransformExprs(E->getExprs(), E->getNumExprs(), true, Inits
                         , &ArgumentChanged)) return ExprError();
...
/// the corresponding label statement by semantic analysis.
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXOperatorCallExpr(CXXOperatorCallExpr *E){
        switch(E->getOperator()){
            ...
            case OO_Call: {
                ...
                if(Object.isInvalid()) return ExprError();
...
#include "clang/Basic/OperatorKinds.def"
        ...
        if(Callee.isInvalid()) return ExprError();
        ExprResult First;
        if(E->getOperator() == OO_Amp) First = getDerived().TransformAddressOfOperand(E->getArg(0));
        else First = getDerived().TransformExpr(E->getArg(0));
        if(First.isInvalid()) return ExprError();
        ExprResult Second;
        if(E->getNumArgs() == 2){
            Second = getDerived().TransformExpr(E->getArg(1));
            if(Second.isInvalid()) return ExprError();
        }
        if(! getDerived().AlwaysRebuild() &&
            Callee.get() == E->getCallee() &&
            First.get() == E->getArg(0) &&
            (E->getNumArgs() != 2 || Second.get() == E->getArg(1))) return SemaRef.MaybeBindToTemporary(E);
        Sema::FPContractStateRAII FPContractState(getSema());
        getSema().FPFeatures.fp_contract = E->isFPContractable();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXMemberCallExpr(CXXMemberCallExpr *E){
        return getDerived().TransformCallExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCUDAKernelCallExpr(CUDAKernelCallExpr *E){
        // Transform the callee.
        ExprResult Callee = getDerived().TransformExpr(E->getCallee());
        if(Callee.isInvalid()) return ExprError();
        // Transform exec config.
        ExprResult EC = getDerived().TransformCallExpr(E->getConfig());
        if(EC.isInvalid()) return ExprError();
        // Transform arguments.
        bool ArgChanged = false;
        SmallVector<Expr *, 8> Args;
        if(getDerived().TransformExprs(E->getArgs(), E->getNumArgs(), true, Args
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXNamedCastExpr(CXXNamedCastExpr *E){
        TypeSourceInfo *Type = getDerived().TransformType(E->getTypeInfoAsWritten());
        if(! Type) return ExprError();
        ExprResult SubExpr
            = getDerived().TransformExpr(E->getSubExprAsWritten());
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXStaticCastExpr(CXXStaticCastExpr *E){
        return getDerived().TransformCXXNamedCastExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXDynamicCastExpr(CXXDynamicCastExpr *E){
        return getDerived().TransformCXXNamedCastExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXReinterpretCastExpr(CXXReinterpretCastExpr *E){
        return getDerived().TransformCXXNamedCastExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXConstCastExpr(CXXConstCastExpr *E){
        return getDerived().TransformCXXNamedCastExpr(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXFunctionalCastExpr(CXXFunctionalCastExpr *E){
        TypeSourceInfo *Type = getDerived().TransformType(E->getTypeInfoAsWritten());
        if(! Type) return ExprError();
        ExprResult SubExpr
            = getDerived().TransformExpr(E->getSubExprAsWritten());
    ...
    ExprResult TreeTransform<Derived>::TransformCXXTypeidExpr(CXXTypeidExpr *E){
        if(E->isTypeOperand()){
            ...
            if(! TInfo) return ExprError();
            if(! getDerived().AlwaysRebuild() &&
                TInfo == E->getTypeOperandSourceInfo()) return E;
        ...
        if(SubExpr.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            SubExpr.get() == E->getExprOperand()) return E;
        return getDerived().RebuildCXXTypeidExpr(E->getType()
                                                , E->getLocStart()
                                                , SubExpr.get()
                                                , E->getLocEnd());
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXBoolLiteralExpr(CXXBoolLiteralExpr *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXNullPtrLiteralExpr(CXXNullPtrLiteralExpr *E){
        return E;
    }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXDefaultArgExpr(CXXDefaultArgExpr *E){
        ParmVarDecl *Param
            = cast_or_null<ParmVarDecl>(getDerived().TransformDecl(E->getLocStart()
                                                                  , E->getParam()));
        if(! Param) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            Param == E->getParam()) return E;
        return getDerived().RebuildCXXDefaultArgExpr(E->getUsedLocation(), Param);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXDefaultInitExpr(CXXDefaultInitExpr *E){
        FieldDecl *Field
            = cast_or_null<FieldDecl>(getDerived().TransformDecl(E->getLocStart()
                                                                , E->getField()));
        if(! Field) return ExprError();
        if(! getDerived().AlwaysRebuild() && Field == E->getField()) return E;
        return getDerived().RebuildCXXDefaultInitExpr(E->getExprLoc(), Field);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXScalarValueInitExpr(CXXScalarValueInitExpr *E){
        TypeSourceInfo *T = getDerived().TransformType(E->getTypeSourceInfo());
        if(! T) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            T == E->getTypeSourceInfo()) return E;
    ...
    ExprResult TreeTransform<Derived>::TransformCXXNewExpr(CXXNewExpr *E){
        ...
        if(! AllocTypeInfo) return ExprError();
        // Transform the size of the array we're allocating (if any).
        ExprResult ArraySize = getDerived().TransformExpr(E->getArraySize());
        if(ArraySize.isInvalid()) return ExprError();
        // Transform the placement arguments (if any).
        bool ArgumentChanged = false;
        SmallVector<Expr *, 8> PlacementArgs;
        if(getDerived().TransformExprs(E->getPlacementArgs()
                                      , E->getNumPlacementArgs(), true
                                      , PlacementArgs, &ArgumentChanged)) return ExprError();
        // Transform the initializer (if any).
        Expr *OldInit = E->getInitializer();
        ExprResult NewInit;
        if(OldInit) NewInit = getDerived().TransformInitializer(OldInit, true);
        if(NewInit.isInvalid()) return ExprError();
        ...
        if(E->getOperatorNew()){
            OperatorNew = cast_or_null<FunctionDecl>(
                getDerived().TransformDecl(E->getLocStart()
                                          , E->getOperatorNew()));
            if(! OperatorNew) return ExprError();
        }
        ...
        if(E->getOperatorDelete()){
            OperatorDelete = cast_or_null<FunctionDecl>(
                getDerived().TransformDecl(E->getLocStart()
                                          , E->getOperatorDelete()));
            if(! OperatorDelete) return ExprError();
        }
        if(! getDerived().AlwaysRebuild() &&
            ...
            if(OperatorNew) SemaRef.MarkFunctionReferenced(E->getLocStart(), OperatorNew);
            if(OperatorDelete) SemaRef.MarkFunctionReferenced(E->getLocStart(), OperatorDelete);
            if(E->isArray() && ! E->getAllocatedType()->isDependentType()){
                QualType ElementType
                    = SemaRef.Context.getBaseElementType(E->getAllocatedType());
                if(const RecordType *RecordT = ElementType->getAs<RecordType>()){
                    CXXRecordDecl *Record = cast<CXXRecordDecl>(RecordT->getDecl());
                    if(CXXDestructorDecl *Destructor = SemaRef.LookupDestructor(Record)) SemaRef.MarkFunctionReferenced(E->getLocStart(), Destructor);
                }
            }
            return E;
        }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXPseudoDestructorExpr(CXXPseudoDestructorExpr *E){
        ExprResult Base = getDerived().TransformExpr(E->getBase());
        if(Base.isInvalid()) return ExprError();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformUnresolvedLookupExpr(UnresolvedLookupExpr *Old){
        ...
        for(UnresolvedLookupExpr::decls_iterator I = Old->decls_begin()
           , E = Old->decls_end(); I != E; ++ I){
            ...
            if(! InstD){
                ...
                if(isa<UsingShadowDecl>(*I)) continue;
                else{
                    R.clear();
                    return ExprError();
                }
            ...
            if(isa<UsingDecl>(InstD)){
                ...
                for(auto *I : UD->shadows()) R.addDecl(I);
                continue;
            }
            R.addDecl(InstD);
        }
    ...
    ExprResult TreeTransform<Derived>::TransformTypeTraitExpr(TypeTraitExpr *E){
        ...
        for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I){
            ...
            if(! FromTL.getAs<PackExpansionTypeLoc>()){
                ...
                if(To.isNull()) return ExprError();
                if(To == From->getType()) Args.push_back(From);
                else{
        ...
        if(! getDerived().AlwaysRebuild() && ! ArgChanged) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformArrayTypeTraitExpr(ArrayTypeTraitExpr *E){
        TypeSourceInfo *T = getDerived().TransformType(E->getQueriedTypeSourceInfo());
        if(! T) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            T == E->getQueriedTypeSourceInfo()) return E;
        ExprResult SubExpr;
        {
            EnterExpressionEvaluationContext Unevaluated(SemaRef, Sema::Unevaluated);
            SubExpr = getDerived().TransformExpr(E->getDimensionExpression());
            if(SubExpr.isInvalid()) return ExprError();
            if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getDimensionExpression()) return E;
        }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformExpressionTraitExpr(ExpressionTraitExpr *E){
        ExprResult SubExpr;
        {
            EnterExpressionEvaluationContext Unevaluated(SemaRef, Sema::Unevaluated);
            SubExpr = getDerived().TransformExpr(E->getQueriedExpression());
            if(SubExpr.isInvalid()) return ExprError();
            if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getQueriedExpression()) return E;
        }
    ...
    ExprResult TreeTransform<Derived>::TransformParenDependentScopeDeclRefExpr(
        ParenExpr *PE, DependentScopeDeclRefExpr *DRE, bool AddrTaken, TypeSourceInfo * *RecoveryTSI){
        ...
        if(! NewDRE.isUsable()) return NewDRE;
        // We got an expr, wrap it up in parens.
        if(! getDerived().AlwaysRebuild() && NewDRE.get() == DRE) return PE;
        return getDerived().RebuildParenExpr(NewDRE.get(), PE->getLParen()
                                            , PE->getRParen());
    }

    template <typename Derived>
    ExprResult TreeTransform<Derived>::TransformDependentScopeDeclRefExpr(DependentScopeDeclRefExpr *E){
    ...
    ExprResult TreeTransform<Derived>::TransformDependentScopeDeclRefExpr(
        DependentScopeDeclRefExpr *E, bool IsAddressOfOperand, TypeSourceInfo * *RecoveryTSI){
        ...
        if(! E->hasExplicitTemplateArgs()){
            if(! getDerived().AlwaysRebuild() &&
                ...
                NameInfo.getName() == E->getDeclName()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXConstructExpr(CXXConstructExpr *E){
        ...
        if((E->getNumArgs() == 1 ||
            ...
            ! E->isListInitialization()) return getDerived().TransformExpr(E->getArg(0));
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXInheritedCtorInitExpr(CXXInheritedCtorInitExpr *E){
        QualType T = getDerived().TransformType(E->getType());
        if(T.isNull()) return ExprError();
...
/// transform the subexpression and return that.
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXBindTemporaryExpr(CXXBindTemporaryExpr *E){
...
/// just transform the subexpression and return that.
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformExprWithCleanups(ExprWithCleanups *E){
        return getDerived().TransformExpr(E->getSubExpr());
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXTemporaryObjectExpr(CXXTemporaryObjectExpr *E){
        TypeSourceInfo *T = getDerived().TransformType(E->getTypeSourceInfo());
        if(! T) return ExprError();
        CXXConstructorDecl *Constructor
            = cast_or_null<CXXConstructorDecl>(
    ...
    ExprResult TreeTransform<Derived>::TransformLambdaExpr(LambdaExpr *E){
        ...
        for(LambdaExpr::capture_iterator C = E->capture_begin()
           , CEnd = E->capture_end();
            C != CEnd; ++ C){
            if(! E->isInitCapture(C)) continue;
            EnterExpressionEvaluationContext EEEC(getSema()
                                                 , Sema::PotentiallyEvaluated);
            ExprResult NewExprInitResult = getDerived().TransformInitializer(
                C->getCapturedVar()->getInit()
                                                                            , C->getCapturedVar()->getInitStyle() == VarDecl::CallInit);
            if(NewExprInitResult.isInvalid()) return ExprError();
        ...
        TypeSourceInfo *NewCallOpTSI = nullptr;
        {
            ...
            if(NewCallOpType.isNull()) return ExprError();
        ...
        for(LambdaExpr::capture_iterator C = E->capture_begin()
           , CEnd = E->capture_end();
            ...
            if(E->isInitCapture(C)){
                ...
                if(! NewVD) Invalid = true;
                else getDerived().transformedLocalDecl(OldVD, NewVD);
        ...
        if(! FinishedExplicitCaptures) getSema().finishLambdaExplicitCaptures(LSI);
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXUnresolvedConstructExpr(CXXUnresolvedConstructExpr *E){
        TypeSourceInfo *T = getDerived().TransformType(E->getTypeSourceInfo());
        if(! T) return ExprError();
        bool ArgumentChanged = false;
        SmallVector<Expr *, 8> Args;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXDependentScopeMemberExpr(CXXDependentScopeMemberExpr *E){
        ...
        if(! E->hasExplicitTemplateArgs()){
            ...
            if(! getDerived().AlwaysRebuild() &&
                ...
                FirstQualifierInScope == E->getFirstQualifierFoundInScope()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformUnresolvedMemberExpr(UnresolvedMemberExpr *Old){
        // Transform the base of the expression.
        ExprResult Base((Expr *)nullptr);
        QualType BaseType;
        ...
        }else BaseType = getDerived().TransformType(Old->getBaseType());
        NestedNameSpecifierLoc QualifierLoc;
        if(Old->getQualifierLoc()){
            QualifierLoc
                = getDerived().TransformNestedNameSpecifierLoc(Old->getQualifierLoc());
        ...
        if(Old->getNamingClass()){
            CXXRecordDecl *NamingClass
                = cast_or_null<CXXRecordDecl>(getDerived().TransformDecl(
                        Old->getMemberLoc()
                                                                        , Old->getNamingClass()));
            if(! NamingClass) return ExprError();
            R.setNamingClass(NamingClass);
        }
        TemplateArgumentListInfo TransArgs;
        if(Old->hasExplicitTemplateArgs()){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXNoexceptExpr(CXXNoexceptExpr *E){
        EnterExpressionEvaluationContext Unevaluated(SemaRef, Sema::Unevaluated);
        ExprResult SubExpr = getDerived().TransformExpr(E->getOperand());
        if(SubExpr.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() && SubExpr.get() == E->getOperand()) return E;
        return getDerived().RebuildCXXNoexceptExpr(E->getSourceRange(), SubExpr.get());
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformPackExpansionExpr(PackExpansionExpr *E){
        ExprResult Pattern = getDerived().TransformExpr(E->getPattern());
        if(Pattern.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() && Pattern.get() == E->getPattern()) return E;
    ...
    ExprResult TreeTransform<Derived>::TransformSizeOfPackExpr(SizeOfPackExpr *E){
        ...
        if(E->isPartiallySubstituted()) PackArgs = E->getPartialArguments();
            ...
            if(ShouldExpand){
                ...
                }else if(auto *TTPD = dyn_cast<TemplateTemplateParmDecl>(Pack)) ArgStorage = TemplateArgument(TemplateName(TTPD), None);
                else{
                    auto *VD		= cast<ValueDecl>(Pack);
                    ExprResult DRE	= getSema().BuildDeclRefExpr(VD, VD->getType()
                                                                , VK_RValue, E->getPackLoc());
                    if(DRE.isInvalid()) return ExprError();
                                                    ...
                                                    , E->getPackLoc());
        {
            ...
            if(TransformTemplateArguments(PackLocIterator(*this, PackArgs.begin())
                                         ...
                                         , TransformedPackArgs,	/*Uneval*/ true)) return ExprError();
        }
        ...
        for(auto &Loc : TransformedPackArgs.arguments()){
            ...
            if(Loc.getArgument().isPackExpansion()) PartialSubstitution = true;
        }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformSubstNonTypeTemplateParmPackExpr(SubstNonTypeTemplateParmPackExpr *E){
        // Default behavior is to do nothing with this transformation.
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformSubstNonTypeTemplateParmExpr(SubstNonTypeTemplateParmExpr *E){
        // Default behavior is to do nothing with this transformation.
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformFunctionParmPackExpr(FunctionParmPackExpr *E){
        // Default behavior is to do nothing with this transformation.
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformMaterializeTemporaryExpr(MaterializeTemporaryExpr *E){
    ...
    ExprResult TreeTransform<Derived>::TransformCXXFoldExpr(CXXFoldExpr *E){
        ...
        if(! Expand){
            ...
            if(LHS.isInvalid()) return true;
            ExprResult RHS =
                E->getRHS() ? getDerived().TransformExpr(E->getRHS()) : ExprResult();
            if(RHS.isInvalid()) return true;
            if(! getDerived().AlwaysRebuild() &&
                LHS.get() == E->getLHS() && RHS.get() == E->getRHS()) return E;
        ...
        if(Result.isUnset()) return getDerived().RebuildEmptyCXXFoldExpr(E->getEllipsisLoc()
                                                                        , E->getOperator());
        return Result;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformCXXStdInitializerListExpr(CXXStdInitializerListExpr *E){
        return getDerived().TransformExpr(E->getSubExpr());
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCStringLiteral(ObjCStringLiteral *E){
        return SemaRef.MaybeBindToTemporary(E);
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCBoolLiteralExpr(ObjCBoolLiteralExpr *E){
        return E;
    }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCArrayLiteral(ObjCArrayLiteral *E){
        // Transform each of the elements.
        SmallVector<Expr *, 8> Elements;
        bool ArgChanged = false;
        if(getDerived().TransformExprs(E->getElements(), E->getNumElements()
                                      ,	/*IsCall=*/ false, Elements, &ArgChanged)) return ExprError();
        if(! getDerived().AlwaysRebuild() && ! ArgChanged) return SemaRef.MaybeBindToTemporary(E);
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCDictionaryLiteral(ObjCDictionaryLiteral *E){
        // Transform each of the elements.
        ...
        for(unsigned I = 0, N = E->getNumElements(); I != N; ++ I){
            ...
            if(OrigElement.isPackExpansion()){
                ...
                if(! Expand){
                    ...
                    if(Key.isInvalid()) return ExprError();
                    if(Key.get() != OrigElement.Key) ArgChanged = true;
                    ExprResult Value = getDerived().TransformExpr(OrigElement.Value);
                    if(Value.isInvalid()) return ExprError();
                    if(Value.get() != OrigElement.Value) ArgChanged = true;
                ...
                for(unsigned I = 0; I != *NumExpansions; ++ I){
                    ...
                    if(Key.get()->containsUnexpandedParameterPack() ||
                        Value.get()->containsUnexpandedParameterPack()) Element.EllipsisLoc = OrigElement.EllipsisLoc;
    ...
    ExprResult TreeTransform<Derived>::TransformObjCEncodeExpr(ObjCEncodeExpr *E){
        ...
        if(! EncodedTypeInfo) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            EncodedTypeInfo == E->getEncodedTypeSourceInfo()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCIndirectCopyRestoreExpr(ObjCIndirectCopyRestoreExpr *E){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCBridgedCastExpr(ObjCBridgedCastExpr *E){
        TypeSourceInfo *TSInfo
            = getDerived().TransformType(E->getTypeInfoAsWritten());
        if(! TSInfo) return ExprError();
        ExprResult Result = getDerived().TransformExpr(E->getSubExpr());
        if(Result.isInvalid()) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            TSInfo == E->getTypeInfoAsWritten() &&
            Result.get() == E->getSubExpr()) return E;
    ...
    template <typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCAvailabilityCheckExpr(ObjCAvailabilityCheckExpr *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCMessageExpr(ObjCMessageExpr *E){
        // Transform arguments.
        bool ArgChanged = false;
        SmallVector<Expr *, 8> Args;
        Args.reserve(E->getNumArgs());
        ...
        if(E->getReceiverKind() == ObjCMessageExpr::Class){
            ...
            if(! ReceiverTypeInfo) return ExprError();
            // If nothing changed, just retain the existing message send.
            if(! getDerived().AlwaysRebuild() &&
                ReceiverTypeInfo == E->getClassReceiverTypeInfo() && ! ArgChanged) return SemaRef.MaybeBindToTemporary(E);
            ...
            if(! E->getMethodDecl()) return ExprError();
        ...
        if(Receiver.isInvalid()) return ExprError();
        // If nothing changed, just retain the existing message send.
        if(! getDerived().AlwaysRebuild() &&
            Receiver.get() == E->getInstanceReceiver() && ! ArgChanged) return SemaRef.MaybeBindToTemporary(E);
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCSelectorExpr(ObjCSelectorExpr *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCProtocolExpr(ObjCProtocolExpr *E){
        return E;
    }

    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCIvarRefExpr(ObjCIvarRefExpr *E){
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCPropertyRefExpr(ObjCPropertyRefExpr *E){
        // 'super' and types never change. Property never changes. Just
        // retain the existing expression.
        if(! E->isObjectReceiver()) return E;
        // Transform the base expression.
        ExprResult Base = getDerived().TransformExpr(E->getBase());
        if(Base.isInvalid()) return ExprError();
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformObjCSubscriptRefExpr(ObjCSubscriptRefExpr *E){
        // Transform the base expression.
        ExprResult Base = getDerived().TransformExpr(E->getBaseExpr());
        if(Base.isInvalid()) return ExprError();
        ...
        // If nothing changed, just retain the existing expression.
        if(! getDerived().AlwaysRebuild() &&
            Key.get() == E->getKeyExpr() && Base.get() == E->getBaseExpr()) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformShuffleVectorExpr(ShuffleVectorExpr *E){
        bool ArgumentChanged = false;
        SmallVector<Expr *, 8> SubExprs;
        SubExprs.reserve(E->getNumSubExprs());
        if(getDerived().TransformExprs(E->getSubExprs(), E->getNumSubExprs(), false
                                      , SubExprs, &ArgumentChanged)) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            ! ArgumentChanged) return E;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::TransformConvertVectorExpr(ConvertVectorExpr *E){
        ExprResult SrcExpr = getDerived().TransformExpr(E->getSrcExpr());
        if(SrcExpr.isInvalid()) return ExprError();
        TypeSourceInfo *Type = getDerived().TransformType(E->getTypeSourceInfo());
        if(! Type) return ExprError();
        if(! getDerived().AlwaysRebuild() &&
            Type == E->getTypeSourceInfo() &&
            SrcExpr.get() == E->getSrcExpr()) return E;
    ...
    ExprResult TreeTransform<Derived>::TransformBlockExpr(BlockExpr *E){
        ...
        if(! params.empty()) blockScope->TheDecl->setParams(params);
        ...
        if(! SemaRef.getDiagnostics().hasErrorOccurred()){
            for(const auto &I : oldBlock->captures()){
                ...
                if(isa<ParmVarDecl>(oldCapture) &&
                    cast<ParmVarDecl>(oldCapture)->isParameterPack()) continue;
                VarDecl *newCapture =
...
//= ==----------------------------------------------------------------------===//
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildPointerType(QualType PointeeType, SourceLocation Star){
        return SemaRef.BuildPointerType(PointeeType, Star
                                       , getDerived().getBaseEntity());
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildBlockPointerType(QualType PointeeType, SourceLocation Star){
        return SemaRef.BuildBlockPointerType(PointeeType, Star
                                            , getDerived().getBaseEntity());
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildReferenceType(QualType ReferentType, bool WrittenAsLValue, SourceLocation Sigil){
        return SemaRef.BuildReferenceType(ReferentType, WrittenAsLValue
                                         , Sigil, getDerived().getBaseEntity());
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildMemberPointerType(QualType PointeeType, QualType ClassType, SourceLocation Sigil){
    ...
    QualType TreeTransform<Derived>::RebuildObjCTypeParamType(
        const ObjCTypeParamDecl *Decl, SourceLocation ProtocolLAngleLoc, ArrayRef<ObjCProtocolDecl *> Protocols, ArrayRef<SourceLocation> ProtocolLocs, SourceLocation ProtocolRAngleLoc){
    ...
    QualType TreeTransform<Derived>::RebuildObjCObjectType(
        QualType BaseType, SourceLocation Loc, SourceLocation TypeArgsLAngleLoc, ArrayRef<TypeSourceInfo *> TypeArgs, SourceLocation TypeArgsRAngleLoc, SourceLocation ProtocolLAngleLoc, ArrayRef<ObjCProtocolDecl *> Protocols, ArrayRef<SourceLocation> ProtocolLocs, SourceLocation ProtocolRAngleLoc){
    ...
    QualType TreeTransform<Derived>::RebuildObjCObjectPointerType(
        QualType PointeeType, SourceLocation Star){
        return SemaRef.Context.getObjCObjectPointerType(PointeeType);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildArrayType(QualType ElementType, ArrayType::ArraySizeModifier SizeMod, const llvm::APInt *Size, Expr *SizeExpr, unsigned IndexTypeQuals, SourceRange BracketsRange){
        if(SizeExpr || ! Size)
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildConstantArrayType(QualType ElementType, ArrayType::ArraySizeModifier SizeMod, const llvm::APInt &Size, unsigned IndexTypeQuals, SourceRange BracketsRange){
        return getDerived().RebuildArrayType(ElementType, SizeMod, &Size, nullptr
                                            , IndexTypeQuals, BracketsRange);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildIncompleteArrayType(QualType ElementType, ArrayType::ArraySizeModifier SizeMod, unsigned IndexTypeQuals, SourceRange BracketsRange){
        return getDerived().RebuildArrayType(ElementType, SizeMod, nullptr, nullptr
                                            , IndexTypeQuals, BracketsRange);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildVariableArrayType(QualType ElementType, ArrayType::ArraySizeModifier SizeMod, Expr *SizeExpr, unsigned IndexTypeQuals, SourceRange BracketsRange){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildDependentSizedArrayType(QualType ElementType, ArrayType::ArraySizeModifier SizeMod, Expr *SizeExpr, unsigned IndexTypeQuals, SourceRange BracketsRange){
        return getDerived().RebuildArrayType(ElementType, SizeMod, nullptr
                                            , SizeExpr
                                            , IndexTypeQuals, BracketsRange);
    }
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildVectorType(QualType ElementType, unsigned NumElements, VectorType::VectorKind VecKind){
        // FIXME: semantic checking!
        return SemaRef.Context.getVectorType(ElementType, NumElements, VecKind);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildExtVectorType(QualType ElementType, unsigned NumElements, SourceLocation AttributeLoc){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildDependentSizedExtVectorType(QualType ElementType, Expr *SizeExpr, SourceLocation AttributeLoc){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildTypeOfExprType(Expr *E, SourceLocation Loc){
    ...
    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildDecltypeType(Expr *E, SourceLocation Loc){
        return SemaRef.BuildDecltypeType(E, Loc);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildUnaryTransformType(QualType BaseType, UnaryTransformType::UTTKind UKind, SourceLocation Loc){
        return SemaRef.BuildUnaryTransformType(BaseType, UKind, Loc);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildTemplateSpecializationType(
        TemplateName Template, SourceLocation TemplateNameLoc, TemplateArgumentListInfo &TemplateArgs){
        return SemaRef.CheckTemplateIdType(Template, TemplateNameLoc, TemplateArgs);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildAtomicType(QualType ValueType, SourceLocation KWLoc){
        return SemaRef.BuildAtomicType(ValueType, KWLoc);
    }

    template<typename Derived>
    QualType TreeTransform<Derived>::RebuildPipeType(QualType ValueType, SourceLocation KWLoc){
        return SemaRef.BuildPipeType(ValueType, KWLoc);
    }

    template<typename Derived>
    TemplateName TreeTransform<Derived>::RebuildTemplateName(CXXScopeSpec &SS, bool TemplateKW, TemplateDecl *Template){
        return SemaRef.Context.getQualifiedTemplateName(SS.getScopeRep(), TemplateKW
                                                       , Template);
    }

    template<typename Derived>
    TemplateName TreeTransform<Derived>::RebuildTemplateName(CXXScopeSpec &SS, const IdentifierInfo &Name, SourceLocation NameLoc, QualType ObjectType, NamedDecl *FirstQualifierInScope){
    ...
    template<typename Derived>
    TemplateName TreeTransform<Derived>::RebuildTemplateName(CXXScopeSpec &SS, OverloadedOperatorKind Operator, SourceLocation NameLoc, QualType ObjectType){
        UnqualifiedId Name;
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::RebuildCXXOperatorCallExpr(OverloadedOperatorKind Op, SourceLocation OpLoc, Expr *OrigCallee, Expr *First, Expr *Second){
        Expr *Callee		= OrigCallee->IgnoreParenCasts();
        bool isPostIncDec	= Second && (Op == OO_PlusPlus || Op == OO_MinusMinus);
        if(First->getObjectKind() == OK_ObjCProperty){
            BinaryOperatorKind Opc = BinaryOperator::getOverloadedOpcode(Op);
            if(BinaryOperator::isAssignmentOp(Opc)) return SemaRef.checkPseudoObjectAssignment(/*Scope=*/ nullptr, OpLoc, Opc
                                                                                              , First, Second);
        ...
        if(UnresolvedLookupExpr *ULE = dyn_cast<UnresolvedLookupExpr>(Callee)){
            ...
            if(! isa<CXXMethodDecl>(ND)) Functions.addDecl(ND);
        }
    ...
    template<typename Derived>
    ExprResult TreeTransform<Derived>::RebuildCXXPseudoDestructorExpr(Expr *Base, SourceLocation OperatorLoc, bool isArrow, CXXScopeSpec &SS, TypeSourceInfo *ScopeType, SourceLocation CCLoc, SourceLocation TildeLoc, PseudoDestructorTypeStorage Destroyed){
        QualType BaseType = Base->getType();
    ...
    StmtResult TreeTransform<Derived>::TransformCapturedStmt(CapturedStmt *S){
        ...
        for(unsigned I = 0; I < NumParams; ++ I){
            ...
            }else Params.push_back(std::make_pair(StringRef(), QualType()));
        }
