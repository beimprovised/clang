    ...
    class DSAStackTy final {
        ...
        private:
            ...
            struct SharingMapTy final {
                ...
                SharingMapTy(OpenMPDirectiveKind DKind, DeclarationNameInfo Name, Scope *CurScope, SourceLocation Loc)
        ...
        public:
            ...
            void push(OpenMPDirectiveKind DKind, const DeclarationNameInfo &DirName, Scope *CurScope, SourceLocation Loc){
            ...
            const std::pair<OMPCriticalDirective *, llvm::APSInt>getCriticalWithHint(const DeclarationNameInfo &Name) const {
                auto I = Criticals.find(Name.getAsString());
                if(I != Criticals.end()) return I->second;
            ...
            void addDSA(ValueDecl *D, Expr *E, OpenMPClauseKind A, DeclRefExpr *PrivateCopy = nullptr);
            ...
            /// predicate.
            DSAVarData hasDSA(ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, const llvm::function_ref<bool (OpenMPDirectiveKind)> &DPred, bool FromParent);
            /// \brief Checks if the specified variables has data-sharing attributes which
            /// match specified \a CPred predicate in any innermost directive which
            /// matches \a DPred predicate.
            DSAVarData hasInnermostDSA(ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, const llvm::function_ref<bool (OpenMPDirectiveKind)> &DPred, bool FromParent);
            /// \brief Checks if the specified variables has explicit data-sharing
            /// attributes which match specified \a CPred predicate at the specified
            /// OpenMP region.
            bool hasExplicitDSA(ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, unsigned Level, bool NotLastprivate = false);
            ...
            bool hasDirective(const llvm::function_ref<bool (OpenMPDirectiveKind
                                                            ...
                                                            , SourceLocation)> &DPred, bool FromParent);
            ...
            OpenMPDirectiveKind getParentDirective() const {
                if(Stack.size() > 2) return Stack[Stack.size() - 2].Directive;
                return OMPD_unknown;
            }
            ...
            bool isParentOrderedRegion() const {
                if(Stack.size() > 2) return Stack[Stack.size() - 2].OrderedRegion.getInt();
                return false;
            }
            /// \brief Returns optional parameter for the ordered region.
            Expr *getParentOrderedRegionParam() const {
                if(Stack.size() > 2) return Stack[Stack.size() - 2].OrderedRegion.getPointer();
                return nullptr;
            }
            ...
            bool isParentNowaitRegion() const {
                if(Stack.size() > 2) return Stack[Stack.size() - 2].NowaitRegion;
                return false;
            }
            /// \brief Marks parent region as cancel region.
            void setParentCancelRegion(bool Cancel = true){
                if(Stack.size() > 2) Stack[Stack.size() - 2].CancelRegion =
            ...
            void setParentTeamsRegionLoc(SourceLocation TeamsRegionLoc){
                if(Stack.size() > 2) Stack[Stack.size() - 2].InnerTeamsRegionLoc = TeamsRegionLoc;
            }
            ...
            SourceLocation getInnerTeamsRegionLoc() const {
                if(Stack.size() > 1) return Stack.back().InnerTeamsRegionLoc;
            ...
            bool checkMappableExprComponentListsForDecl(
                ValueDecl *VD, bool CurrentRegionOnly, const llvm::function_ref<
                bool (OMPClauseMappableExprCommon::MappableExprComponentListRef
                     , OpenMPClauseKind)> &Check){
                auto	SI	= Stack.rbegin();
                auto	SE	= Stack.rend();
                if(SI == SE) return false;
                if(CurrentRegionOnly) SE = std::next(SI);
                else ++ SI;
                for(; SI != SE; ++ SI){
                    auto MI = SI->MappedExprComponents.find(VD);
                    if(MI != SI->MappedExprComponents.end())
                            for(auto &L : MI->second.Components)
                                    if(Check(L, MI->second.Kind)) return true;
                }
                return false;
            }
            ...
            bool checkMappableExprComponentListsForDeclAtLevel(
                ValueDecl *VD, unsigned Level, const llvm::function_ref<
                bool (OMPClauseMappableExprCommon::MappableExprComponentListRef
                     , OpenMPClauseKind)> &Check){
                auto	StartI	= std::next(Stack.begin());
                auto	EndI	= Stack.end();
                if(std::distance(StartI, EndI) <= (int) Level) return false;
            ...
            void addMappableExpressionComponents(
                ValueDecl *VD, OMPClauseMappableExprCommon::MappableExprComponentListRef Components, OpenMPClauseKind WhereFoundClauseKind){
                assert(Stack.size() > 1 &&
            ...
            llvm::iterator_range<DoacrossDependMapTy::const_iterator>getDoacrossDependClauses() const {
                assert(Stack.size() > 1);
...
DSAStackTy::DSAVarData DSAStackTy::getDSA(StackTy::reverse_iterator &Iter, ValueDecl *D){
    ...
    if(Iter == std::prev(Stack.rend())){
        ...
        //  directive.
        if(VD && ! VD->isFunctionOrMethodVarDecl() && ! isa<ParmVarDecl>(D)) DVar.CKind = OMPC_shared;
        // OpenMP [2.9.1.2, Data-sharing Attribute Rules for Variables Referenced
        // in a region but not in construct]
        //  Variables with static storage duration that are declared in called
        //  routines in the region are shared.
        if(VD && VD->hasGlobalStorage()) DVar.CKind = OMPC_shared;
        // Non-static data members are shared by default.
        if(FD) DVar.CKind = OMPC_shared;
        return DVar;
    }
        ...
        case DSA_unspecified:
            ...
            if(isOpenMPTaskingDirective(DVar.DKind)){
                ...
                for(StackTy::reverse_iterator I = std::next(Iter), EE = Stack.rend();
                    ...
                    if(isParallelOrTaskRegion(I->Directive)) break;
                }
                DVar.CKind =
...
ValueDecl *DSAStackTy::getParentLoopControlVariable(unsigned I){
    assert(Stack.size() > 2 && "Data-sharing attributes stack is empty");
    if(Stack[Stack.size() - 2].LCVMap.size() < I) return nullptr;
    for(auto &Pair : Stack[Stack.size() - 2].LCVMap)
            if(Pair.second.first == I) return Pair.first;
    return nullptr;
}

void DSAStackTy::addDSA(ValueDecl *D, Expr *E, OpenMPClauseKind A, DeclRefExpr *PrivateCopy){
    D = getCanonicalDecl(D);
...
bool DSAStackTy::isOpenMPLocal(VarDecl *D, StackTy::reverse_iterator Iter){
    ...
    if(Stack.size() > 2){
        ...
        while(I != E && ! isParallelOrTaskRegion(I->Directive)) ++ I;
        if(I == E) return false;
        TopScope = I->CurScope ? I->CurScope->getParent() : nullptr;
        Scope *CurScope = getCurScope();
        while(CurScope != TopScope && ! CurScope->isDeclScope(D)) CurScope = CurScope->getParent();
...
DSAStackTy::DSAVarData DSAStackTy::getTopDSA(ValueDecl *D, bool FromParent){
    ...
    if(VD && VD->isStaticDataMember()){
        ...
        if(DVarTemp.CKind != OMPC_unknown && DVarTemp.RefExpr) return DVar;
        DVar.CKind = OMPC_shared;
        return DVar;
    }
    ...
    if(auto *CTSD = dyn_cast_or_null<ClassTemplateSpecializationDecl>(RD))
            if(auto *CTD = CTSD->getSpecializedTemplate()) RD = CTD->getTemplatedDecl();
    if(IsConstant &&
        ...
        if(DVarTemp.CKind == OMPC_firstprivate && DVarTemp.RefExpr) return DVar;
        DVar.CKind = OMPC_shared;
        return DVar;
    }
    // Explicitly specified attributes and local variables with predetermined
    ...
    if(FromParent && StartI != EndI) StartI = std::next(StartI);
...
DSAStackTy::DSAVarData DSAStackTy::getImplicitDSA(ValueDecl *D, bool FromParent){
    D = getCanonicalDecl(D);
...
DSAStackTy::DSAVarData DSAStackTy::hasDSA(ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, const llvm::function_ref<bool (OpenMPDirectiveKind)> &DPred, bool FromParent){
    D = getCanonicalDecl(D);
    auto	StartI	= std::next(Stack.rbegin());
    auto	EndI	= Stack.rend();
    if(FromParent && StartI != EndI) StartI = std::next(StartI);
    for(auto I = StartI, EE = EndI; I != EE; ++ I){
        if(! DPred(I->Directive) && ! isParallelOrTaskRegion(I->Directive)) continue;
        DSAVarData DVar = getDSA(I, D);
        if(CPred(DVar.CKind)) return DVar;
    }
    return DSAVarData();
}

DSAStackTy::DSAVarData DSAStackTy::hasInnermostDSA(
    ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, const llvm::function_ref<bool (OpenMPDirectiveKind)> &DPred, bool FromParent){
    D = getCanonicalDecl(D);
    auto	StartI	= std::next(Stack.rbegin());
    auto	EndI	= Stack.rend();
    if(FromParent && StartI != EndI) StartI = std::next(StartI);
    if(StartI == EndI || ! DPred(StartI->Directive)) return DSAVarData();
    DSAVarData DVar = getDSA(StartI, D);
    return CPred(DVar.CKind) ? DVar : DSAVarData();
}

bool DSAStackTy::hasExplicitDSA(
    ValueDecl *D, const llvm::function_ref<bool (OpenMPClauseKind)> &CPred, unsigned Level, bool NotLastprivate){
    if(CPred(ClauseKindMode)) return true;
    D = getCanonicalDecl(D);
    auto	StartI	= std::next(Stack.begin());
    auto	EndI	= Stack.end();
    if(std::distance(StartI, EndI) <= (int) Level) return false;
...
bool DSAStackTy::hasDirective(
    ...
    if(FromParent && StartI != EndI) StartI = std::next(StartI);
    for(auto I = StartI, EE = EndI; I != EE; ++ I)
            if(DPred(I->Directive, I->DirectiveName, I->ConstructLoc)) return true;
    return false;
}
...
bool ImplicitDeviceFunctionChecker::VisitCallExpr(CallExpr *Call){
    if(FunctionDecl *Callee = Call->getDirectCallee()) return VisitFunctionDecl(Callee);
    return true;
}
...
bool ImplicitDeviceFunctionChecker::VisitCXXConstructExpr(CXXConstructExpr *E){
    ...
    if(auto *Destructor = RD->getDestructor()) VisitCXXDestructorDecl(Destructor);
    return VisitFunctionDecl(Constructor);
}

bool ImplicitDeviceFunctionChecker::VisitCXXDestructorDecl(CXXDestructorDecl *D){
    assert(D);
...
bool Sema::IsOpenMPCapturedByRef(ValueDecl *D, unsigned Level){
    ...
    if(DSAStack->hasExplicitDirective(isOpenMPTargetExecutionDirective, Level)){
        ...
        if(Ty->isReferenceType()) Ty = Ty->castAs<ReferenceType>()->getPointeeType();
        ...
        DSAStack->checkMappableExprComponentListsForDeclAtLevel(
            ...
            MapExprComponents
                ...
                // behavior.
                if(WhereFoundClauseKind != OMPC_map) return false;
                auto EI = MapExprComponents.rbegin();
                auto EE = MapExprComponents.rend();

                assert(EI != EE && "Invalid map expression!");
                if(isa<DeclRefExpr>(EI->getAssociatedExpression())) IsVariableUsedInMapClause |= EI->getAssociatedDeclaration() == D;
                ++ EI;
    ...
    if(! IsByRef &&
        ...
        Ctx.getDeclAlign(D) > Ctx.getTypeAlignInChars(Ctx.getUIntPtrType()))) IsByRef = true;
    return IsByRef;
}
...
VarDecl *Sema::IsOpenMPCapturedDecl(ValueDecl *D){
    ...
    if(VD && ! VD->hasLocalStorage() &&
        ...
        if(isOpenMPTargetExecutionDirective(DSAStack->getCurrentDirective()) &&
            ! DSAStack->isClauseParsingMode()) return VD;
    ...
    if(DSAStack->getCurrentDirective() != OMPD_unknown &&
        ...
        if(Info.first ||
            ...
            (VD && DSAStack->isForceVarCapturing())) return VD ? VD : Info.second;
        auto DVarPrivate = DSAStack->getTopDSA(D, DSAStack->isClauseParsingMode());
        if(DVarPrivate.CKind != OMPC_unknown && isOpenMPPrivate(DVarPrivate.CKind)) return VD ? VD : cast<VarDecl>(DVarPrivate.PrivateCopy->getDecl());
        DVarPrivate = DSAStack->hasDSA(
            D, isOpenMPPrivate, [](OpenMPDirectiveKind) -> bool {return true;}
                                      , DSAStack->isClauseParsingMode());
        if(DVarPrivate.CKind != OMPC_unknown) return VD ? VD : cast<VarDecl>(DVarPrivate.PrivateCopy->getDecl());
    }
    return nullptr;
}
...
void Sema::StartOpenMPDSABlock(OpenMPDirectiveKind DKind, const DeclarationNameInfo &DirName, Scope *CurScope, SourceLocation Loc){
            ...
            if(auto *Clause = dyn_cast<OMPLastprivateClause>(C)){
                ...
                for(auto *DE : Clause->varlists()){
                    ...
                    if(DVar.CKind == OMPC_lastprivate){
                        ...
                        if(VDPrivate->isInvalidDecl()) continue;
                ...
                if(PrivateCopies.size() == Clause->varlist_size()) Clause->setPrivateCopies(PrivateCopies);
            }
        }
    }
    DSAStack->pop();
...
ExprResult Sema::ActOnOpenMPIdExpression(Scope *CurScope, CXXScopeSpec &ScopeSpec, const DeclarationNameInfo &Id){
...
Sema::DeclGroupPtrTy Sema::ActOnOpenMPThreadprivateDirective(SourceLocation Loc, ArrayRef<Expr *> VarList){
...
OMPThreadPrivateDecl *Sema::CheckOMPThreadPrivateDecl(SourceLocation Loc, ArrayRef<Expr *> VarList){
    ...
    for(auto &RefExpr : VarList){
        ...
        if(RequireCompleteType(ILoc, VD->getType()
                              , diag::err_omp_threadprivate_incomplete_type)) continue;
        // OpenMP [2.9.2, Restrictions, C/C++, p.10]
        ...
        if(auto *ML = Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPThreadPrivate(VD);
    }
...
static void ReportOriginalDSA(Sema &SemaRef, DSAStackTy *Stack, const ValueDecl *D, DSAStackTy::DSAVarData DVar, bool IsLoopIterVar = false){
    ...
    auto	*VD			= dyn_cast<VarDecl>(D);
    if(IsLoopIterVar){
        if(DVar.CKind == OMPC_private) Reason = PDSA_LoopIterVarPrivate;
        else if(DVar.CKind == OMPC_lastprivate) Reason = PDSA_LoopIterVarLastprivate;
        else Reason = PDSA_LoopIterVarLinear;
    }else if(isOpenMPTaskingDirective(DVar.DKind) &&
        DVar.CKind == OMPC_firstprivate){
        Reason		= PDSA_TaskVarFirstprivate;
        ReportLoc	= DVar.ImplicitDSALoc;
    }else if(VD && VD->isStaticLocal()) Reason = PDSA_StaticLocalVarShared;
    else if(VD && VD->isStaticDataMember()) Reason = PDSA_StaticMemberShared;
    else if(VD && VD->isFileVarDecl()) Reason = PDSA_GlobalVarShared;
    else if(D->getType().isConstant(SemaRef.getASTContext())) Reason = PDSA_ConstVarShared;
        ...
        public:
            void VisitDeclRefExpr(DeclRefExpr *E){
                if(E->isTypeDependent() || E->isValueDependent() ||
                    E->containsUnexpandedParameterPack() || E->isInstantiationDependent()) return;
                if(auto *VD = dyn_cast<VarDecl>(E->getDecl())){
                    ...
                    if(isOpenMPTaskingDirective(DKind) && DVar.CKind != OMPC_shared &&
                        ! Stack->isLoopControlVariable(VD).first) ImplicitFirstprivate.push_back(E);
                }
            }
            void VisitMemberExpr(MemberExpr *E){
                ...
                if(isa<CXXThisExpr>(E->getBase()->IgnoreParens())){
                    if(auto *FD = dyn_cast<FieldDecl>(E->getMemberDecl())){
                        ...
                        if(isOpenMPTaskingDirective(DKind) && DVar.CKind != OMPC_shared &&
                            ! Stack->isLoopControlVariable(FD).first) ImplicitFirstprivate.push_back(E);
                    }
                }
            }
            ...
            void VisitStmt(Stmt *S){
                for(auto *C : S->children())
                        if(C && ! isa<OMPExecutableDirective>(C)) Visit(C);
            }
...
static OMPCapturedExprDecl *buildCaptureDecl(Sema &S, IdentifierInfo *Id, Expr *CaptureExpr, bool WithInit, bool AsExpression, unsigned CaptureLevel){
    assert(CaptureExpr);
    ASTContext	&C		= S.getASTContext();
    Expr		*Init	= AsExpression ? CaptureExpr : CaptureExpr->IgnoreImpCasts();
    QualType	Ty		= Init->getType();
    if(CaptureExpr->getObjectKind() == OK_Ordinary && CaptureExpr->isGLValue()){
        if(S.getLangOpts().CPlusPlus) Ty = C.getLValueReferenceType(Ty);
        else{
            Ty = C.getPointerType(Ty);
            ExprResult Res =
                S.CreateBuiltinUnaryOp(CaptureExpr->getExprLoc(), UO_AddrOf, Init);
            if(! Res.isUsable()) return nullptr;
            Init = Res.get();
        }
        WithInit = true;
    }
    ...
    if(! WithInit) CED->addAttr(OMPCaptureNoInitAttr::CreateImplicit(C, SourceRange()));
...
static DeclRefExpr *buildCapture(Sema &S, ValueDecl *D, Expr *CaptureExpr, bool WithInit, unsigned CaptureLevel = 1){
    OMPCapturedExprDecl *CD;
    if(auto *VD = S.IsOpenMPCapturedDecl(D)) CD = cast<OMPCapturedExprDecl>(VD);
    else CD = buildCaptureDecl(S, D->getIdentifier(), CaptureExpr, WithInit
                              ,	/*AsExpression=*/ false, CaptureLevel);
    return buildDeclRefExpr(S, CD, CD->getType().getNonReferenceType()
                           , CaptureExpr->getExprLoc());
}

static ExprResult buildCapture(Sema &S, Expr *CaptureExpr, DeclRefExpr * &Ref, unsigned CaptureLevel = 1){
    if(! Ref){
    ...
    if(! S.getLangOpts().CPlusPlus &&
        ...
        Ref->getType()->isPointerType()) Res = S.CreateBuiltinUnaryOp(CaptureExpr->getExprLoc(), UO_Deref, Ref);
    if(! Res.isUsable()) return ExprError();
    return CaptureExpr->isGLValue() ? Res : S.DefaultLvalueConversion(Res.get());
}

StmtResult Sema::ActOnOpenMPRegionEnd(StmtResult S, ArrayRef<OMPClause *> Clauses){
    if(! S.isUsable()){
    ...
    for(auto *Clause : Clauses){
        if(isOpenMPPrivate(Clause->getClauseKind()) ||
            ...
            for(auto *VarRef : Clause->children())
                    if(auto *E = cast_or_null<Expr>(VarRef)) MarkDeclarationsReferencedInExpr(E);
            ...
            if(auto *C = OMPClauseWithPreInit::get(Clause)){
                if(auto *DS = cast_or_null<DeclStmt>(C->getPreInitStmt()))
                        for(auto *D : DS->decls()) MarkVariableReferenced(D->getLocation(), cast<VarDecl>(D));
            }
            if(auto *C = OMPClauseWithPostUpdate::get(Clause))
                    if(auto *E = C->getPostUpdateExpr()) MarkDeclarationsReferencedInExpr(E);
        }
        if(Clause->getClauseKind() == OMPC_schedule) SC = cast<OMPScheduleClause>(Clause);
        else if(Clause->getClauseKind() == OMPC_ordered) OC = cast<OMPOrderedClause>(Clause);
        else if(Clause->getClauseKind() == OMPC_linear) LCs.push_back(cast<OMPLinearClause>(Clause));
    }
...
static bool CheckNestingOfRegions(Sema &SemaRef, DSAStackTy *Stack, OpenMPDirectiveKind CurrentRegion, const DeclarationNameInfo &CurrentName, OpenMPDirectiveKind CancelRegion, SourceLocation StartLoc){
    ...
    if(Stack->getCurScope()){
        ...
        if(ParentRegion == OMPD_unknown &&
            ...
            ! isOpenMPTargetExecutionDirective(CurrentRegion))) return false;
        if(CurrentRegion == OMPD_cancellation_point ||
            ...
            if(DeadLock){
                SemaRef.Diag(StartLoc
                    ...
                    << CurrentName.getName();
                if(PreviousCriticalLoc.isValid()) SemaRef.Diag(PreviousCriticalLoc
...
static bool checkIfClauses(Sema &S, OpenMPDirectiveKind Kind, ArrayRef<OMPClause *> Clauses, ArrayRef<OpenMPDirectiveKind> AllowedNameModifiers){
    ...
    for(const auto *C : Clauses){
        if(const auto *IC = dyn_cast_or_null<OMPIfClause>(C)){
            ...
            if(CurNM == OMPD_unknown) continue;
    ...
    if(FoundNameModifiers[OMPD_unknown] && NamedModifiersNumber > 0){
        if(NamedModifiersNumber == AllowedNameModifiers.size()){
            ...
            for(unsigned Cnt = 0, End = AllowedNameModifiers.size(); Cnt < End;
                ...
                if(! FoundNameModifiers[NM]){
                    ...
                    Values	+= "'";
                    if(AllowedCnt + 2 == TotalAllowedNum) Values += " or ";
                    else if(AllowedCnt + 1 != TotalAllowedNum) Values += Sep;
                    ++ AllowedCnt;
                }
            }
        ...
        for(auto Loc : NameModifierLoc) S.Diag(Loc, diag::note_omp_previous_named_if_clause);
        ErrorFound = true;
    }
    return ErrorFound;
}
...
StmtResult Sema::ActOnOpenMPExecutableDirective(
    OpenMPDirectiveKind Kind, const DeclarationNameInfo &DirName, OpenMPDirectiveKind CancelRegion, ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    StmtResult Res = StmtError();
    if(CheckNestingOfRegions(*this, DSAStack, Kind, DirName, CancelRegion
                            , StartLoc)) return StmtError();
    ...
    if(! AllowedNameModifiers.empty()) ErrorFound = checkIfClauses(*this, Kind, Clauses, AllowedNameModifiers) ||
                ErrorFound;
...
Sema::DeclGroupPtrTy Sema::ActOnOpenMPDeclareSimdDirective(
    DeclGroupPtrTy DG, OMPDeclareSimdDeclAttr::BranchStateTy BS, Expr *Simdlen, ArrayRef<Expr *> Uniforms, ArrayRef<Expr *> Aligneds, ArrayRef<Expr *> Alignments, ArrayRef<Expr *> Linears, ArrayRef<unsigned> LinModifiers, ArrayRef<Expr *> Steps, SourceRange SR){
    assert(Aligneds.size() == Alignments.size());
    assert(Linears.size() == LinModifiers.size());
    assert(Linears.size() == Steps.size());
    if(! DG || DG.get().isNull()) return DeclGroupPtrTy();
    if(! DG.get().isSingleDecl()){
        Diag(SR.getBegin(), diag::err_omp_single_decl_in_declare_simd);
        return DG;
    }
    auto *ADecl = DG.get().getSingleDecl();
    if(auto *FTD = dyn_cast<FunctionTemplateDecl>(ADecl)) ADecl = FTD->getTemplatedDecl();
    ...
    // expression.
    ExprResult SL;
    if(Simdlen) SL = VerifyPositiveIntegerConstantInClause(Simdlen, OMPC_simdlen);
    // OpenMP [2.8.2, declare simd construct, Description]
    ...
    for(auto *E : Alignments){
        ExprResult Align;
        if(E) Align = VerifyPositiveIntegerConstantInClause(E, OMPC_aligned);
    ...
    for(auto *E : Linears){
                ...
                if(auto *PVD = dyn_cast<ParmVarDecl>(DRE->getDecl())){
                    ...
                    if(FD->getNumParams() > PVD->getFunctionScopeIndex() &&
                        ...
                        if(E->isValueDependent() || E->isTypeDependent() ||
                            ...
                            E->containsUnexpandedParameterPack()) continue;
        ...
        if(isa<CXXThisExpr>(E)){
            ...
            UniformedLinearThis = E;
            if(E->isValueDependent() || E->isTypeDependent() ||
                E->isInstantiationDependent() || E->containsUnexpandedParameterPack()) continue;
    ...
    for(auto *E : Steps){
        ...
        if(auto *DRE = dyn_cast<DeclRefExpr>(Step))
                if(auto *PVD = dyn_cast<ParmVarDecl>(DRE->getDecl())){
                    ...
                    if(UniformedArgs.count(CanonPVD) == 0){
                        ...
                        CanonPVD->getType()->hasIntegerRepresentation()) NewSteps.push_back(Step);
                    else{
        ...
        if(Step && ! Step->isValueDependent() && ! Step->isTypeDependent() &&
            ...
            NewStep = PerformOpenMPImplicitIntegerConversion(Step->getExprLoc(), Step)
                .get();
            if(NewStep) NewStep = VerifyIntegerConstantExpression(NewStep).get();
        }
...
StmtResult Sema::ActOnOpenMPParallelDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
...
/// for IR generation.
    class OpenMPIterationSpaceChecker {
        ...
        public:
            ...
            Expr *BuildNumIterations(Scope *S, const bool LimitedType, llvm::MapVector<Expr *, DeclRefExpr *> &Captures) const;
            /// \brief Build the precondition expression for the loops.
            Expr *BuildPreCond(Scope *S, Expr *Cond, llvm::MapVector<Expr *, DeclRefExpr *> &Captures) const;
            /// \brief Build reference expression to the counter be used for codegen.
            DeclRefExpr *BuildCounterVar(llvm::MapVector<Expr *, DeclRefExpr *> &Captures, DSAStackTy &DSA) const;
        ...
        private:
            ...
            bool SetUB(Expr *NewUB, bool LessOp, bool StrictOp, SourceRange SR, SourceLocation SL);
    ...
    static Expr *getExprAsWritten(Expr *E){
        if(auto *ExprTemp = dyn_cast<ExprWithCleanups>(E)) E = ExprTemp->getSubExpr();
        if(auto *MTE = dyn_cast<MaterializeTemporaryExpr>(E)) E = MTE->GetTemporaryExpr();
        while(auto *Binder = dyn_cast<CXXBindTemporaryExpr>(E)) E = Binder->getSubExpr();
        if(auto *ICE = dyn_cast<ImplicitCastExpr>(E)) E = ICE->getSubExprAsWritten();
        return E->IgnoreParens();
    }

    bool OpenMPIterationSpaceChecker::SetLCDeclAndLB(ValueDecl *NewLCDecl, Expr *NewLCRefExpr, Expr *NewLB){
        // State consistency checking to ensure correct usage.
        assert(LCDecl == nullptr && LB == nullptr && LCRef == nullptr &&
            UB == nullptr && Step == nullptr && ! TestIsLessOp && ! TestIsStrictOp);
        if(! NewLCDecl || ! NewLB) return true;
        ...
        if(auto *CE = dyn_cast_or_null<CXXConstructExpr>(NewLB))
                if(const CXXConstructorDecl *Ctor = CE->getConstructor())
                        if((Ctor->isCopyOrMoveConstructor() ||
                            ...
                            CE->getNumArgs() > 0 && CE->getArg(0) != nullptr) NewLB = CE->getArg(0)->IgnoreParenImpCasts();
        LB = NewLB;
        return false;
    }

    bool OpenMPIterationSpaceChecker::SetUB(Expr *NewUB, bool LessOp, bool StrictOp, SourceRange SR, SourceLocation SL){
        // State consistency checking to ensure correct usage.
    ...
    bool OpenMPIterationSpaceChecker::CheckInit(Stmt *S, bool EmitDiags){
        ...
        //
        if(! S){
            if(EmitDiags) SemaRef.Diag(DefaultLoc, diag::err_omp_loop_not_canonical_init);
            return true;
        }
        if(auto *ExprTemp = dyn_cast<ExprWithCleanups>(S))
                if(! ExprTemp->cleanupsHaveSideEffects()) S = ExprTemp->getSubExpr();
        InitSrcRange = S->getSourceRange();
        if(Expr *E = dyn_cast<Expr>(S)) S = E->IgnoreParens();
        if(auto *BO = dyn_cast<BinaryOperator>(S)){
            if(BO->getOpcode() == BO_Assign){
                auto *LHS = BO->getLHS()->IgnoreParens();
                if(auto *DRE = dyn_cast<DeclRefExpr>(LHS)){
                    if(auto *CED = dyn_cast<OMPCapturedExprDecl>(DRE->getDecl()))
                            if(auto *ME = dyn_cast<MemberExpr>(getExprAsWritten(CED->getInit()))) return SetLCDeclAndLB(ME->getMemberDecl(), ME, BO->getRHS());
                    return SetLCDeclAndLB(DRE->getDecl(), DRE, BO->getRHS());
                }
                if(auto *ME = dyn_cast<MemberExpr>(LHS)){
                    if(ME->isArrow() &&
                        isa<CXXThisExpr>(ME->getBase()->IgnoreParenImpCasts())) return SetLCDeclAndLB(ME->getMemberDecl(), ME, BO->getRHS());
                }
            }
        ...
        if(Dependent() || SemaRef.CurContext->isDependentContext()) return false;
        if(EmitDiags){
...
/// variable (which may be the loop variable) if possible.
    static const ValueDecl *GetInitLCDecl(Expr *E){
        ...
        if(auto *CE = dyn_cast_or_null<CXXConstructExpr>(E))
                if(const CXXConstructorDecl *Ctor = CE->getConstructor())
                        if((Ctor->isCopyOrMoveConstructor() ||
                            Ctor->isConvertingConstructor(/*AllowExplicit=*/ false)) &&
                            CE->getNumArgs() > 0 && CE->getArg(0) != nullptr) E = CE->getArg(0)->IgnoreParenImpCasts();
        if(auto *DRE = dyn_cast_or_null<DeclRefExpr>(E)){
            if(auto *VD = dyn_cast<VarDecl>(DRE->getDecl())){
                if(auto *CED = dyn_cast<OMPCapturedExprDecl>(VD))
                        if(auto *ME = dyn_cast<MemberExpr>(getExprAsWritten(CED->getInit()))) return getCanonicalDecl(ME->getMemberDecl());
                return getCanonicalDecl(VD);
            }
        }
        if(auto *ME = dyn_cast_or_null<MemberExpr>(E))
                if(ME->isArrow() && isa<CXXThisExpr>(ME->getBase()->IgnoreParenImpCasts())) return getCanonicalDecl(ME->getMemberDecl());
        return nullptr;
    }
    ...
    bool OpenMPIterationSpaceChecker::CheckIncRHS(Expr *RHS){
        ...
        if(auto *BO = dyn_cast<BinaryOperator>(RHS)){
            if(BO->isAdditiveOp()){
                ...
                if(GetInitLCDecl(BO->getLHS()) == LCDecl) return SetStep(BO->getRHS(), ! IsAdd);
                if(IsAdd && GetInitLCDecl(BO->getRHS()) == LCDecl) return SetStep(BO->getLHS(), false);
            }
        }else if(auto *CE = dyn_cast<CXXOperatorCallExpr>(RHS)){
            bool IsAdd = CE->getOperator() == OO_Plus;
            if((IsAdd || CE->getOperator() == OO_Minus) && CE->getNumArgs() == 2){
                if(GetInitLCDecl(CE->getArg(0)) == LCDecl) return SetStep(CE->getArg(1), ! IsAdd);
                if(IsAdd && GetInitLCDecl(CE->getArg(1)) == LCDecl) return SetStep(CE->getArg(0), false);
            }
        }
        if(Dependent() || SemaRef.CurContext->isDependentContext()) return false;
    ...
    bool OpenMPIterationSpaceChecker::CheckInc(Expr *S){
        ...
        if(auto *UO = dyn_cast<UnaryOperator>(S)){
            ...
            switch(BO->getOpcode()){
                case BO_AddAssign:
                case BO_SubAssign:
                    if(GetInitLCDecl(BO->getLHS()) == LCDecl) return SetStep(BO->getRHS(), BO->getOpcode() == BO_SubAssign);
                    break;
                case BO_Assign:
                    if(GetInitLCDecl(BO->getLHS()) == LCDecl) return CheckIncRHS(BO->getRHS());
                    break;
                default:
                    break;
            }
            ...
            switch(CE->getOperator()){
                ...
                case OO_PlusEqual:
                case OO_MinusEqual:
                    if(GetInitLCDecl(CE->getArg(0)) == LCDecl) return SetStep(CE->getArg(1), CE->getOperator() == OO_MinusEqual);
                    break;
                case OO_Equal:
                    if(GetInitLCDecl(CE->getArg(0)) == LCDecl) return CheckIncRHS(CE->getArg(1));
                    break;
                default:
                    break;
            }
    ...
    static ExprResult tryBuildCapture(Sema &SemaRef, Expr *Capture, llvm::MapVector<Expr *, DeclRefExpr *> &Captures, unsigned CaptureLevel = 1){
        if(SemaRef.CurContext->isDependentContext()) return ExprResult(Capture);
        if(Capture->isEvaluatable(SemaRef.Context, Expr::SE_AllowSideEffects))
                return SemaRef.PerformImplicitConversion(
                    Capture->IgnoreImpCasts(), Capture->getType(), Sema::AA_Converting
                                                        , /*AllowExplicit=*/ true);
        auto I = Captures.find(Capture);
        if(I != Captures.end()) return buildCapture(SemaRef, Capture, I->second, CaptureLevel);
...
/// \brief Build the expression to calculate the number of iterations.
    Expr *OpenMPIterationSpaceChecker::BuildNumIterations(
        Scope *S, const bool LimitedType, llvm::MapVector<Expr *, DeclRefExpr *> &Captures) const {
        ExprResult	Diff;
        ...
        if(! Diff.isUsable()) return nullptr;
        // Upper - Lower [- 1]
        if(TestIsStrictOp)
        ...
        if(! NewStep.isUsable()) return nullptr;
    ...
    Expr *OpenMPIterationSpaceChecker::BuildPreCond(
        Scope *S, Expr *Cond, llvm::MapVector<Expr *, DeclRefExpr *> &Captures) const {
        ...
        if(! NewLB.isUsable() || ! NewUB.isUsable()) return nullptr;
...
/// \brief Build reference expression to the counter be used for codegen.
    DeclRefExpr *OpenMPIterationSpaceChecker::BuildCounterVar(
        ...
        if(! VD){
            ...
            // as captured again.
            if(! isOpenMPPrivate(Data.CKind) || ! Data.RefExpr) Captures.insert(std::make_pair(LCRef, Ref));
            return Ref;
        }
    ...
    Expr *OpenMPIterationSpaceChecker::BuildPrivateCounterVar() const {
        if(LCDecl && ! LCDecl->isInvalidDecl()){
            ...
            if(PrivateVar->isInvalidDecl()) return nullptr;
    ...
    if(AssociatedLoops > 0 &&
        ...
        if(! ISC.CheckInit(Init, /*EmitDiags=*/ false)){
            if(auto *D = ISC.GetLoopDecl()){
                auto *VD = dyn_cast<VarDecl>(D);
                if(! VD){
                    if(auto *Private = IsOpenMPCapturedDecl(D)) VD = Private;
                    else{
...
static bool CheckOpenMPIterationSpace(
    OpenMPDirectiveKind DKind, Stmt *S, Sema &SemaRef, DSAStackTy &DSA, unsigned CurrentNestedLoopCount, unsigned NestedLoopCount, Expr *CollapseLoopCountExpr, Expr *OrderedLoopCountExpr, llvm::DenseMap<ValueDecl *, Expr *> &VarsWithImplicitDSA, LoopIterationSpace &ResultIterSpace, llvm::MapVector<Expr *, DeclRefExpr *> &Captures){
    ...
    if(ISC.CheckInit(Init)) return true;
    ...
    if(auto *LCDecl = ISC.GetLoopDecl()){
        ...
        if(((isOpenMPSimdDirective(DKind) && DVar.CKind != OMPC_unknown &&
            ...
            if(DVar.RefExpr == nullptr) DVar.CKind = PredeterminedCKind;
    ...
    if(ISC.Dependent() || SemaRef.CurContext->isDependentContext() || HasErrors) return HasErrors;
...
static ExprResult BuildCounterInit(Sema &SemaRef, Scope *S, SourceLocation Loc, ExprResult VarRef, ExprResult Start, llvm::MapVector<Expr *, DeclRefExpr *> &Captures){
    ...
    if(! NewStart.isUsable()) return ExprError();
...
static ExprResult BuildCounterUpdate(Sema &SemaRef, Scope *S, SourceLocation Loc, ExprResult VarRef, ExprResult Start, ExprResult Iter, ExprResult Step, bool Subtract, llvm::MapVector<Expr *, DeclRefExpr *> *Captures = nullptr){
    ...
    if(! VarRef.isUsable() || ! Start.isUsable() || ! Iter.isUsable() ||
        ! Step.isUsable()) return ExprError();
    ExprResult NewStep = Step;
    if(Captures) NewStep = tryBuildCapture(SemaRef, Step.get(), *Captures);
    if(NewStep.isInvalid()) return ExprError();
    ExprResult Update =
        SemaRef.BuildBinOp(S, Loc, BO_Mul, Iter.get(), NewStep.get());
    if(! Update.isUsable()) return ExprError();
    // Try to build 'VarRef = Start, VarRef (+|-)= Iter * Step' or
    // 'VarRef = Start (+|-) Iter * Step'.
    ExprResult NewStart = Start;
    if(Captures) NewStart = tryBuildCapture(SemaRef, Start.get(), *Captures);
    if(NewStart.isInvalid()) return ExprError();
...
static ExprResult WidenIterationCount(unsigned Bits, Expr *E, Sema &SemaRef){
    if(E == nullptr) return ExprError();
    auto		&C		= SemaRef.Context;
    QualType	OldType = E->getType();
    unsigned	HasBits = C.getTypeSize(OldType);
    if(HasBits >= Bits) return ExprResult(E);
...
static bool FitsInto(unsigned Bits, bool Signed, Expr *E, Sema &SemaRef){
    ...
    llvm::APSInt Result;
    if(E->isIntegerConstantExpr(Result, SemaRef.Context)) return Signed ? Result.isSignedIntN(Bits) : Result.isIntN(Bits);
    return false;
}
...
static Stmt *buildPreInits(ASTContext &Context, llvm::MapVector<Expr *, DeclRefExpr *> &Captures){
    if(! Captures.empty()){
        ...
        for(auto &Pair : Captures) PreInits.push_back(Pair.second->getDecl());
...
static unsigned CheckOpenMPLoop(
    OpenMPDirectiveKind DKind, Expr *CollapseLoopCountExpr, Expr *OrderedLoopCountExpr, Stmt *AStmt, Sema &SemaRef, DSAStackTy &DSA, llvm::DenseMap<ValueDecl *, Expr *> &VarsWithImplicitDSA, OMPLoopDirective::HelperExprs &Built, bool CoalescedSchedule = false, bool CoalescedDistSchedule = false){
    ...
    for(unsigned Cnt = 0; Cnt < NestedLoopCount; ++ Cnt){
        if(CheckOpenMPIterationSpace(DKind, CurStmt, SemaRef, DSA, Cnt
                                    ...
                                    , IterSpaces[Cnt], Captures)) return 0;
    ...
    if(SemaRef.CurContext->isDependentContext()) return NestedLoopCount;
    ...
    ExprResult LastIteration64 = WidenIterationCount(
        ...
        .get()
                                                    , SemaRef);
    if(! LastIteration32.isUsable() || ! LastIteration64.isUsable()) return NestedLoopCount;
    auto	&C							= SemaRef.Context;
    ...
    if(LastIteration32.isUsable() &&
        ...
        FitsInto(
            32	/* Bits */
                ...
                , LastIteration64.get(), SemaRef))) LastIteration = LastIteration32;
    ...
    if(! LastIteration.isUsable()) return 0;
    ...
    if(! IsConstant){
        ...
        NumIterations = SemaRef.BuildBinOp(
            CurScope, SourceLocation(), BO_Add, SaveRef.get()
                                          , SemaRef.ActOnIntegerConstant(SourceLocation(), 1).get());
        if(! NumIterations.isUsable()) return 0;
    }
    ...
    SmallVector<Expr *, 4> LoopMultipliers;
    {
        ...
        for(int Cnt = NestedLoopCount - 1; Cnt >= 0; -- Cnt){
            ...
            if(Cnt != 0 && Iter.isUsable()) Iter = SemaRef.BuildBinOp(CurScope, UpdLoc, BO_Rem, Iter.get()
                                                                     , IS.NumIterations);
            if(! Iter.isUsable()){
                HasErrors = true;
                break;
            ...
            if(Cnt != 0){
                if(Div.isUnset()) Div = IS.NumIterations;
                else Div = SemaRef.BuildBinOp(CurScope, UpdLoc, BO_Mul, Div.get()
                                             , IS.NumIterations);
                // Add parentheses (for debugging purposes only).
                if(Div.isUsable()) Div = tryBuildCapture(SemaRef, Div.get(), Captures);
                if(! Div.isUsable()){
                    HasErrors = true;
                    break;
                }
    ...
    for(auto Pair : DSA.getDoacrossDependClauses()){
        if(Pair.first->getDependencyKind() == OMPC_DEPEND_source) Pair.first->setCounterValue(CounterVal);
        else{
...
static std::pair<bool, bool>generateCoalescedSchedule(Sema &SemaRef, ArrayRef<OMPClause *> Clauses){
...
static Expr *getCollapseNumberExpr(ArrayRef<OMPClause *> Clauses){
    ...
    if(CollapseClauses.begin() != CollapseClauses.end()) return (*CollapseClauses.begin())->getNumForLoops();
    return nullptr;
}
...
static Expr *getOrderedNumberExpr(ArrayRef<OMPClause *> Clauses){
    auto OrderedClauses =
        OMPExecutableDirective::getClausesOfKind<OMPOrderedClause>(Clauses);
    if(OrderedClauses.begin() != OrderedClauses.end()) return (*OrderedClauses.begin())->getNumForLoops();
    return nullptr;
}
...
static bool checkSimdlenSafelenSpecified(Sema &S, const ArrayRef<OMPClause *> Clauses){
    ...
    for(auto *Clause : Clauses){
        if(Clause->getClauseKind() == OMPC_safelen) Safelen = cast<OMPSafelenClause>(Clause);
        else if(Clause->getClauseKind() == OMPC_simdlen) Simdlen = cast<OMPSimdlenClause>(Clause);
        if(Safelen && Simdlen) break;
    }
    if(Simdlen && Safelen){
        ...
        if(SimdlenLength->isValueDependent() || SimdlenLength->isTypeDependent() ||
            ...
            SimdlenLength->containsUnexpandedParameterPack()) return false;
        if(SafelenLength->isValueDependent() || SafelenLength->isTypeDependent() ||
            SafelenLength->isInstantiationDependent() ||
            SafelenLength->containsUnexpandedParameterPack()) return false;
...
StmtResult Sema::ActOnOpenMPSimdDirective(
    ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc, llvm::DenseMap<ValueDecl *, Expr *> &VarsWithImplicitDSA){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
    ...
    if(NestedLoopCount == 0) return StmtError();
        ...
        for(auto C : Clauses){
            if(auto *LC = dyn_cast<OMPLinearClause>(C))
                    if(FinishOpenMPLinearClause(*LC, cast<DeclRefExpr>(B.IterationVarRef)
                                               ...
                                               , DSAStack)) return StmtError();
        }
    }
    if(checkSimdlenSafelenSpecified(*this, Clauses)) return StmtError();
    getCurFunction()->setHasBranchProtectedScope();
...
StmtResult Sema::ActOnOpenMPSectionsDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
    auto BaseStmt = AStmt;
    while(auto *CS = dyn_cast_or_null<CapturedStmt>(BaseStmt)) BaseStmt = CS->getCapturedStmt();
    if(auto *C = dyn_cast_or_null<CompoundStmt>(BaseStmt)){
        auto S = C->children();
        if(S.begin() == S.end()) return StmtError();
...
StmtResult Sema::ActOnOpenMPSectionDirective(Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
...
StmtResult Sema::ActOnOpenMPSingleDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
    ...
    for(auto *Clause : Clauses){
        if(Clause->getClauseKind() == OMPC_nowait) Nowait = Clause;
        else if(Clause->getClauseKind() == OMPC_copyprivate) Copyprivate = Clause;
...
StmtResult Sema::ActOnOpenMPMasterDirective(Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
...
StmtResult Sema::ActOnOpenMPCriticalDirective(
    const DeclarationNameInfo &DirName, ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
    ...
    for(auto *C : Clauses){
        if(C->getClauseKind() == OMPC_hint){
            ...
            if(E->isTypeDependent() || E->isValueDependent() ||
                E->isInstantiationDependent()) DependentHint = true;
            else{
    ...
    if(Pair.first && DirName.getName() && ! DependentHint){
        if(llvm::APSInt::compareValues(Hint, Pair.second) != 0){
            ...
            }else Diag(StartLoc, diag::note_omp_critical_no_hint) << 0;
            if(auto *C = Pair.first->getSingleClause<OMPHintClause>()){
                Diag(C->getLocStart(), diag::note_omp_critical_hint_here)
                    << 1
                    << C->getHint()->EvaluateKnownConstInt(Context).toString(
                    /*Radix=*/ 10, /*Signed=*/ false);
            }else Diag(Pair.first->getLocStart(), diag::note_omp_critical_no_hint) << 1;
        }
    }
    getCurFunction()->setHasBranchProtectedScope();
    ...
    auto *Dir = OMPCriticalDirective::Create(Context, DirName, StartLoc, EndLoc
                                            , Clauses, AStmt);
    if(! Pair.first && DirName.getName() && ! DependentHint) DSAStack->addCriticalWithHint(Dir, Hint);
    return Dir;
}
...
StmtResult Sema::ActOnOpenMPParallelSectionsDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
    auto BaseStmt = AStmt;
    while(auto *CS = dyn_cast_or_null<CapturedStmt>(BaseStmt)) BaseStmt = CS->getCapturedStmt();
...
StmtResult Sema::ActOnOpenMPTaskDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
...
StmtResult Sema::ActOnOpenMPTaskyieldDirective(SourceLocation StartLoc, SourceLocation EndLoc){
    return OMPTaskyieldDirective::Create(Context, StartLoc, EndLoc);
}

StmtResult Sema::ActOnOpenMPBarrierDirective(SourceLocation StartLoc, SourceLocation EndLoc){
    return OMPBarrierDirective::Create(Context, StartLoc, EndLoc);
}

StmtResult Sema::ActOnOpenMPTaskwaitDirective(SourceLocation StartLoc, SourceLocation EndLoc){
    return OMPTaskwaitDirective::Create(Context, StartLoc, EndLoc);
}

StmtResult Sema::ActOnOpenMPTaskgroupDirective(Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
...
StmtResult Sema::ActOnOpenMPFlushDirective(ArrayRef<OMPClause *> Clauses, SourceLocation StartLoc, SourceLocation EndLoc){
...
StmtResult Sema::ActOnOpenMPOrderedDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    ...
    for(auto *C : Clauses){
        ...
        }else if(C->getClauseKind() == OMPC_threads) TC = cast<OMPThreadsClause>(C);
        else if(C->getClauseKind() == OMPC_simd) SC = cast<OMPSIMDClause>(C);
    }
    if(! ErrorFound && ! SC &&
    ...
    if((! AStmt && ! DependFound) || ErrorFound) return StmtError();
    if(AStmt){
        assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
...
/// construct.
    class OpenMPAtomicUpdateChecker {
        ...
        private:
            bool checkBinaryOperation(BinaryOperator *AtomicBinOp, unsigned DiagId = 0, unsigned NoteId = 0);
    };
}	// namespace
...
bool OpenMPAtomicUpdateChecker::checkBinaryOperation(
    ...
    if(ErrorFound != NoError && DiagId != 0 && NoteId != 0){
        ...
        return true;
    }else if(SemaRef.CurContext->isDependentContext()) E = X = UpdateExpr = nullptr;
    return ErrorFound != NoError;
}

bool OpenMPAtomicUpdateChecker::checkStatement(Stmt *S, unsigned DiagId, unsigned NoteId){
    ExprAnalysisErrorCode	ErrorFound = NoError;
    SourceLocation			ErrorLoc, NoteLoc;
    SourceRange				ErrorRange, NoteRange;
    // Allowed constructs are:
    ...
    if(auto *AtomicBody = dyn_cast<Expr>(S)){
        ...
        if(AtomicBody->getType()->isScalarType() ||
            ...
            if(auto *AtomicCompAssignOp = dyn_cast<CompoundAssignOperator>(
                ...
                if(checkBinaryOperation(AtomicBinOp, DiagId, NoteId)) return true;
    ...
    if(ErrorFound == NoError && E && X){
        ...
        if(Update.isInvalid()) return true;
...
StmtResult Sema::ActOnOpenMPAtomicDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    ...
    if(auto *EWC = dyn_cast<ExprWithCleanups>(Body)) Body = EWC->getSubExpr();
    Expr	*X				= nullptr;
    Expr	*V				= nullptr;
    Expr	*E				= nullptr;
    Expr	*UE				= nullptr;
    ...
    if(AtomicKind == OMPC_read){
        ...
        if(ErrorFound != NoError){
            ...
            Diag(NoteLoc, diag::note_omp_atomic_read_write) << ErrorFound
                                                            << NoteRange;
            return StmtError();
        }else if(CurContext->isDependentContext()) V = X = nullptr;
        ...
        if(ErrorFound != NoError){
            ...
            Diag(NoteLoc, diag::note_omp_atomic_read_write) << ErrorFound
                                                            << NoteRange;
            return StmtError();
        }else if(CurContext->isDependentContext()) E = X = nullptr;
        ...
        if(Checker.checkStatement(
                ...
                : diag::err_omp_atomic_not_expression_statement
                                 , diag::note_omp_atomic_update)) return StmtError();
        if(! CurContext->isDependentContext()){
            E				= Checker.getExpr();
            X				= Checker.getX();
        ...
        if(auto *AtomicBody = dyn_cast<Expr>(Body)){
            ...
            if(ErrorFound != NoError){
                ...
                return StmtError();
            }else if(CurContext->isDependentContext()) UE = V = E = X = nullptr;
        }else{
            // If clause is a capture:
            //  { v = x; x = expr; }
            //  { v = x; x++; }
            ...
            if(auto *CS = dyn_cast<CompoundStmt>(Body)){
                ...
                if(CS->size() == 2){
                    ...
                    if(auto *EWC = dyn_cast<ExprWithCleanups>(First)) First = EWC->getSubExpr()->IgnoreParenImpCasts();
                    if(auto *EWC = dyn_cast<ExprWithCleanups>(Second)) Second = EWC->getSubExpr()->IgnoreParenImpCasts();
...
StmtResult Sema::ActOnOpenMPTargetDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    CapturedStmt *CS = cast<CapturedStmt>(AStmt);
    // 1.2.2 OpenMP Language Terminology
    // Structured block - An executable statement with a single entry at the
...
StmtResult Sema::ActOnOpenMPTargetParallelDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    CapturedStmt *CS = cast<CapturedStmt>(AStmt);
    // 1.2.2 OpenMP Language Terminology
    // Structured block - An executable statement with a single entry at the
...
static bool HasMapClause(ArrayRef<OMPClause *> Clauses){
    for(ArrayRef<OMPClause *>::iterator I = Clauses.begin(), E = Clauses.end();
        I != E; ++ I)
            if(*I != nullptr && (*I)->getClauseKind() == OMPC_map) return true;
    return false;
}

StmtResult Sema::ActOnOpenMPTargetDataDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    assert(isa<CapturedStmt>(AStmt) && "Captured statement expected");
...
StmtResult Sema::ActOnOpenMPTargetEnterDataDirective(ArrayRef<OMPClause *> Clauses, SourceLocation StartLoc, SourceLocation EndLoc){
...
StmtResult Sema::ActOnOpenMPTargetExitDataDirective(ArrayRef<OMPClause *> Clauses, SourceLocation StartLoc, SourceLocation EndLoc){
...
StmtResult Sema::ActOnOpenMPTargetUpdateDirective(ArrayRef<OMPClause *> Clauses, SourceLocation StartLoc, SourceLocation EndLoc){
    bool seenMotionClause = false;
    for(auto *C : Clauses)
            if(C->getClauseKind() == OMPC_to || C->getClauseKind() == OMPC_from) seenMotionClause = true;
    if(! seenMotionClause){
...
StmtResult Sema::ActOnOpenMPTeamsDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    CapturedStmt *CS = cast<CapturedStmt>(AStmt);
    // 1.2.2 OpenMP Language Terminology
    // Structured block - An executable statement with a single entry at the
...
StmtResult Sema::ActOnOpenMPCancellationPointDirective(SourceLocation StartLoc, SourceLocation EndLoc, OpenMPDirectiveKind CancelRegion){
...
StmtResult Sema::ActOnOpenMPCancelDirective(ArrayRef<OMPClause *> Clauses, SourceLocation StartLoc, SourceLocation EndLoc, OpenMPDirectiveKind CancelRegion){
    if(CancelRegion != OMPD_parallel && CancelRegion != OMPD_for &&
        CancelRegion != OMPD_sections && CancelRegion != OMPD_taskgroup){
        Diag(StartLoc, diag::err_omp_wrong_cancel_region)
            << getOpenMPDirectiveName(CancelRegion);
...
StmtResult Sema::ActOnOpenMPTaskLoopDirective(
    ...
    if(checkGrainsizeNumTasksClauses(*this, Clauses)) return StmtError();
    getCurFunction()->setHasBranchProtectedScope();
...
StmtResult Sema::ActOnOpenMPTargetTeamsDirective(ArrayRef<OMPClause *> Clauses, Stmt *AStmt, SourceLocation StartLoc, SourceLocation EndLoc){
    if(! AStmt) return StmtError();
    CapturedStmt *CS = cast<CapturedStmt>(AStmt);
    // 1.2.2 OpenMP Language Terminology
    // Structured block - An executable statement with a single entry at the
...
OMPClause *Sema::ActOnOpenMPSingleExprClause(OpenMPClauseKind Kind, Expr *Expr, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
static unsigned getOpenMPCaptureLevel(OpenMPDirectiveKind DKind, OpenMPClauseKind CKind){
    switch(CKind){
        case OMPC_num_threads:
        case OMPC_if:
            switch(DKind){
...
OMPClause *Sema::ActOnOpenMPIfClause(OpenMPDirectiveKind NameModifier, Expr *Condition, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation NameModifierLoc, SourceLocation ColonLoc, SourceLocation EndLoc){
    ...
    if(! Condition->isValueDependent() && ! Condition->isTypeDependent() &&
        ...
        if(Val.isInvalid()) return nullptr;
...
OMPClause *Sema::ActOnOpenMPFinalClause(Expr *Condition, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    Expr *ValExpr = Condition;
    if(! Condition->isValueDependent() && ! Condition->isTypeDependent() &&
        ! Condition->isInstantiationDependent() &&
        ! Condition->containsUnexpandedParameterPack()){
...
ExprResult Sema::PerformOpenMPImplicitIntegerConversion(SourceLocation Loc, Expr *Op){
    if(! Op) return ExprError();
    class IntConvertDiagnoser : public ICEConvertDiagnoser {
        public:
            IntConvertDiagnoser()
                : ICEConvertDiagnoser(/*AllowScopedEnumerations*/ false, false, true){}
            SemaDiagnosticBuilder diagnoseNotInt(Sema &S, SourceLocation Loc, QualType T) override {
                return S.Diag(Loc, diag::err_omp_not_integral) << T;
            }
            SemaDiagnosticBuilder diagnoseIncomplete(Sema &S, SourceLocation Loc, QualType T) override {
                return S.Diag(Loc, diag::err_omp_incomplete_type) << T;
            }
            SemaDiagnosticBuilder diagnoseExplicitConv(Sema &S, SourceLocation Loc, QualType T, QualType ConvTy) override {
                return S.Diag(Loc, diag::err_omp_explicit_conversion) << T << ConvTy;
            }
            SemaDiagnosticBuilder noteExplicitConv(Sema &S, CXXConversionDecl *Conv, QualType ConvTy) override {
                return S.Diag(Conv->getLocation(), diag::note_omp_conversion_here)
                       << ConvTy->isEnumeralType() << ConvTy;
            }
            SemaDiagnosticBuilder diagnoseAmbiguous(Sema &S, SourceLocation Loc, QualType T) override {
                return S.Diag(Loc, diag::err_omp_ambiguous_conversion) << T;
            }
            SemaDiagnosticBuilder noteAmbiguous(Sema &S, CXXConversionDecl *Conv, QualType ConvTy) override {
                return S.Diag(Conv->getLocation(), diag::note_omp_conversion_here)
                       << ConvTy->isEnumeralType() << ConvTy;
            }
            SemaDiagnosticBuilder diagnoseConversion(Sema &, SourceLocation, QualType, QualType) override {
...
OMPClause *Sema::ActOnOpenMPNumThreadsClause(Expr *NumThreads, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    Expr	*ValExpr		= NumThreads;
    Stmt	*HelperValStmt	= nullptr;
    // OpenMP [2.5, Restrictions]
    //  The num_threads expression must evaluate to a positive integer value.
    if(! IsNonNegativeIntegerValue(ValExpr, *this, OMPC_num_threads
                                  ,	/*StrictlyPositive=*/ true)) return nullptr;
    OpenMPDirectiveKind DKind = DSAStack->getCurrentDirective();
    if((DKind == OMPD_teams_distribute_parallel_for ||
        DKind == OMPD_teams_distribute_parallel_for_simd ||
        isOpenMPTargetExecutionDirective(DKind)) &&
...
ExprResult Sema::VerifyPositiveIntegerConstantInClause(Expr *E, OpenMPClauseKind CKind, bool StrictlyPositive){
    if(! E) return ExprError();
    if(E->isValueDependent() || E->isTypeDependent() ||
        E->isInstantiationDependent() || E->containsUnexpandedParameterPack()) return E;
    llvm::APSInt	Result;
    ExprResult		ICE = VerifyIntegerConstantExpression(E, &Result);
    if(ICE.isInvalid()) return ExprError();
    ...
    if(CKind == OMPC_collapse && DSAStack->getAssociatedLoops() == 1) DSAStack->setAssociatedLoops(Result.getExtValue());
    else if(CKind == OMPC_ordered) DSAStack->setAssociatedLoops(Result.getExtValue());
    return ICE;
}

OMPClause *Sema::ActOnOpenMPSafelenClause(Expr *Len, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    // OpenMP [2.8.1, simd construct, Description]
    // The parameter of the safelen clause must be a constant
    // positive integer expression.
    ExprResult Safelen = VerifyPositiveIntegerConstantInClause(Len, OMPC_safelen);
    if(Safelen.isInvalid()) return nullptr;
    return new(Context)
           OMPSafelenClause(Safelen.get(), StartLoc, LParenLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPSimdlenClause(Expr *Len, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    // OpenMP [2.8.1, simd construct, Description]
    ...
    if(Simdlen.isInvalid()) return nullptr;
    return new(Context)
           OMPSimdlenClause(Simdlen.get(), StartLoc, LParenLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPCollapseClause(Expr *NumForLoops, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    ...
    if(NumForLoopsResult.isInvalid()) return nullptr;
    return new(Context)
           OMPCollapseClause(NumForLoopsResult.get(), StartLoc, LParenLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPOrderedClause(SourceLocation StartLoc, SourceLocation EndLoc, SourceLocation LParenLoc, Expr *NumForLoops){
    // OpenMP [2.7.1, loop construct, Description]
    // OpenMP [2.8.1, simd construct, Description]
    // OpenMP [2.9.6, distribute construct, Description]
...
OMPClause *Sema::ActOnOpenMPSimpleClause(
    OpenMPClauseKind Kind, unsigned Argument, SourceLocation ArgumentLoc, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    OMPClause *Res = nullptr;
    switch(Kind){
        case OMPC_default:
            Res =
...
static std::string getListOfPossibleValues(OpenMPClauseKind K, unsigned First, unsigned Last, ArrayRef<unsigned> Exclude = llvm::None){
    ...
    for(unsigned i = First; i < Last; ++ i){
        ...
        Values	+= "'";
        if(i == Bound - Skipped) Values += " or ";
        else if(i != Bound + 1 - Skipped) Values += ", ";
    }
    return Values;
}

OMPClause *Sema::ActOnOpenMPDefaultClause(OpenMPDefaultClauseKind Kind, SourceLocation KindKwLoc, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPProcBindClause(OpenMPProcBindClauseKind Kind, SourceLocation KindKwLoc, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPSingleExprWithArgClause(
    OpenMPClauseKind Kind, ArrayRef<unsigned> Argument, Expr *Expr, SourceLocation StartLoc, SourceLocation LParenLoc, ArrayRef<SourceLocation> ArgumentLoc, SourceLocation DelimLoc, SourceLocation EndLoc){
    OMPClause *Res = nullptr;
    switch(Kind){
        case OMPC_schedule:
...
static bool checkScheduleModifiers(Sema &S, OpenMPScheduleClauseModifier M1, OpenMPScheduleClauseModifier M2, SourceLocation M1Loc, SourceLocation M2Loc){
    if(M1 == OMPC_SCHEDULE_MODIFIER_unknown && M1Loc.isValid()){
        ...
        if(M2 != OMPC_SCHEDULE_MODIFIER_unknown) Excluded.push_back(M2);
        if(M2 == OMPC_SCHEDULE_MODIFIER_nonmonotonic) Excluded.push_back(OMPC_SCHEDULE_MODIFIER_monotonic);
        if(M2 == OMPC_SCHEDULE_MODIFIER_monotonic) Excluded.push_back(OMPC_SCHEDULE_MODIFIER_nonmonotonic);
...
OMPClause *Sema::ActOnOpenMPScheduleClause(
    ...
    if(checkScheduleModifiers(*this, M1, M2, M1Loc, M2Loc) ||
        checkScheduleModifiers(*this, M2, M1, M2Loc, M1Loc)) return nullptr;
    // OpenMP, 2.7.1, Loop Construct, Restrictions
...
OMPClause *Sema::ActOnOpenMPClause(OpenMPClauseKind Kind, SourceLocation StartLoc, SourceLocation EndLoc){
    OMPClause *Res = nullptr;
    switch(Kind){
        case OMPC_ordered:
...
OMPClause *Sema::ActOnOpenMPNowaitClause(SourceLocation StartLoc, SourceLocation EndLoc){
    DSAStack->setNowaitRegion();
    return new(Context) OMPNowaitClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPUntiedClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPUntiedClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPMergeableClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPMergeableClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPReadClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPReadClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPWriteClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPWriteClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPUpdateClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPUpdateClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPCaptureClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPCaptureClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPSeqCstClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPSeqCstClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPThreadsClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPThreadsClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPSIMDClause(SourceLocation StartLoc, SourceLocation EndLoc){
    return new(Context) OMPSIMDClause(StartLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPNogroupClause(SourceLocation StartLoc, SourceLocation EndLoc){
...
ExprResult Sema::getOpenMPCapturedExpr(VarDecl *Capture, ExprValueKind VK, ExprObjectKind OK, SourceLocation Loc){
...
static std::pair<ValueDecl *, bool>getPrivateItem(Sema &S, Expr * &RefExpr, SourceLocation &ELoc, SourceRange &ERange, bool AllowArraySection = false){
    if(RefExpr->isTypeDependent() || RefExpr->isValueDependent() ||
        RefExpr->containsUnexpandedParameterPack()) return std::make_pair(nullptr, true);
    // OpenMP [3.1, C/C++]
    ...
    if(AllowArraySection){
        if(auto *ASE = dyn_cast_or_null<ArraySubscriptExpr>(RefExpr)){
            ...
            while(auto *TempASE = dyn_cast<ArraySubscriptExpr>(Base)) Base = TempASE->getBase()->IgnoreParenImpCasts();
            RefExpr		= Base;
            IsArrayExpr = ArraySubscript;
        }else if(auto *OASE = dyn_cast_or_null<OMPArraySectionExpr>(RefExpr)){
            auto *Base = OASE->getBase()->IgnoreParenImpCasts();
            while(auto *TempOASE = dyn_cast<OMPArraySectionExpr>(Base)) Base = TempOASE->getBase()->IgnoreParenImpCasts();
            while(auto *TempASE = dyn_cast<ArraySubscriptExpr>(Base)) Base = TempASE->getBase()->IgnoreParenImpCasts();
            RefExpr		= Base;
    ...
    if((! DE || ! isa<VarDecl>(DE->getDecl())) &&
        ...
        if(IsArrayExpr != NoArrayExpr) S.Diag(ELoc, diag::err_omp_expected_base_var_name)	<< IsArrayExpr
                                                                                            << ERange;
        else{
            S.Diag(ELoc
                  , AllowArraySection
...
OMPClause *Sema::ActOnOpenMPPrivateClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    SmallVector<Expr *, 8>	PrivateCopies;
    for(auto &RefExpr : VarList){
        ...
        if(RequireCompleteType(ELoc, Type, diag::err_omp_private_incomplete_type)) continue;
        ...
        DeclRefExpr *Ref = nullptr;
        if(! VD && ! CurContext->isDependentContext()) Ref = buildCapture(*this, D, SimpleRefExpr, /*WithInit=*/ false);
...
namespace {
    class DiagsUninitializedSeveretyRAII {
        ...
        public:
            DiagsUninitializedSeveretyRAII(DiagnosticsEngine &Diags, SourceLocation Loc, bool IsIgnored)
            ...
            ~DiagsUninitializedSeveretyRAII(){
                if(! IsIgnored) Diags.popMappings(SavedLoc);
            }
    };
}

OMPClause *Sema::ActOnOpenMPFirstprivateClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    SmallVector<Expr *, 8>	PrivateCopies;
    ...
    for(auto &RefExpr : VarList){
        ...
        if(RequireCompleteType(ELoc, Type
                              , diag::err_omp_firstprivate_incomplete_type)) continue;
        Type = Type.getNonReferenceType();
        ...
        if(Type->isArrayType()){
            ...
            if(Result.isInvalid()) VDPrivate->setInvalidDecl();
            else VDPrivate->setInit(Result.getAs<Expr>());
        ...
        if(! VD && ! CurContext->isDependentContext()){
            if(TopDVar.CKind == OMPC_lastprivate) Ref = TopDVar.PrivateCopy;
                ...
                if(! IsOpenMPCapturedDecl(D)) ExprCaptures.push_back(Ref->getDecl());
            }
        }
...
OMPClause *Sema::ActOnOpenMPLastprivateClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    ...
    for(auto &RefExpr : VarList){
        ...
        if(RequireCompleteType(ELoc, Type
                              , diag::err_omp_lastprivate_incomplete_type)) continue;
        Type = Type.getNonReferenceType();
        ...
        if(AssignmentOp.isInvalid()) continue;
...
OMPClause *Sema::ActOnOpenMPSharedClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8> Vars;
    for(auto &RefExpr : VarList){
        assert(RefExpr && "NULL expr in OpenMP lastprivate clause.");
        SourceLocation	ELoc;
        ...
        DeclRefExpr *Ref = nullptr;
        if(! VD && IsOpenMPCapturedDecl(D) && ! CurContext->isDependentContext()) Ref = buildCapture(*this, D, SimpleRefExpr, /*WithInit=*/ true);
        ...
        public:
            bool VisitDeclRefExpr(DeclRefExpr *E){
                if(VarDecl *VD = dyn_cast<VarDecl>(E->getDecl())){
                    ...
                    if(DVar.CKind == OMPC_shared && ! DVar.RefExpr) return false;
                    if(DVar.CKind != OMPC_unknown) return true;
                    DSAStackTy::DSAVarData DVarPrivate = Stack->hasDSA(
                        VD, isOpenMPPrivate, [](OpenMPDirectiveKind) -> bool {return true;}
                                                                      , false);
                    if(DVarPrivate.CKind != OMPC_unknown) return true;
                    return false;
                }
                return false;
            }
...
static T filterLookupForUDR(SmallVectorImpl<UnresolvedSet<8> > &Lookups, const llvm::function_ref<T(ValueDecl *)> &Gen){
    for(auto &Set : Lookups){
        for(auto *D : Set)
                if(auto Res = Gen(cast<ValueDecl>(D))) return Res;
    }
    return T();
}

static ExprResult buildDeclareReductionRef(Sema &SemaRef, SourceLocation Loc, SourceRange Range, Scope *S, CXXScopeSpec &ReductionIdScopeSpec, const DeclarationNameInfo &ReductionId, QualType Ty, CXXCastPath &BasePath, Expr *UnresolvedReduction){
    if(ReductionIdScopeSpec.isInvalid()) return ExprError();
    ...
    if(S){
        ...
        for(auto *D : ULE->decls()){
            if(D == PrevD) Lookups.push_back(UnresolvedSet<8>());
            else if(auto *DRD = cast<OMPDeclareReductionDecl>(D)) Lookups.back().addDecl(DRD);
            PrevD = D;
        }
    }
    ...
    if(auto *VD = filterLookupForUDR<ValueDecl *>(
            Lookups, [&SemaRef, Ty](ValueDecl *D) -> ValueDecl * {
                if(! D->isInvalidDecl() &&
                SemaRef.Context.hasSameType(D->getType(), Ty)) return D;
                return nullptr;
    ...
    if(auto *VD = filterLookupForUDR<ValueDecl *>(
            Lookups, [&SemaRef, Ty, Loc](ValueDecl *D) -> ValueDecl * {
                ...
                ! Ty.isMoreQualifiedThan(D->getType())) return D;
                return nullptr;
            })){
    ...
    switch(OOK){
        ...
        case OO_None:
            if(auto II = DN.getAsIdentifierInfo()){
                if(II->isStr("max")) BOK = BO_GT;
                else if(II->isStr("min")) BOK = BO_LT;
            }
            break;
    }
    SourceRange ReductionIdRange;
    if(ReductionIdScopeSpec.isValid()) ReductionIdRange.setBegin(ReductionIdScopeSpec.getBeginLoc());
    ...
    for(auto RefExpr : VarList){
        ...
        if(Res.second){
            ...
            if(CurContext->isDependentContext() &&
                ...
                isa<UnresolvedLookupExpr>(DeclareReductionRef.get()))) ReductionOps.push_back(DeclareReductionRef.get());
            else ReductionOps.push_back(nullptr);
        }
        ValueDecl *D = Res.first;
        if(! D) continue;
        QualType	Type;
        ...
        if(ASE) Type = ASE->getType().getNonReferenceType();
        else if(OASE){
            auto BaseType = OMPArraySectionExpr::getBaseOriginalType(OASE->getBase());
            if(auto *ATy = BaseType->getAsArrayTypeUnsafe()) Type = ATy->getElementType();
            else Type = BaseType->getPointeeType();
            Type = Type.getNonReferenceType();
        }else Type = Context.getBaseElementType(D->getType().getNonReferenceType());
        auto *VD = dyn_cast<VarDecl>(D);
        // OpenMP [2.9.3.3, Restrictions, C/C++, p.3]
        //  A variable that appears in a private clause must not have an incomplete
        //  type or a reference type.
        if(RequireCompleteType(ELoc, Type
                              , diag::err_omp_reduction_incomplete_type)) continue;
        ...
        if(DVar.CKind == OMPC_reduction){
            ...
            if(DVar.RefExpr) Diag(DVar.RefExpr->getExprLoc(), diag::note_omp_referenced);
        ...
        ExprResult	DeclareReductionRef = buildDeclareReductionRef(
            *this, ELoc, ERange, DSAStack->getCurScope(), ReductionIdScopeSpec
                                                                  , ReductionId, Type, BasePath, IR == ER ? nullptr : *IR);
        if(DeclareReductionRef.isInvalid()) continue;
        if(CurContext->isDependentContext() &&
            (DeclareReductionRef.isUnset() ||
        ...
        if(OASE ||
            ...
            Context.getAsArrayType(D->getType().getNonReferenceType())) PrivateTy = D->getType().getNonReferenceType();
        // Private copy.
        ...
        if(DeclareReductionRef.isUsable()){
            ...
            switch(BOK){
                ...
                case BO_LOr:
                    ...
                    if(Type->isScalarType() || Type->isAnyComplexType()) Init = ActOnIntegerConstant(ELoc, /*Val=*/ 0).get();
                    break;
                case BO_Mul:
                case BO_LAnd:
                ...
                case BO_And: {
                    ...
                    if(auto *ComplexTy = OrigType->getAs<ComplexType>()) Type = ComplexTy->getElementType();
                        ...
                        if(Type->isPointerType()){
                            ...
                            auto CastExpr = BuildCStyleCastExpr(
                                SourceLocation(), Context.getTrivialTypeSourceInfo(Type, ELoc)
                                                               , SourceLocation(), Init);
                            if(CastExpr.isInvalid()) continue;
                            Init = CastExpr.get();
                        }
        ...
        if(Init && DeclareReductionRef.isUnset()){
            AddInitializerToDecl(RHSVD, Init, /*DirectInit=*/ false
                                , /*TypeMayContainAuto=*/ false);
        }else if(! Init) ActOnUninitializedDecl(RHSVD, /*TypeMayContainAuto=*/ false);
        if(RHSVD->isInvalidDecl()) continue;
        ...
        if(DeclareReductionRef.isUsable()){
            ...
            if(ReductionOp.isInvalid()) continue;
        }
        DeclRefExpr *Ref		= nullptr;
...
bool Sema::CheckOpenMPLinearModifier(OpenMPLinearClauseKind LinKind, SourceLocation LinLoc){
...
bool Sema::CheckOpenMPLinearDecl(ValueDecl *D, SourceLocation ELoc, OpenMPLinearClauseKind LinKind, QualType Type){
    auto *VD = dyn_cast_or_null<VarDecl>(D);
    // A variable must not have an incomplete type or a reference type.
    if(RequireCompleteType(ELoc, Type, diag::err_omp_linear_incomplete_type)) return true;
...
OMPClause *Sema::ActOnOpenMPLinearClause(
    ArrayRef<Expr *> VarList, Expr *Step, SourceLocation StartLoc, SourceLocation LParenLoc, OpenMPLinearClauseKind LinKind, SourceLocation LinLoc, SourceLocation ColonLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    SmallVector<Expr *, 8>	Privates;
    SmallVector<Expr *, 8>	Inits;
    SmallVector<Decl *, 4>	ExprCaptures;
    SmallVector<Expr *, 4>	ExprPostUpdates;
    if(CheckOpenMPLinearModifier(LinKind, LinLoc)) LinKind = OMPC_LINEAR_val;
    for(auto &RefExpr : VarList){
        ...
        if(CheckOpenMPLinearDecl(D, ELoc, LinKind, Type)) continue;
        ...
        if(LinKind == OMPC_LINEAR_uval) InitExpr = VD ? VD->getInit() : SimpleRefExpr;
        else InitExpr = VD ? SimpleRefExpr : Ref;
    ...
    if(Step && ! Step->isValueDependent() && ! Step->isTypeDependent() &&
        ...
        if(IsConstant && ! Result.isNegative() && ! Result.isStrictlyPositive()) Diag(StepLoc, diag::warn_omp_linear_step_zero) << Vars[0]
                                                                                                                                << (Vars.size() > 1);
...
static bool FinishOpenMPLinearClause(OMPLinearClause &Clause, DeclRefExpr *IV, Expr *NumIterations, Sema &SemaRef, Scope *S, DSAStackTy *Stack){
    ...
    if(Step == nullptr) Step = SemaRef.ActOnIntegerConstant(SourceLocation(), 1).get();
    else if(CalcStep) Step = cast<BinaryOperator>(CalcStep)->getLHS();
    bool	HasErrors	= false;
    ...
    for(auto &RefExpr : Clause.varlists()){
        ...
        Expr	*CapturedRef;
        if(LinKind == OMPC_LINEAR_uval) CapturedRef = cast<VarDecl>(DE->getDecl())->getInit();
        else
                CapturedRef =
...
OMPClause *Sema::ActOnOpenMPAlignedClause(
    ArrayRef<Expr *> VarList, Expr *Alignment, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation ColonLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8> Vars;
    for(auto &RefExpr : VarList){
        assert(RefExpr && "NULL expr in OpenMP linear clause.");
        SourceLocation	ELoc;
        ...
        DeclRefExpr *Ref = nullptr;
        if(! VD && IsOpenMPCapturedDecl(D)) Ref = buildCapture(*this, D, SimpleRefExpr,	/*WithInit=*/ true);
    ...
    if(Alignment != nullptr){
        ...
        if(AlignResult.isInvalid()) return nullptr;
...
OMPClause *Sema::ActOnOpenMPCopyinClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    SmallVector<Expr *, 8>	SrcExprs;
    SmallVector<Expr *, 8>	DstExprs;
    SmallVector<Expr *, 8>	AssignmentOps;
...
OMPClause *Sema::ActOnOpenMPCopyprivateClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    SmallVector<Expr *, 8>	Vars;
    SmallVector<Expr *, 8>	SrcExprs;
    SmallVector<Expr *, 8>	DstExprs;
    SmallVector<Expr *, 8>	AssignmentOps;
...
OMPClause *Sema::ActOnOpenMPFlushClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    if(VarList.empty()) return nullptr;
    return OMPFlushClause::Create(Context, StartLoc, LParenLoc, EndLoc, VarList);
}

OMPClause *Sema::ActOnOpenMPDependClause(OpenMPDependClauseKind DepKind, SourceLocation DepLoc, SourceLocation ColonLoc, ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    ...
    if((DepKind != OMPC_DEPEND_sink && DepKind != OMPC_DEPEND_source) ||
        ...
        if(DepKind != OMPC_DEPEND_source && DepKind != OMPC_DEPEND_sink &&
            Vars.empty()) return nullptr;
    }
    auto *C = OMPDependClause::Create(Context, StartLoc, LParenLoc, EndLoc
                                     , DepKind, DepLoc, ColonLoc, Vars);
    if(DepKind == OMPC_DEPEND_sink || DepKind == OMPC_DEPEND_source) DSAStack->addDoacrossDependClause(C, OpsOffs);
    return C;
}

OMPClause *Sema::ActOnOpenMPDeviceClause(Expr *Device, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    Expr *ValExpr = Device;
...
static bool IsCXXRecordForMappable(Sema &SemaRef, SourceLocation Loc, DSAStackTy *Stack, CXXRecordDecl *RD){
    ...
    // true.
    if(SemaRef.getLangOpts().OpenMPIgnoreUnmappableTypes) return true;
    if(! RD || RD->isInvalidDecl()) return true;
    ...
    for(auto &I : RD->bases()){
        if(! IsCXXRecordForMappable(SemaRef, I.getLocStart(), Stack
                                   , I.getType()->getAsCXXRecordDecl())) IsCorrect = false;
    }
    return IsCorrect;
}
...
static bool CheckTypeMappable(SourceLocation SL, SourceRange SR, Sema &SemaRef, DSAStackTy *Stack, QualType QTy){
    ...
    if(QTy->isIncompleteType(&ND)){
        ...
        return false;
            ...
            if(! RD->isInvalidDecl() && ! IsCXXRecordForMappable(SemaRef, SL, Stack, RD)) return false;
    return true;
}
...
static bool CheckArrayExpressionDoesNotReferToWholeSize(Sema &SemaRef, const Expr *E, QualType BaseQTy){
    ...
    if(isa<ArraySubscriptExpr>(E) || (OASE && OASE->getColonLoc().isInvalid())){
        if(auto *ATy = dyn_cast<ConstantArrayType>(BaseQTy.getTypePtr())) return ATy->getSize().getSExtValue() != 1;
    ...
    if(LowerBound){
        ...
        if(! LowerBound->EvaluateAsInt(ConstLowerBound, SemaRef.getASTContext())) return false;																																																	// Can't get the integer value as a constant.
        if(ConstLowerBound.getSExtValue()) return true;
    }
    ...
    // pointee.
    if(BaseQTy->isPointerType()) return false;
    ...
    if(! CATy) return false;
    llvm::APSInt ConstLength;
    if(! Length->EvaluateAsInt(ConstLength, SemaRef.getASTContext())) return false;																																											// Can't get the integer value as a constant.
...
static bool CheckArrayExpressionDoesNotReferToUnitySize(Sema &SemaRef, const Expr *E, QualType BaseQTy){
    ...
    if(isa<ArraySubscriptExpr>(E) || (OASE && OASE->getColonLoc().isInvalid())) return false;
    assert(OASE && "Expecting array section if not an array subscript.");
    auto *Length = OASE->getLength();
...
static Expr *CheckMapClauseExpressionBase(
    Sema &SemaRef, Expr *E, OMPClauseMappableExprCommon::MappableExprComponentList &CurComponents, OpenMPClauseKind CKind){
    ...
    while(! RelevantExpr){
        ...
        if(auto *CurE = dyn_cast<DeclRefExpr>(E)){
            if(! isa<VarDecl>(CurE->getDecl())) break;
            RelevantExpr = CurE;
        ...
        if(auto *CurE = dyn_cast<ArraySubscriptExpr>(E)){
            ...
            if(CheckArrayExpressionDoesNotReferToWholeSize(SemaRef, CurE
                                                          , E->getType())) AllowWholeSizeArraySection = false;
        ...
        if(auto *CurE = dyn_cast<OMPArraySectionExpr>(E)){
            ...
            //  If the type of a list item is a reference to a type T then the type
            //  will be considered to be T for all purposes of this clause.
            if(CurType->isReferenceType()) CurType = CurType->getPointeeType();
            ...
            if(AllowWholeSizeArraySection){
                ...
                if(NotWhole || IsPointer) AllowWholeSizeArraySection = false;
...
static bool CheckMapConflicts(
    Sema &SemaRef, DSAStackTy *DSAS, ValueDecl *VD, Expr *E, bool CurrentRegionOnly, OMPClauseMappableExprCommon::MappableExprComponentListRef CurComponents, OpenMPClauseKind CKind){
    assert(VD && E);
    SourceLocation	ELoc	= E->getExprLoc();
    SourceRange		ERange	= E->getSourceRange();
    ...
    bool FoundError = DSAS->checkMappableExprComponentListsForDecl(
        ...
        StackComponents
            ...
            for(; CI != CE && SI != SE; ++ CI, ++ SI){
                ...
                SI->getAssociatedExpression()->getStmtClass()) break;
                // Are we dealing with different variables/fields?
                if(CI->getAssociatedDeclaration() != SI->getAssociatedDeclaration()) break;
            }
            ...
            for(; SI != SE; ++ SI){
                QualType Type;
                if(auto *ASE =
                dyn_cast<ArraySubscriptExpr>(SI->getAssociatedExpression())) Type = ASE->getBase()->IgnoreParenImpCasts()->getType();
                ...
                CheckArrayExpressionDoesNotReferToWholeSize(
                    SemaRef, SI->getAssociatedExpression(), Type)) break;
            }
            ...
            if(CI == CE && SI == SE){
                if(CurrentRegionOnly){
                    if(CKind == OMPC_map) SemaRef.Diag(ELoc, diag::err_omp_map_shared_storage) << ERange;
                    else{
            ...
            if(! CurrentRegionOnly && SI != SE) EnclosingExpr = RE;
    ...
    if(CurrentRegionOnly) return FoundError;
    // OpenMP 4.5 [2.15.5.1, map Clause, Restrictions, p.5]
    //  If any part of the original storage of a list item has corresponding
...
static void checkMappableExpressionList(Sema &SemaRef, DSAStackTy *DSAS, OpenMPClauseKind CKind, MappableVarListInfo &MVLI, SourceLocation StartLoc, OpenMPMapClauseKind MapType = OMPC_MAP_unknown, bool IsMapTypeImplicit = false){
    ...
    for(auto &RE : MVLI.VarList){
        ...
        if(CheckMapConflicts(SemaRef, DSAS, CurDeclaration, SimpleExpr
                            , /*CurrentRegionOnly=*/ true, CurComponents, CKind)) break;
        if(CKind == OMPC_map &&
            CheckMapConflicts(SemaRef, DSAS, CurDeclaration, SimpleExpr
                             , /*CurrentRegionOnly=*/ false, CurComponents, CKind)) break;
        // OpenMP 4.5 [2.10.5, target update Construct]
        // OpenMP 4.5 [2.15.5.1, map Clause, Restrictions, C++, p.1]
...
OMPClause *Sema::ActOnOpenMPMapClause(OpenMPMapClauseKind MapTypeModifier, OpenMPMapClauseKind MapType, bool IsMapTypeImplicit, SourceLocation MapLoc, SourceLocation ColonLoc, ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
QualType Sema::ActOnOpenMPDeclareReductionType(SourceLocation TyLoc, TypeResult ParsedType){
    assert(ParsedType.isUsable());

    QualType ReductionType = GetTypeFromParser(ParsedType.get());
    if(ReductionType.isNull()) return QualType();
...
Sema::DeclGroupPtrTy Sema::ActOnOpenMPDeclareReductionDirectiveStart(
    Scope *S, DeclContext *DC, DeclarationName Name, ArrayRef<std::pair<QualType, SourceLocation> > ReductionTypes, AccessSpecifier AS, Decl *PrevDeclInScope){
    ...
    if(S != nullptr){
        ...
        while(Filter.hasNext()){
            ...
            if(InCompoundScope){
                ...
                if(I == UsedAsPrevious.end()) UsedAsPrevious[PrevDecl] = false;
                if(auto *D = PrevDecl->getPrevDeclInScope()) UsedAsPrevious[D] = true;
            }
...
void Sema::ActOnOpenMPDeclareReductionCombinerStart(Scope *S, Decl *D){
    ...
    if(S != nullptr) PushDeclContext(S, DRD);
    else CurContext = DRD;
    PushExpressionEvaluationContext(PotentiallyEvaluated);
...
void Sema::ActOnOpenMPDeclareReductionCombinerEnd(Decl *D, Expr *Combiner){
    ...
    if(Combiner != nullptr) DRD->setCombiner(Combiner);
    else DRD->setInvalidDecl();
}
...
void Sema::ActOnOpenMPDeclareReductionInitializerEnd(Decl *D, Expr *Initializer){
    ...
    PopDeclContext();
    PopFunctionScopeInfo();
    if(Initializer != nullptr) DRD->setInitializer(Initializer);
    else DRD->setInvalidDecl();
}
...
Sema::DeclGroupPtrTy Sema::ActOnOpenMPDeclareReductionDirectiveEnd(
    ...
    for(auto *D : DeclReductions.get()){
        if(IsValid){
            auto *DRD = cast<OMPDeclareReductionDecl>(D);
            if(S != nullptr) PushOnScopeChains(DRD, S, /*AddToContext=*/ false);
        }else D->setInvalidDecl();
    }
    return DeclReductions;
}

OMPClause *Sema::ActOnOpenMPNumTeamsClause(Expr *NumTeams, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPThreadLimitClause(Expr *ThreadLimit, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPPriorityClause(Expr *Priority, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPGrainsizeClause(Expr *Grainsize, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPNumTasksClause(Expr *NumTasks, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPHintClause(Expr *Hint, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    // OpenMP [2.13.2, critical construct, Description]
    // ... where hint-expression is an integer constant expression that evaluates
    // to a valid lock hint.
    ExprResult HintExpr = VerifyPositiveIntegerConstantInClause(Hint, OMPC_hint);
    if(HintExpr.isInvalid()) return nullptr;
    return new(Context)
           OMPHintClause(HintExpr.get(), StartLoc, LParenLoc, EndLoc);
}

OMPClause *Sema::ActOnOpenMPDistScheduleClause(
    OpenMPDistScheduleClauseKind Kind, Expr *ChunkSize, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation KindLoc, SourceLocation CommaLoc, SourceLocation EndLoc){
...
OMPClause *Sema::ActOnOpenMPDefaultmapClause(
    OpenMPDefaultmapClauseModifier M, OpenMPDefaultmapClauseKind Kind, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation MLoc, SourceLocation KindLoc, SourceLocation EndLoc){
...
void Sema::ActOnOpenMPDeclareTargetName(Scope *CurScope, CXXScopeSpec &ScopeSpec, const DeclarationNameInfo &Id, OMPDeclareTargetDeclAttr::MapTypeTy MT, NamedDeclSetType &SameDirectiveDecls){
    LookupResult Lookup(*this, Id, LookupOrdinaryName);
    LookupParsedName(Lookup, CurScope, &ScopeSpec, true);
    if(Lookup.isAmbiguous()) return;
    Lookup.suppressDiagnostics();
    if(! Lookup.isSingleResult()){
    ...
    if(isa<VarDecl>(ND) || isa<FunctionDecl>(ND)){
        if(! SameDirectiveDecls.insert(cast<NamedDecl>(ND->getCanonicalDecl()))) Diag(Id.getLoc(), diag::err_omp_declare_target_multiple) << Id.getName();
        if(! ND->hasAttr<OMPDeclareTargetDeclAttr>()){
            Attr *A = OMPDeclareTargetDeclAttr::CreateImplicit(Context, MT);
            ND->addAttr(A);
            if(ASTMutationListener *ML = Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPDeclareTarget(ND, A);
    ...
    }else Diag(Id.getLoc(), diag::err_omp_invalid_target_decl) << Id.getName();
}

static void checkDeclInTargetContext(SourceLocation SL, SourceRange SR, Sema &SemaRef, Decl *D){
    if(! D) return;
    Decl *LD = nullptr;
    if(isa<TagDecl>(D)) LD = cast<TagDecl>(D)->getDefinition();
        ...
        if(cast<VarDecl>(D)->isImplicit()){
            Attr *A = OMPDeclareTargetDeclAttr::CreateImplicit(
                SemaRef.Context, OMPDeclareTargetDeclAttr::MT_To);
            D->addAttr(A);
            if(ASTMutationListener *ML = SemaRef.Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPDeclareTarget(D, A);
            return;
        }
    }else if(isa<FunctionDecl>(D)){
        const FunctionDecl *FD = nullptr;
        if(cast<FunctionDecl>(D)->hasBody(FD)) LD = const_cast<FunctionDecl *>(FD);
        ...
        if(LD == D){
            if(! SemaRef.getLangOpts().OpenMPImplicitDeclareTarget)
                    if(! D->hasAttr<OMPDeclareTargetDeclAttr>()) SemaRef.Diag(LD->getLocation(), diag::warn_omp_not_in_target_context);
            return;
        }
    }
    if(! LD) LD = D;
        ...
        if(LD->isOutOfLine()){
            ...
            while(DC){
                if(isa<FunctionDecl>(DC) &&
                    cast<FunctionDecl>(DC)->hasAttr<OMPDeclareTargetDeclAttr>()) break;
                DC = DC->getParent();
            }
            if(DC) return;
...
static bool checkValueDeclInTarget(SourceLocation SL, SourceRange SR, Sema &SemaRef, DSAStackTy *Stack, ValueDecl *VD){
    if(VD->hasAttr<OMPDeclareTargetDeclAttr>()) return true;
    if(! CheckTypeMappable(SL, SR, SemaRef, Stack, VD->getType())) return false;
    return true;
}
...
void Sema::checkDeclIsAllowedInOpenMPTarget(Expr *E, Decl *D){
        ...
        if((E || ! VD->getType()->isIncompleteType()) &&
            ...
            if(isa<VarDecl>(VD) || isa<FunctionDecl>(VD)){
                Attr *A = OMPDeclareTargetDeclAttr::CreateImplicit(
                    Context, OMPDeclareTargetDeclAttr::MT_To);
                VD->addAttr(A);
                if(ASTMutationListener *ML = Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPDeclareTarget(VD, A);
            }
            return;
        }
    }
    if(TemplateDecl *TD = dyn_cast<TemplateDecl>(D)){
        ...
        Attr *A = OMPDeclareTargetDeclAttr::CreateImplicit(
            Context, OMPDeclareTargetDeclAttr::MT_To);
        TD->addAttr(A);
        if(ASTMutationListener *ML = Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPDeclareTarget(TD, A);
        return;
    }
    if(! E){
        ...
        if(! D->hasAttr<OMPDeclareTargetDeclAttr>() &&
            ...
            Attr *A = OMPDeclareTargetDeclAttr::CreateImplicit(
                Context, OMPDeclareTargetDeclAttr::MT_To);
            D->addAttr(A);
            if(ASTMutationListener *ML = Context.getASTMutationListener()) ML->DeclarationMarkedOpenMPDeclareTarget(D, A);
        }
        return;
    }
...
OMPClause *Sema::ActOnOpenMPToClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    MappableVarListInfo MVLI(VarList);
    checkMappableExpressionList(*this, DSAStack, OMPC_to, MVLI, StartLoc);
    if(MVLI.ProcessedVarList.empty()) return nullptr;
    return OMPToClause::Create(Context, StartLoc, LParenLoc, EndLoc
                              , MVLI.ProcessedVarList, MVLI.VarBaseDeclarations
                              , MVLI.VarComponents);
}

OMPClause *Sema::ActOnOpenMPFromClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    MappableVarListInfo MVLI(VarList);
...
OMPClause *Sema::ActOnOpenMPUseDevicePtrClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    MappableVarListInfo		MVLI(VarList);
    SmallVector<Expr *, 8>	PrivateCopies;
    SmallVector<Expr *, 8>	Inits;
    for(auto &RefExpr : VarList){
        ...
        DeclRefExpr *Ref = nullptr;
        if(! VD) Ref = buildCapture(*this, D, SimpleRefExpr, /*WithInit=*/ true);
...
OMPClause *Sema::ActOnOpenMPIsDevicePtrClause(ArrayRef<Expr *> VarList, SourceLocation StartLoc, SourceLocation LParenLoc, SourceLocation EndLoc){
    MappableVarListInfo MVLI(VarList);
    for(auto &RefExpr : VarList){
        assert(RefExpr && "NULL expr in OpenMP use_device_ptr clause.");
        SourceLocation	ELoc;
