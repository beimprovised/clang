...
/// Common pre(post)-action for different OpenMP constructs.
    class CommonActionTy final : public PrePostActionTy {
        ...
        public:
            CommonActionTy(llvm::Value *EnterCallee, ArrayRef<llvm::Value *> EnterArgs, llvm::Value *ExitCallee, ArrayRef<llvm::Value *> ExitArgs, bool Conditional = false)
...
/// a block.
static void GetNVPTXBarrier(CodeGenFunction &CGF, int ID, llvm::Value *NumThreadsVal){
    CGBuilderTy &Bld	= CGF.Builder;
...
static llvm::Value *GetThreadLimit(CodeGenFunction &CGF, bool isSPMDExecutionMode){
    CGBuilderTy &Bld = CGF.Builder;
...
void CGOpenMPRuntimeNVPTX::emitDistributeStaticInit(
    CodeGenFunction &CGF, SourceLocation Loc, OpenMPDistScheduleClauseKind SchedKind, unsigned IVSize, bool IVSigned, bool Ordered, Address IL, Address LB, Address UB, Address ST, llvm::Value *Chunk, bool CoalescedDistSchedule){
    ...
    if(CoalescedDistSchedule) ParallelNestingLevel ++;
}

void CGOpenMPRuntimeNVPTX::emitForStaticFinish(CodeGenFunction &CGF, SourceLocation Loc, bool CoalescedDistSchedule){
    // If we are generating a coalesced schedule for the directive
    // 'target teams distribute parallel for', then the 'distribute' and 'for'
    // parts have been combined and the 'parallel' codegen has been elided.
...
QualType CGOpenMPRuntimeNVPTX::getDataSharingSlotQty(bool UseFixedDataSize, bool IsMaster){
    if(UseFixedDataSize){
        if(IsMaster) return getDataSharingMasterSlotQty();
...
LValue CGOpenMPRuntimeNVPTX::getDataSharingRootSlotLValue(CodeGenFunction &CGF, bool IsMaster){
...
void CGOpenMPRuntimeNVPTX::initializeDataSharing(CodeGenFunction &CGF, bool IsMaster){
...
// generated call.
llvm::Function *CGOpenMPRuntimeNVPTX::createKernelInitializerFunction(llvm::Function *WorkerFunction){
    ...
    class OpenMPFinder : public ConstStmtVisitor<OpenMPFinder> {
        ...
        private:
            ...
            Stmt *getBody(const FunctionDecl *FD){
                ...
                if(FunctionTemplateDecl *FunTmpl = FD->getPrimaryTemplate()){
                    if(const FunctionDecl *TD = FunTmpl->getTemplatedDecl()) D = TD;
                }else if(const FunctionDecl *PD = FD->getTemplateInstantiationPattern()) D = PD;
            ...
            void VisitFunctionDecl(const FunctionDecl *FD){
                ...
                if(Body){
                    ...
                    if(Visited.insert(Body).second) Visit(Body);
        ...
        public:
            ...
            void Visit(const Stmt *S){
                if(MatchesOpenMP) return;
                ConstStmtVisitor<OpenMPFinder>::Visit(S);
                for(const Stmt *Child : S->children())
                        if(Child && ! MatchesOpenMP) Visit(Child);
            }
            ...
            void VisitCXXConstructExpr(const CXXConstructExpr *E){
                ...
                if(! MatchesOpenMP && RD) VisitFunctionDecl(RD->getDestructor());
            }
...
/// path is taken (e.g. when building the worker loop).
    class OrphanedParallelFinder : public ConstStmtVisitor<OrphanedParallelFinder> {
        ...
        public:
            ...
            void Visit(const Stmt *S){
                if(ContainsOrphanedParallel) return;
                ConstStmtVisitor<OrphanedParallelFinder>::Visit(S);
                for(const Stmt *Child : S->children())
                        if(Child && ! ContainsOrphanedParallel) Visit(Child);
            }
            ...
            void VisitCXXConstructExpr(const CXXConstructExpr *E){
                ...
                QualType			Ty	= E->getType();
                const CXXRecordDecl *RD = Ty->getAsCXXRecordDecl();
                if(! ContainsOrphanedParallel && RD) FindParallel(RD->getDestructor());
            }

            // Found an OMP directive.
            void VisitOMPExecutableDirective(const Stmt *S){
                const OMPExecutableDirective &D = *cast<OMPExecutableDirective>(S);
                if(D.hasAssociatedStmt()) Visit(cast<CapturedStmt>(D.getAssociatedStmt())->getCapturedStmt());
            }
...
void CGOpenMPRuntimeNVPTX::WorkerFunctionState::createWorkerFunction(CodeGenModule &CGM){
...
void CGOpenMPRuntimeNVPTX::emitWorkerLoop(CodeGenFunction &CGF, WorkerFunctionState &WST){
    //
...
void CGOpenMPRuntimeNVPTX::emitGenericEntryHeader(CodeGenFunction &CGF, EntryFunctionState &EST, WorkerFunctionState &WST){
...
void CGOpenMPRuntimeNVPTX::emitGenericEntryFooter(CodeGenFunction &CGF, EntryFunctionState &EST){
...
void CGOpenMPRuntimeNVPTX::emitSPMDEntryHeader(
    CodeGenFunction &CGF, EntryFunctionState &EST, const OMPExecutableDirective &D){
    auto &Bld = CGF.Builder;
...
void CGOpenMPRuntimeNVPTX::emitSPMDEntryFooter(CodeGenFunction &CGF, EntryFunctionState &EST){
...
llvm::Value *CGOpenMPRuntimeNVPTX::getThreadID(CodeGenFunction &CGF, SourceLocation Loc){
    assert(CGF.CurFn && "No function in current CodeGenFunction.");
    return GetGlobalThreadId(CGF);
}

llvm::Value *CGOpenMPRuntimeNVPTX::getParallelLevel(CodeGenFunction &CGF, SourceLocation Loc){
...
void CGOpenMPRuntimeNVPTX::registerParallelContext(
    ...
    if(isSPMDExecutionMode() && InL0()) return;
    CurrentParallelContext = CGF.CurCodeDecl;
    if(isOpenMPParallelDirective(S.getDirectiveKind()) ||
        isOpenMPSimdDirective(S.getDirectiveKind())) createDataSharingInfo(CGF);
}

void CGOpenMPRuntimeNVPTX::createOffloadEntry(llvm::Constant *ID, llvm::Constant *Addr, uint64_t Size, llvm::ConstantInt *Flags){
...
/// discard all CompoundStmts intervening between two constructs
    static const Stmt *ignoreCompoundStmts(const Stmt *Body){
        while(auto *CS = dyn_cast_or_null<CompoundStmt>(Body)) Body = CS->body_front();
        return Body;
    }
...
// check for inner (nested) SPMD teams construct, if any
    static const OMPExecutableDirective *getNestedTeamsSPMDDirective(const OMPExecutableDirective &D){
        const CapturedStmt &CS = *cast<CapturedStmt>(D.getAssociatedStmt());
        if(auto *NestedDir = dyn_cast_or_null<OMPExecutableDirective>(
                ignoreCompoundStmts(CS.getCapturedStmt()))){
            OpenMPDirectiveKind DirectiveKind = NestedDir->getDirectiveKind();
            if(isOpenMPTeamsDirective(DirectiveKind) &&
                isOpenMPParallelDirective(DirectiveKind)) return NestedDir;
        }
        return nullptr;
    }

    static CGOpenMPRuntimeNVPTX::ExecutionMode GetExecutionMode(CodeGenModule &CGM, const OMPExecutableDirective &D){
        if(CGM.getLangOpts().OpenMPNoSPMD) return CGOpenMPRuntimeNVPTX::ExecutionMode::GENERIC;
    ...
    class ExecutionModeRAII {
        ...
        public:
            ExecutionModeRAII(CGOpenMPRuntimeNVPTX::ExecutionMode &CurrMode, CGOpenMPRuntimeNVPTX::ExecutionMode ThisMode, bool &IsOrphaned, bool ThisOrphaned)
...
void CGOpenMPRuntimeNVPTX::emitGenericKernel(const OMPExecutableDirective &D, StringRef ParentName, llvm::Function * &OutlinedFn, llvm::Constant * &OutlinedFnID, bool IsOffloadEntry, const RegionCodeGenTy &CodeGen){
    ...
    class NVPTXPrePostActionTy : public PrePostActionTy {
        ...
        public:
            NVPTXPrePostActionTy(CGOpenMPRuntimeNVPTX &RT, CGOpenMPRuntimeNVPTX::EntryFunctionState &EST, CGOpenMPRuntimeNVPTX::WorkerFunctionState &WST)
...
static const OMPExecutableDirective *getSPMDDirective(const OMPExecutableDirective &D){
...
void CGOpenMPRuntimeNVPTX::EntryFunctionState::setRequiresOMPRuntime(const OMPExecutableDirective &TD){
    ...
    if(auto *C = D.getSingleClause<OMPScheduleClause>()) ScheduleKind = C->getScheduleKind();
...
void CGOpenMPRuntimeNVPTX::EntryFunctionState::setRequiresDataSharing(const OMPExecutableDirective &TD){
    const OMPExecutableDirective &D = *getSPMDDirective(TD);
...
void CGOpenMPRuntimeNVPTX::emitSPMDKernel(const OMPExecutableDirective &D, StringRef ParentName, llvm::Function * &OutlinedFn, llvm::Constant * &OutlinedFnID, bool IsOffloadEntry, const RegionCodeGenTy &CodeGen){
    ...
    class NVPTXPrePostActionTy : public PrePostActionTy {
        ...
        public:
            NVPTXPrePostActionTy(CGOpenMPRuntimeNVPTX &RT, CGOpenMPRuntimeNVPTX::EntryFunctionState &EST, const OMPExecutableDirective &D)
                : RT(RT), EST(EST), D(D){}
            void Enter(CodeGenFunction &CGF) override {
...
void CGOpenMPRuntimeNVPTX::emitTargetOutlinedFunction(
    const OMPExecutableDirective &D, StringRef ParentName, llvm::Function * &OutlinedFn, llvm::Constant * &OutlinedFnID, bool IsOffloadEntry, const RegionCodeGenTy &CodeGen){
...
void CGOpenMPRuntimeNVPTX::emitNumThreadsClause(CodeGenFunction &CGF, llvm::Value *NumThreads, SourceLocation Loc){
    // Do nothing in case of SPMD Execution Mode and at level 0.
    if(isSPMDExecutionMode() && InL0()) return;
    CGOpenMPRuntime::emitNumThreadsClause(CGF, NumThreads, Loc);
}

void CGOpenMPRuntimeNVPTX::emitProcBindClause(CodeGenFunction &CGF, OpenMPProcBindClauseKind ProcBind, SourceLocation Loc){
    // Do nothing in case of SPMD Execution Mode and at level 0.
    if(isSPMDExecutionMode() && InL0()) return;
    CGOpenMPRuntime::emitProcBindClause(CGF, ProcBind, Loc);
}

void CGOpenMPRuntimeNVPTX::emitForDispatchFinish(CodeGenFunction &CGF, const OMPLoopDirective &S, SourceLocation Loc, unsigned IVSize, bool IVSigned){
    ...
    if(auto *C = S.getSingleClause<OMPDistScheduleClause>()) DistChunked = C->getChunkSize() != nullptr;
    if(isSPMDExecutionMode() && InL1() && DistChunked &&
        (Kind == OMPD_target_teams_distribute_parallel_for ||
        Kind == OMPD_teams_distribute_parallel_for ||
        Kind == OMPD_target_teams_distribute_parallel_for_simd ||
        Kind == OMPD_teams_distribute_parallel_for_simd)) SyncCTAThreads(CGF);
}
...
/// \brief Base class for handling code generation inside OpenMP regions.
    class CGOpenMPRegionInfo : public CodeGenFunction::CGCapturedStmtInfo {
        public:
            ...
            CGOpenMPRegionInfo(const CapturedStmt &CS, const CGOpenMPRegionKind RegionKind, const RegionCodeGenTy &CodeGen, OpenMPDirectiveKind Kind, bool HasCancel)
                : CGCapturedStmtInfo(CS, CR_OpenMP), RegionKind(RegionKind)
                  , CodeGen(CodeGen), Kind(Kind), HasCancel(HasCancel){}

            CGOpenMPRegionInfo(const CGOpenMPRegionKind RegionKind, const RegionCodeGenTy &CodeGen, OpenMPDirectiveKind Kind, bool HasCancel)
...
/// \brief API for captured statement code generation in OpenMP constructs.
    class CGOpenMPOutlinedRegionInfo final : public CGOpenMPRegionInfo {
        public:
            CGOpenMPOutlinedRegionInfo(const CapturedStmt &CS, const VarDecl *ThreadIDVar, const RegionCodeGenTy &CodeGen, OpenMPDirectiveKind Kind, bool HasCancel)
...
/// \brief API for captured statement code generation in OpenMP constructs.
    class CGOpenMPSimdOutlinedRegionInfo : public CGOpenMPRegionInfo {
        public:
            CGOpenMPSimdOutlinedRegionInfo(const CapturedStmt &CS, const VarDecl *LaneIDVar, const VarDecl *NumLanesVar, const RegionCodeGenTy &CodeGen, OpenMPDirectiveKind Kind)
...
static void DoOnSharedLoopBounds(
    const OMPExecutableDirective &D, const llvm::function_ref<void (const VarDecl *, const VarDecl *)> &Exec){
...
llvm::Value *CGOpenMPRuntimeNVPTX::emitTeamsOutlinedFunction(
    const OMPExecutableDirective &D, const VarDecl *ThreadIDVar, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen, unsigned CaptureLevel, unsigned ImplicitParamStop){
...
llvm::Value *CGOpenMPRuntimeNVPTX::emitParallelOutlinedFunction(
    ...
    if(isSPMDExecutionMode() && InL0()){
        ...
        if(auto *OPD = dyn_cast<OMPParallelDirective>(&D)) HasCancel = OPD->hasCancel();
        else if(auto *OPSD = dyn_cast<OMPParallelSectionsDirective>(&D)) HasCancel = OPSD->hasCancel();
        else if(auto *OPFD = dyn_cast<OMPParallelForDirective>(&D)) HasCancel = OPFD->hasCancel();
...
llvm::Value *CGOpenMPRuntimeNVPTX::emitSimdOutlinedFunction(
    const OMPExecutableDirective &D, const VarDecl *LaneIDVar, const VarDecl *NumLanesVar, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen){
    llvm::Function *OutlinedFun = nullptr;
...
void CGOpenMPRuntimeNVPTX::registerCtorDtorEntry(
    unsigned DeviceID, unsigned FileID, StringRef RegionName, unsigned Line, llvm::Function *Fn, bool IsDtor){
...
void CGOpenMPRuntimeNVPTX::createDataSharingInfo(CodeGenFunction &CGF){
    ...
    ASTContext &C = CGM.getContext();
    if(DataSharingInfoMap.find(Context) != DataSharingInfoMap.end()) return;
    auto &Info = DataSharingInfoMap[Context];

    // Get the body of the region. The region context is either a function or a
    // captured declaration.
    const Stmt *Body;
    if(auto *D = dyn_cast<CapturedDecl>(Context)) Body = D->getBody();
    else Body = cast<FunctionDecl>(Context)->getBody();
    ...
    while(! WorkList.empty()){
        ...
        if(auto *Dir = dyn_cast<OMPExecutableDirective>(CurStmt)){
            if(isOpenMPParallelDirective(Dir->getDirectiveKind()) ||
                isOpenMPSimdDirective(Dir->getDirectiveKind())) CapturedDirs.push_back(Dir);
            else{
        ...
        for(CapturedStmt::const_capture_init_iterator I = CS->capture_init_begin()
           , E = CS->capture_init_end();
            ...
            if(CurField->hasCapturedVLAType()){
                ...
                if(auto *OD = dyn_cast<OMPCapturedExprDecl>(OrigVD)) OrigVD = cast<VarDecl>(
                            cast<DeclRefExpr>(OD->getInit()->IgnoreImpCasts())->getDecl());
                // If the variable does not have local storage it is always a reference.
                if(! OrigVD->hasLocalStorage()) DST = DataSharingInfo::DST_Ref;
                else{
                    ...
                    if(isa<llvm::LoadInst>(Val)) DST = DataSharingInfo::DST_Ref;
                    else if(isa<llvm::BitCastInst>(Val)) DST = DataSharingInfo::DST_Cast;
                }
            }
            // Do not insert the same declaration twice.
            if(AlreadySharedDecls.count(CurVD)) continue;
            AlreadySharedDecls.insert(CurVD);
            Info.add(CurVD, DST);
            if(DST == DataSharingInfo::DST_Ref) ElemTy = C.getPointerType(ElemTy);
        ...
        DoOnSharedLoopBounds(
            *Dir, [&AlreadySharedDecls, &C, &Info, &SharedMasterRD, &SharedWarpRD
            , &Dir, &CGF](const VarDecl *LB, const VarDecl *UB){
                // Do not insert the same declaration twice.
                if(AlreadySharedDecls.count(LB)) return;
                ...
                if(auto *PreInits = cast_or_null<DeclStmt>(L->getPreInits()))
                        for(const auto *I : PreInits->decls()) CGF.EmitOMPHelperVar(cast<VarDecl>(I));
            });
    }
...
static LValue castValueToUintptr(CodeGenFunction &CGF, QualType SrcType, StringRef Name, LValue AddrLV){
    // If the value is a pointer we don't have to do anything.
    if(SrcType->isAnyPointerType()) return AddrLV;
...
void CGOpenMPRuntimeNVPTX::createDataSharingPerFunctionInfrastructure(CodeGenFunction &EnclosingCGF){
    ...
    // to do anything.
    if(DSI.CapturesValues.empty()) return;
    ...
    // to create a new one.
    if(EnclosingFuncInfo.InitializationFunction) return;
    ...
    for(auto *F : MasterRD->fields()){
        ...
        if(CapturesIt->second != DataSharingInfo::DST_Ref) ArgTy = Ctx.getPointerType(ArgTy);
        StringRef BaseName =
    ...
    FunctionArgList ArgList;
    for(auto &I : ArgImplDecls) ArgList.push_back(&I);
    auto &CGFI =
    ...
    auto &&Sequential = [this, &DSI, &Ctx, MasterRD, &NewAddressPtrs
        , &OrigAddresses](CodeGenFunction &CGF
            ...
            for(unsigned i = 0; i < OrigAddresses.size(); ++ i, ++ FI, ++ CapturesIt){
                ...
                if(CapturesIt->second == DataSharingInfo::DST_Ref) continue;
    ...
    for(unsigned i = 0; i < OrigAddresses.size(); ++ i, ++ FI){
        ...
        }else OriginalVal = CGF.LoadCXXThis();
...
static void CreateAddressStoreForVariable(
    CodeGenFunction &CGF, const VarDecl *VD, QualType Ty, const CGOpenMPRuntimeNVPTX::DataSharingInfo &DSI, llvm::Value *SlotAddr, Address StoreAddr, llvm::Value *LaneID = nullptr){
    auto	&Ctx	= CGF.getContext();
    auto	&Bld	= CGF.Builder;

    unsigned Idx = 0;
    for(; Idx < DSI.CapturesValues.size(); ++ Idx)
            if(DSI.CapturesValues[Idx].first == VD) break;
    ...
    }else CGF.EmitStoreOfScalar(Arg, StoreAddr,	/*Volatile=*/ false
...
llvm::Function *CGOpenMPRuntimeNVPTX::createDataSharingParallelWrapper(
    llvm::Function &OutlinedParallelFn, const OMPExecutableDirective &D, const Decl *CurrentContext, bool IsSimd){
    auto				&Ctx	= CGM.getContext();
    const CapturedStmt	&CS		= *cast<CapturedStmt>(D.getAssociatedStmt());
    ...
    for(CapturedStmt::const_capture_iterator CI = CS.capture_begin()
       , CE = CS.capture_end();
        ...
        StringRef Name;
        if(CI->capturesThis()) Name = "this";
        else Name = CI->getCapturedVar()->getName();
        QualType ElemTy = CurField->getType();
        // If this is a capture by copy the element type has to be the pointer to
        // the data.
        if(CI->capturesVariableByCopy()) ElemTy = Ctx.getPointerType(ElemTy);
    ...
    if(IsSimd) emitParallelismLevelCode(CGF, ParallelLevelGen, Sequential, L1ParallelGen
                                       , Sequential);
    else emitParallelismLevelCode(CGF, ParallelLevelGen, L0ParallelGen
...
void CGOpenMPRuntimeNVPTX::emitParallelismLevelCode(
    CodeGenFunction &CGF, const llvm::function_ref<llvm::Value *()> &ParallelLevelGen, const RegionCodeGenTy &Level0, const RegionCodeGenTy &Level1, const RegionCodeGenTy &Sequential){
    auto &Bld = CGF.Builder;
    ...
    if(! OnlyInL1 && ! OnlySequential){
        ...
        if(NextBB) CGF.EmitBlock(NextBB);
    }
...
void CGOpenMPRuntimeNVPTX::emitGenericParallelCall(
    CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn, ArrayRef<llvm::Value *> CapturedVars, const Expr *IfCond){
    ...
    if(IfCond) emitOMPIfClause(CGF, IfCond, ThenGen, SeqGen);
    else{
        CodeGenFunction::RunCleanupsScope	Scope(CGF);
...
void CGOpenMPRuntimeNVPTX::emitSPMDParallelCall(
    ...
    }else emitGenericParallelCall(CGF, Loc, OutlinedFn, CapturedVars, IfCond);
}

void CGOpenMPRuntimeNVPTX::emitParallelCall(
    CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn, ArrayRef<llvm::Value *> CapturedVars, const Expr *IfCond){
    if(! CGF.HaveInsertPoint()) return;
    if(isSPMDExecutionMode()) emitSPMDParallelCall(CGF, Loc, OutlinedFn, CapturedVars, IfCond);
    else emitGenericParallelCall(CGF, Loc, OutlinedFn, CapturedVars, IfCond);
}

void CGOpenMPRuntimeNVPTX::emitSimdCall(CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn, ArrayRef<llvm::Value *> CapturedVars){
    if(! CGF.HaveInsertPoint()) return;
    llvm::Function	*Fn		= cast<llvm::Function>(OutlinedFn);
    llvm::Function	*WFn	= WrapperFunctionsMap[Fn];
    assert(WFn && "Wrapper function does not exist??");
...
void CGOpenMPRuntimeNVPTX::emitCriticalRegion(
    CodeGenFunction &CGF, StringRef CriticalName, const RegionCodeGenTy &CriticalOpGen, SourceLocation Loc, const Expr *Hint){
...
bool CGOpenMPRuntimeNVPTX::generateCoalescedSchedule(
    OpenMPScheduleClauseKind ScheduleKind, bool ChunkSizeOne, bool Ordered) const {
...
//
bool CGOpenMPRuntimeNVPTX::generateCoalescedSchedule(
    OpenMPDistScheduleClauseKind DistScheduleKind, OpenMPScheduleClauseKind ScheduleKind, bool DistChunked, bool ChunkSizeOne, bool Ordered) const {
    return isSPMDExecutionMode() &&
...
bool CGOpenMPRuntimeNVPTX::requiresBarrier(const OMPLoopDirective &S) const {
    ...
    OpenMPScheduleClauseKind	ScheduleKind	= OMPC_SCHEDULE_unknown;
    if(auto *C = S.getSingleClause<OMPScheduleClause>()) ScheduleKind = C->getScheduleKind();
...
CGOpenMPRuntimeNVPTX::CGOpenMPRuntimeNVPTX(CodeGenModule &CGM)
    ...
    if(! CGM.getLangOpts().OpenMPIsDevice) llvm_unreachable("OpenMP NVPTX can only handle device code.");
}

void CGOpenMPRuntimeNVPTX::emitNumTeamsClause(CodeGenFunction &CGF, const Expr *NumTeams, const Expr *ThreadLimit, SourceLocation Loc){}

static void emitPostUpdateForReductionClause(
    CodeGenFunction &CGF, const OMPExecutableDirective &D, const llvm::function_ref<llvm::Value *(CodeGenFunction &)> &CondGen){
    if(! CGF.HaveInsertPoint()) return;
    ...
    if(DoneBB) CGF.EmitBlock(DoneBB, /*IsFinished=*/ true);
}

void CGOpenMPRuntimeNVPTX::emitTeamsCall(CodeGenFunction &CGF, const OMPExecutableDirective &D, SourceLocation Loc, llvm::Value *OutlinedFn, ArrayRef<llvm::Value *> CapturedVars){
    if(isSPMDExecutionMode()){
        // OutlinedFn(&GTid, &zero, CapturedStruct);
        auto	ThreadIDAddr	= emitThreadIDAddress(CGF, Loc);
        Address ZeroAddr		=
...
llvm::Function *CGOpenMPRuntimeNVPTX::emitRegistrationFunction(){
    ...
    for(auto &DS : DataSharingFunctionInfoMap){
        ...
        for(auto &I : HeaderBB){
            ...
            if(It == DSI.ValuesToBeReplaced.end()) continue;
        ...
        if(LastAlloca) InsertPtr = LastAlloca->getNextNode();
        else InsertPtr = &(*HeaderBB.begin());
        ...
        if(LastNonAllocaReplacement) InsertPtr = LastNonAllocaReplacement->getNextNode();
        ...
        if(DSI.IsEntryPoint) InitializeEntryPoint(InsertPtr);
...
static llvm::Value *CreateRuntimeShuffleFunction(CodeGenFunction &CGF, QualType ShuffleTy, llvm::Value *Elem, llvm::Value *Offset){
    auto	&CGM	= CGF.CGM;
...
static void EmitDirectionSpecializedReduceDataCopy(
    COPY_DIRECTION Direction, CodeGenFunction &CGF, QualType ReductionArrayTy, ArrayRef<const Expr *> Privates, Address SrcBase, Address DestBase, llvm::Value *OffsetVal = nullptr, llvm::Value *IndexVal = nullptr, llvm::Value *WidthVal = nullptr){
    auto	&CGM	= CGF.CGM;
    auto	&C		= CGM.getContext();
    auto	&Bld	= CGF.Builder;
...
// address = base + index * ElementSizeInChars.
                ...
                if(! BitSize) BitSize = getArrayTypeSize(CGF.ConvertTypeForMem(Private->getType()));
...
// for each ReduceData Element and ReduceData subelement.
        ...
        if(Direction == ReduceData_To_Global ||
            ...
            if(Direction == ReduceData_To_Global) DestBase = Address(DestOrSrcBasePtrVal, CGF.getPointerAlign());
...
llvm::Value *EmitCopyToScratchpad(CodeGenModule &CGM, ArrayRef<const Expr *> Privates, QualType ReductionArrayTy){
    //
...
llvm::Value *EmitReduceScratchpadFunction(CodeGenModule &CGM, ArrayRef<const Expr *> Privates, QualType ReductionArrayTy, llvm::Value *ReduceFn){
    auto &C = CGM.getContext();
    //
...
void CGOpenMPRuntimeNVPTX::emitReduction(
    CodeGenFunction &CGF, SourceLocation Loc, ArrayRef<const Expr *> Privates, ArrayRef<const Expr *> LHSExprs, ArrayRef<const Expr *> RHSExprs, ArrayRef<const Expr *> ReductionOps, bool WithNowait, bool SimpleReduction, OpenMPDirectiveKind ReductionKind){
    if(! CGF.HaveInsertPoint()) return;
    ...
    // reduction directive.
    if(ReductionKind == OMPD_distribute_parallel_for) return;
    if(TeamsReduction){
