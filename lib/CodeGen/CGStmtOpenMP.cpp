...
/// for captured expressions.
    class OMPLexicalScope final : public CodeGenFunction::LexicalScope {
                ...
                if(auto *CPI = OMPClauseWithPreInit::get(C)){
                    if(auto *PreInit = cast_or_null<DeclStmt>(CPI->getPreInitStmt())){
                        for(const auto *I : PreInit->decls()){
                            if(! I->hasAttr<OMPCaptureNoInitAttr>()) CGF.EmitVarDecl(cast<VarDecl>(*I));
                            else{
        ...
        public:
            OMPLexicalScope(CodeGenFunction &CGF, const OMPExecutableDirective &S, bool AsInlined = false)
...
llvm::Value *CodeGenFunction::getTypeSize(QualType Ty){
    ...
    if(SizeInChars.isZero()){
        ...
        if(SizeInChars.isZero()) return llvm::ConstantInt::get(SizeTy, /*V=*/ 0);
        Size = Builder.CreateNUWMul(Size, CGM.getSize(SizeInChars));
    }else Size = CGM.getSize(SizeInChars);
    return Size;
}

void CodeGenFunction::GenerateOpenMPCapturedVars(
    const CapturedStmt &S, SmallVectorImpl<llvm::Value *> &CapturedVars, unsigned CaptureLevel){
       ...
       , E = S.capture_init_end();
        ...
        if(CurCap->capturesVariable() || CurCap->capturesVariableByCopy()){
            ...
            if(auto *C = dyn_cast<OMPCapturedExprDecl>(Var))
                    ...
                    if(C->getCaptureLevel() < CaptureLevel) continue;
        }
        if(CurField->hasCapturedVLAType()){
            auto	VAT		= CurField->getCapturedVLAType();
            auto	*Val	= VLASizeMap[VAT->getSizeExpr()];
            CapturedVars.push_back(Val);
        }else if(CurCap->capturesThis()) CapturedVars.push_back(CXXThisValue);
...
llvm::Function *CodeGenFunction::GenerateOpenMPCapturedStmtFunction(
    const CapturedStmt &S, bool UseCapturedArgumentsOnly, unsigned CaptureLevel, unsigned ImplicitParamStop, bool NonAliasedMaps){
    assert(
        CapturedStmtInfo &&
    ...
    if(ImplicitParamStop == 0) ImplicitParamStop = CD->getContextParamPosition();
    if(! UseCapturedArgumentsOnly) Args.append(CD->param_begin()
    ...
    for(auto *FD : RD->fields()){
        ...
        }else if(I->capturesThis()) II = &getContext().Idents.get("this");
        else{
        ...
        if(NonAliasedMaps && ArgType->isAnyPointerType()) ArgType = ArgType.withRestrict();
    ...
    if(CD->isNothrow()) F->addFnAttr(llvm::Attribute::NoUnwind);
...
void CodeGenFunction::EmitOMPAggregateAssign(
    Address DestAddr, Address SrcAddr, QualType OriginalType, const llvm::function_ref<void (Address, Address)> &CopyGen){
            ...
            if(auto *OVE = dyn_cast<OpaqueValueExpr>(CE->getCallee()))
                    if(auto *DRE =
                        dyn_cast<DeclRefExpr>(OVE->getSourceExpr()->IgnoreImpCasts()))
                            if(auto *DRD = dyn_cast<OMPDeclareReductionDecl>(DRE->getDecl())) return DRD;
    return nullptr;
}
...
static void EmitOMPAggregateInit(CodeGenFunction &CGF, Address DestAddr, QualType Type, const Expr *Init, Address SrcAddr = Address::invalid()){
    ...
    if(DRD) SrcBegin = SrcAddr.getPointer();
    auto DestBegin = DestAddr.getPointer();
    // Cast from pointer to array type to pointer to single element.
    ...
    // Emit copy.
    {
        ...
        }else CGF.EmitAnyExprToMem(Init, DestElementCurrent, ElementTy.getQualifiers()
                                  ,	/*IsInitializer=*/ false);
    }
    if(DRD){
        // Shift the address forward by one element.
...
void CodeGenFunction::EmitOMPCopy(QualType OriginalType, Address DestAddr, Address SrcAddr, const VarDecl *DestVD, const VarDecl *SrcVD, const Expr *Copy){
...
bool CodeGenFunction::EmitOMPFirstprivateClause(const OMPExecutableDirective &D, OMPPrivateScope &PrivateScope){
    if(! HaveInsertPoint()) return false;
...
void CodeGenFunction::EmitOMPPrivateClause(
    const OMPExecutableDirective &D, CodeGenFunction::OMPPrivateScope &PrivateScope){
...
bool CodeGenFunction::EmitOMPLastprivateClauseInit(
    ...
    for(const auto *C : D.getClausesOfKind<OMPLastprivateClause>()){
        ...
        if(isOpenMPTaskLoopDirective(D.getDirectiveKind())) break;
        auto	IRef		= C->varlist_begin();
        auto	IDestRef	= C->destination_exprs().begin();
        for(auto *IInit : C->private_copies()){
...
void CodeGenFunction::EmitOMPLastprivateClauseFinal(
    ...
    if(auto *LoopDirective = dyn_cast<OMPLoopDirective>(&D)){
        ...
        for(auto F : LoopDirective->finals()){
            ...
            if(NoFinals) AlreadyEmittedVars.insert(D);
            else LoopCountersAndUpdates[D] = F;
            ++ IC;
        }
    }
    for(const auto *C : D.getClausesOfKind<OMPLastprivateClause>()){
        ...
        for(auto *AssignOp : C->assignment_ops()){
            ...
            if(AlreadyEmittedVars.insert(CanonicalVD).second){
                ...
                // variable.
                if(auto *FinalExpr = LoopCountersAndUpdates.lookup(CanonicalVD)) EmitIgnoredExpr(FinalExpr);
                auto	*SrcVD	= cast<VarDecl>(cast<DeclRefExpr>(*ISrcRef)->getDecl());
                auto	*DestVD = cast<VarDecl>(cast<DeclRefExpr>(*IDestRef)->getDecl());
        ...
        if(auto *PostUpdate = C->getPostUpdateExpr()) EmitIgnoredExpr(PostUpdate);
    }
    if(IsLastIterCond) EmitBlock(DoneBB, /*IsFinished=*/ true);
}
...
static Address castToBase(CodeGenFunction &CGF, QualType BaseTy, QualType ElTy, LValue BaseLV, llvm::Value *Addr){
    ...
    while((BaseTy->isPointerType() || BaseTy->isReferenceType()) &&
        ...
        if(TopTmp.isValid()) CGF.Builder.CreateStore(Tmp.getPointer(), TopTmp);
        else MostTopTmp = Tmp;
        TopTmp	= Tmp;
        BaseTy	= BaseTy->getPointeeType();
    }
    llvm::Type *Ty = BaseLV.getPointer()->getType();
    if(Tmp.isValid()) Ty = Tmp.getElementType();
...
static LValue loadToBegin(CodeGenFunction &CGF, QualType BaseTy, QualType ElTy, LValue BaseLV){
    ...
    while((BaseTy->isPointerType() || BaseTy->isReferenceType()) &&
        ! CGF.getContext().hasSameType(BaseTy, ElTy)){
        if(auto *PtrTy = BaseTy->getAs<PointerType>()) BaseLV = CGF.EmitLoadOfPointerLValue(BaseLV.getAddress(), PtrTy);
        else{
...
static void emitCommonOMPParallelDirective(CodeGenFunction &CGF, const OMPExecutableDirective &S, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen, unsigned CaptureLevel = 1){
...
void CodeGenFunction::EmitOMPLoopBody(const OMPLoopDirective &D, JumpDest LoopExit){
    RunCleanupsScope BodyScope(*this);
    // Update counters values on current iteration.
    for(auto I : D.updates()) EmitIgnoredExpr(I);
    // Update the linear variables.
    for(const auto *C : D.getClausesOfKind<OMPLinearClause>()){
        for(auto *U : C->updates()) EmitIgnoredExpr(U);
    }
...
void CodeGenFunction::EmitOMPInnerLoop(
    const Stmt &S, bool RequiresCleanup, const Expr *LoopCond, const Expr *IncExpr, const llvm::function_ref<void (CodeGenFunction &)> &BodyGen, const llvm::function_ref<void (CodeGenFunction &)> &PostIncGen){
    ...
    if(RequiresCleanup) ExitBlock = createBasicBlock("omp.inner.for.cond.cleanup");
...
void CodeGenFunction::EmitOMPLinearClauseFinal(
    const OMPLoopDirective &D, const llvm::function_ref<llvm::Value *(CodeGenFunction &)> &CondGen){
    if(! HaveInsertPoint()) return;
    llvm::BasicBlock *DoneBB = nullptr;
    ...
    if(DoneBB) EmitBlock(DoneBB, /*IsFinished=*/ true);
}
...
static void emitPreCond(CodeGenFunction &CGF, const OMPLoopDirective &S, const Expr *Cond, llvm::BasicBlock *TrueBlock, llvm::BasicBlock *FalseBlock, uint64_t TrueCount){
    if(! CGF.HaveInsertPoint()) return;
    {
        ...
        for(auto I : S.inits()) CGF.EmitIgnoredExpr(I);
    }
...
static void emitSimdlenSafelenClause(CodeGenFunction &CGF, const OMPExecutableDirective &D, bool IsMonotonic){
    ...
    if(const auto *C = D.getSingleClause<OMPSimdlenClause>()){
        ...
        if(! IsMonotonic) CGF.LoopStack.setParallel(! D.getSingleClause<OMPSafelenClause>());
...
void CodeGenFunction::EmitOMPSimdInit(const OMPLoopDirective &D, bool IsMonotonic){
...
void CodeGenFunction::EmitOMPSimdFinal(
    ...
    for(auto F : D.finals()){
        ...
        if(LocalDeclMap.count(OrigVD) || CapturedStmtInfo->lookup(OrigVD) ||
            ...
            if(CED) OrigAddr = EmitLValue(CED->getInit()->IgnoreImpCasts()).getAddress();
            else{
                DeclRefExpr DRE(const_cast<VarDecl *>(PrivateVD)
                               , /*RefersToEnclosingVariableOrCapture=*/ false
...
void CodeGenFunction::EmitOMPHelperVar(const VarDecl *VDecl){
    ...
    if(LocalDeclMap.find(VDecl) == LocalDeclMap.end()) EmitVarDecl(*VDecl);
}

static void emitDeviceOMPSimdDirective(CodeGenFunction &CGF, const OMPExecutableDirective &S, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen){
    CGF.CGM.getOpenMPRuntime().registerParallelContext(CGF, S);
    auto									CS = cast<CapturedStmt>(S.getAssociatedStmt());
    llvm::SmallVector<llvm::Value *, 16>	CapturedVars;
...
void CodeGenFunction::EmitOMPSimdLoop(const OMPLoopDirective &S, bool OutlinedSimd){
    auto &&CodeGen = [&S, OutlinedSimd](CodeGenFunction &CGF, PrePostActionTy &){
            // Do not emit this code if it was already emitted before
            // in the same scope.
            if(! requiresAdditionalIterationVar(S.getDirectiveKind())) OMPLoopScope PreInitScope(CGF, S);
            // if (PreCond) {
            ...
            CGF.EmitOMPLinearClauseInit(S);
            {
                ...
                if(! LastprivateAlreadyEmitted) CGF.EmitOMPPrivateClause(S, LoopScope);
                CGF.EmitOMPReductionClauseInit(S, LoopScope);
                bool HasLastprivateClause = false;
                if(! LastprivateAlreadyEmitted) HasLastprivateClause = CGF.EmitOMPLastprivateClauseInit(S, LoopScope);
                ...
                if(HasLastprivateClause) CGF.EmitOMPLastprivateClauseFinal(S, /*NoFinals=*/ true);
    ...
    if(OutlinedSimd) emitDeviceOMPSimdDirective(*this, S, OMPD_simd, CodeGen);
    else{
        if(S.getDirectiveKind() == OMPD_teams_distribute_simd ||
            S.getDirectiveKind() == OMPD_target_teams_distribute_simd) CGM.getOpenMPRuntime().emitInlinedDirective(*this, OMPD_simd, CodeGen);
        else{
...
void CodeGenFunction::EmitOMPOuterLoop(
    bool DynamicOrOrdered, bool IsMonotonic, bool IsDistribute, const OMPLoopDirective &S, OMPPrivateScope &LoopScope, bool Ordered, Address LB, Address UB, Address ST, Address IL, llvm::Value *Chunk, const RegionCodeGenTy &CodeGenDistributeLoopContent){
    ...
    // create a block to stage a loop exit along.
    auto ExitBlock = LoopExit.getBlock();
    if(LoopScope.requiresCleanups()) ExitBlock = createBasicBlock("omp.dispatch.cleanup");
    ...
    if(DynamicOrOrdered) EmitIgnoredExpr(S.getInit());
    // Create a block for the increment.
    auto Continue = getJumpDestInCurrentScope("omp.dispatch.inc");
    BreakContinueStack.push_back(BreakContinue(LoopExit, Continue));
    // Generate !llvm.loop.parallel metadata for loads and stores for loops
    // with dynamic/guided scheduling and without ordered clause.
    if(! isOpenMPSimdDirective(S.getDirectiveKind())) LoopStack.setParallel(! IsMonotonic);
    else EmitOMPSimdInit(S, IsMonotonic);
    ...
    EmitOMPInnerLoop(
        ...
        , LoopExit](CodeGenFunction &CGF){
            if(isOpenMPLoopBoundSharingDirective(S.getDirectiveKind()) &&
            IsDistribute) CodeGenDistributeLoopContent(CGF);
            else{
    ...
    if(DynamicOrOrdered) RT.emitForDispatchFinish(*this, S, S.getLocEnd(), IVSize, IVSigned);
    else/* !DynamicOrOrdered */
            RT.emitForStaticFinish(*this, S.getLocEnd());
}

void CodeGenFunction::EmitOMPForOuterLoop(
    const OpenMPScheduleTy &ScheduleKind, bool IsMonotonic, const OMPLoopDirective &S, OMPPrivateScope &LoopScope, bool Ordered, Address LB, Address UB, Address ST, Address IL, llvm::Value *Chunk){
    auto &RT = CGM.getOpenMPRuntime();
...
void CodeGenFunction::EmitOMPDistributeOuterLoop(
    OpenMPDistScheduleClauseKind ScheduleKind, const OMPLoopDirective &S, OMPPrivateScope &LoopScope, Address LB, Address UB, Address ST, Address IL, llvm::Value *Chunk, const RegionCodeGenTy &CodeGenDistributeLoopContent){
    auto &RT = CGM.getOpenMPRuntime();
...
static void emitCommonOMPTeamsDirective(CodeGenFunction &CGF, const OMPExecutableDirective &S, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen, unsigned CaptureLevel = 1, unsigned ImplicitParamStop = 0){
    auto	CS			= cast<CapturedStmt>(S.getAssociatedStmt());
...
namespace {
    struct ScheduleKindModifiersTy {
        ...
        ScheduleKindModifiersTy(OpenMPScheduleClauseKind Kind, OpenMPScheduleClauseModifier M1, OpenMPScheduleClauseModifier M2)
...
bool CodeGenFunction::EmitOMPWorksharingLoop(const OMPLoopDirective &S){
    ...
    // Check pre-condition.
    {
        ...
        if(auto *OrderedClause = S.getSingleClause<OMPOrderedClause>()){
            if(OrderedClause->getNumForLoops()) RT.emitDoacrossInit(*this, S);
            else Ordered = true;
        }
        ...
        // Emit 'then' code.
        {
            ...
            if(auto *C = S.getSingleClause<OMPScheduleClause>()){
                ...
                if(const auto *Ch = C->getChunkSize()){
                    llvm::APSInt Result;
                    if(ConstantFoldsToSimpleInteger(Ch, Result)) ChunkSizeOne = Result == 1;
            ...
            if(RT.generateCoalescedSchedule(ScheduleKind.Schedule, ChunkSizeOne
                ...
                if(isOpenMPSimdDirective(S.getDirectiveKind())) EmitOMPSimdInit(S, /*IsMonotonic=*/ true);
...
void CodeGenFunction::EmitOMPForDirective(const OMPForDirective &S){
    ...
    if(CGM.getOpenMPRuntime().requiresBarrier(S) ||
        ! S.getSingleClause<OMPNowaitClause>() || HasLastprivates) CGM.getOpenMPRuntime().emitBarrierCall(*this, S.getLocStart(), OMPD_for);
}
...
void CodeGenFunction::EmitOMPForSimdDirective(const OMPForSimdDirective &S){
    ...
    // Emit an implicit barrier at the end.
    if(! S.getSingleClause<OMPNowaitClause>() || HasLastprivates) CGM.getOpenMPRuntime().emitBarrierCall(*this, S.getLocStart(), OMPD_for);
}

static LValue createSectionLVal(CodeGenFunction &CGF, QualType Ty, const Twine &Name, llvm::Value *Init = nullptr){
    auto LVal = CGF.MakeAddrLValue(CGF.CreateMemTemp(Ty, Name), Ty);
    if(Init) CGF.EmitStoreThroughLValue(RValue::get(Init), LVal, /*isInit*/ true);
    return LVal;
}
...
void CodeGenFunction::EmitSections(const OMPExecutableDirective &S){
    ...
    if(auto *OSD = dyn_cast<OMPSectionsDirective>(&S)) HasCancel = OSD->hasCancel();
    else if(auto *OPSD = dyn_cast<OMPParallelSectionsDirective>(&S)) HasCancel = OPSD->hasCancel();
...
void CodeGenFunction::EmitOMPCriticalDirective(const OMPCriticalDirective &S){
    ...
    Expr *Hint = nullptr;
    if(auto *HintClause = S.getSingleClause<OMPHintClause>()) Hint = HintClause->getHint();
    OMPLexicalScope Scope(*this, S,	/*AsInlined=*/ true);
...
void CodeGenFunction::EmitOMPTaskBasedDirective(const OMPExecutableDirective &S, const RegionCodeGenTy &BodyGen, const TaskGenTy &TaskGen, OMPTaskDataTy &Data){
    ...
    if(const auto *Clause = S.getSingleClause<OMPFinalClause>()){
        ...
        bool	CondConstant;
        if(ConstantFoldsToSimpleInteger(Cond, CondConstant)) Data.Final.setInt(CondConstant);
        else Data.Final.setPointer(EvaluateExprAsBool(Cond));
    }else{
    ...
    for(const auto *C : S.getClausesOfKind<OMPDependClause>())
            for(auto *IRef : C->varlists()) Data.Dependences.push_back(std::make_pair(C->getDependencyKind(), IRef));
...
void CodeGenFunction::EmitOMPDistributeLoop(
    const OMPLoopDirective &S, const RegionCodeGenTy &CodeGenDistributeLoopContent){
    // Emit the loop iteration variable.
    auto	IVExpr	= cast<DeclRefExpr>(S.getIterationVariable());
    auto	IVDecl	= cast<VarDecl>(IVExpr->getDecl());
    EmitVarDecl(*IVDecl);
                ...
                EmitOMPInnerLoop(S, LoopScope.requiresCleanups()
                    ...
                    , &CodeGenDistributeLoopContent](CodeGenFunction &CGF){
                        ...
                        OMPD_target_teams_distribute){
                            ...
                            S.getDirectiveKind())) CodeGenDistributeLoopContent(CGF);
                    }
                                , [](CodeGenFunction &){});
                EmitBlock(LoopExit.getBlock());
                // Tell the runtime we are done.
...
void CodeGenFunction::EmitOMPOrderedDirective(const OMPOrderedDirective &S){
    if(! S.getAssociatedStmt()){
        for(const auto *DC : S.getClausesOfKind<OMPDependClause>()) CGM.getOpenMPRuntime().emitDoacrossOrdered(*this, DC);
        return;
    }
...
static void emitSimpleAtomicStore(CodeGenFunction &CGF, bool IsSeqCst, LValue LVal, RValue RVal){
    if(LVal.isGlobalReg()) CGF.EmitStoreThroughGlobalRegLValue(RVal, LVal);
    else{
...
void CodeGenFunction::emitOMPSimpleStore(LValue LVal, RValue RVal, QualType RValTy, SourceLocation Loc){
...
static void EmitOMPAtomicReadExpr(CodeGenFunction &CGF, bool IsSeqCst, const Expr *X, const Expr *V, SourceLocation Loc){
    ...
    // list.
    if(IsSeqCst) CGF.CGM.getOpenMPRuntime().emitFlush(CGF, llvm::None, Loc);
...
static std::pair<bool, RValue> emitOMPAtomicRMW(CodeGenFunction &CGF, LValue X, RValue Update, BinaryOperatorKind BO, llvm::AtomicOrdering AO, bool IsXLHSInRHSPart){
    ...
    if(BO == BO_Comma || ! Update.isScalar() ||
        ...
        ! Context.getTargetInfo().hasBuiltinAtomic(
            Context.getTypeSize(X.getType()), Context.toBits(X.getAlignment()))) return std::make_pair(false, RValue::get(nullptr));
    ...
    switch(BO){
        case BO_Add:
            ...
            break;
        case BO_Sub:
            if(! IsXLHSInRHSPart) return std::make_pair(false, RValue::get(nullptr));
...
std::pair<bool, RValue> CodeGenFunction::EmitOMPAtomicSimpleUpdateExpr(
    LValue X, RValue E, BinaryOperatorKind BO, bool IsXLHSInRHSPart, llvm::AtomicOrdering AO, SourceLocation Loc, const llvm::function_ref<RValue(RValue)> &CommonGen){
...
void CodeGenFunction::EmitOMPAtomicDirective(const OMPAtomicDirective &S){
    ...
    if(const auto *EWC = dyn_cast<ExprWithCleanups>(CS)) enterFullExpression(EWC);
    // Processing for statements under 'atomic capture'.
    if(const auto *Compound = dyn_cast<CompoundStmt>(CS)){
        for(const auto *C : Compound->body()){
            if(const auto *EWC = dyn_cast<ExprWithCleanups>(C)) enterFullExpression(EWC);
        }
    }
...
static void emitCommonOMPTargetDirective(CodeGenFunction &CGF, const OMPExecutableDirective &S, OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen){
    ...
    if(auto *C = S.getSingleClause<OMPDeviceClause>()) Device = C->getDevice();
    ...
    if(IfCond){
        bool Val;
        if(CGF.ConstantFoldsToSimpleInteger(IfCond, Val) && ! Val) IsOffloadEntry = false;
    }
    if(CGM.getLangOpts().OMPTargetTriples.empty()) IsOffloadEntry = false;
    assert(CGF.CurFuncDecl && "No parent declaration for target region!");
    StringRef ParentName;
    // In case we have Ctors/Dtors we use the complete type variant to produce
    // the mangling of the device outlined kernel.
    if(auto *D = dyn_cast<CXXConstructorDecl>(CGF.CurFuncDecl)) ParentName = CGM.getMangledName(GlobalDecl(D, Ctor_Complete));
    else if(auto *D = dyn_cast<CXXDestructorDecl>(CGF.CurFuncDecl)) ParentName = CGM.getMangledName(GlobalDecl(D, Dtor_Complete));
    else ParentName =
...
void TargetCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetDirective &S){
    CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
    (void) CGF.EmitOMPFirstprivateClause(S, PrivateScope);
    CGF.EmitOMPPrivateClause(S, PrivateScope);
    (void) PrivateScope.Privatize();
...
void CodeGenFunction::EmitOMPTargetDeviceFunction(CodeGenModule &CGM, StringRef ParentName, const OMPTargetDirective &S){
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &Action){
...
static void TargetParallelCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetParallelDirective &S){
    Action.Enter(CGF);
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &){
            CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
            (void) CGF.EmitOMPFirstprivateClause(S, PrivateScope);
...
void CodeGenFunction::EmitOMPTargetParallelDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetParallelDirective &S){
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &Action){
...
static void TargetSimdCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetSimdDirective &S){
    CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
    (void) CGF.EmitOMPFirstprivateClause(S, PrivateScope);
    CGF.EmitOMPPrivateClause(S, PrivateScope);
    (void) PrivateScope.Privatize();
...
static void TargetParallelForCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetParallelForDirective &S){
    Action.Enter(CGF);
    // Emit directive as a combined directive that consists of two implicit
    // directives: 'parallel' with 'for' directive.
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &){
...
void CodeGenFunction::EmitOMPTargetParallelForDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetParallelForDirective &S){
...
static void TargetParallelForSimdCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetParallelForSimdDirective &S){
    Action.Enter(CGF);
    // Emit directive as a combined directive that consists of two implicit
    // directives: 'parallel' with 'for' directive.
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &){
...
void CodeGenFunction::EmitOMPTargetParallelForSimdDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetParallelForSimdDirective &S){
    // Emit SPMD target parallel for region as a standalone region.
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &Action){
...
static void TargetTeamsDistributeParallelForCodegen(
    CodeGenModule &CGM, CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetTeamsDistributeParallelForDirective &S){
    Action.Enter(CGF);
...
void CodeGenFunction::EmitOMPTargetTeamsDistributeParallelForDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetTeamsDistributeParallelForDirective &S){
    // Emit SPMD target parallel for region as a standalone region.
...
static void TargetTeamsDistributeParallelForSimdCodegen(
    CodeGenModule &CGM, CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetTeamsDistributeParallelForSimdDirective &S){
    Action.Enter(CGF);
    auto &&CodeGen = [&CGM, &S](CodeGenFunction &CGF, PrePostActionTy &){
            CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
            CGF.EmitOMPReductionClauseInit(S, PrivateScope);
...
void CodeGenFunction::EmitOMPTargetTeamsDistributeParallelForSimdDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetTeamsDistributeParallelForSimdDirective &S){
    // Emit SPMD target parallel for region as a standalone region.
    auto &&CodeGen = [&S, &CGM](CodeGenFunction &CGF, PrePostActionTy &Action){
...
static void TargetTeamsCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetTeamsDirective &S){
    Action.Enter(CGF);
...
static void TargetTeamsDistributeCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetTeamsDistributeDirective &S){
    Action.Enter(CGF);
    auto &&CGDistributeInlined = [&S](CodeGenFunction &CGF, PrePostActionTy &){
            CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
            CGF.EmitOMPReductionClauseInit(S, PrivateScope);
...
void CodeGenFunction::EmitOMPTargetTeamsDistributeDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetTeamsDistributeDirective &S){
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &Action){
...
static void TargetTeamsDistributeSimdCodegen(CodeGenFunction &CGF, PrePostActionTy &Action, const OMPTargetTeamsDistributeSimdDirective &S){
    Action.Enter(CGF);
    auto &&CGDistributeInlined = [&S](CodeGenFunction &CGF, PrePostActionTy &){
            CodeGenFunction::OMPPrivateScope PrivateScope(CGF);
            (void) CGF.EmitOMPFirstprivateClause(S, PrivateScope);
...
void CodeGenFunction::EmitOMPTargetTeamsDistributeSimdDeviceFunction(
    CodeGenModule &CGM, StringRef ParentName, const OMPTargetTeamsDistributeSimdDirective &S){
    auto &&CodeGen = [&S](CodeGenFunction &CGF, PrePostActionTy &Action){
...
CodeGenFunction::JumpDest CodeGenFunction::getOMPCancelDestination(OpenMPDirectiveKind Kind){
    if(Kind == OMPD_parallel || Kind == OMPD_task) return ReturnBlock;
...
void CodeGenFunction::EmitOMPUseDevicePtrClause(
    const OMPClause &NC, OMPPrivateScope &PrivateScope, const llvm::DenseMap<const ValueDecl *, Address> &CaptureDeviceAddrMap){
...
void CodeGenFunction::EmitOMPTargetDataDirective(const OMPTargetDataDirective &S){
    ...
    // Check if we have any if clause associated with the directive.
    const Expr *IfCond = nullptr;
    if(auto *C = S.getSingleClause<OMPIfClause>()) IfCond = C->getCondition();
    // Check if we have any device clause associated with the directive.
    const Expr *Device = nullptr;
    if(auto *C = S.getSingleClause<OMPDeviceClause>()) Device = C->getDevice();
...
void CodeGenFunction::EmitOMPTargetEnterDataDirective(const OMPTargetEnterDataDirective &S){
    // If we don't have target devices, don't bother emitting the data mapping
    // code.
    if(CGM.getLangOpts().OMPTargetTriples.empty()) return;
    // Check if we have any if clause associated with the directive.
    const Expr *IfCond = nullptr;
    if(auto *C = S.getSingleClause<OMPIfClause>()) IfCond = C->getCondition();
    // Check if we have any device clause associated with the directive.
...
void CodeGenFunction::EmitOMPTaskLoopBasedDirective(const OMPLoopDirective &S){
    ...
    auto &&BodyGen = [CS, &S](CodeGenFunction &CGF, PrePostActionTy &){
            ...
            if(isOpenMPSimdDirective(S.getDirectiveKind())) CGF.EmitOMPSimdInit(S);
            OMPPrivateScope LoopScope(CGF);
            // Emit helper vars inits.
