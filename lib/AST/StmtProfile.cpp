...
namespace {
    class StmtProfiler : public ConstStmtVisitor<StmtProfiler> {
        ...
        public:
            StmtProfiler(llvm::FoldingSetNodeID &ID, const ASTContext &Context, bool Canonical)
...
void StmtProfiler::VisitStmt(const Stmt *S){
    ...
    for(const Stmt *SubStmt : S->children()){
        if(SubStmt) Visit(SubStmt);
        else ID.AddInteger(0);
    }
}

void StmtProfiler::VisitDeclStmt(const DeclStmt *S){
    VisitStmt(S);
    for(const auto *D : S->decls()) VisitDecl(D);
}
...
void StmtProfiler::VisitGCCAsmStmt(const GCCAsmStmt *S){
    ...
    for(unsigned I = 0, N = S->getNumClobbers(); I != N; ++ I) VisitStringLiteral(S->getClobberStringLiteral(I));
}
...
void StmtProfiler::VisitObjCAtCatchStmt(const ObjCAtCatchStmt *S){
    ...
    if(S->getCatchParamDecl()) VisitType(S->getCatchParamDecl()->getType());
}
...
void StmtProfiler::VisitObjCAtSynchronizedStmt(const ObjCAtSynchronizedStmt *S){
    VisitStmt(S);
}
...
void StmtProfiler::VisitObjCAutoreleasePoolStmt(const ObjCAutoreleasePoolStmt *S){
    VisitStmt(S);
}
...
#include "clang/Basic/OpenMPKinds.def"
    ...
    void OMPClauseProfiler::VistOMPClauseWithPreInit(const OMPClauseWithPreInit *C){
        if(auto *S = C->getPreInitStmt()) Profiler->VisitStmt(S);
    }

    void OMPClauseProfiler::VistOMPClauseWithPostUpdate(const OMPClauseWithPostUpdate *C){
        VistOMPClauseWithPreInit(C);
        if(auto *E = C->getPostUpdateExpr()) Profiler->VisitStmt(E);
    }

    void OMPClauseProfiler::VisitOMPIfClause(const OMPIfClause *C){
        VistOMPClauseWithPreInit(C);
        if(C->getCondition()) Profiler->VisitStmt(C->getCondition());
    }
    ...
    void OMPClauseProfiler::VisitOMPNumThreadsClause(const OMPNumThreadsClause *C){
        VistOMPClauseWithPreInit(C);
        if(C->getNumThreads()) Profiler->VisitStmt(C->getNumThreads());
    }

    void OMPClauseProfiler::VisitOMPSafelenClause(const OMPSafelenClause *C){
        if(C->getSafelen()) Profiler->VisitStmt(C->getSafelen());
    }

    void OMPClauseProfiler::VisitOMPSimdlenClause(const OMPSimdlenClause *C){
        if(C->getSimdlen()) Profiler->VisitStmt(C->getSimdlen());
    }

    void OMPClauseProfiler::VisitOMPCollapseClause(const OMPCollapseClause *C){
        if(C->getNumForLoops()) Profiler->VisitStmt(C->getNumForLoops());
    }
    ...
    void OMPClauseProfiler::VisitOMPScheduleClause(const OMPScheduleClause *C){
        VistOMPClauseWithPreInit(C);
        if(auto *S = C->getChunkSize()) Profiler->VisitStmt(S);
    }

    void OMPClauseProfiler::VisitOMPOrderedClause(const OMPOrderedClause *C){
        if(auto *Num = C->getNumForLoops()) Profiler->VisitStmt(Num);
    }
    ...
    void OMPClauseProfiler::VisitOMPFirstprivateClause(const OMPFirstprivateClause *C){
        VisitOMPClauseList(C);
        VistOMPClauseWithPreInit(C);
        for(auto *E : C->private_copies()){
            if(E) Profiler->VisitStmt(E);
    ...
    void OMPClauseProfiler::VisitOMPLastprivateClause(const OMPLastprivateClause *C){
        VisitOMPClauseList(C);
    ...
    void OMPClauseProfiler::VisitOMPLinearClause(const OMPLinearClause *C){
        ...
        if(C->getStep()) Profiler->VisitStmt(C->getStep());
        if(C->getCalcStep()) Profiler->VisitStmt(C->getCalcStep());
    }
    void OMPClauseProfiler::VisitOMPAlignedClause(const OMPAlignedClause *C){
        VisitOMPClauseList(C);
        if(C->getAlignment()) Profiler->VisitStmt(C->getAlignment());
    }
    ...
    void OMPClauseProfiler::VisitOMPCopyprivateClause(const OMPCopyprivateClause *C){
        VisitOMPClauseList(C);
        for(auto *E : C->source_exprs()){
            if(E) Profiler->VisitStmt(E);
        }
    ...
    void OMPClauseProfiler::VisitOMPNumTeamsClause(const OMPNumTeamsClause *C){
        if(C->getNumTeams()) Profiler->VisitStmt(C->getNumTeams());
    }
    void OMPClauseProfiler::VisitOMPThreadLimitClause(const OMPThreadLimitClause *C){
        if(C->getThreadLimit()) Profiler->VisitStmt(C->getThreadLimit());
    }
    void OMPClauseProfiler::VisitOMPPriorityClause(const OMPPriorityClause *C){
        if(C->getPriority()) Profiler->VisitStmt(C->getPriority());
    }
    void OMPClauseProfiler::VisitOMPGrainsizeClause(const OMPGrainsizeClause *C){
        if(C->getGrainsize()) Profiler->VisitStmt(C->getGrainsize());
    }
    void OMPClauseProfiler::VisitOMPNumTasksClause(const OMPNumTasksClause *C){
        if(C->getNumTasks()) Profiler->VisitStmt(C->getNumTasks());
    }
    void OMPClauseProfiler::VisitOMPHintClause(const OMPHintClause *C){
        if(C->getHint()) Profiler->VisitStmt(C->getHint());
    }
    ...
    void OMPClauseProfiler::VisitOMPUseDevicePtrClause(const OMPUseDevicePtrClause *C){
        VisitOMPClauseList(C);
    }
    void OMPClauseProfiler::VisitOMPIsDevicePtrClause(const OMPIsDevicePtrClause *C){
        VisitOMPClauseList(C);
    }
}

void StmtProfiler::VisitOMPExecutableDirective(const OMPExecutableDirective *S){
    VisitStmt(S);
...
void StmtProfiler::VisitOMPParallelForDirective(const OMPParallelForDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPParallelForSimdDirective(const OMPParallelForSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPParallelSectionsDirective(const OMPParallelSectionsDirective *S){
    VisitOMPExecutableDirective(S);
}
...
void StmtProfiler::VisitOMPTargetEnterDataDirective(const OMPTargetEnterDataDirective *S){
    VisitOMPExecutableDirective(S);
}

void StmtProfiler::VisitOMPTargetExitDataDirective(const OMPTargetExitDataDirective *S){
    VisitOMPExecutableDirective(S);
}

void StmtProfiler::VisitOMPTargetParallelDirective(const OMPTargetParallelDirective *S){
    VisitOMPExecutableDirective(S);
}

void StmtProfiler::VisitOMPTargetParallelForDirective(const OMPTargetParallelForDirective *S){
    VisitOMPExecutableDirective(S);
}
...
void StmtProfiler::VisitOMPCancellationPointDirective(const OMPCancellationPointDirective *S){
    VisitOMPExecutableDirective(S);
}
...
void StmtProfiler::VisitOMPTaskLoopSimdDirective(const OMPTaskLoopSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPDistributeDirective(const OMPDistributeDirective *S){
    VisitOMPLoopDirective(S);
}

void OMPClauseProfiler::VisitOMPDistScheduleClause(const OMPDistScheduleClause *C){
    VistOMPClauseWithPreInit(C);
    if(auto *S = C->getChunkSize()) Profiler->VisitStmt(S);
}
...
void StmtProfiler::VisitOMPTargetUpdateDirective(const OMPTargetUpdateDirective *S){
    VisitOMPExecutableDirective(S);
}

void StmtProfiler::VisitOMPDistributeParallelForDirective(const OMPDistributeParallelForDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPDistributeParallelForSimdDirective(const OMPDistributeParallelForSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPDistributeSimdDirective(const OMPDistributeSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetParallelForSimdDirective(const OMPTargetParallelForSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetSimdDirective(const OMPTargetSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTeamsDistributeDirective(const OMPTeamsDistributeDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTeamsDistributeSimdDirective(const OMPTeamsDistributeSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetTeamsDirective(const OMPTargetTeamsDirective *S){
    VisitOMPExecutableDirective(S);
}

void StmtProfiler::VisitOMPTeamsDistributeParallelForDirective(const OMPTeamsDistributeParallelForDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetTeamsDistributeParallelForDirective(const OMPTargetTeamsDistributeParallelForDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetTeamsDistributeParallelForSimdDirective(const OMPTargetTeamsDistributeParallelForSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetTeamsDistributeDirective(const OMPTargetTeamsDistributeDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTargetTeamsDistributeSimdDirective(const OMPTargetTeamsDistributeSimdDirective *S){
    VisitOMPLoopDirective(S);
}

void StmtProfiler::VisitOMPTeamsDistributeParallelForSimdDirective(const OMPTeamsDistributeParallelForSimdDirective *S){
    VisitOMPLoopDirective(S);
}
...
void StmtProfiler::VisitDeclRefExpr(const DeclRefExpr *S){
    VisitExpr(S);
    if(! Canonical) VisitNestedNameSpecifier(S->getQualifier());
    VisitDecl(S->getDecl());
    if(! Canonical) VisitTemplateArguments(S->getTemplateArgs(), S->getNumTemplateArgs());
}
...
void StmtProfiler::VisitUnaryExprOrTypeTraitExpr(const UnaryExprOrTypeTraitExpr *S){
    VisitExpr(S);
    ID.AddInteger(S->getKind());
    if(S->isArgumentType()) VisitType(S->getArgumentType());
}
...
void StmtProfiler::VisitCompoundAssignOperator(const CompoundAssignOperator *S){
...
void StmtProfiler::VisitBinaryConditionalOperator(const BinaryConditionalOperator *S){
    VisitExpr(S);
}
...
void StmtProfiler::VisitDesignatedInitExpr(const DesignatedInitExpr *S){
    ...
    for(const DesignatedInitExpr::Designator &D : S->designators()){
        ...
        if(D.isArrayDesignator()) ID.AddInteger(1);
        else{
...
void StmtProfiler::VisitDesignatedInitUpdateExpr(const DesignatedInitUpdateExpr *S){
...
void StmtProfiler::VisitGenericSelectionExpr(const GenericSelectionExpr *S){
    ...
    for(unsigned i = 0; i != S->getNumAssocs(); ++ i){
        ...
        if(T.isNull()) ID.AddPointer(nullptr);
        else VisitType(T);
...
void StmtProfiler::VisitPseudoObjectExpr(const PseudoObjectExpr *S){
    ...
    for(PseudoObjectExpr::const_semantics_iterator
        i = S->semantics_begin(), e = S->semantics_end(); i != e; ++ i)
            ...
            if(const OpaqueValueExpr *OVE = dyn_cast<OpaqueValueExpr>(*i)) Visit(OVE->getSourceExpr());
}
...
void StmtProfiler::VisitCXXOperatorCallExpr(const CXXOperatorCallExpr *S){
    if(S->isTypeDependent()){
        ...
        if(S->getOperator() == OO_Arrow) return Visit(S->getArg(0));
        UnaryOperatorKind	UnaryOp		= UO_Extension;
        BinaryOperatorKind	BinaryOp	= BO_Comma;
        Stmt::StmtClass		SC			= DecodeOperatorCall(S, UnaryOp, BinaryOp);

        ID.AddInteger(SC);
        for(unsigned I = 0, N = S->getNumArgs(); I != N; ++ I) Visit(S->getArg(I));
        if(SC == Stmt::UnaryOperatorClass) ID.AddInteger(UnaryOp);
        else if(SC == Stmt::BinaryOperatorClass ||
            SC == Stmt::CompoundAssignOperatorClass) ID.AddInteger(BinaryOp);
        else assert(SC == Stmt::ArraySubscriptExprClass);
        return;
    }
    VisitCallExpr(S);
...
void StmtProfiler::VisitCXXReinterpretCastExpr(const CXXReinterpretCastExpr *S){
    VisitCXXNamedCastExpr(S);
}
...
void StmtProfiler::VisitCXXStdInitializerListExpr(const CXXStdInitializerListExpr *S){
    VisitExpr(S);
}

void StmtProfiler::VisitCXXTypeidExpr(const CXXTypeidExpr *S){
    VisitExpr(S);
    if(S->isTypeOperand()) VisitType(S->getTypeOperandSourceInfo()->getType());
}
...
void StmtProfiler::VisitMSPropertySubscriptExpr(const MSPropertySubscriptExpr *S){
    VisitExpr(S);
}
...
void StmtProfiler::VisitCXXInheritedCtorInitExpr(const CXXInheritedCtorInitExpr *S){
    VisitExpr(S);
    VisitDecl(S->getConstructor());
}
...
void StmtProfiler::VisitCXXTemporaryObjectExpr(const CXXTemporaryObjectExpr *S){
...
void StmtProfiler::VisitCXXScalarValueInitExpr(const CXXScalarValueInitExpr *S){
    VisitExpr(S);
}
...
void StmtProfiler::VisitCXXPseudoDestructorExpr(const CXXPseudoDestructorExpr *S){
    VisitExpr(S);
    ID.AddBoolean(S->isArrow());
    VisitNestedNameSpecifier(S->getQualifier());
    ID.AddBoolean(S->getScopeTypeInfo() != nullptr);
    if(S->getScopeTypeInfo()) VisitType(S->getScopeTypeInfo()->getType());
    ID.AddBoolean(S->getDestroyedTypeInfo() != nullptr);
    if(S->getDestroyedTypeInfo()) VisitType(S->getDestroyedType());
    else ID.AddPointer(S->getDestroyedTypeIdentifier());
}
...
void StmtProfiler::VisitOverloadExpr(const OverloadExpr *S){
    ...
    if(S->hasExplicitTemplateArgs()) VisitTemplateArguments(S->getTemplateArgs(), S->getNumTemplateArgs());
}
...
void StmtProfiler::VisitTypeTraitExpr(const TypeTraitExpr *S){
    ...
    for(unsigned I = 0, N = S->getNumArgs(); I != N; ++ I) VisitType(S->getArg(I)->getType());
}
...
void StmtProfiler::VisitDependentScopeDeclRefExpr(const DependentScopeDeclRefExpr *S){
    VisitExpr(S);
...
void StmtProfiler::VisitCXXUnresolvedConstructExpr(const CXXUnresolvedConstructExpr *S){
    VisitExpr(S);
    VisitType(S->getTypeAsWritten());
}

void StmtProfiler::VisitCXXDependentScopeMemberExpr(const CXXDependentScopeMemberExpr *S){
...
void StmtProfiler::VisitSizeOfPackExpr(const SizeOfPackExpr *S){
    ...
    if(S->isPartiallySubstituted()){
        ...
        for(const auto &TA : Args) VisitTemplateArgument(TA);
    }else ID.AddInteger(0);
}

void StmtProfiler::VisitSubstNonTypeTemplateParmPackExpr(const SubstNonTypeTemplateParmPackExpr *S){
    VisitExpr(S);
    VisitDecl(S->getParameterPack());
    VisitTemplateArgument(S->getArgumentPack());
}

void StmtProfiler::VisitSubstNonTypeTemplateParmExpr(const SubstNonTypeTemplateParmExpr *E){
...
void StmtProfiler::VisitFunctionParmPackExpr(const FunctionParmPackExpr *S){
    ...
    for(FunctionParmPackExpr::iterator I = S->begin(), E = S->end(); I != E; ++ I) VisitDecl(*I);
}

void StmtProfiler::VisitMaterializeTemporaryExpr(const MaterializeTemporaryExpr *S){
    VisitExpr(S);
}
...
void StmtProfiler::VisitObjCIndirectCopyRestoreExpr(const ObjCIndirectCopyRestoreExpr *S){
    VisitExpr(S);
...
void StmtProfiler::VisitObjCAvailabilityCheckExpr(const ObjCAvailabilityCheckExpr *S){
    VisitExpr(S);
}
...
void StmtProfiler::VisitType(QualType T){
    if(Canonical) T = Context.getCanonicalType(T);
...
void StmtProfiler::VisitNestedNameSpecifier(NestedNameSpecifier *NNS){
    if(Canonical) NNS = Context.getCanonicalNestedNameSpecifier(NNS);
    ID.AddPointer(NNS);
}

void StmtProfiler::VisitTemplateName(TemplateName Name){
    if(Canonical) Name = Context.getCanonicalTemplateName(Name);
    Name.Profile(ID);
}

void StmtProfiler::VisitTemplateArguments(const TemplateArgumentLoc *Args, unsigned NumArgs){
    ID.AddInteger(NumArgs);
    for(unsigned I = 0; I != NumArgs; ++ I) VisitTemplateArgument(Args[I].getArgument());
}
...
void StmtProfiler::VisitTemplateArgument(const TemplateArgument &Arg){
    ...
    switch(Arg.getKind()){
        ...
        case TemplateArgument::Pack:
            for(const auto &P : Arg.pack_elements()) VisitTemplateArgument(P);
            break;
    }
}

void Stmt::Profile(llvm::FoldingSetNodeID &ID, const ASTContext &Context, bool Canonical) const {
