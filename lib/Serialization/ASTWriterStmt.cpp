...
void ASTStmtWriter::AddTemplateKWAndArgsInfo(
    ...
    for(unsigned i = 0; i != ArgInfo.NumTemplateArgs; ++ i) Record.AddTemplateArgumentLoc(Args[i]);
}
...
void ASTStmtWriter::VisitCompoundStmt(CompoundStmt *S){
    ...
    for(auto *CS : S->body()) Record.AddStmt(CS);
...
void ASTStmtWriter::VisitSwitchStmt(SwitchStmt *S){
    ...
    for(SwitchCase *SC = S->getSwitchCaseList(); SC;
        SC = SC->getNextSwitchCase()) Record.push_back(Writer.RecordSwitchCaseID(SC));
...
void ASTStmtWriter::VisitDeclStmt(DeclStmt *S){
    ...
    for(DeclGroupRef::iterator D = DG.begin(), DEnd = DG.end(); D != DEnd; ++ D) Record.AddDeclRef(*D);
...
void ASTStmtWriter::VisitGCCAsmStmt(GCCAsmStmt *S){
    ...
    // Clobbers
    for(unsigned I = 0, N = S->getNumClobbers(); I != N; ++ I) Record.AddStmt(S->getClobberStringLiteral(I));
...
void ASTStmtWriter::VisitMSAsmStmt(MSAsmStmt *S){
    ...
    // Clobbers
    for(unsigned I = 0, N = S->getNumClobbers(); I != N; ++ I) Record.AddString(S->getClobber(I));
    // Outputs
    for(unsigned I = 0, N = S->getNumOutputs(); I != N; ++ I){
        Record.AddStmt(S->getOutputExpr(I));
...
void ASTStmtWriter::VisitCapturedStmt(CapturedStmt *S){
    ...
    // Capture inits
    for(auto *I : S->capture_inits()) Record.AddStmt(I);
    // Body
    Record.AddStmt(S->getCapturedStmt());
    // Captures
    for(const auto &I : S->captures()){
        if(I.capturesThis() || I.capturesVariableArrayType()) Record.AddDeclRef(nullptr);
        else Record.AddDeclRef(I.getCapturedVar());
...
void ASTStmtWriter::VisitDeclRefExpr(DeclRefExpr *E){
    ...
    if((! E->hasTemplateKWAndArgsInfo()) && (! E->hasQualifier()) &&
        ...
        nk == DeclarationName::Identifier) AbbrevToUse = Writer.getDeclRefExprAbbrev();
    if(E->hasQualifier()) Record.AddNestedNameSpecifierLoc(E->getQualifierLoc());
    if(E->getDecl() != E->getFoundDecl()) Record.AddDeclRef(E->getFoundDecl());
    if(E->hasTemplateKWAndArgsInfo()) AddTemplateKWAndArgsInfo(*E->getTrailingObjects<ASTTemplateKWAndArgsInfo>()
...
void ASTStmtWriter::VisitIntegerLiteral(IntegerLiteral *E){
    ...
    if(E->getValue().getBitWidth() == 32) AbbrevToUse = Writer.getIntegerLiteralAbbrev();
...
void ASTStmtWriter::VisitStringLiteral(StringLiteral *E){
    ...
    for(unsigned I = 0, N = E->getNumConcatenated(); I != N; ++ I) Record.AddSourceLocation(E->getStrTokenLoc(I));
...
void ASTStmtWriter::VisitParenListExpr(ParenListExpr *E){
    ...
    for(unsigned i = 0; i != E->NumExprs; ++ i) Record.AddStmt(E->Exprs[i]);
...
void ASTStmtWriter::VisitOffsetOfExpr(OffsetOfExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumExpressions(); I != N; ++ I) Record.AddStmt(E->getIndexExpr(I));
...
void ASTStmtWriter::VisitUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *E){
    VisitExpr(E);
    Record.push_back(E->getKind());
    if(E->isArgumentType()) Record.AddTypeSourceInfo(E->getArgumentTypeInfo());
    else{
        Record.push_back(0);
...
void ASTStmtWriter::VisitCallExpr(CallExpr *E){
    ...
    for(CallExpr::arg_iterator Arg = E->arg_begin(), ArgEnd = E->arg_end();
        Arg != ArgEnd; ++ Arg) Record.AddStmt(*Arg);
...
void ASTStmtWriter::VisitMemberExpr(MemberExpr *E){
    ...
    if(E->HasTemplateKWAndArgsInfo){
        ...
        for(unsigned i = 0; i != NumTemplateArgs; ++ i) Record.AddTemplateArgumentLoc(E->getTemplateArgs()[i]);
    }
    Record.push_back(E->hadMultipleCandidates());
...
void ASTStmtWriter::VisitObjCIndirectCopyRestoreExpr(ObjCIndirectCopyRestoreExpr *E){
    VisitExpr(E);
    Record.AddStmt(E->getSubExpr());
...
void ASTStmtWriter::VisitCastExpr(CastExpr *E){
    ...
    for(CastExpr::path_iterator
        PI = E->path_begin(), PE = E->path_end(); PI != PE; ++ PI) Record.AddCXXBaseSpecifier(**PI);
}
...
void ASTStmtWriter::VisitBinaryConditionalOperator(BinaryConditionalOperator *E){
    VisitExpr(E);
...
void ASTStmtWriter::VisitImplicitCastExpr(ImplicitCastExpr *E){
    VisitCastExpr(E);
    if(E->path_size() == 0) AbbrevToUse = Writer.getExprImplicitCastAbbrev();
...
void ASTStmtWriter::VisitInitListExpr(InitListExpr *E){
    ...
    if(isArrayFiller) Record.AddStmt(E->getArrayFiller());
    else Record.AddDeclRef(E->getInitializedFieldInUnion());
    ...
    if(isArrayFiller){
        ...
        for(unsigned I = 0, N = E->getNumInits(); I != N; ++ I) Record.AddStmt(E->getInit(I) != filler ? E->getInit(I) : nullptr);
    }else{
        for(unsigned I = 0, N = E->getNumInits(); I != N; ++ I) Record.AddStmt(E->getInit(I));
    }
...
void ASTStmtWriter::VisitDesignatedInitExpr(DesignatedInitExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumSubExprs(); I != N; ++ I) Record.AddStmt(E->getSubExpr(I));
...
void ASTStmtWriter::VisitShuffleVectorExpr(ShuffleVectorExpr *E){
    VisitExpr(E);
    Record.push_back(E->getNumSubExprs());
    for(unsigned I = 0, N = E->getNumSubExprs(); I != N; ++ I) Record.AddStmt(E->getExpr(I));
    Record.AddSourceLocation(E->getBuiltinLoc());
    Record.AddSourceLocation(E->getRParenLoc());
...
void ASTStmtWriter::VisitPseudoObjectExpr(PseudoObjectExpr *E){
    ...
    for(PseudoObjectExpr::semantics_iterator
        i = E->semantics_begin(), e = E->semantics_end(); i != e; ++ i) Record.AddStmt(*i);
...
void ASTStmtWriter::VisitAtomicExpr(AtomicExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumSubExprs(); I != N; ++ I) Record.AddStmt(E->getSubExprs()[I]);
    Record.AddSourceLocation(E->getBuiltinLoc());
    Record.AddSourceLocation(E->getRParenLoc());
...
void ASTStmtWriter::VisitObjCArrayLiteral(ObjCArrayLiteral *E){
    ...
    for(unsigned i = 0; i < E->getNumElements(); i ++) Record.AddStmt(E->getElement(i));
...
void ASTStmtWriter::VisitObjCDictionaryLiteral(ObjCDictionaryLiteral *E){
    ...
    for(unsigned i = 0; i < E->getNumElements(); i ++){
        ...
        if(E->HasPackExpansions){
            ...
            if(Element.NumExpansions) NumExpansions = *Element.NumExpansions + 1;
...
void ASTStmtWriter::VisitObjCPropertyRefExpr(ObjCPropertyRefExpr *E){
    ...
    }else Record.AddDeclRef(E->getExplicitProperty());
    Record.AddSourceLocation(E->getLocation());
...
void ASTStmtWriter::VisitObjCMessageExpr(ObjCMessageExpr *E){
    ...
    for(unsigned i = 0, e = E->getNumStoredSelLocs(); i != e; ++ i) Record.AddSourceLocation(Locs[i]);
...
void ASTStmtWriter::VisitObjCAtTryStmt(ObjCAtTryStmt *S){
    ...
    for(unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++ I) Record.AddStmt(S->getCatchStmt(I));
    if(S->getFinallyStmt()) Record.AddStmt(S->getFinallyStmt());
...
void ASTStmtWriter::VisitCXXTryStmt(CXXTryStmt *S){
    ...
    for(unsigned i = 0, e = S->getNumHandlers(); i != e; ++ i) Record.AddStmt(S->getHandler(i));
...
void ASTStmtWriter::VisitCXXConstructExpr(CXXConstructExpr *E){
    VisitExpr(E);
    Record.push_back(E->getNumArgs());
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) Record.AddStmt(E->getArg(I));
...
void ASTStmtWriter::VisitLambdaExpr(LambdaExpr *E){
    ...
    if(E->HasArrayIndexVars) NumArrayIndexVars = E->getArrayIndexStarts()[E->NumCaptures];
...
void ASTStmtWriter::VisitCXXPseudoDestructorExpr(CXXPseudoDestructorExpr *E){
    ...
    if(E->getDestroyedTypeIdentifier()) Record.AddSourceLocation(E->getDestroyedTypeLoc());
    else Record.AddTypeSourceInfo(E->getDestroyedTypeInfo());
...
void ASTStmtWriter::VisitExprWithCleanups(ExprWithCleanups *E){
    ...
    for(unsigned i = 0, e = E->getNumObjects(); i != e; ++ i) Record.AddDeclRef(E->getObject(i));
    Record.push_back(E->cleanupsHaveSideEffects());
    Record.AddStmt(E->getSubExpr());
    Code = serialization::EXPR_EXPR_WITH_CLEANUPS;
}

void ASTStmtWriter::VisitCXXDependentScopeMemberExpr(CXXDependentScopeMemberExpr *E){
    VisitExpr(E);
    ...
    if(! E->isImplicitAccess()) Record.AddStmt(E->getBase());
    else Record.AddStmt(nullptr);
...
void ASTStmtWriter::VisitDependentScopeDeclRefExpr(DependentScopeDeclRefExpr *E){
    VisitExpr(E);
...
void ASTStmtWriter::VisitCXXUnresolvedConstructExpr(CXXUnresolvedConstructExpr *E){
    VisitExpr(E);
    Record.push_back(E->arg_size());
    for(CXXUnresolvedConstructExpr::arg_iterator
        ArgI = E->arg_begin(), ArgE = E->arg_end(); ArgI != ArgE; ++ ArgI) Record.AddStmt(*ArgI);
    Record.AddTypeSourceInfo(E->getTypeSourceInfo());
    Record.AddSourceLocation(E->getLParenLoc());
    Record.AddSourceLocation(E->getRParenLoc());
...
void ASTStmtWriter::VisitTypeTraitExpr(TypeTraitExpr *E){
    ...
    Record.AddSourceRange(E->getSourceRange());
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) Record.AddTypeSourceInfo(E->getArg(I));
...
void ASTStmtWriter::VisitSizeOfPackExpr(SizeOfPackExpr *E){
    ...
    if(E->isPartiallySubstituted()){
        for(const auto &TA : E->getPartialArguments()) Record.AddTemplateArgument(TA);
    }else if(! E->isValueDependent()) Record.push_back(E->getPackLength());
    Code = serialization::EXPR_SIZEOF_PACK;
}

void ASTStmtWriter::VisitSubstNonTypeTemplateParmExpr(SubstNonTypeTemplateParmExpr *E){
    VisitExpr(E);
...
void ASTStmtWriter::VisitSubstNonTypeTemplateParmPackExpr(SubstNonTypeTemplateParmPackExpr *E){
    VisitExpr(E);
...
void OMPClauseWriter::VisitOMPPrivateClause(OMPPrivateClause *C){
    ...
    Record.AddSourceLocation(C->getLParenLoc());
    for(auto *VE : C->varlists()) Record.AddStmt(VE);
    for(auto *VE : C->private_copies()) Record.AddStmt(VE);
}
...
void OMPClauseWriter::VisitOMPFirstprivateClause(OMPFirstprivateClause *C){
    ...
    for(auto *VE : C->varlists()) Record.AddStmt(VE);
    for(auto *VE : C->private_copies()) Record.AddStmt(VE);
    for(auto *VE : C->inits()) Record.AddStmt(VE);
}
...
void OMPClauseWriter::VisitOMPLastprivateClause(OMPLastprivateClause *C){
    ...
    Record.AddSourceLocation(C->getLParenLoc());
    for(auto *VE : C->varlists()) Record.AddStmt(VE);
    for(auto *E : C->private_copies()) Record.AddStmt(E);
    for(auto *E : C->source_exprs()) Record.AddStmt(E);
    for(auto *E : C->destination_exprs()) Record.AddStmt(E);
    for(auto *E : C->assignment_ops()) Record.AddStmt(E);
}
...
void OMPClauseWriter::VisitOMPReductionClause(OMPReductionClause *C){
    ...
    for(auto *VE : C->varlists()) Record.AddStmt(VE);
    for(auto *VE : C->privates()) Record.AddStmt(VE);
    for(auto *E : C->lhs_exprs()) Record.AddStmt(E);
    for(auto *E : C->rhs_exprs()) Record.AddStmt(E);
    for(auto *E : C->reduction_ops()) Record.AddStmt(E);
}
...
void OMPClauseWriter::VisitOMPLinearClause(OMPLinearClause *C){
    ...
    for(auto *VE : C->privates()) Record.AddStmt(VE);
    for(auto *VE : C->inits()) Record.AddStmt(VE);
    for(auto *VE : C->updates()) Record.AddStmt(VE);
    for(auto *VE : C->finals()) Record.AddStmt(VE);
...
void OMPClauseWriter::VisitOMPMapClause(OMPMapClause *C){
    ...
    Record.AddSourceLocation(C->getColonLoc());
    for(auto *E : C->varlists()) Record.AddStmt(E);
    for(auto *D : C->all_decls()) Record.AddDeclRef(D);
    for(auto N : C->all_num_lists()) Record.push_back(N);
    for(auto N : C->all_lists_sizes()) Record.push_back(N);
...
void ASTStmtWriter::VisitOMPExecutableDirective(OMPExecutableDirective *E){
    ...
    for(unsigned i = 0; i < E->getNumClauses(); ++ i) ClauseWriter.writeClause(E->getClause(i));
    if(E->hasAssociatedStmt()) Record.AddStmt(E->getAssociatedStmt());
}
...
void ASTStmtWriter::VisitOMPLoopDirective(OMPLoopDirective *D){
    ...
    for(auto I : D->counters()) Record.AddStmt(I);
    for(auto I : D->private_counters()) Record.AddStmt(I);
    for(auto I : D->inits()) Record.AddStmt(I);
    for(auto I : D->updates()) Record.AddStmt(I);
    for(auto I : D->finals()) Record.AddStmt(I);
}
...
void ASTStmtWriter::VisitOMPTargetEnterDataDirective(OMPTargetEnterDataDirective *D){
    VisitStmt(D);
    Record.push_back(D->getNumClauses());
    VisitOMPExecutableDirective(D);
...
void ASTStmtWriter::VisitOMPTargetExitDataDirective(OMPTargetExitDataDirective *D){
    VisitStmt(D);
    Record.push_back(D->getNumClauses());
    VisitOMPExecutableDirective(D);
...
void ASTStmtWriter::VisitOMPTargetParallelDirective(OMPTargetParallelDirective *D){
    VisitStmt(D);
    Record.push_back(D->getNumClauses());
    VisitOMPExecutableDirective(D);
...
void ASTStmtWriter::VisitOMPTargetParallelForDirective(OMPTargetParallelForDirective *D){
    VisitOMPLoopDirective(D);
    Record.push_back(D->hasCancel() ? 1 : 0);
...
void ASTStmtWriter::VisitOMPCancellationPointDirective(OMPCancellationPointDirective *D){
    VisitStmt(D);
    VisitOMPExecutableDirective(D);
...
void ASTStmtWriter::VisitOMPTargetParallelForSimdDirective(OMPTargetParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
...
void ASTStmtWriter::VisitOMPTeamsDistributeDirective(OMPTeamsDistributeDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::STMT_OMP_TEAMS_DISTRIBUTE_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTeamsDistributeSimdDirective(OMPTeamsDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
...
void ASTStmtWriter::VisitOMPTeamsDistributeParallelForDirective(OMPTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::STMT_OMP_TEAMS_DISTRIBUTE_PARALLEL_FOR_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTargetTeamsDistributeParallelForDirective(OMPTargetTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::STMT_OMP_TARGET_TEAMS_DISTRIBUTE_PARALLEL_FOR_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTargetTeamsDistributeParallelForSimdDirective(OMPTargetTeamsDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::
        STMT_OMP_TARGET_TEAMS_DISTRIBUTE_PARALLEL_FOR_SIMD_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTargetTeamsDistributeDirective(OMPTargetTeamsDistributeDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::STMT_OMP_TARGET_TEAMS_DISTRIBUTE_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTargetTeamsDistributeSimdDirective(OMPTargetTeamsDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
    Code = serialization::STMT_OMP_TARGET_TEAMS_DISTRIBUTE_SIMD_DIRECTIVE;
}

void ASTStmtWriter::VisitOMPTeamsDistributeParallelForSimdDirective(OMPTeamsDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
