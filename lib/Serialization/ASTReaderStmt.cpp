...
namespace clang {
    class ASTStmtReader : public StmtVisitor<ASTStmtReader> {
        ...
        void ReadDeclarationNameLoc(DeclarationNameLoc &DNLoc, DeclarationName Name, const ASTReader::RecordData &R, unsigned &I){
            Reader.ReadDeclarationNameLoc(F, DNLoc, Name, R, I);
        }

        void ReadDeclarationNameInfo(DeclarationNameInfo &NameInfo, const ASTReader::RecordData &R, unsigned &I){
...
void ASTStmtReader::ReadTemplateKWAndArgsInfo(ASTTemplateKWAndArgsInfo &Args, TemplateArgumentLoc *ArgsLocArray, unsigned NumTemplateArgs){
    SourceLocation				TemplateKWLoc = ReadSourceLocation(Record, Idx);
    TemplateArgumentListInfo	ArgInfo;
    ArgInfo.setLAngleLoc(ReadSourceLocation(Record, Idx));
    ArgInfo.setRAngleLoc(ReadSourceLocation(Record, Idx));
    for(unsigned i = 0; i != NumTemplateArgs; ++ i) ArgInfo.addArgument(
...
void ASTStmtReader::VisitCompoundStmt(CompoundStmt *S){
    ...
    while(NumStmts --) Stmts.push_back(Reader.ReadSubStmt());
...
void ASTStmtReader::VisitSwitchStmt(SwitchStmt *S){
    ...
    if(Record[Idx ++]) S->setAllEnumCasesCovered();
    SwitchCase *PrevSC = nullptr;
    for(unsigned N = Record.size(); Idx != N; ++ Idx){
        SwitchCase *SC = Reader.getSwitchCaseWithID(Record[Idx]);
        if(PrevSC) PrevSC->setNextSwitchCase(SC);
        else S->setSwitchCaseList(SC);
        PrevSC = SC;
    }
}
...
void ASTStmtReader::VisitDeclStmt(DeclStmt *S){
    ...
    if(Idx + 1 == Record.size()){
        ...
        for(unsigned N = Record.size(); Idx != N; ) Decls.push_back(ReadDecl(Record, Idx));
...
void ASTStmtReader::VisitGCCAsmStmt(GCCAsmStmt *S){
    ...
    for(unsigned I = 0; I != NumClobbers; ++ I) Clobbers.push_back(cast_or_null<StringLiteral>(Reader.ReadSubStmt()));
...
void ASTStmtReader::VisitMSAsmStmt(MSAsmStmt *S){
    ...
    for(unsigned i = 0, e = S->NumAsmToks; i != e; ++ i) AsmToks.push_back(ReadToken(Record, Idx));
...
void ASTStmtReader::VisitDeclRefExpr(DeclRefExpr *E){
    ...
    if(E->hasTemplateKWAndArgsInfo()) NumTemplateArgs = Record[Idx ++];
    if(E->hasQualifier())
            new(E->getTrailingObjects<NestedNameSpecifierLoc>())
            NestedNameSpecifierLoc(
                Reader.ReadNestedNameSpecifierLoc(F, Record, Idx));
    if(E->hasFoundDecl()) *E->getTrailingObjects<NamedDecl *>() = ReadDeclAs<NamedDecl>(Record, Idx);
...
void ASTStmtReader::VisitStringLiteral(StringLiteral *E){
    ...
    for(unsigned I = 0, N = E->getNumConcatenated(); I != N; ++ I) E->setStrTokenLoc(I, ReadSourceLocation(Record, Idx));
}
...
void ASTStmtReader::VisitParenListExpr(ParenListExpr *E){
    ...
    for(unsigned i = 0; i != NumExprs; ++ i) E->Exprs[i] = Reader.ReadSubStmt();
    E->NumExprs		= NumExprs;
...
void ASTStmtReader::VisitOffsetOfExpr(OffsetOfExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumExpressions(); I != N; ++ I) E->setIndexExpr(I, Reader.ReadSubExpr());
}
...
void ASTStmtReader::VisitUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *E){
    ...
    if(Record[Idx] == 0){
        ...
        ++ Idx;
    }else E->setArgument(GetTypeSourceInfo(Record, Idx));
    E->setOperatorLoc(ReadSourceLocation(Record, Idx));
    E->setRParenLoc(ReadSourceLocation(Record, Idx));
}
...
void ASTStmtReader::VisitCallExpr(CallExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) E->setArg(I, Reader.ReadSubExpr());
}
...
void ASTStmtReader::VisitObjCIndirectCopyRestoreExpr(ObjCIndirectCopyRestoreExpr *E){
    VisitExpr(E);
...
void ASTStmtReader::VisitBinaryConditionalOperator(BinaryConditionalOperator *E){
    VisitExpr(E);
...
void ASTStmtReader::VisitInitListExpr(InitListExpr *E){
    VisitExpr(E);
    if(InitListExpr *SyntForm = cast_or_null<InitListExpr>(Reader.ReadSubStmt())) E->setSyntacticForm(SyntForm);
    ...
    }else E->ArrayFillerOrUnionFieldInit = ReadDeclAs<FieldDecl>(Record, Idx);
    ...
    if(isArrayFiller){
        for(unsigned I = 0; I != NumInits; ++ I){
            ...
            for(unsigned I = 0; I != NumInits; ++ I) E->updateInit(Reader.getContext(), I, Reader.ReadSubExpr());
}
...
void ASTStmtReader::VisitDesignatedInitExpr(DesignatedInitExpr *E){
    ...
    for(unsigned I = 0; I != NumSubExprs; ++ I) E->setSubExpr(I, Reader.ReadSubExpr());
...
void ASTStmtReader::VisitShuffleVectorExpr(ShuffleVectorExpr *E){
    ...
    SmallVector<Expr *, 16> Exprs;
    unsigned				NumExprs = Record[Idx ++];
    while(NumExprs --) Exprs.push_back(Reader.ReadSubExpr());
...
void ASTStmtReader::VisitAtomicExpr(AtomicExpr *E){
    ...
    for(unsigned I = 0; I != E->NumSubExprs; ++ I) E->SubExprs[I] = Reader.ReadSubExpr();
    E->BuiltinLoc	= ReadSourceLocation(Record, Idx);
    E->RParenLoc	= ReadSourceLocation(Record, Idx);
}
...
void ASTStmtReader::VisitObjCArrayLiteral(ObjCArrayLiteral *E){
    ...
    for(unsigned I = 0, N = NumElements; I != N; ++ I) Elements[I] = Reader.ReadSubExpr();
...
void ASTStmtReader::VisitObjCMessageExpr(ObjCMessageExpr *E){
    ...
    if(Record[Idx ++]) E->setMethodDecl(ReadDeclAs<ObjCMethodDecl>(Record, Idx));
    else E->setSelector(Reader.ReadSelector(F, Record, Idx));
    E->LBracLoc = ReadSourceLocation(Record, Idx);
    E->RBracLoc = ReadSourceLocation(Record, Idx);
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) E->setArg(I, Reader.ReadSubExpr());
    SourceLocation *Locs = E->getStoredSelLocs();
    for(unsigned I = 0; I != NumStoredSelLocs; ++ I) Locs[I] = ReadSourceLocation(Record, Idx);
}
...
void ASTStmtReader::VisitObjCAtTryStmt(ObjCAtTryStmt *S){
    ...
    for(unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++ I) S->setCatchStmt(I, cast_or_null<ObjCAtCatchStmt>(Reader.ReadSubStmt()));
    if(HasFinally) S->setFinallyStmt(Reader.ReadSubStmt());
...
void ASTStmtReader::VisitCXXTryStmt(CXXTryStmt *S){
    ...
    for(unsigned i = 0, e = S->getNumHandlers(); i != e; ++ i) S->getStmts()[i + 1] = Reader.ReadSubStmt();
}
...
void ASTStmtReader::VisitCXXConstructExpr(CXXConstructExpr *E){
    ...
    if(E->NumArgs) E->Args = new(Reader.getContext())Stmt *[E->NumArgs];
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) E->setArg(I, Reader.ReadSubExpr());
...
void ASTStmtReader::VisitLambdaExpr(LambdaExpr *E){
    ...
    for(LambdaExpr::capture_init_iterator C = E->capture_init_begin()
       , CEnd = E->capture_init_end();
        C != CEnd; ++ C) *C = Reader.ReadSubExpr();
    // Read array capture index variables.
    if(NumArrayIndexVars > 0){
        unsigned *ArrayIndexStarts = E->getArrayIndexStarts();
        for(unsigned I = 0; I != NumCaptures + 1; ++ I) ArrayIndexStarts[I] = Record[Idx ++];
        VarDecl * *ArrayIndexVars = E->getArrayIndexVars();
        for(unsigned I = 0; I != NumArrayIndexVars; ++ I) ArrayIndexVars[I] = ReadDeclAs<VarDecl>(Record, Idx);
    }
}

void ASTStmtReader::VisitCXXStdInitializerListExpr(CXXStdInitializerListExpr *E){
    VisitExpr(E);
...
void ASTStmtReader::VisitCXXPseudoDestructorExpr(CXXPseudoDestructorExpr *E){
    ...
    if(II) E->setDestroyedType(II, ReadSourceLocation(Record, Idx));
    else E->setDestroyedType(GetTypeSourceInfo(Record, Idx));
}
...
void ASTStmtReader::VisitExprWithCleanups(ExprWithCleanups *E){
    ...
    for(unsigned i = 0; i != NumObjects; ++ i) E->getTrailingObjects<BlockDecl *>()[i] =
...
void ASTStmtReader::VisitCXXUnresolvedConstructExpr(CXXUnresolvedConstructExpr *E){
    VisitExpr(E);
    assert(Record[Idx] == E->arg_size() && "Read wrong record during creation ?");
    ++ Idx;	// NumArgs;
    for(unsigned I = 0, N = E->arg_size(); I != N; ++ I) E->setArg(I, Reader.ReadSubExpr());
    E->Type = GetTypeSourceInfo(Record, Idx);
    E->setLParenLoc(ReadSourceLocation(Record, Idx));
    E->setRParenLoc(ReadSourceLocation(Record, Idx));
}
...
void ASTStmtReader::VisitTypeTraitExpr(TypeTraitExpr *E){
    ...
    for(unsigned I = 0, N = E->getNumArgs(); I != N; ++ I) Args[I] = GetTypeSourceInfo(Record, Idx);
}
...
void ASTStmtReader::VisitSizeOfPackExpr(SizeOfPackExpr *E){
    ...
    if(E->isPartiallySubstituted()){
        ...
        for(auto *I = E->getTrailingObjects<TemplateArgument>()
           , *E = I + NumPartialArgs;
            I != E; ++ I) new(I) TemplateArgument(Reader.ReadTemplateArgument(F, Record, Idx));
    }else if(! E->isValueDependent()) E->Length = Record[Idx ++];
}

void ASTStmtReader::VisitSubstNonTypeTemplateParmExpr(SubstNonTypeTemplateParmExpr *E){
    VisitExpr(E);
...
void ASTStmtReader::VisitSubstNonTypeTemplateParmPackExpr(SubstNonTypeTemplateParmPackExpr *E){
    VisitExpr(E);
    E->Param = ReadDeclAs<NonTypeTemplateParmDecl>(Record, Idx);
    TemplateArgument ArgPack = Reader.ReadTemplateArgument(F, Record, Idx);
    if(ArgPack.getKind() != TemplateArgument::Pack) return;
...
void ASTStmtReader::VisitFunctionParmPackExpr(FunctionParmPackExpr *E){
    ...
    for(unsigned i = 0, n = E->NumParameters; i != n; ++ i) Parms[i] = ReadDeclAs<ParmVarDecl>(Record, Idx);
}
...
void OMPClauseReader::VisitOMPPrivateClause(OMPPrivateClause *C){
    ...
    for(unsigned i = 0; i != NumVars; ++ i) Vars.push_back(Reader->Reader.ReadSubExpr());
    C->setVarRefs(Vars);
    Vars.clear();
    for(unsigned i = 0; i != NumVars; ++ i) Vars.push_back(Reader->Reader.ReadSubExpr());
...
void OMPClauseReader::VisitOMPCopyinClause(OMPCopyinClause *C){
    ...
    for(unsigned i = 0; i != NumVars; ++ i) Exprs.push_back(Reader->Reader.ReadSubExpr());
    C->setVarRefs(Exprs);
    Exprs.clear();
    for(unsigned i = 0; i != NumVars; ++ i) Exprs.push_back(Reader->Reader.ReadSubExpr());
...
void OMPClauseReader::VisitOMPMapClause(OMPMapClause *C){
    ...
    for(unsigned i = 0; i < UniqueDecls; ++ i) Decls.push_back(
                Reader->Reader.ReadDeclAs<ValueDecl>(Reader->F, Record, Idx));
    C->setUniqueDecls(Decls);

    SmallVector<unsigned, 16> ListsPerDecl;
    ListsPerDecl.reserve(UniqueDecls);
    for(unsigned i = 0; i < UniqueDecls; ++ i) ListsPerDecl.push_back(Record[Idx ++]);
    C->setDeclNumLists(ListsPerDecl);

    SmallVector<unsigned, 32> ListSizes;
    ListSizes.reserve(TotalLists);
    for(unsigned i = 0; i < TotalLists; ++ i) ListSizes.push_back(Record[Idx ++]);
...
void ASTStmtReader::VisitOMPExecutableDirective(OMPExecutableDirective *E){
    ...
    for(unsigned i = 0; i < E->getNumClauses(); ++ i) Clauses.push_back(ClauseReader.readClause());
    E->setClauses(Clauses);
    if(E->hasAssociatedStmt()) E->setAssociatedStmt(Reader.ReadSubStmt());
}
...
void ASTStmtReader::VisitOMPLoopDirective(OMPLoopDirective *D){
    ...
    for(unsigned i = 0; i < CollapsedNum; ++ i) Sub.push_back(Reader.ReadSubExpr());
    D->setCounters(Sub);
    Sub.clear();
    for(unsigned i = 0; i < CollapsedNum; ++ i) Sub.push_back(Reader.ReadSubExpr());
...
void ASTStmtReader::VisitOMPTargetEnterDataDirective(OMPTargetEnterDataDirective *D){
    VisitStmt(D);
    ++ Idx;
    VisitOMPExecutableDirective(D);
}

void ASTStmtReader::VisitOMPTargetExitDataDirective(OMPTargetExitDataDirective *D){
    VisitStmt(D);
    ++ Idx;
    VisitOMPExecutableDirective(D);
}

void ASTStmtReader::VisitOMPTargetParallelDirective(OMPTargetParallelDirective *D){
    VisitStmt(D);
    ++ Idx;
    VisitOMPExecutableDirective(D);
}

void ASTStmtReader::VisitOMPTargetParallelForDirective(OMPTargetParallelForDirective *D){
    VisitOMPLoopDirective(D);
    D->setHasCancel(Record[Idx ++]);
}
...
void ASTStmtReader::VisitOMPCancellationPointDirective(OMPCancellationPointDirective *D){
    VisitStmt(D);
    VisitOMPExecutableDirective(D);
...
void ASTStmtReader::VisitOMPTargetParallelForSimdDirective(OMPTargetParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}
...
void ASTStmtReader::VisitOMPTeamsDistributeParallelForDirective(OMPTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void ASTStmtReader::VisitOMPTargetTeamsDistributeParallelForDirective(OMPTargetTeamsDistributeParallelForDirective *D){
    VisitOMPLoopDirective(D);
}

void ASTStmtReader::VisitOMPTargetTeamsDistributeParallelForSimdDirective(OMPTargetTeamsDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void ASTStmtReader::VisitOMPTargetTeamsDistributeDirective(OMPTargetTeamsDistributeDirective *D){
    VisitOMPLoopDirective(D);
}

void ASTStmtReader::VisitOMPTargetTeamsDistributeSimdDirective(OMPTargetTeamsDistributeSimdDirective *D){
    VisitOMPLoopDirective(D);
}

void ASTStmtReader::VisitOMPTeamsDistributeParallelForSimdDirective(OMPTeamsDistributeParallelForSimdDirective *D){
    VisitOMPLoopDirective(D);
}
        ...
        switch((StmtCode) Cursor.readRecord(Entry.ID, Record)){
            ...
            case EXPR_MEMBER: {
                ...
                if(HasTemplateKWAndArgsInfo){
                    ...
                    for(unsigned i = 0; i != NumTemplateArgs; ++ i) ArgInfo.addArgument(ReadTemplateArgumentLoc(F, Record, Idx));
                }
                ...
                if(HadMultipleCandidates) cast<MemberExpr>(S)->setHadMultipleCandidates(true);
                break;
            }
